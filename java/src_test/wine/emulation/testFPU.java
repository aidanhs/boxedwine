...
public class testFPU extends testCPU {
    ...
    // same as base class, but doesn't clear memory
    ...
    private float getTopFloat() {
        newInstruction(0xd9, 0);
        pushCode8(rm(true, 2, 0));
        runCPU();
        return Float.intBitsToFloat(memory.readd(HEAP_ADDRESS));
    }

    private void writeF(float f) {
        memory.writed(HEAP_ADDRESS, Float.floatToRawIntBits(f));
        memory.writed(HEAP_ADDRESS - 4, 0xCDCDCDCD);
        memory.writed(HEAP_ADDRESS + 4, 0xCDCDCDCD);
    }
    private void writeD(long f) {
        memory.writed(HEAP_ADDRESS, (int)f);
        memory.writed(HEAP_ADDRESS+4, (int)(f>>>32));
        memory.writed(HEAP_ADDRESS - 4, 0xCDCDCDCD);
        memory.writed(HEAP_ADDRESS + 8, 0xCDCDCDCD);
    }
    ...
    private void doF32Instruction(int op1, int group1, int op2, int group2, float x, float y, float r) {
        ...
        pushCode8(rm(true, group1, 0));
        pushCode32(HEAP_ADDRESS);
        runCPU();
    ...
    public void FSTFloat(int op, int group, float f, boolean pop) {
        ...
        fldf32(f);
        memory.writed(HEAP_ADDRESS, 0xCDCDCDCD);

        newInstruction(op, 0);
        pushCode8(rm(true, group, 0));
        runCPU();
        float result = Float.intBitsToFloat(memory.readd(HEAP_ADDRESS));
