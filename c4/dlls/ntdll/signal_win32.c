...
extern DWORD EXC_CallHandler(EXCEPTION_RECORD *record, EXCEPTION_REGISTRATION_RECORD *frame,
	...
	PEXCEPTION_HANDLER handler, PEXCEPTION_HANDLER nested_handler, PNT_TIB tib);
...
static inline TEB *get_current_teb(void)
{
	unsigned long tmp;
	_asm mov tmp, esp;
	return (TEB *)(tmp & ~signal_stack_mask);
}
...
static NTSTATUS call_stack_handlers(EXCEPTION_RECORD *rec, CONTEXT *context)
{
	...
	while (frame != (EXCEPTION_REGISTRATION_RECORD*)~0UL)
	{
		...
		res = EXC_CallHandler(rec, frame, context, &dispatch, frame->Handler, raise_handler, &NtCurrentTeb()->Tib);
...
static inline void *init_handler(const ucontext_t *sigcontext, WORD *fs, WORD *gs)
{
	...
	// clear alignment flag
	_asm pushf;
	_asm and esp, ~0x40000;
	_asm popf;
...
#if !defined __sun && !defined IGNORE_FS_GS  /* see above for Solaris handling */
	{
...
{
    FLOATING_SAVE_AREA* p = &context->FloatSave;
...
*/
__declspec(naked) void WINAPI RtlCaptureContext(PCONTEXT ContextRecord) {
	__asm {
		push eax
		mov  eax, ContextRecord
		mov [eax]CONTEXT.ContextFlags, CONTEXT_FULL
		mov word ptr [eax]CONTEXT.SegGs, gs
		mov word ptr [eax]CONTEXT.SegFs, fs
		mov word ptr [eax]CONTEXT.SegEs, es
		mov word ptr [eax]CONTEXT.SegDs, ds
		...
		mov word ptr [eax]CONTEXT.SegCs, cs
		pushf
		pop [eax]CONTEXT.EFlags
		lea edx, [esp+12]
		mov [eax]CONTEXT.Esp, edx
		mov word ptr [eax]CONTEXT.SegSs, ss
		pop [eax]CONTEXT.Eax
	}
}
...
*/
__declspec(naked) DECLSPEC_NORETURN void WINAPI __wine_restore_regs(PCONTEXT ContextRecord) {
	__asm {
		mov  eax, ContextRecord
		//mov gs, word ptr[eax]CONTEXT.SegGs
		//mov fs, word ptr[eax]CONTEXT.SegFs
		//mov es, word ptr[eax]CONTEXT.SegEs
		//mov ds, word ptr[eax]CONTEXT.SegDs
		mov edi, [eax]CONTEXT.Edi
		mov esi, [eax]CONTEXT.Esi
		mov ebx, [eax]CONTEXT.Ebx
		mov edx, [eax]CONTEXT.Edx		
		mov ebp, [eax]CONTEXT.Ebp
		push [eax]CONTEXT.EFlags
		popf
		//mov cs, word ptr[eax]CONTEXT.SegCs
		//mov ss, word ptr[eax]CONTEXT.SegSs
		mov esp, [eax]CONTEXT.Esp
		mov ecx, [eax]CONTEXT.Eip
		mov [esp-4], ecx
		mov ecx, [eax]CONTEXT.Ecx
	...
	PEXCEPTION_RECORD pRecord, PVOID retval, CONTEXT *context)
{
	...
	while ((frame != (EXCEPTION_REGISTRATION_RECORD*)~0UL) && (frame != pEndFrame))
	{
		...
		res = EXC_CallHandler(pRecord, frame, context, &dispatch, frame->Handler, unwind_handler, &NtCurrentTeb()->Tib);
		TRACE("handler at %p returned %x\n", frame->Handler, res);
...
_declspec(naked) void DECLSPEC_NORETURN call_thread_entry_point(LPTHREAD_START_ROUTINE entry, void *arg) {
	_asm push ebp;
	_asm mov ebp, esp;
	_asm push ebx;
	_asm push esi;
	...
	_asm push [ebp + 12];
	_asm push [ebp + 8];
	_asm call call_thread_func;
}

_declspec(naked) void DECLSPEC_NORETURN call_thread_exit_func(int status, void(*func)(int), void *frame) {
	_asm mov eax, [esp + 4];
	_asm mov ecx, [esp + 8];
	_asm mov ebp, [esp + 12];
	_asm lea esp, [ebp - 20];
...
_declspec(naked) void DECLSPEC_NORETURN call_thread_func_wrapper(LPTHREAD_START_ROUTINE entry, void *arg) {
	_asm push ebp;
	_asm mov ebp, esp;
	_asm sub esp, 4;
	_asm push [ebp + 12];
...
{
	return pthread_getspecific(1);
}

/**************************************************************************
*           _chkstk   (NTDLL.@)
*/
_declspec(naked) void WINAPI _chkstk() {
	_asm neg eax;
	_asm add eax, esp;
	_asm xchg eax, esp;
	_asm mov eax, [eax];
...
*           _alloca_probe   (NTDLL.@)
*/
_declspec(naked) void WINAPI _alloca_probe() {
	_asm neg eax;
	_asm add eax, esp;
	_asm xchg eax, esp;
	_asm mov eax, [eax];
...
*/
_declspec(naked) void call_exception_handler() {
	_asm push ebp;
	_asm mov ebp, esp;
	_asm sub esp, 12;
	_asm push[ebp + 12];
	_asm push edx;	/* nested_handler (passed in edx). */
	_asm push ebx;  /* TIB pased in ebx */
	_asm mov[ebx], esp;
	_asm push[ebp + 20];
	_asm push[ebp + 16];
	_asm push[ebp + 12];
...
_declspec(naked) DWORD EXC_CallHandler(EXCEPTION_RECORD *record, EXCEPTION_REGISTRATION_RECORD *frame,
	CONTEXT *context, EXCEPTION_REGISTRATION_RECORD **dispatcher,
	PEXCEPTION_HANDLER handler, PEXCEPTION_HANDLER nested_handler, PNT_TIB tib) {
	_asm push ebp;
	_asm mov ebp, esp;
	...
	_asm push[ebp + 12];
	_asm push[ebp + 8];
	_asm call call_exception_handler;
	_asm pop ebx;
	_asm pop edx;
	_asm leave;
	_asm ret;
