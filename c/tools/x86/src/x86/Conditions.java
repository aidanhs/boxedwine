...
public class Conditions extends Base {
    ...
    public void condsrc(FileOutputStream fos, String inst, String c, String cond, int bits) throws IOException {
        ...
        out(fos, "        out(data, \") {\");");
        out(fos, "        out(data, r"+bits+"(op->r1));");
        out(fos, "        out(data, \" = read"+width+"(MMU_PARAM_CPU \");");
    ...
    public void condition(FileOutputStream fos, String name, String condition) throws IOException {
        ...
        out(fos, "    if ("+condition+") {");
        out(fos, "        cpu->reg[op->r1].u16 = readw(MMU_PARAM_CPU eaa16(cpu, op));");
        out(fos, "    }");
        out(fos, "    CYCLES(1);");
        out(fos, "    NEXT();");
        out(fos, "}");
        ...
        out(fos, "    if ("+condition+") {");
        out(fos, "        cpu->reg[op->r1].u16 = readw(MMU_PARAM_CPU eaa32(cpu, op));");
        out(fos, "    }");
        out(fos, "    CYCLES(1);");
        out(fos, "    NEXT();");
        out(fos, "}");
        ...
        out(fos, "    if ("+condition+") {");
        out(fos, "        cpu->reg[op->r1].u32 = readd(MMU_PARAM_CPU eaa16(cpu, op));");
        out(fos, "    }");
        out(fos, "    CYCLES(1);");
        out(fos, "    NEXT();");
        out(fos, "}");
        ...
        out(fos, "    if ("+condition+") {");
        out(fos, "        cpu->reg[op->r1].u32 = readd(MMU_PARAM_CPU eaa32(cpu, op));");
        out(fos, "    }");
        out(fos, "    CYCLES(1);");
        out(fos, "    NEXT();");
        out(fos, "}");
