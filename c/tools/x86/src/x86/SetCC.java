...
public class SetCC extends Base {
    ...
    public void ccsrc(FileOutputStream fos, String inst, String c, String cond) throws IOException {
        ...
        out(fos, "        out(data, \") {\");");
        out(fos, "        out(data, r8(op->r1));");
        out(fos, "        out(data, \" = 1;} else {\");");
        out(fos, "        out(data, r8(op->r1));");
        out(fos, "        out(data, \" = 0;} CYCLES(2);\");");
        out(fos, "    } else if (op->func == set"+c+"_mem16) {");
        out(fos, "        out(data, \"if (\");");
        out(fos, "        out(data, getCondition(data, "+cond+"));");
        out(fos, "        out(data, \") {writeb(MMU_PARAM_CPU \");");
        out(fos, "        out(data, getEaa16(op));");
        out(fos, "        out(data, \", 1);} else {writeb(MMU_PARAM_CPU \");");
        out(fos, "        out(data, getEaa16(op));");
        out(fos, "        out(data, \", 0);} CYCLES(2);\");");
    ...
    public void cc(FileOutputStream fos, String name, String condition) throws IOException {
        ...
        out(fos, "    if ("+condition+") {");
        out(fos, "        writeb(MMU_PARAM_CPU eaa16(cpu, op), 1);");
        out(fos, "    } else {");
        out(fos, "        writeb(MMU_PARAM_CPU eaa16(cpu, op), 0);");
        out(fos, "    }");
        out(fos, "    CYCLES(2);");
        out(fos, "    NEXT();");
        out(fos, "}");
        ...
        out(fos, "    if ("+condition+") {");
        out(fos, "        writeb(MMU_PARAM_CPU eaa32(cpu, op), 1);");
        out(fos, "    } else {");
        out(fos, "        writeb(MMU_PARAM_CPU eaa32(cpu, op), 0);");
        out(fos, "    }");
        out(fos, "    CYCLES(2);");
        out(fos, "    NEXT();");
        out(fos, "}");
