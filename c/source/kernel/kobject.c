...
void closeKObject(struct KObject* kobject) {
 ...
 void writeStat(struct Memory* memory, U32 buf, BOOL is64, U64 st_dev, U64 st_ino, U32 st_mode, U64 st_rdev, U64 st_size, U32 st_blksize, U64 st_blocks, U64 mtime, U32 linkCount) {
     if (is64) {
        ...
        writeq(memory, buf, st_dev);buf+=8;//st_dev               // 0
        buf+=4; // padding                                          // 8
        writed(memory, buf, (U32)st_ino); buf += 4;//__st_ino     // 12
        writed(memory, buf, st_mode); buf += 4;//st_mode          // 16
        writed(memory, buf, linkCount); buf += 4;//st_nlink       // 20
        writed(memory, buf, UID); buf += 4;//st_uid               // 24
        writed(memory, buf, GID); buf += 4;//st_gid               // 28
        writeq(memory, buf, st_rdev); buf += 8;//st_rdev          // 32
        buf+=4;                                                     // 40
        writeq(memory, buf, st_size); buf += 8;//st_size          // 44
        writed(memory, buf, st_blksize); buf += 4;//st_blksize    // 52
        writeq(memory, buf, st_blocks); buf += 8; //st_blocks     // 56
        writed(memory, buf, t); buf += 4; // st_atime             // 64
        writed(memory, buf, n); buf += 4; // st_atime_nsec        // 68
        writed(memory, buf, t); buf += 4; // st_mtime             // 72
        writed(memory, buf, n); buf += 4; // st_mtime_nsec        // 76
        writed(memory, buf, t); buf += 4; // st_ctime             // 80
        writed(memory, buf, n); buf += 4; // st_ctime_nsec        // 84
        writeq(memory, buf, st_ino); // st_ino                    // 88
     } else {
        U32 t = (U32)(mtime/1000); // ms to sec
