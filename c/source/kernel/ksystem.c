...
U32 syscall_uname(struct KThread* thread, U32 address) {
    writeNativeString(thread->process->memory, address, "Linux");
    writeNativeString(thread->process->memory, address + 65, "Linux");
    writeNativeString(thread->process->memory, address + 130, "3.11.0-12-generic");
    writeNativeString(thread->process->memory, address + 260, "i686");
    return 0;
}
...
U32 syscall_ugetrlimit(struct KThread* thread, U32 resource, U32 rlim) {
    switch (resource) {
        case 2: // RLIMIT_DATA
            writed(thread->process->memory, rlim, MAX_DATA_SIZE);
            writed(thread->process->memory, rlim + 4, MAX_DATA_SIZE);
            break;
        case 3: // RLIMIT_STACK
            writed(thread->process->memory, rlim, MAX_STACK_SIZE);
            writed(thread->process->memory, rlim + 4, MAX_STACK_SIZE);
            break;
        case 4: // RLIMIT_CORE
            writed(thread->process->memory, rlim, 1024 * 1024 * 4);
            writed(thread->process->memory, rlim + 4, 1024 * 1024 * 4);
            break;
        case 5: // RLIMIT_DATA
            writed(thread->process->memory, rlim, MAX_DATA_SIZE);
            writed(thread->process->memory, rlim + 4, MAX_DATA_SIZE);
        ...
        case 6: // RLIMIT_MEMLOCK
            writed(thread->process->memory, rlim, 64 * 1024 * 1024);
            writed(thread->process->memory, rlim + 4, 64 * 1024 * 1024);
            break;
        case 7: // RLIMIT_NOFILE
            writed(thread->process->memory, rlim, MAX_NUMBER_OF_FILES);
            writed(thread->process->memory, rlim + 4, MAX_NUMBER_OF_FILES);
            break;
        case 9: // RLIMIT_AS
            writed(thread->process->memory, rlim, MAX_ADDRESS_SPACE);
            writed(thread->process->memory, rlim + 4, MAX_ADDRESS_SPACE);
            break;
        default:
...
U32 syscall_getrusuage(struct KThread* thread, U32 who, U32 usuage) {
    if (who==0) { // RUSAGE_SELF
        // user time
        writed(thread->process->memory, usuage, (U32)(thread->userTime / 1000000l));
        writed(thread->process->memory, usuage + 4, (U32)(thread->userTime % 1000000l));
        // system time
        writed(thread->process->memory, usuage + 8, (U32)(thread->kernelTime / 1000000l));
        writed(thread->process->memory, usuage + 12, (U32)(thread->kernelTime % 1000000l));
    }
    return 0;
}
...
U32 syscall_times(struct KThread* thread, U32 buf) {
    if (buf) {
        writed(thread->process->memory, buf, (U32)thread->userTime * 10); // user time
        writed(thread->process->memory, buf + 4, (U32)thread->kernelTime * 10); // system time
...
U32 syscall_clock_gettime(struct KThread* thread, U32 clock_id, U32 tp) {    
    if (clock_id==0) { // CLOCK_REALTIME
        ...
        writed(thread->process->memory, tp, (U32)(m / 1000000l));
        writed(thread->process->memory, tp + 4, (U32)(m % 1000000l) * 1000);
        ...
        writed(thread->process->memory, tp, (U32)(diff / 1000000l));
        writed(thread->process->memory, tp + 4, (U32)(diff % 1000000l) * 1000);
    } else {
...
U32 syscall_gettimeofday(struct KThread* thread, U32 tv, U32 tz) {
    ...
    writed(thread->process->memory, tv, (U32)(m / 1000000l));
    writed(thread->process->memory, tv + 4, (U32)(m % 1000000l));
    return 0;
}
...
U32 syscall_sysinfo(struct KThread* thread, U32 address) {
    writed(thread->process->memory, address, getMilliesSinceStart()/1000); address+=4;
    writed(thread->process->memory, address, 0); address+=4;
    writed(thread->process->memory, address, 0); address+=4;
    writed(thread->process->memory, address, 0); address+=4;
    writed(thread->process->memory, address, getPageCount()); address+=4;
    writed(thread->process->memory, address, getFreePageCount()); address+=4;
    writed(thread->process->memory, address, 0); address+=4;
    writed(thread->process->memory, address, 0); address+=4;
    writed(thread->process->memory, address, 0); address+=4;
    writed(thread->process->memory, address, 0); address+=4;
    writew(thread->process->memory, address, getProcessCount()); address+=2;
    writed(thread->process->memory, address, 0); address+=4;
    writed(thread->process->memory, address, 0); address+=4;
    writed(thread->process->memory, address, PAGE_SIZE);
    return 0;
}
...
void walkStack(struct CPU* cpu, U32 eip, U32 ebp, U32 indent) {
    ...
    if (isValidReadAddress(cpu->memory, ebp)) {
        prevEbp = readd(cpu->memory, ebp); 
        returnEip = readd(cpu->memory, ebp+4); 
        if (prevEbp==0)
            return;
