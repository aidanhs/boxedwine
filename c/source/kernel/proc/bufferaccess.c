...
U32 buffer_write(MMU_ARG struct FsOpenNode* node, U32 address, U32 len) {
    U32 pos = node->idata;
    if (pos+len>node->func->dataLen)
        len = node->func->dataLen-pos;
    memcopyToNative(MMU_PARAM address, ((char*)node->func->data)+pos, len);
    return len;
}
...
U32 buffer_map(MMU_ARG struct FsOpenNode* node, U32 address, U32 len, S32 prot, S32 flags, U64 off) {
    return 0;
}
