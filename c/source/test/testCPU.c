...
void setup() {
	if (!memory) {
		...
		allocPages(memory, &ramPageWR, TRUE, (STACK_ADDRESS >> PAGE_SHIFT)-1, 1, 0);
		allocPages(memory, &ramPageWR, TRUE, CODE_ADDRESS >> PAGE_SHIFT, 1, 0);
		allocPages(memory, &ramPageWR, TRUE, HEAP_ADDRESS >> PAGE_SHIFT, 1, 0);
	}
	initCPU(cpu, memory);	
