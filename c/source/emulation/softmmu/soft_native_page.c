...
#else
        U16* result = (U16*)(memory->ramPage[address >> PAGE_SHIFT]+(address & PAGE_MASK));
        return *result;
#endif
}
...
#else
        U32* result = (U32*)(memory->ramPage[address >> PAGE_SHIFT]+(address & PAGE_MASK));
        return *result;
#endif
}
...
static U8* native_physicalAddress(struct Memory* memory, U32 address) {
    return (U8*)(memory->ramPage[address >> PAGE_SHIFT]+(address & PAGE_MASK));
}
