...
static INLINE HANDLE HEAP_CreateSystemHeap(void)
{
    ...
    event = CreateEventA_k( NULL, TRUE, FALSE, "__wine_system_heap_event" );

    if (!(map = CreateFileMappingA_k( INVALID_HANDLE_VALUE, NULL, SEC_COMMIT | PAGE_READWRITE,
                                    0, SYSTEM_HEAP_SIZE, "__wine_system_heap" ))) return 0;
    created = (GetLastError_k() != ERROR_ALREADY_EXISTS);

    if (!(base = MapViewOfFileEx_k( map, FILE_MAP_ALL_ACCESS, 0, 0, 0, SYSTEM_HEAP_BASE )))
    {
    ...
    {
        systemHeap = RtlCreateHeap_k( HEAP_SHARED, base, SYSTEM_HEAP_SIZE,
    ...
    else
    {
        ...
        WaitForSingleObject_k( event, INFINITE );
        systemHeap = base;
    }
    CloseHandle_k( map );
    return systemHeap;
...
HANDLE WINAPI HeapCreate_k(
    ...
    {
        ret = RtlCreateHeap_k( flags, NULL, maxSize, initialSize, NULL, NULL );
        if (!ret) SetLastError_k( ERROR_NOT_ENOUGH_MEMORY );
    }
    return ret;
}
 ...
 *	FALSE: Failure
 */
BOOL WINAPI HeapDestroy_k( HANDLE heap /* [in] Handle of heap */ )
{
    if (heap == systemHeap)
    {
    ...
    if (!RtlDestroyHeap_k( heap )) return TRUE;
 ...
 */
SIZE_T WINAPI HeapCompact_k( HANDLE heap, DWORD flags )
{
    return RtlCompactHeap_k( heap, flags );
}
 ...
 *	FALSE: Failure
 */
BOOL WINAPI HeapValidate_k(
              HANDLE heap, /* [in] Handle to the heap */
              DWORD flags,   /* [in] Bit flags that control access during operation */
              LPCVOID block  /* [in] Optional pointer to memory block to validate */
) {
    return RtlValidateHeap_k( heap, flags, block );
}
 ...
 *	FALSE: Failure
 */
BOOL WINAPI HeapUnlock_k(
              HANDLE heap /* [in] Handle to the heap to unlock */
) {
    return RtlUnlockHeap_k( heap );
}
 ...
 */
DWORD WINAPI GetProcessHeaps_k( DWORD count, HANDLE *heaps )
{
    return RtlGetProcessHeaps_k( count, heaps );
}
 ...
 *           HeapAlloc    (KERNEL32.@)
 */
LPVOID WINAPI HeapAlloc_k( HANDLE heap, DWORD flags, SIZE_T size )
{
    return RtlAllocateHeap_k( heap, flags, size );
}

BOOL WINAPI HeapFree_k( HANDLE heap, DWORD flags, LPVOID ptr )
{
    return RtlFreeHeap_k( heap, flags, ptr );
}

LPVOID WINAPI HeapReAlloc_k( HANDLE heap, DWORD flags, LPVOID ptr, SIZE_T size )
{
    return RtlReAllocateHeap_k( heap, flags, ptr, size );
}

SIZE_T WINAPI HeapSize_k( HANDLE heap, DWORD flags, LPCVOID ptr )
{
    return RtlSizeHeap_k( heap, flags, ptr );
}

BOOL WINAPI HeapQueryInformation_k( HANDLE heap, HEAP_INFORMATION_CLASS info_class,
                                  PVOID info, SIZE_T size_in, PSIZE_T size_out)
{
    NTSTATUS ret = RtlQueryHeapInformation_k( heap, info_class, info, size_in, size_out );
    if (ret) SetLastError_k( RtlNtStatusToDosError_k(ret) );
    return !ret;
}

BOOL WINAPI HeapSetInformation_k( HANDLE heap, HEAP_INFORMATION_CLASS infoclass, PVOID info, SIZE_T size)
{
...
HGLOBAL WINAPI GlobalAlloc_k(
   ...
   {
      palloc=HeapAlloc_k(GetProcessHeap_k(), hpflags, size);
   ...
   else  /* HANDLE */
   {
      ...
      pintern = HeapAlloc_k(GetProcessHeap_k(), 0, sizeof(GLOBAL32_INTERN));
      if (pintern)
      {
          ...
          if (size)
          {
              palloc = HeapAlloc_k(GetProcessHeap_k(), hpflags, size+HGLOBAL_STORAGE);
              if (!palloc)
              {
                  HeapFree_k(GetProcessHeap_k(), 0, pintern);
                  pintern = NULL;
              }
              else
              {
 ...
 *
 */
LPVOID WINAPI GlobalLock_k(HGLOBAL hmem)
{
    PGLOBAL32_INTERN pintern;
    LPVOID           palloc;

    if (ISPOINTER(hmem))
        return IsBadReadPtr_k(hmem, 1) ? NULL : hmem;

    RtlLockHeap_k(GetProcessHeap_k());
    __TRY
    {
        ...
        if (pintern->Magic == MAGIC_GLOBAL_USED)
        {
            ...
            if (!pintern->Pointer)
                SetLastError_k(ERROR_DISCARDED);
    ...
    __ENDTRY
    RtlUnlockHeap_k(GetProcessHeap_k());
    return palloc;
}
 ...
 *
 */
BOOL WINAPI GlobalUnlock_k(HGLOBAL hmem)
{
    PGLOBAL32_INTERN pintern;
    BOOL locked;
    ...
    __TRY
    {
        ...
        if(pintern->Magic==MAGIC_GLOBAL_USED)
        {
            if(pintern->LockCount)
            {
                ...
                if (!locked) SetLastError_k(NO_ERROR);
            }
            else
            {
                WARN("%p not locked\n", hmem);
                SetLastError_k(ERROR_NOT_LOCKED);
                locked = FALSE;
            }
        }
        else
...
HGLOBAL WINAPI GlobalHandle_k(
    ...
    __TRY
    {
        ...
        /* will fail.                                                      */
        if (ISPOINTER(pmem)) {
            if (HeapValidate_k( GetProcessHeap_k(), 0, pmem )) {
        ...
        if (maybe_intern->Magic == MAGIC_GLOBAL_USED) {
            ...
            if (HeapValidate_k( GetProcessHeap_k(), 0, (const char *)test - HGLOBAL_STORAGE ) && /* obj(-handle) valid arena? */
                HeapValidate_k( GetProcessHeap_k(), 0, maybe_intern ))  /* intern valid arena? */
            {
                RtlUnlockHeap_k(GetProcessHeap_k());
                return handle;
            }
 ...
 *      NULL: Failure
 */
HGLOBAL WINAPI GlobalReAlloc_k(
   ...
   if(flags & GMEM_MODIFY) /* modify flags */
   {
      if( ISPOINTER(hmem) && (flags & GMEM_MOVEABLE))
      {
         ...
         {
             size = HeapSize_k(GetProcessHeap_k(), 0, hmem);
             hnew = GlobalAlloc_k(flags, size);
             palloc = GlobalLock_k(hnew);
   ...
   else
   {
      if(ISPOINTER(hmem))
      {
         ...
         hnew=HeapReAlloc_k(GetProcessHeap_k(), heap_flags, hmem, size);
      }
      else
      {
...
/* See also the same comment in GlobalFree() */
         ...
         if(size!=0)
         {
            ...
            if(pintern->Pointer)
            {
               ...
               else if((palloc = HeapReAlloc_k(GetProcessHeap_k(), heap_flags,
            ...
            else
            {
                ...
                else if((palloc=HeapAlloc_k(GetProcessHeap_k(), heap_flags, size+HGLOBAL_STORAGE))
                   == NULL)
                    hnew = 0;
                else
                {
         ...
         else
         {
            if (pintern->LockCount == 0)
            {
                if(pintern->Pointer)
                {
                    HeapFree_k(GetProcessHeap_k(), 0, (char *) pintern->Pointer-HGLOBAL_STORAGE);
                    pintern->Pointer = NULL;
                }
                hnew = hmem;
            }
 ...
 *
 */
HGLOBAL WINAPI GlobalFree_k(HGLOBAL hmem)
{
    PGLOBAL32_INTERN pintern;
    HGLOBAL hreturned;
    ...
    __TRY
    {
        ...
        {
            if(!HeapFree_k(GetProcessHeap_k(), 0, hmem))
            {
                SetLastError_k(ERROR_INVALID_HANDLE);
                hreturned = hmem;
            }
        ...
        else  /* HANDLE */
        {
            ...
            if(pintern->Magic==MAGIC_GLOBAL_USED)
            {
                ...
                if(pintern->Pointer)
                    if(!HeapFree_k(GetProcessHeap_k(), 0, (char *)(pintern->Pointer)-HGLOBAL_STORAGE))
                        hreturned=hmem;
                if(!HeapFree_k(GetProcessHeap_k(), 0, pintern))
                    hreturned=hmem;
            }
            else
            {
 ...
 *
 */
SIZE_T WINAPI GlobalSize_k(HGLOBAL hmem)
{
   SIZE_T               retval;
   PGLOBAL32_INTERN     pintern;
   ...
   if(ISPOINTER(hmem))
   {
      retval=HeapSize_k(GetProcessHeap_k(), 0, hmem);

      if (retval == ~0ul) /* It might be a GMEM_MOVEABLE data pointer */
      {
          retval = HeapSize_k(GetProcessHeap_k(), 0, (char*)hmem - HGLOBAL_STORAGE);
   ...
   else
   {
      ...
      if(pintern->Magic==MAGIC_GLOBAL_USED)
      {
         ...
         {
             retval = HeapSize_k(GetProcessHeap_k(), 0, (char *)pintern->Pointer - HGLOBAL_STORAGE );
             if (retval != ~0ul) retval -= HGLOBAL_STORAGE;
         }
      }
      else
 ...
 *           GlobalWire   (KERNEL32.@)
 */
LPVOID WINAPI GlobalWire_k(HGLOBAL hmem)
{
   return GlobalLock_k( hmem );
}
 ...
 */
BOOL WINAPI GlobalUnWire_k(HGLOBAL hmem)
{
   return GlobalUnlock_k( hmem);
}
 ...
 *           GlobalFix   (KERNEL32.@)
 */
VOID WINAPI GlobalFix_k(HGLOBAL hmem)
{
    GlobalLock_k( hmem );
}
 ...
 */
VOID WINAPI GlobalUnfix_k(HGLOBAL hmem)
{
   GlobalUnlock_k( hmem);
}
 ...
 *
 */
UINT WINAPI GlobalFlags_k(HGLOBAL hmem)
{
   DWORD                retval;
   PGLOBAL32_INTERN     pintern;
 ...
 */
SIZE_T WINAPI GlobalCompact_k( DWORD minfree )
{
 ...
 *  and global heap.
 */
HLOCAL WINAPI LocalAlloc_k(
                UINT flags, /* [in] Allocation attributes */
                SIZE_T size /* [in] Number of bytes to allocate */
) {
    return GlobalAlloc_k( flags, size );
}
 ...
 */
SIZE_T WINAPI LocalCompact_k( UINT minfree )
{
 ...
 *  and global heap.
 */
UINT WINAPI LocalFlags_k(
              HLOCAL handle /* [in] Handle of memory object */
) {
    return GlobalFlags_k( handle );
}
 ...
 *  and global heap.
 */
HLOCAL WINAPI LocalFree_k(
                HLOCAL handle /* [in] Handle of memory object */
) {
    return GlobalFree_k( handle );
}
 ...
 *  and global heap.
 */
HLOCAL WINAPI LocalHandle_k(
                LPCVOID ptr /* [in] Address of local memory block */
) {
    return GlobalHandle_k( ptr );
}
 ...
 *  and global heap.
 */
LPVOID WINAPI LocalLock_k(
              HLOCAL handle /* [in] Address of local memory object */
) {
    return GlobalLock_k( handle );
}
 ...
 *  and global heap.
 */
HLOCAL WINAPI LocalReAlloc_k(
                HLOCAL handle, /* [in] Handle of memory object */
                SIZE_T size,   /* [in] New size of block */
                UINT flags     /* [in] How to reallocate object */
) {
    return GlobalReAlloc_k( handle, size, flags );
}
 ...
 */
SIZE_T WINAPI LocalShrink_k( HGLOBAL handle, UINT newsize )
{
 ...
 *  and global heap.
 */
SIZE_T WINAPI LocalSize_k(
              HLOCAL handle /* [in] Handle of memory object */
) {
    return GlobalSize_k( handle );
}
 ...
 *	TRUE
 */
BOOL WINAPI GlobalMemoryStatusEx_k( LPMEMORYSTATUSEX lpmemex )
{
    static MEMORYSTATUSEX	cached_memstatus;
    static time_t cache_lastchecked = 0;
    SYSTEM_INFO si;
#ifdef linux
    FILE *f;
	...
	*lpmemex = cached_memstatus;
    ...
    {
        lpmemex->dwMemoryLoad = (DWORD)((lpmemex->ullTotalPhys-lpmemex->ullAvailPhys)
 ...
 *      None
 */
VOID WINAPI GlobalMemoryStatus_k( LPMEMORYSTATUS lpBuffer )
{
    MEMORYSTATUSEX memstatus;
    OSVERSIONINFOW osver;
    IMAGE_NT_HEADERS *nt = RtlImageNtHeader_k( GetModuleHandleW_k(0) );

    /* Because GlobalMemoryStatus is identical to GlobalMemoryStatusEX save
       for one extra field in the struct, and the lack of a bug, we simply
       call GlobalMemoryStatusEx and copy the values across. */
    memstatus.dwLength = sizeof(memstatus);
    GlobalMemoryStatusEx_k(&memstatus);
    ...
    GetVersionExW_k(&osver);

    if ( osver.dwMajorVersion >= 5 || osver.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS )
    {
        lpBuffer->dwTotalPhys = (SIZE_T)(min( memstatus.ullTotalPhys, MAXDWORD ));
        lpBuffer->dwAvailPhys = (SIZE_T)(min( memstatus.ullAvailPhys, MAXDWORD ));
        /* Limit value for apps that do not expect so much memory. Remove last 512 kb to make Sacrifice demo happy. */
        lpBuffer->dwTotalPageFile = (SIZE_T)(min( memstatus.ullTotalPageFile, 0xfff7ffff ));
        lpBuffer->dwAvailPageFile = (SIZE_T)(min( memstatus.ullAvailPageFile, MAXDWORD ));
        lpBuffer->dwTotalVirtual = (SIZE_T)(min( memstatus.ullTotalVirtual, MAXDWORD ));
        lpBuffer->dwAvailVirtual = (SIZE_T)(min( memstatus.ullAvailVirtual, MAXDWORD ));

    }
    else /* duplicate NT bug */
    {
        lpBuffer->dwTotalPhys = (SIZE_T)memstatus.ullTotalPhys;
        lpBuffer->dwAvailPhys = (SIZE_T)memstatus.ullAvailPhys;
        lpBuffer->dwTotalPageFile = (SIZE_T)memstatus.ullTotalPageFile;
        lpBuffer->dwAvailPageFile = (SIZE_T)memstatus.ullAvailPageFile;
        lpBuffer->dwTotalVirtual = (SIZE_T)memstatus.ullTotalVirtual;
        lpBuffer->dwAvailVirtual = (SIZE_T)memstatus.ullAvailVirtual;
    }
...
#include "memory.h"
HANDLE WINAPI GetProcessHeap_k() {
    return (HANDLE)readd(readd(NtCurrentTeb_k()+TEB_PEB)+PEB_HEAP);
}