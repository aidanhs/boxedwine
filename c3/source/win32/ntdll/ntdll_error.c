...
{
    writed(NtCurrentTeb_k()+TEB_LAST_STATUS, status);
    return RtlNtStatusToDosErrorNoTeb_k( status );
}
...
{
    return readd(NtCurrentTeb_k()+TEB_LAST_STATUS);
}
 ...
 */
DWORD WINAPI RtlGetLastWin32Error_k(void)
{
 ...
 */
NTSTATUS WINAPI NtRaiseHardError_k( NTSTATUS ErrorStatus, ULONG NumberOfParameters,
 ...
 *  Nothing.
 */
void WINAPI RtlSetLastWin32Error_k( DWORD err )
{
    writed(NtCurrentTeb_k()+TEB_LAST_ERROR, err);
}
 ...
 *  Nothing.
 */
void WINAPI RtlSetLastWin32ErrorAndNtStatusFromNtStatus_k( NTSTATUS status )
{
    writed(NtCurrentTeb_k()+TEB_LAST_ERROR, RtlNtStatusToDosError_k( status ));
}
