                            ...
                            ULONG level, const void **ret, int want_dir )
{
    ...
    root = RtlImageDirectoryEntryToData_k( hmod, TRUE, IMAGE_DIRECTORY_ENTRY_RESOURCE, &size );
    ...
    if (PRIMARYLANGID(info->Language) == LANG_NEUTRAL)
    {
        ...
        if (SUBLANGID(info->Language) != SUBLANG_SYS_DEFAULT)
        {
            ...
            pos = push_language( list, pos, LANGIDFROMLCID(readd(NtCurrentTeb_k()+TEB_LOCALE)) );
...
#endif
{
    ...
    __TRY
    {
        ...
        if (!RtlImageDirectoryEntryToData_k( hmod, TRUE, IMAGE_DIRECTORY_ENTRY_RESOURCE, &dirsize ))
        ...
        else
        {
            if (ptr)
            {
                if (is_data_file_module(hmod))
                {
                    ...
                    *ptr = RtlImageRvaToVa_k( RtlImageNtHeader_k(mod), mod, entry->OffsetToData, NULL );
                }
