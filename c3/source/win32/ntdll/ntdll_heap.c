...
static INLINE BOOL HEAP_Commit( SUBHEAP *subheap, ARENA_INUSE *pArena, SIZE_T data_size )
{
    ...
    if (NtAllocateVirtualMemory_k( NtCurrentProcess_k(), &ptr, 0,
...
static INLINE BOOL HEAP_Decommit( SUBHEAP *subheap, void *ptr )
{
    ...
    if (NtFreeVirtualMemory_k( NtCurrentProcess_k(), &addr, &decommit_size, MEM_DECOMMIT ))
    {
...
static void HEAP_MakeInUseBlockFree( SUBHEAP *subheap, ARENA_INUSE *pArena )
{
        ...
        (subheap != &subheap->heap->subheap))
    {
        ...
        /* Free the memory */
        subheap->magic = 0;
        NtFreeVirtualMemory_k( NtCurrentProcess_k(), &addr, &size, MEM_RELEASE );
        return;
    }
...
static void *allocate_large_block( HEAP *heap, DWORD flags, SIZE_T size )
{
    ...
    if (NtAllocateVirtualMemory_k( NtCurrentProcess_k(), &address, 5,
...
static void free_large_block( HEAP *heap, DWORD flags, void *ptr )
{
    ...
    NtFreeVirtualMemory_k( NtCurrentProcess_k(), &address, &size, MEM_RELEASE );
}
                                    ...
                                    SIZE_T commitSize, SIZE_T totalSize )
{
    ...
    if (!address)
    {
        ...
        if (NtAllocateVirtualMemory_k( NtCurrentProcess_k(), &address, 5, &totalSize,
                                     MEM_RESERVE, get_protection_type( flags ) ))
        {
            WARN("Could not allocate %08lx bytes\n", totalSize );
            return NULL;
        }
        if (NtAllocateVirtualMemory_k( NtCurrentProcess_k(), &address, 0,
    ...
    else
    {
        ...
        {
            RtlInitializeCriticalSection_k( &heap->critSection );
        ...
        if (flags & HEAP_SHARED)
        {
            ...
            NtDuplicateObject_k( NtCurrentProcess_k(), sem, NtCurrentProcess_k(), &sem, 0, 0,
                               DUP_HANDLE_MAKE_GLOBAL | DUP_HANDLE_SAME_ACCESS | DUP_HANDLE_CLOSE_SOURCE );
            heap->critSection.LockSemaphore = sem;
            RtlFreeHeap_k( processHeap, 0, heap->critSection.DebugInfo );
    ...
    if (pArena->magic == ARENA_PENDING_MAGIC)
    {
        ...
        while (ptr < end)
        {
            if (*ptr != ARENA_FREE_FILLER)
            {
                ...
                if (!*ptr) { HEAP_Dump( subheap->heap ); DbgBreakPoint_k(); }
                return FALSE;
            }
            ptr++;
        }
                              ...
                              *             does not complain    */
{
    ...
    if (!(flags & HEAP_NO_SERIALIZE))
        RtlEnterCriticalSection_k( &heapPtr->critSection );
    ...
    if (block)  /* only check this single memory block */
    {
        ...
        if (!(flags & HEAP_NO_SERIALIZE))
            RtlLeaveCriticalSection_k( &heapPtr->critSection );
        return ret;
    }
    ...
    if (!(flags & HEAP_NO_SERIALIZE)) RtlLeaveCriticalSection_k( &heapPtr->critSection );
    return ret;
}
...
void heap_set_debug_flags( HANDLE handle )
{
    ...
    ULONG global_flags = RtlGetNtGlobalFlags_k();
    ULONG flags = 0;
        ...
        ((flags & HEAP_FREE_CHECKING_ENABLED) || RUNNING_ON_VALGRIND))
    {
        ...
        if (!NtAllocateVirtualMemory_k( NtCurrentProcess_k(), &ptr, 4, &size, MEM_COMMIT, PAGE_READWRITE ))
        {
 ...
 */
HANDLE WINAPI RtlCreateHeap_k( ULONG flags, PVOID addr, SIZE_T totalSize, SIZE_T commitSize,
                             PVOID unknown, PRTL_HEAP_DEFINITION definition )
{
    ...
    if (processHeap)
    {
        ...
        RtlEnterCriticalSection_k( &processHeap->critSection );
        list_add_head( &processHeap->entry, &heapPtr->entry );
        RtlLeaveCriticalSection_k( &processHeap->critSection );
    }
    else if (!addr)
    {
 ...
 */
HANDLE WINAPI RtlDestroyHeap_k( HANDLE heap )
{
    ...
    RtlDeleteCriticalSection_k( &heapPtr->critSection );
 ...
 */
PVOID WINAPI RtlAllocateHeap_k( HANDLE heap, ULONG flags, SIZE_T size )
{
    ARENA_FREE *pArena;
    ARENA_INUSE *pInUse;
    SUBHEAP *subheap;
    ...
    {
        if (flags & HEAP_GENERATE_EXCEPTIONS) RtlRaiseStatus_k( STATUS_NO_MEMORY );
        return NULL;
    }
    if (rounded_size < HEAP_MIN_DATA_SIZE) rounded_size = HEAP_MIN_DATA_SIZE;

    if (!(flags & HEAP_NO_SERIALIZE)) RtlEnterCriticalSection_k( &heapPtr->critSection );

    if (rounded_size >= HEAP_MIN_LARGE_BLOCK_SIZE && (flags & HEAP_GROWABLE))
    {
        void *ret = allocate_large_block( heap, flags, size );
        if (!(flags & HEAP_NO_SERIALIZE)) RtlLeaveCriticalSection_k( &heapPtr->critSection );
        if (!ret && (flags & HEAP_GENERATE_EXCEPTIONS)) RtlRaiseStatus_k( STATUS_NO_MEMORY );
 ...
 *  Failure: FALSE.
 */
BOOLEAN WINAPI RtlFreeHeap_k( HANDLE heap, ULONG flags, PVOID ptr )
{
    ARENA_INUSE *pInUse;
    SUBHEAP *subheap;
    HEAP *heapPtr;
    ...
    if (!heapPtr)
    {
        RtlSetLastWin32ErrorAndNtStatusFromNtStatus_k( STATUS_INVALID_HANDLE );
        return FALSE;
    }
...
error:
    if (!(flags & HEAP_NO_SERIALIZE)) RtlLeaveCriticalSection_k( &heapPtr->critSection );
    RtlSetLastWin32ErrorAndNtStatusFromNtStatus_k( STATUS_INVALID_PARAMETER );
 ...
 *  Failure: NULL.
 */
PVOID WINAPI RtlReAllocateHeap_k( HANDLE heap, ULONG flags, PVOID ptr, SIZE_T size )
{
    ARENA_INUSE *pArena;
    HEAP *heapPtr;
    SUBHEAP *subheap;
...
oom:
    if (!(flags & HEAP_NO_SERIALIZE)) RtlLeaveCriticalSection_k( &heapPtr->critSection );
    if (flags & HEAP_GENERATE_EXCEPTIONS) RtlRaiseStatus_k( STATUS_NO_MEMORY );
    RtlSetLastWin32ErrorAndNtStatusFromNtStatus_k( STATUS_NO_MEMORY );
 ...
 */
ULONG WINAPI RtlCompactHeap_k( HANDLE heap, ULONG flags )
{
 ...
 */
BOOLEAN WINAPI RtlLockHeap_k( HANDLE heap )
{
    HEAP *heapPtr = HEAP_GetPtr( heap );
 ...
 *  Failure: FALSE, if heap is invalid.
 */
BOOLEAN WINAPI RtlUnlockHeap_k( HANDLE heap )
{
    HEAP *heapPtr = HEAP_GetPtr( heap );
    if (!heapPtr) return FALSE;
    RtlLeaveCriticalSection_k( &heapPtr->critSection );
 ...
 */
SIZE_T WINAPI RtlSizeHeap_k( HANDLE heap, ULONG flags, const void *ptr )
{
    SIZE_T ret;
 ...
 */
BOOLEAN WINAPI RtlValidateHeap_k( HANDLE heap, ULONG flags, LPCVOID ptr )
{
    HEAP *heapPtr = HEAP_GetPtr( heap );
    if (!heapPtr) return FALSE;
 ...
 */
NTSTATUS WINAPI RtlWalkHeap_k( HANDLE heap, PVOID entry_ptr )
{
    ...
    if (!(heapPtr->flags & HEAP_NO_SERIALIZE)) RtlEnterCriticalSection_k( &heapPtr->critSection );
    ...
    if (ptr == (char *)currentheap->base + currentheap->headerSize)
    {
        ...
        entry->Region.dwCommittedSize = currentheap->commitSize;
        entry->Region.dwUnCommittedSize =
                currentheap->size - currentheap->commitSize;
        entry->Region.lpFirstBlock = /* first valid block */
                (char *)currentheap->base + currentheap->headerSize;
        entry->Region.lpLastBlock  = /* first invalid block */
...
HW_end:
    if (!(heapPtr->flags & HEAP_NO_SERIALIZE)) RtlLeaveCriticalSection_k( &heapPtr->critSection );
    return ret;
}
 ...
 *  Failure: 0.
 */
ULONG WINAPI RtlGetProcessHeaps_k( ULONG count, HANDLE *heaps )
{
 ...
 */
NTSTATUS WINAPI RtlQueryHeapInformation_k( HANDLE heap, HEAP_INFORMATION_CLASS info_class,
