 ...
 *  of source is assumed to be 0.
 */
NTSTATUS WINAPI RtlInitAnsiStringEx_k(PANSI_STRING target, PCSZ source)
{
    if (source)
    {
 ...
 */
void WINAPI RtlFreeAnsiString_k( PSTRING str )
{
    if (str->Buffer)
    {
        RtlFreeHeap_k( GetProcessHeap_k(), 0, str->Buffer );
        RtlZeroMemory_k( str, sizeof(*str) );
    }
}
 ...
 */
void WINAPI RtlFreeOemString_k( PSTRING str )
{
    RtlFreeAnsiString_k( str );
}
 ...
 *	RtlCopyString   (NTDLL.@)
 */
void WINAPI RtlCopyString_k( STRING *dst, const STRING *src )
{
    if (src)
    {
 ...
 *  of source is assumed to be 0.
 */
void WINAPI RtlInitUnicodeString_k(
 ...
 *  of source is assumed to be 0.
 */
NTSTATUS WINAPI RtlInitUnicodeStringEx_k(
    PUNICODE_STRING target, /* [I/O] Buffered unicode string to be initialized */
    PCWSTR source)          /* [I]   '\0' terminated unicode string used to initialize target */
{
    if (source != NULL) {
 ...
 *     Failure: FALSE
 */
BOOLEAN WINAPI RtlCreateUnicodeString_k( PUNICODE_STRING target, LPCWSTR src )
{
    int len = (strlenW(src) + 1) * sizeof(WCHAR);
    if (!(target->Buffer = RtlAllocateHeap_k( GetProcessHeap_k(), 0, len ))) return FALSE;
 ...
 *     Failure: FALSE
 */
BOOLEAN WINAPI RtlCreateUnicodeStringFromAsciiz_k( PUNICODE_STRING target, LPCSTR src )
{
    STRING ansi;
    RtlInitAnsiString( &ansi, src );
    return !RtlAnsiStringToUnicodeString_k( target, &ansi, TRUE );
}
 ...
 *     nothing
 */
void WINAPI RtlFreeUnicodeString_k( PUNICODE_STRING str )
{
    if (str->Buffer)
    {
        RtlFreeHeap_k( GetProcessHeap_k(), 0, str->Buffer );
 ...
 *     nothing
 */
void WINAPI RtlCopyUnicodeString_k( UNICODE_STRING *dst, const UNICODE_STRING *src )
{
    if (src)
    {
        unsigned int len = min( src->Length, dst->MaximumLength );
 ...
 */
NTSTATUS WINAPI RtlDuplicateUnicodeString_k(
    int add_nul,                  /* [I] flag */
    ...
    UNICODE_STRING *destination)  /* [O] destination for the duplicated unicode string */
{
    if (source == NULL || destination == NULL ||
        ...
        if (source->Length == 0 && add_nul != 3) {
            ...
            destination->Buffer = (PWSTR)RtlAllocateHeap_k(GetProcessHeap_k(), 0, destination_max_len);
 ...
 *     nothing
 */
void WINAPI RtlEraseUnicodeString_k( UNICODE_STRING *str )
{
    if (str->Buffer)
    {
 ...
 */
LONG WINAPI RtlCompareString_k( const STRING *s1, const STRING *s2, BOOLEAN CaseInsensitive )
{
    unsigned int len;
    LONG ret = 0;
    LPCSTR p1, p2;
    ...
    if (CaseInsensitive)
    {
        while (!ret && len--) ret = RtlUpperChar_k(*p1++) - RtlUpperChar_k(*p2++);
    }
    else
    {
 ...
 */
LONG WINAPI RtlCompareUnicodeString_k( const UNICODE_STRING *s1, const UNICODE_STRING *s2,
 ...
 */
BOOLEAN WINAPI RtlEqualString_k( const STRING *s1, const STRING *s2, BOOLEAN CaseInsensitive )
{
    if (s1->Length != s2->Length) return FALSE;
    return !RtlCompareString_k( s1, s2, CaseInsensitive );
}
 ...
 */
BOOLEAN WINAPI RtlEqualUnicodeString_k( const UNICODE_STRING *s1, const UNICODE_STRING *s2,
                                      BOOLEAN CaseInsensitive )
{
    if (s1->Length != s2->Length) return FALSE;
    return !RtlCompareUnicodeString_k( s1, s2, CaseInsensitive );
}
 ...
 */
BOOLEAN WINAPI RtlPrefixString_k( const STRING *s1, const STRING *s2, BOOLEAN ignore_case )
{
    unsigned int i;
    ...
    if (ignore_case)
    {
        for (i = 0; i < s1->Length; i++)
            if (RtlUpperChar_k(s1->Buffer[i]) != RtlUpperChar_k(s2->Buffer[i])) return FALSE;
    }
    else
    {
        for (i = 0; i < s1->Length; i++)
 ...
 */
BOOLEAN WINAPI RtlPrefixUnicodeString_k( const UNICODE_STRING *s1,
 ...
 */
NTSTATUS WINAPI RtlEqualComputerName_k(const UNICODE_STRING *left,
                                     const UNICODE_STRING *right)
{
    NTSTATUS ret;
    STRING upLeft, upRight;

    if (!(ret = RtlUpcaseUnicodeStringToOemString_k( &upLeft, left, TRUE )))
    {
       if (!(ret = RtlUpcaseUnicodeStringToOemString_k( &upRight, right, TRUE )))
       {
         ret = RtlEqualString_k( &upLeft, &upRight, FALSE );
         RtlFreeOemString_k( &upRight );
       }
       RtlFreeOemString_k( &upLeft );
    }
    return ret;
}
 ...
 *  The comparison is case insensitive.
 */
NTSTATUS WINAPI RtlEqualDomainName_k(const UNICODE_STRING *left,
                                   const UNICODE_STRING *right)
{
    return RtlEqualComputerName_k(left, right);
}
 ...
 */
WCHAR WINAPI RtlAnsiCharToUnicodeChar_k(LPSTR *ansi)
{
    WCHAR str;
    ...
    RtlMultiByteToUnicodeN_k(&str, sizeof(WCHAR), NULL, *ansi, charSize);
    *ansi += charSize;
 ...
 */
NTSTATUS WINAPI RtlAnsiStringToUnicodeString_k(
...
{
    DWORD total = RtlAnsiStringToUnicodeSize_k( ansi );

    if (total > 0xffff) return STATUS_INVALID_PARAMETER_2;
    uni->Length = (USHORT)(total - sizeof(WCHAR));
    if (doalloc)
    {
        uni->MaximumLength = (USHORT)total;
        if (!(uni->Buffer = RtlAllocateHeap_k( GetProcessHeap_k(), 0, total )))
            return STATUS_NO_MEMORY;
    }
    else if (total > uni->MaximumLength) return STATUS_BUFFER_OVERFLOW;

    RtlMultiByteToUnicodeN_k( uni->Buffer, uni->Length, NULL, ansi->Buffer, ansi->Length );
    ...
    BOOLEAN doalloc)     /* [I]   TRUE=Allocate new buffer for uni, FALSE=Use existing buffer */
{
    ...
    RtlOemToUnicodeN_k( uni->Buffer, uni->Length, NULL, oem->Buffer, oem->Length );
    uni->Buffer[uni->Length / sizeof(WCHAR)] = 0;
    return STATUS_SUCCESS;
}
 ...
 */
NTSTATUS WINAPI RtlUnicodeStringToAnsiString_k(
    ...
    BOOLEAN doalloc)           /* [I]   TRUE=Allocate new buffer for ansi, FALSE=Use existing buffer */
{
    ...
    DWORD len = RtlUnicodeStringToAnsiSize_k( uni );

    ansi->Length = (USHORT)(len - 1);
    if (doalloc)
    {
        ansi->MaximumLength = (USHORT)len;
        if (!(ansi->Buffer = RtlAllocateHeap_k( GetProcessHeap_k(), 0, len )))
            return STATUS_NO_MEMORY;
    }
    ...
    RtlUnicodeToMultiByteN_k( ansi->Buffer, ansi->Length, NULL, uni->Buffer, uni->Length );
 ...
 */
NTSTATUS WINAPI RtlUnicodeStringToOemString_k( STRING *oem,
                                             const UNICODE_STRING *uni,
                                             BOOLEAN doalloc )
{
    NTSTATUS ret = STATUS_SUCCESS;
    DWORD len = RtlUnicodeStringToOemSize_k( uni );

    oem->Length = (USHORT)(len - 1);
    if (doalloc)
    {
        oem->MaximumLength = (USHORT)len;
        if (!(oem->Buffer = RtlAllocateHeap_k( GetProcessHeap_k(), 0, len )))
            return STATUS_NO_MEMORY;
    }
    ...
    RtlUnicodeToOemN_k( oem->Buffer, oem->Length, NULL, uni->Buffer, uni->Length );
 ...
 */
NTSTATUS WINAPI RtlMultiByteToUnicodeN_k( LPWSTR dst, DWORD dstlen, LPDWORD reslen,
 ...
 *  NTSTATUS code
 */
NTSTATUS WINAPI RtlOemToUnicodeN_k( LPWSTR dst, DWORD dstlen, LPDWORD reslen,
                                  LPCSTR src, DWORD srclen )
{
 ...
 *  NTSTATUS code
 */
NTSTATUS WINAPI RtlUnicodeToMultiByteN_k( LPSTR dst, DWORD dstlen, LPDWORD reslen,
 ...
 *  NTSTATUS code
 */
NTSTATUS WINAPI RtlUnicodeToOemN_k( LPSTR dst, DWORD dstlen, LPDWORD reslen,
                                  LPCWSTR src, DWORD srclen )
{
 ...
 */
CHAR WINAPI RtlUpperChar_k( CHAR ch )
{
    if (ch >= 'a' && ch <= 'z') {
        return ch - 'a' + 'A';
    } else {
 ...
 */
void WINAPI RtlUpperString_k( STRING *dst, const STRING *src )
{
    unsigned int i, len = min(src->Length, dst->MaximumLength);

    for (i = 0; i < len; i++) dst->Buffer[i] = RtlUpperChar_k(src->Buffer[i]);
    dst->Length = len;
}
 ...
 *  The uppercase character value.
 */
WCHAR WINAPI RtlUpcaseUnicodeChar_k( WCHAR wch )
{
    return toupperW(wch);
}
 ...
 */
WCHAR WINAPI RtlDowncaseUnicodeChar_k(WCHAR wch)
{
    return tolowerW(wch);
}
 ...
 */
NTSTATUS WINAPI RtlUpcaseUnicodeString_k( UNICODE_STRING *dest,
                                        ...
                                        BOOLEAN doalloc)
{
    ...
    if (doalloc)
    {
        ...
        if (!(dest->Buffer = RtlAllocateHeap_k( GetProcessHeap_k(), 0, len )))
            return STATUS_NO_MEMORY;
    }
 ...
 *  might not be '\0' terminated. dest->Length is only set upon success.
 */
NTSTATUS WINAPI RtlDowncaseUnicodeString_k(
    UNICODE_STRING *dest,
    const UNICODE_STRING *src,
    BOOLEAN doalloc)
{
 ...
 */
NTSTATUS WINAPI RtlUpcaseUnicodeStringToAnsiString_k( STRING *dst,
                                                    const UNICODE_STRING *src,
                                                    BOOLEAN doalloc )
{
    NTSTATUS ret;
    ...
    if (!(ret = RtlUpcaseUnicodeString_k( &upcase, src, TRUE )))
    {
        ret = RtlUnicodeStringToAnsiString_k( dst, &upcase, doalloc );
        RtlFreeUnicodeString_k( &upcase );
    }
    return ret;
}
 ...
 *  writes terminating 0
 */
NTSTATUS WINAPI RtlUpcaseUnicodeStringToOemString_k( STRING *dst,
                                                   const UNICODE_STRING *src,
                                                   BOOLEAN doalloc )
{
    NTSTATUS ret;
    ...
    if (!(ret = RtlUpcaseUnicodeString_k( &upcase, src, TRUE )))
    {
        ret = RtlUnicodeStringToOemString_k( dst, &upcase, doalloc );
        RtlFreeUnicodeString_k( &upcase );
    }
    return ret;
}
 ...
 */
NTSTATUS WINAPI RtlUpcaseUnicodeStringToCountedOemString_k( STRING *oem,
                                                          const UNICODE_STRING *uni,
                                                          BOOLEAN doalloc )
{
    NTSTATUS ret;
    ...
    ret = RtlUpcaseUnicodeString_k( &upcase, uni, FALSE );
    if (ret == STATUS_BUFFER_OVERFLOW) ret = RtlUpcaseUnicodeString_k( &upcase, uni, TRUE );

    if (!ret)
    {
        DWORD len = RtlUnicodeStringToOemSize_k( &upcase ) - 1;
        oem->Length = (USHORT)len;
        if (doalloc)
        {
            oem->MaximumLength = (USHORT)len;
            if (!(oem->Buffer = RtlAllocateHeap_k( GetProcessHeap_k(), 0, len )))
        ...
        RtlUnicodeToOemN_k( oem->Buffer, oem->Length, NULL, upcase.Buffer, upcase.Length );
    done:
        if (upcase.Buffer != tmp) RtlFreeUnicodeString_k( &upcase );
    }
    return ret;
}
 ...
 *  NTSTATUS code
 */
NTSTATUS WINAPI RtlUpcaseUnicodeToMultiByteN_k( LPSTR dst, DWORD dstlen, LPDWORD reslen,
                                              LPCWSTR src, DWORD srclen )
{
    NTSTATUS ret;
    LPWSTR upcase;
    ...
    if (!(upcase = RtlAllocateHeap_k( GetProcessHeap_k(), 0, srclen ))) return STATUS_NO_MEMORY;
    for (i = 0; i < srclen/sizeof(WCHAR); i++) upcase[i] = toupperW(src[i]);
    ret = RtlUnicodeToMultiByteN_k( dst, dstlen, reslen, upcase, srclen );
    RtlFreeHeap_k( GetProcessHeap_k(), 0, upcase );
    return ret;
}
 ...
 *  NTSTATUS code
 */
NTSTATUS WINAPI RtlUpcaseUnicodeToOemN_k( LPSTR dst, DWORD dstlen, LPDWORD reslen,
                                        LPCWSTR src, DWORD srclen )
{
    NTSTATUS ret;
    LPWSTR upcase;
    ...
    if (!(upcase = RtlAllocateHeap_k( GetProcessHeap_k(), 0, srclen ))) return STATUS_NO_MEMORY;
    for (i = 0; i < srclen/sizeof(WCHAR); i++) upcase[i] = toupperW(src[i]);
    ret = RtlUnicodeToOemN_k( dst, dstlen, reslen, upcase, srclen );
    RtlFreeHeap_k( GetProcessHeap_k(), 0, upcase );
    return ret;
}
 ...
 *  The calculated size.
 */
UINT WINAPI RtlOemStringToUnicodeSize_k( const STRING *str )
{
 ...
 *  The calculated size.
 */
DWORD WINAPI RtlAnsiStringToUnicodeSize_k( const STRING *str )
{
    DWORD ret;
    RtlMultiByteToUnicodeSize_k( &ret, str->Buffer, str->Length );
 ...
 *  STATUS_SUCCESS.
 */
NTSTATUS WINAPI RtlMultiByteToUnicodeSize_k( DWORD *size, LPCSTR str, UINT len )
{
 ...
 *  STATUS_SUCCESS.
 */
NTSTATUS WINAPI RtlUnicodeToMultiByteSize_k( PULONG size, LPCWSTR str, ULONG len )
{
 ...
 *  The calculated size.
 */
DWORD WINAPI RtlUnicodeStringToAnsiSize_k( const UNICODE_STRING *str )
{
    DWORD ret;
    RtlUnicodeToMultiByteSize_k( &ret, str->Buffer, str->Length );
    return ret + 1;
}
 ...
 *  The calculated size.
 */
DWORD WINAPI RtlUnicodeStringToOemSize_k( const UNICODE_STRING *str )
{
 ...
 */
NTSTATUS WINAPI RtlAppendAsciizToString_k(
 ...
 *  dest is never '\0' terminated.
 */
NTSTATUS WINAPI RtlAppendStringToString_k(
    STRING *dest,       /* [I/O] Buffered character string to which src is concatenated */
 ...
 */
NTSTATUS WINAPI RtlAppendUnicodeToString_k(
 ...
 *  MaximumLength is odd as the native function does.
 */
NTSTATUS WINAPI RtlAppendUnicodeStringToString_k(
    UNICODE_STRING *dest,      /* [I/O] Buffered unicode string to which src is concatenated */
 ...
 */
NTSTATUS WINAPI RtlFindCharInUnicodeString_k(
    int flags,                          /* [I] Flags */
 ...
 */
BOOLEAN WINAPI RtlIsTextUnicode_k( LPCVOID buf, INT len, INT *pf )
{
 ...
 */
NTSTATUS WINAPI RtlCharToInteger_k(
    ...
    ULONG *value)  /* [O] Destination for the converted value */
{
	...
	if (digit < 0 || digit >= (int)base) {
	    *value = bMinus ? (ULONG)(-(LONG)RunningTotal) : RunningTotal;
	    return STATUS_SUCCESS;
	} /* if */
 ...
 */
NTSTATUS WINAPI RtlIntegerToChar_k(
 ...
 */
NTSTATUS WINAPI RtlIntegerToUnicodeString_k(
    ULONG value,         /* [I] Value to be converted */
    ULONG base,          /* [I] Number base for conversion (allowed 0, 2, 8, 10 or 16) */
 ...
 */
NTSTATUS WINAPI RtlGUIDFromString_k(PUNICODE_STRING str, GUID* guid)
{
  int i = 0;
 ...
 */
NTSTATUS WINAPI RtlStringFromGUID_k(const GUID* guid, UNICODE_STRING *str)
{
  ...
  str->Buffer = RtlAllocateHeap_k(GetProcessHeap_k(), 0, str->MaximumLength);
  if (!str->Buffer)
  {
