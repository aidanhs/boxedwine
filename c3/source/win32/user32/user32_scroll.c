...
static SCROLLBAR_INFO *SCROLL_GetInternalInfo( HWND hwnd, INT nBar, BOOL alloc )
{
    ...
    if (!wndPtr || wndPtr == WND_DESKTOP) return NULL;
    switch(nBar)
    {
        case SB_HORZ:
    ...
    if (!infoPtr && alloc)
    {
        ...
        else if ((winInfoPtr = HeapAlloc_k( GetProcessHeap_k(), 0, sizeof(WINSCROLLBAR_INFO) )))
        {
            /* Set default values */
                                     ...
                                     INT *thumbPos )
{
    ...
    switch(nBar)
    {
      case SB_HORZ:
        ...
        lprect->bottom += GetSystemMetrics_k(SM_CYHSCROLL);
	...
	break;
      ...
      case SB_VERT:
        ...
        if((wndPtr->dwExStyle & WS_EX_LEFTSCROLLBAR) != 0)
        {
            ...
            lprect->left -= GetSystemMetrics_k(SM_CXVSCROLL);
        }
        else
        {
            lprect->left = lprect->right;
            lprect->right += GetSystemMetrics_k(SM_CXVSCROLL);
        }
    ...
    if (pixels <= 2*GetSystemMetrics_k(SM_CXVSCROLL) + SCROLL_MIN_RECT)
    {
    ...
    else
    {
        ...
        *arrowSize = GetSystemMetrics_k(SM_CXVSCROLL);
        pixels -= (2 * (GetSystemMetrics_k(SM_CXVSCROLL) - SCROLL_ARROW_THUMB_OVERLAP));

        if (info->page)
        {
	    *thumbSize = MulDiv_k(pixels,info->page,(info->maxVal-info->minVal+1));
            if (*thumbSize < SCROLL_MIN_THUMB) *thumbSize = SCROLL_MIN_THUMB;
        }
        else *thumbSize = GetSystemMetrics_k(SM_CXVSCROLL);
        ...
        else
        {
            ...
            if (info->minVal >= max)
                ...
                *thumbPos = *arrowSize - SCROLL_ARROW_THUMB_OVERLAP
		  + MulDiv_k(pixels, (info->curVal-info->minVal),(max - info->minVal));
        }
    }
    return vertical;
}
                                  ...
                                  BOOL vertical, INT pos )
{
    ...
    if ((pixels -= 2*(GetSystemMetrics_k(SM_CXVSCROLL) - SCROLL_ARROW_THUMB_OVERLAP)) <= 0)
        return infoPtr->minVal;

    if (infoPtr->page)
    {
        thumbSize = MulDiv_k(pixels,infoPtr->page,(infoPtr->maxVal-infoPtr->minVal+1));
        if (thumbSize < SCROLL_MIN_THUMB) thumbSize = SCROLL_MIN_THUMB;
    }
    else thumbSize = GetSystemMetrics_k(SM_CXVSCROLL);
    ...
    pos = max( 0, pos - (GetSystemMetrics_k(SM_CXVSCROLL) - SCROLL_ARROW_THUMB_OVERLAP) );
    ...
    return infoPtr->minVal + MulDiv_k(pos, range, pixels);
}
                                           ...
                                           POINT pt, BOOL bDragging )
{
    ...
    if ( (bDragging && !SCROLL_PtInRectEx( &rect, pt, vertical )) ||
	 (!PtInRect_k( &rect, pt )) ) return SCROLL_NOWHERE;
                               ...
                               BOOL top_pressed, BOOL bottom_pressed )
{
  ...
  DrawFrameControl_k( hdc, &r, DFC_SCROLL,
				    ...
				    BOOL top_selected, BOOL bottom_selected )
{
    ...
    if (nBar == SB_CTL)
    {
      hBrush = (HBRUSH)SendMessageW_k( GetParent_k(hwnd), WM_CTLCOLORSCROLLBAR,
    ...
    hSavePen = SelectObject_k( hdc, SYSCOLOR_GetPen(COLOR_WINDOWFRAME) );
    hSaveBrush = SelectObject_k( hdc, hBrush );
    ...
    {
        PatBlt_k( hdc, r.left, r.top,
                     r.right - r.left, r.bottom - r.top,
                     PATCOPY );

        /* cleanup and return */
        SelectObject_k( hdc, hSavePen );
        SelectObject_k( hdc, hSaveBrush );
        return;
    }
    ...
    if (vertical)
    {
        ...
        PatBlt_k( hdc, r.left, r.top + thumbSize,
                  r.right - r.left,
    ...
    else  /* horizontal */
    {
        ...
        PatBlt_k( hdc, r.left + thumbSize, r.top,
    ...
    /* Draw the thumb */
    DrawEdge_k( hdc, &r, EDGE_RAISED, BF_RECT | BF_MIDDLE  );
                                 ...
                                 BOOL top_selected, BOOL bottom_selected )
{
    ...
    if (!thumbPos)  /* No thumb to draw */
    {
        PatBlt_k( hdc, r.left, r.top, r.right - r.left, r.bottom - r.top, PATCOPY );
    ...
    if (vertical)
    {
        PatBlt_k( hdc, r.left, r.top, r.right - r.left,
                thumbPos - (arrowSize - SCROLL_ARROW_THUMB_OVERLAP),
                top_selected ? 0x0f0000 : PATCOPY );
        r.top += thumbPos - (arrowSize - SCROLL_ARROW_THUMB_OVERLAP);
        PatBlt_k( hdc, r.left, r.top + thumbSize, r.right - r.left,
                r.bottom - r.top - thumbSize,
                bottom_selected ? 0x0f0000 : PATCOPY );
        r.bottom = r.top + thumbSize;
    }
    else  /* horizontal */
    {
        ...
        r.left += thumbPos - (arrowSize - SCROLL_ARROW_THUMB_OVERLAP);
        PatBlt_k( hdc, r.left + thumbSize, r.top, r.right - r.left - thumbSize,
    ...
    SelectObject_k( hdc, GetSysColorBrush_k(COLOR_BTNFACE) );
    Rectangle_k( hdc, r.left+1, r.top+1, r.right-1, r.bottom-1 );
    DrawEdge_k( hdc, &r, EDGE_RAISED, BF_RECT );
			   ...
			   BOOL arrows, BOOL interior )
{
    ...
    if (arrows && arrowSize)
    {
	if( vertical == SCROLL_trackVertical && GetCapture_k() == hwnd )
    ...
    if(hwnd==GetFocus_k() && (nBar==SB_CTL))
    {
        if (!vertical)
        {
            SetCaretPos_k(thumbPos+1, rect.top+1);
        }
        else
        {
            SetCaretPos_k(rect.top+1, thumbPos+1);
        }
    }
}
...
static void SCROLL_DrawSizeGrip( HWND hwnd,  HDC hdc)
{
    ...
    FillRect_k( hdc, &rc, GetSysColorBrush_k(COLOR_SCROLLBAR) );
    rc.left = max( rc.left, rc.right - GetSystemMetrics_k(SM_CXVSCROLL) - 1 );
    rc.top  = max( rc.top, rc.bottom - GetSystemMetrics_k(SM_CYHSCROLL) - 1 );
    DrawFrameControl_k( hdc, &rc, DFC_SCROLL, DFCS_SCROLLSIZEGRIP );
}
...
static void SCROLL_HandleKbdEvent(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    SendMessageW_k(GetParent_k(hwnd),
        ((GetWindowLongW_k( hwnd, GWL_STYLE ) & SBS_VERT) ?
...
static void SCROLL_HandleScrollEvent( HWND hwnd, INT nBar, UINT msg, POINT pt)
{
    ...
    if (nBar == SB_CTL && (GetWindowLongW_k( hwnd, GWL_STYLE ) & (SBS_SIZEGRIP | SBS_SIZEBOX)))
    {
        switch(msg)
        {
            case WM_LBUTTONDOWN:  /* Initialise mouse tracking */
                HideCaret_k(hwnd);  /* hide caret while holding down LBUTTON */
                SetCapture_k( hwnd );
                prevPt = pt;
                SCROLL_trackHitTest  = hittest = SCROLL_THUMB;
                break;
            case WM_MOUSEMOVE:
                GetClientRect_k(GetParent_k(GetParent_k(hwnd)),&rect);
                prevPt = pt;
                break;
            case WM_LBUTTONUP:
                ReleaseCapture_k();
                SCROLL_trackHitTest  = hittest = SCROLL_NOWHERE;
                if (hwnd==GetFocus_k()) ShowCaret_k(hwnd);
                break;
            case WM_SYSTIMER:
                pt = prevPt;
                break;
    ...
    hdc = GetDCEx_k( hwnd, 0, DCX_CACHE | ((nBar == SB_CTL) ? 0 : DCX_WINDOW));
    vertical = SCROLL_GetScrollBarRect( hwnd, nBar, &rect,
                                        &arrowSize, &thumbSize, &thumbPos );
    hwndOwner = (nBar == SB_CTL) ? GetParent_k(hwnd) : hwnd;
    ...
    switch(msg)
    {
      case WM_LBUTTONDOWN:  /* Initialise mouse tracking */
          ...
          prevPt = pt;
          if (nBar == SB_CTL && (GetWindowLongW_k(hwnd, GWL_STYLE) & WS_TABSTOP)) SetFocus_k( hwnd );
          SetCapture_k( hwnd );
          break;
    ...
    case SCROLL_TOP_ARROW:
        ...
        if (hittest == SCROLL_trackHitTest)
        {
            ...
            {
                SendMessageW_k( hwndOwner, vertical ? WM_VSCROLL : WM_HSCROLL,
                                SB_LINEUP, (LPARAM)hwndCtl );
	    }

	    SetSystemTimer_k( hwnd, SCROLL_TIMER, (msg == WM_LBUTTONDOWN) ?
                            SCROLL_FIRST_DELAY : SCROLL_REPEAT_DELAY, NULL );
        }
        else KillSystemTimer_k( hwnd, SCROLL_TIMER );
        break;
...
void SCROLL_TrackScrollBar( HWND hwnd, INT scrollbar, POINT pt )
{
    ...
    do
    {
        ...
        if (CallMsgFilterW_k( &msg, MSGF_SCROLLBAR )) continue;
    ...
    } while (msg.message != WM_LBUTTONUP && GetCapture_k() == hwnd);
}
...
static void SCROLL_CreateScrollBar(HWND hwnd, LPCREATESTRUCTW lpCreate)
{
    ...
    if (lpCreate->style & (SBS_SIZEGRIP | SBS_SIZEBOX))
    {
        if (lpCreate->style & SBS_SIZEBOXTOPLEFTALIGN)
            MoveWindow_k( hwnd, lpCreate->x, lpCreate->y, GetSystemMetrics_k(SM_CXVSCROLL)+1,
                        GetSystemMetrics_k(SM_CYHSCROLL)+1, FALSE );
        else if(lpCreate->style & SBS_SIZEBOXBOTTOMRIGHTALIGN)
            MoveWindow_k( hwnd, lpCreate->x+lpCreate->cx-GetSystemMetrics_k(SM_CXVSCROLL)-1, 
                        lpCreate->y+lpCreate->cy-GetSystemMetrics_k(SM_CYHSCROLL)-1,
                        GetSystemMetrics_k(SM_CXVSCROLL)+1,
                        GetSystemMetrics_k(SM_CYHSCROLL)+1, FALSE );
    }
    else if (lpCreate->style & SBS_VERT)
    {
        if (lpCreate->style & SBS_LEFTALIGN)
            MoveWindow_k( hwnd, lpCreate->x, lpCreate->y,
                        GetSystemMetrics_k(SM_CXVSCROLL)+1, lpCreate->cy, FALSE );
        else if (lpCreate->style & SBS_RIGHTALIGN)
            MoveWindow_k( hwnd,
                        lpCreate->x+lpCreate->cx-GetSystemMetrics_k(SM_CXVSCROLL)-1,
                        lpCreate->y,
                        GetSystemMetrics_k(SM_CXVSCROLL)+1, lpCreate->cy, FALSE );
    }
    else  /* SBS_HORZ */
    {
        if (lpCreate->style & SBS_TOPALIGN)
            MoveWindow_k( hwnd, lpCreate->x, lpCreate->y,
                        lpCreate->cx, GetSystemMetrics_k(SM_CYHSCROLL)+1, FALSE );
...
static BOOL SCROLL_GetScrollBarInfo(HWND hwnd, LONG idObject, LPSCROLLBARINFO info)
{
    ...
    OffsetRect_k(&info->rcScrollBar, rect.left, rect.top);
    ...
    if (nBar == SB_CTL && !IsWindowEnabled_k(hwnd))
        info->rgstate[0] |= STATE_SYSTEM_UNAVAILABLE;
    
    pressed = ((nBar == SB_VERT) == SCROLL_trackVertical && GetCapture_k() == hwnd);
...
LRESULT ScrollBarWndProc_common( HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam, BOOL unicode )
{
    ...
    case WM_LBUTTONDOWN:
        if (GetWindowLongW_k( hwnd, GWL_STYLE ) & SBS_SIZEGRIP)
        {
            SendMessageW_k( GetParent_k(hwnd), WM_SYSCOMMAND,
                          SC_SIZE + ((GetWindowLongW_k( hwnd, GWL_EXSTYLE ) & WS_EX_LAYOUTRTL) ?
    ...
    case WM_SETFOCUS:
        {
            ...
            if (!vertical)
            {
                CreateCaret_k(hwnd, (HBITMAP)1, thumbSize-2, rect.bottom-rect.top-2);
                SetCaretPos_k(thumbPos+1, rect.top+1);
            }
            else
            {
                CreateCaret_k(hwnd, (HBITMAP)1, rect.right-rect.left-2,thumbSize-2);
                SetCaretPos_k(rect.top+1, thumbPos+1);
            }
            ShowCaret_k(hwnd);
        }
    ...
    case WM_PAINT:
        {
            ...
            else if (GetWindowLongW_k( hwnd, GWL_STYLE ) & SBS_SIZEBOX)
            {
                RECT rc;
                GetClientRect_k( hwnd, &rc );
                FillRect_k( hdc, &rc, GetSysColorBrush_k(COLOR_SCROLLBAR) );
    ...
    case WM_SETCURSOR:
        if (GetWindowLongW_k( hwnd, GWL_STYLE ) & SBS_SIZEGRIP)
        {
            ULONG_PTR cursor = (GetWindowLongW_k( hwnd, GWL_EXSTYLE ) & WS_EX_LAYOUTRTL) ? IDC_SIZENESW : IDC_SIZENWSE;
            return (LRESULT)SetCursor_k( LoadCursorA_k( 0, (LPSTR)cursor ));
        }
        return DefWindowProcW_k( hwnd, message, wParam, lParam );

    case SBM_SETPOS:
        return SetScrollPos_k( hwnd, SB_CTL, wParam, (BOOL)lParam );
    ...
    case SBM_ENABLE_ARROWS:
        return EnableScrollBar_k( hwnd, SB_CTL, wParam );
 ...
 */
INT WINAPI SetScrollInfo_k(HWND hwnd, INT nBar, const SCROLLINFO *info, BOOL bRedraw)
{
    TRACE("hwnd=%p nBar=%d info=%p, bRedraw=%d\n", hwnd, nBar, info, bRedraw);

    /* Refer SB_CTL requests to the window */
    if (nBar == SB_CTL)
        return SendMessageW_k(hwnd, SBM_SETSCROLLINFO, bRedraw, (LPARAM)info);
    else
 ...
 *  is filled)
 */
BOOL WINAPI GetScrollInfo_k(HWND hwnd, INT nBar, LPSCROLLINFO info)
{
    ...
    if (nBar == SB_CTL)
    {
        SendMessageW_k(hwnd, SBM_GETSCROLLINFO, 0, (LPARAM)info);
        return TRUE;
    }
 ...
 *  TRUE if success
 */
BOOL WINAPI GetScrollBarInfo_k(HWND hwnd, LONG idObject, LPSCROLLBARINFO info)
{
    TRACE("hwnd=%p idObject=%d info=%p\n", hwnd, idObject, info);

    /* Refer OBJID_CLIENT requests to the window */
    if (idObject == OBJID_CLIENT)
        return SendMessageW_k(hwnd, SBM_GETSCROLLBARINFO, 0, (LPARAM)info);
    else
 ...
 */
INT WINAPI SetScrollPos_k( HWND hwnd, INT nBar, INT nPos, BOOL bRedraw)
{
    SCROLLINFO info;
    SCROLLBAR_INFO *infoPtr;
    INT oldPos;
    ...
    info.nPos   = nPos;
    info.fMask  = SIF_POS;
    SetScrollInfo_k( hwnd, nBar, &info, bRedraw );
    return oldPos;
}
 ...
 *    to make sure there was an error (and to know which one).
 */
INT WINAPI GetScrollPos_k(HWND hwnd, INT nBar)
{
    TRACE("hwnd=%p nBar=%d\n", hwnd, nBar);

    /* Refer SB_CTL requests to the window */
    if (nBar == SB_CTL)
        return SendMessageW_k(hwnd, SBM_GETPOS, 0, 0);
    else
 ...
 *    Failure: FALSE
 */
BOOL WINAPI SetScrollRange_k(HWND hwnd, INT nBar, INT minVal, INT maxVal, BOOL bRedraw)
{
    SCROLLINFO info;
 ...
 */
BOOL WINAPI GetScrollRange_k(HWND hwnd, INT nBar, LPINT lpMin, LPINT lpMax)
{
    TRACE("hwnd=%p nBar=%d lpMin=%p lpMax=%p\n", hwnd, nBar, lpMin, lpMax);

    /* Refer SB_CTL requests to the window */
    if (nBar == SB_CTL)
        SendMessageW_k(hwnd, SBM_GETRANGE, (WPARAM)lpMin, (LPARAM)lpMax);
    else
...
static BOOL SCROLL_ShowScrollBar( HWND hwnd, INT nBar, BOOL fShowH, BOOL fShowV )
{
    ...
    {
    case SB_CTL:
        ShowWindow_k( hwnd, fShowH ? SW_SHOW : SW_HIDE );
        return TRUE;
 ...
 *    Failure: FALSE
 */
BOOL WINAPI ShowScrollBar_k(HWND hwnd, INT nBar, BOOL fShow)
{
    if ( !hwnd )
        return FALSE;
 ...
 */
BOOL WINAPI EnableScrollBar_k( HWND hwnd, UINT nBar, UINT flags )
{
    BOOL bFineWithMe;
    SCROLLBAR_INFO *infoPtr;
    ...
    if (nBar == SB_CTL && (flags == ESB_DISABLE_BOTH || flags == ESB_ENABLE_BOTH))
        EnableWindow_k(hwnd, flags == ESB_ENABLE_BOTH);
