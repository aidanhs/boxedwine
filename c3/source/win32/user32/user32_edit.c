...
#define EDIT_NOTIFY_PARENT(es, wNotifyCode) \
	...
	{ /* Notify parent which has created this edit control */ \
	    ...
	    SendMessageW_k(es->hwndParent, WM_COMMAND, \
		     MAKEWPARAM(GetWindowLongPtrW_k((es->hwndSelf),GWLP_ID), wNotifyCode), \
		     (LPARAM)(es->hwndSelf)); \
	} while(0)
...
static DWORD get_app_version(void)
{
    ...
    if (!version)
    {
        ...
        OSVERSIONINFOW info;
        DWORD dwProcVersion = GetProcessVersion_k(0);

	info.dwOSVersionInfoSize = sizeof(OSVERSIONINFOW);
        GetVersionExW_k( &info );
...
static HBRUSH EDIT_NotifyCtlColor(EDITSTATE *es, HDC hdc)
{
	...
	/* why do we notify to es->hwndParent, and we send this one to GetParent()? */
        hbrush = (HBRUSH)SendMessageW_k(GetParent_k(es->hwndSelf), msg, (WPARAM)hdc, (LPARAM)es->hwndSelf);
        if (!hbrush)
            hbrush = (HBRUSH)DefWindowProcW_k(GetParent_k(es->hwndSelf), msg, (WPARAM)hdc, (LPARAM)es->hwndSelf);
        return hbrush;
}
...
static INT EDIT_WordBreakProc(EDITSTATE *es, LPWSTR s, INT index, INT count, INT action)
{
    ...
    if (!es->logAttr)
    {
        ...
        es->logAttr = HeapAlloc_k(GetProcessHeap_k(), 0, sizeof(SCRIPT_LOGATTR) * get_text_length(es));
        ScriptBreak_k(es->text, get_text_length(es), &psa, es->logAttr);
    }
...
static INT EDIT_CallWordBreakProc(EDITSTATE *es, INT start, INT index, INT count, INT action)
{
	INT ret;
		...
		countA = WideCharToMultiByte_k(CP_ACP, 0, es->text + start, count, NULL, 0, NULL, NULL);
		textA = HeapAlloc_k(GetProcessHeap_k(), 0, countA);
		WideCharToMultiByte_k(CP_ACP, 0, es->text + start, count, textA, countA, NULL, NULL);
		...
		HeapFree_k(GetProcessHeap_k(), 0, textA);
	    }
        }
	else
...
static INLINE void EDIT_InvalidateUniscribeData_linedef(LINEDEF *line_def)
{
	if (line_def->ssa)
	{
		ScriptStringFree_k(&line_def->ssa);
		line_def->ssa = NULL;
	}
}
...
static SCRIPT_STRING_ANALYSIS EDIT_UpdateUniscribeData_linedef(EDITSTATE *es, HDC dc, LINEDEF *line_def)
{
	...
	if (line_def->net_length && !line_def->ssa)
	{
		...
		if (!udc)
			udc = GetDC_k(es->hwndSelf);
		if (es->font)
			old_font = SelectObject_k(udc, es->font);
		...
		hr = ScriptStringAnalyse_k(udc, &es->text[index], line_def->net_length,
                                         (int)(1.5*line_def->net_length+16), -1,
		...
		if (es->font)
			SelectObject_k(udc, old_font);
		if (udc != dc)
			ReleaseDC_k(es->hwndSelf, udc);
	}
	...
	if (!(es->style & ES_MULTILINE))
	{
		if (!es->ssa)
		{
			...
			if (es->style & ES_PASSWORD)
				ScriptStringAnalyse_k(udc, &es->password_char, length, (int)(1.5*length+16), -1, SSA_LINK|SSA_FALLBACK|SSA_GLYPHS|SSA_PASSWORD, -1, NULL, NULL, NULL, NULL, NULL, &es->ssa);
			else
				ScriptStringAnalyse_k(udc, es->text, length, (int)(1.5*length+16), -1, SSA_LINK|SSA_FALLBACK|SSA_GLYPHS, -1, NULL, NULL, NULL, NULL, NULL, &es->ssa);
	...
	do {
		if (current_line != start_line)
		{
			if (!current_line || current_line->index + delta > current_position - es->text)
			{
				...
				LINEDEF *new_line = HeapAlloc_k(GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(LINEDEF));
			...
			else if (current_line->index + delta < current_position - es->text)
			{
				...
				HeapFree_k(GetProcessHeap_k(), 0, current_line);
		...
		if (current_line->net_length)
		{
			...
			if (current_line->ssa)
			{
				sz = ScriptString_pSize_k(current_line->ssa);
...
/* Line breaks just look back from the end and find the next break and try that. */
		...
		if (!(es->style & ES_AUTOHSCROLL)) {
		   if (current_line->width > fw && fw > es->char_width) {
			...
			if (prev == 0) { /* Didn't find a line break so force a break */
				...
				if (current_line->ssa)
				{
					count = ScriptString_pcOutChars_k(current_line->ssa);
					piDx = HeapAlloc_k(GetProcessHeap_k(),0,sizeof(INT) * (*count));
					ScriptStringGetLogicalWidths_k(current_line->ssa,piDx);
					...
					if (prev<=0)
						prev = 1;
					HeapFree_k(GetProcessHeap_k(),0,piDx);
				}
				else
	...
	if (hrgn)
	{
		...
		*/
		...
		SetRectRgn_k(hrgn, rc.left, rc.top, rc.right, rc.bottom);
		...
		*/
		...
		tmphrgn = CreateRectRgn_k(rc.left, rc.top, rc.right, rc.bottom);
		CombineRgn_k(hrgn, hrgn, tmphrgn, RGN_OR);
		DeleteObject_k(tmphrgn);
	}
}
...
static void EDIT_CalcLineWidth_SL(EDITSTATE *es)
{
	...
	{
		const SIZE *size;
		size = ScriptString_pSize_k(es->ssa);
...
static INT EDIT_CharFromPos(EDITSTATE *es, INT x, INT y, LPBOOL after_wrap)
{
	...
	if (es->style & ES_MULTILINE) {
		...
		ScriptStringXtoCP_k(line_def->ssa, x , &index, &trailing);
		if (trailing) index++;
		index += line_index;
		if (after_wrap)
		...
		if (es->x_offset)
		{
			if (es->ssa)
			{
				if (es->x_offset>= (INT)get_text_length(es))
				{
					const SIZE *size;
					size = ScriptString_pSize_k(es->ssa);
					xoff = size->cx;
				}
				ScriptStringCPtoX_k(es->ssa, es->x_offset, FALSE, &xoff);
			}
			else
				xoff = 0;
		}
		if (x < 0)
		{
			if (x + xoff > 0 || !es->ssa)
			{
				ScriptStringXtoCP_k(es->ssa, x+xoff, &index, &trailing);
				if (trailing) index++;
			}
			else
				index = 0;
...
static LRESULT EDIT_EM_PosFromChar(EDITSTATE *es, INT index, BOOL after_wrap)
{
	...
	if (es->style & ES_MULTILINE) {
		...
		if (line_def->ssa)
		{
			ScriptStringCPtoX_k(line_def->ssa, (index - 1) - li, TRUE, &x);
			x -= es->x_offset;
		}
		else
			x = es->x_offset;
		...
		if (index)
		{
			if (index >= (INT)get_text_length(es))
			{
				...
				ScriptStringCPtoX_k(es->ssa, index, FALSE, &xi);
			else
				xi = 0;
		}
		x = xi - xoff;
...
static void EDIT_GetLineRect(EDITSTATE *es, INT line, INT scol, INT ecol, LPRECT rc)
{
	...
	if (ssa)
	{
		ScriptStringCPtoX_k(ssa, scol, FALSE, &pt3);
...
static INLINE void text_buffer_changed(EDITSTATE *es)
{
    ...
    HeapFree_k(GetProcessHeap_k(), 0, es->logAttr );
    es->logAttr = NULL;
...
static void EDIT_LockBuffer(EDITSTATE *es)
{
	...
	if (!es->text) {
	    ...
	    if(!es->hloc32W) return;
            ...
            if(es->hloc32A)
            {
                CHAR *textA = LocalLock_k(es->hloc32A);
		HLOCAL hloc32W_new;
		UINT countW_new = MultiByteToWideChar_k(CP_ACP, 0, textA, -1, NULL, 0);
		if(countW_new > es->buffer_size + 1)
		{
		    ...
		    hloc32W_new = LocalReAlloc_k(es->hloc32W, alloc_size, LMEM_MOVEABLE | LMEM_ZEROINIT);
		    if(hloc32W_new)
		    {
			es->hloc32W = hloc32W_new;
			es->buffer_size = LocalSize_k(hloc32W_new)/sizeof(WCHAR) - 1;
			TRACE("Real new size %d+1 WCHARs\n", es->buffer_size);
		    }
		    else
			WARN("FAILED! Will synchronize partially\n");
		}
                es->text = LocalLock_k(es->hloc32W);
		MultiByteToWideChar_k(CP_ACP, 0, textA, -1, es->text, es->buffer_size + 1);
                LocalUnlock_k(es->hloc32A);
	    }
	    else es->text = LocalLock_k(es->hloc32W);
	}
	es->lock_count++;
}
...
static void EDIT_UnlockBuffer(EDITSTATE *es, BOOL force)
{
	...
	if (force || (es->lock_count == 1)) {
		...
		if(es->hloc32A)
		{
		    UINT countA_new = WideCharToMultiByte_k(CP_ACP, 0, es->text, countW, NULL, 0, NULL, NULL);
		    TRACE("Synchronizing with 32-bit ANSI buffer\n");
		    TRACE("%d WCHARs translated to %d bytes\n", countW, countA_new);
		    countA = LocalSize_k(es->hloc32A);
		    if(countA_new > countA)
		    {
			HLOCAL hloc32A_new;
			UINT alloc_size = ROUND_TO_GROW(countA_new);
			TRACE("Resizing 32-bit ANSI buffer from %d to %d bytes\n", countA, alloc_size);
			hloc32A_new = LocalReAlloc_k(es->hloc32A, alloc_size, LMEM_MOVEABLE | LMEM_ZEROINIT);
			if(hloc32A_new)
			{
			    es->hloc32A = hloc32A_new;
			    countA = LocalSize_k(hloc32A_new);
			    TRACE("Real new size %d bytes\n", countA);
			}
			else
			    WARN("FAILED! Will synchronize partially\n");
		    }
		    WideCharToMultiByte_k(CP_ACP, 0, es->text, countW,
                                        LocalLock_k(es->hloc32A), countA, NULL, NULL);
                    LocalUnlock_k(es->hloc32A);
		}

		LocalUnlock_k(es->hloc32W);
		es->text = NULL;
	    }
	    else {
...
static BOOL EDIT_MakeFit(EDITSTATE *es, UINT size)
{
	...
	if (es->hloc32W) {
	    ...
	    if ((hNew32W = LocalReAlloc_k(es->hloc32W, alloc_size, LMEM_MOVEABLE | LMEM_ZEROINIT))) {
		TRACE("Old 32 bit handle %p, new handle %p\n", es->hloc32W, hNew32W);
		es->hloc32W = hNew32W;
		es->buffer_size = LocalSize_k(hNew32W)/sizeof(WCHAR) - 1;
	    }
	}
...
static BOOL EDIT_MakeUndoFit(EDITSTATE *es, UINT size)
{
	...
	if ((es->undo_text = HeapReAlloc_k(GetProcessHeap_k(), HEAP_ZERO_MEMORY, es->undo_text, alloc_size))) {
...
static void EDIT_UpdateTextRegion(EDITSTATE *es, HRGN hrgn, BOOL bErase)
{
    ...
    InvalidateRgn_k(es->hwndSelf, hrgn, bErase);
}
...
static void EDIT_UpdateText(EDITSTATE *es, const RECT *rc, BOOL bErase)
{
    ...
    InvalidateRect_k(es->hwndSelf, rc, bErase);
}
...
static void EDIT_SL_InvalidateText(EDITSTATE *es, INT start, INT end)
{
	...
	if (IntersectRect_k(&rc, &line_rect, &es->format_rect))
...
static void EDIT_ML_InvalidateText(EDITSTATE *es, INT start, INT end)
{
	...
	GetClientRect_k(es->hwndSelf, &rc1);
	IntersectRect_k(&rcWnd, &rc1, &es->format_rect);
	if (sl == el) {
		EDIT_GetLineRect(es, sl, sc, ec, &rcLine);
		if (IntersectRect_k(&rcUpdate, &rcWnd, &rcLine))
...
static void EDIT_UpdateScrollInfo(EDITSTATE *es)
{
	...
	SetScrollInfo_k(es->hwndSelf, SB_VERT, &si, TRUE);
    }
	...
	TRACE("SB_HORZ, nMin=%d, nMax=%d, nPage=%d, nPos=%d\n",
		si.nMin, si.nMax, si.nPage, si.nPos);
	SetScrollInfo_k(es->hwndSelf, SB_HORZ, &si, TRUE);
    }
}
...
static BOOL EDIT_EM_LineScroll_internal(EDITSTATE *es, INT dx, INT dy)
{
	...
	if (dx || dy) {
		...
		GetClientRect_k(es->hwndSelf, &rc1);
		IntersectRect_k(&rc, &rc1, &es->format_rect);
		ScrollWindowEx_k(es->hwndSelf, -dx, dy,
			     ...
			     BOOL after_wrap)
{
	...
	SetCaretPos_k((short)LOWORD(res), (short)HIWORD(res));
}
...
static INT EDIT_PaintText(EDITSTATE *es, HDC dc, INT x, INT y, INT line, INT col, INT count, BOOL rev)
{
	...
	if (!count)
		return 0;
	BkMode = GetBkMode_k(dc);
	BkColor = GetBkColor_k(dc);
	TextColor = GetTextColor_k(dc);
	if (rev) {
	        if (es->composition_len == 0)
	        {
			SetBkColor_k(dc, GetSysColor_k(COLOR_HIGHLIGHT));
			SetTextColor_k(dc, GetSysColor_k(COLOR_HIGHLIGHTTEXT));
			SetBkMode_k( dc, OPAQUE);
	        }
		else
		{
			HFONT current = GetCurrentObject_k(dc,OBJ_FONT);
			GetObjectW_k(current,sizeof(LOGFONTW),&underline_font);
			underline_font.lfUnderline = TRUE;
	            	hUnderline = CreateFontIndirectW_k(&underline_font);
			old_font = SelectObject_k(dc,hUnderline);
	        }
	}
	li = EDIT_EM_LineIndex(es, line);
	if (es->style & ES_MULTILINE) {
		ret = (INT)LOWORD(TabbedTextOutW_k(dc, x, y, es->text + li + col, count,
					es->tabs_count, es->tabs, es->format_rect.left - es->x_offset));
	} else {
		LPWSTR text = es->text;
		TextOutW_k(dc, x, y, text + li + col, count);
		GetTextExtentPoint32W_k(dc, text + li + col, count, &size);
		ret = size.cx;
		if (es->style & ES_PASSWORD)
	...
	if (rev) {
		if (es->composition_len == 0)
		{
			SetBkColor_k(dc, BkColor);
			SetTextColor_k(dc, TextColor);
			SetBkMode_k( dc, BkMode);
		}
		else
		{
			if (old_font)
				SelectObject_k(dc,old_font);
			if (hUnderline)
				DeleteObject_k(hUnderline);
	        }
	}
	return ret;
}
...
static void EDIT_PaintLine(EDITSTATE *es, HDC dc, INT line, BOOL rev)
{
	...
	if (ssa)
		ScriptStringOut_k(ssa, x, y, 0, &es->format_rect, s - li, e - li, FALSE);
	else if (rev && (s != e) &&
...
static void EDIT_AdjustFormatRect(EDITSTATE *es)
{
	...
	GetClientRect_k(es->hwndSelf, &ClientRect);
...
static void EDIT_SetRectNP(EDITSTATE *es, const RECT *rc)
{
	LONG_PTR ExStyle;
	INT bw, bh;
	ExStyle = GetWindowLongPtrW_k(es->hwndSelf, GWL_EXSTYLE);
	
	CopyRect_k(&es->format_rect, rc);
	...
	if (ExStyle & WS_EX_CLIENTEDGE) {
		...
		bw = GetSystemMetrics_k(SM_CXBORDER) + 1;
		bh = GetSystemMetrics_k(SM_CYBORDER) + 1;
...
static LRESULT EDIT_EM_CharFromPos(EDITSTATE *es, INT x, INT y)
{
	...
	pt.x = x;
	pt.y = y;
	GetClientRect_k(es->hwndSelf, &rc);
	if (!PtInRect_k(&rc, pt))
		return -1;
...
static HLOCAL EDIT_EM_GetHandle(EDITSTATE *es)
{
	...
	else
	{
		...
		countA = WideCharToMultiByte_k(CP_ACP, 0, es->text, -1, NULL, 0, NULL, NULL);
		alloc_size = ROUND_TO_GROW(countA);
		if(!(es->hloc32A = LocalAlloc_k(LMEM_MOVEABLE | LMEM_ZEROINIT, alloc_size)))
		{
		    ERR("Could not allocate %d bytes for 32-bit ANSI alias buffer\n", alloc_size);
		    return 0;
		}
		textA = LocalLock_k(es->hloc32A);
		WideCharToMultiByte_k(CP_ACP, 0, es->text, -1, textA, countA, NULL, NULL);
		LocalUnlock_k(es->hloc32A);
	    }
	    hLocal = es->hloc32A;
	}
	...
	TRACE("Returning %p, LocalSize_k() = %ld\n", hLocal, LocalSize_k(hLocal));
	return hLocal;
}
...
static INT EDIT_EM_GetLine(EDITSTATE *es, INT line, LPWSTR dst, BOOL unicode)
{
	...
	{
	    INT ret = WideCharToMultiByte_k(CP_ACP, 0, src, line_len, (LPSTR)dst, dst_len, NULL, NULL);
...
static void EDIT_EM_ReplaceSel(EDITSTATE *es, BOOL can_undo, LPCWSTR lpsz_replace, BOOL send_update, BOOL honor_limit)
{
	...
	if (e != s) {
		...
		bufl = e - s;
		buf = HeapAlloc_k(GetProcessHeap_k(), 0, (bufl + 1) * sizeof(WCHAR));
		if (!buf) return;
	...
	if (strl) {
		...
		if(es->style & ES_UPPERCASE)
			CharUpperBuffW_k(p, strl);
		else if(es->style & ES_LOWERCASE)
			CharLowerBuffW_k(p, strl);
                text_buffer_changed(es);
	}
	if (es->style & ES_MULTILINE)
	{
...
static void EDIT_EM_SetHandle(EDITSTATE *es, HLOCAL hloc)
{
	...
	else
	{
	    ...
	    countA = LocalSize_k(hloc);
	    textA = LocalLock_k(hloc);
	    countW = MultiByteToWideChar_k(CP_ACP, 0, textA, countA, NULL, 0);
	    if(!(hloc32W_new = LocalAlloc_k(LMEM_MOVEABLE | LMEM_ZEROINIT, countW * sizeof(WCHAR))))
	    {
		ERR("Could not allocate new unicode buffer\n");
		return;
	    }
	    textW = LocalLock_k(hloc32W_new);
	    MultiByteToWideChar_k(CP_ACP, 0, textA, countA, textW, countW);
	    LocalUnlock_k(hloc32W_new);
	    LocalUnlock_k(hloc);

	    if(es->hloc32W)
		LocalFree_k(es->hloc32W);

	    es->hloc32W = hloc32W_new;
	    es->hloc32A = hloc;
	}

	es->buffer_size = LocalSize_k(es->hloc32W)/sizeof(WCHAR) - 1;
...
static int calc_min_set_margin_size(HDC dc, INT left, INT right)
{
    ...
    GetTextExtentPointW_k(dc, magic_string, sizeof(magic_string)/sizeof(WCHAR) - 1, &sz);
			       ...
			       WORD left, WORD right, BOOL repaint)
{
	...
	INT default_right_margin = 0; /* in pixels */
        ...
        if (es->font && (left == EC_USEFONTINFO || right == EC_USEFONTINFO)) {
            HDC dc = GetDC_k(es->hwndSelf);
            HFONT old_font = SelectObject_k(dc, es->font);
            GetTextMetricsW_k(dc, &tm);
...
static void EDIT_EM_SetPasswordChar(EDITSTATE *es, WCHAR c)
{
	...
	if (es->password_char == c)
        ...
        style = GetWindowLongW_k( es->hwndSelf, GWL_STYLE );
	es->password_char = c;
	if (c) {
            SetWindowLongW_k( es->hwndSelf, GWL_STYLE, style | ES_PASSWORD );
...
static BOOL EDIT_EM_SetTabStops(EDITSTATE *es, INT count, const INT *tabs)
{
	if (!(es->style & ES_MULTILINE))
		return FALSE;
        HeapFree_k(GetProcessHeap_k(), 0, es->tabs);
	es->tabs_count = count;
	if (!count)
		es->tabs = NULL;
	else {
		es->tabs = HeapAlloc_k(GetProcessHeap_k(), 0, count * sizeof(INT));
...
static BOOL EDIT_EM_Undo(EDITSTATE *es)
{
	...
	utext = HeapAlloc_k(GetProcessHeap_k(), 0, (ulength + 1) * sizeof(WCHAR));
	...
	EDIT_EM_SetSel(es, es->undo_position, es->undo_position + es->undo_insert_count, FALSE);
        ...
        EDIT_NOTIFY_PARENT(es, EN_CHANGE);
	EDIT_EM_ScrollCaret(es);
	HeapFree_k(GetProcessHeap_k(), 0, utext);
...
static void EDIT_WM_Paste(EDITSTATE *es)
{
	...
	OpenClipboard_k(es->hwndSelf);
	if ((hsrc = GetClipboardData_k(CF_UNICODETEXT))) {
		src = GlobalLock_k(hsrc);
...
static void EDIT_WM_Copy(EDITSTATE *es)
{
	...
	len = e - s;
	hdst = GlobalAlloc_k(GMEM_MOVEABLE | GMEM_DDESHARE, (len + 1) * sizeof(WCHAR));
	dst = GlobalLock_k(hdst);
	...
	OpenClipboard_k(es->hwndSelf);
	EmptyClipboard_k();
	SetClipboardData_k(CF_UNICODETEXT, hdst);
	CloseClipboard_k();
}
...
static LRESULT EDIT_WM_Char(EDITSTATE *es, WCHAR c)
{
	...
	control = GetKeyState_k(VK_CONTROL) & 0x8000;
	...
	case 0x03: /* ^C */
		if (!(es->style & ES_PASSWORD))
		    SendMessageW_k(es->hwndSelf, WM_COPY, 0, 0);
		break;
	case 0x16: /* ^V */
	        if (!(es->style & ES_READONLY))
		    SendMessageW_k(es->hwndSelf, WM_PASTE, 0, 0);
		break;
	case 0x18: /* ^X */
	        if (!((es->style & ES_READONLY) || (es->style & ES_PASSWORD)))
		    SendMessageW_k(es->hwndSelf, WM_CUT, 0, 0);
		break;
	case 0x1A: /* ^Z */
	        if (!(es->style & ES_READONLY))
		    SendMessageW_k(es->hwndSelf, WM_UNDO, 0, 0);
		break;
...
static void EDIT_WM_Command(EDITSTATE *es, INT code, INT id, HWND control)
{
	...
	switch (id) {
		...
		case WM_PASTE:
			...
			break;
		case WM_CLEAR:
                        SendMessageW_k(es->hwndSelf, WM_CLEAR, 0, 0);
			break;
		case EM_SETSEL:
...
{
	HMENU menu = LoadMenuA_k(user32_module, "EDITMENU");
	HMENU popup = GetSubMenu_k(menu, 0);
	...
	/* undo */
	EnableMenuItem_k(popup, 0, MF_BYPOSITION | (EDIT_EM_CanUndo(es) && !(es->style & ES_READONLY) ? MF_ENABLED : MF_GRAYED));
	/* cut */
	EnableMenuItem_k(popup, 2, MF_BYPOSITION | ((end - start) && !(es->style & ES_PASSWORD) && !(es->style & ES_READONLY) ? MF_ENABLED : MF_GRAYED));
	/* copy */
	EnableMenuItem_k(popup, 3, MF_BYPOSITION | ((end - start) && !(es->style & ES_PASSWORD) ? MF_ENABLED : MF_GRAYED));
	/* paste */
	EnableMenuItem_k(popup, 4, MF_BYPOSITION | (IsClipboardFormatAvailable_k(CF_UNICODETEXT) && !(es->style & ES_READONLY) ? MF_ENABLED : MF_GRAYED));
	/* delete */
	EnableMenuItem_k(popup, 5, MF_BYPOSITION | ((end - start) && !(es->style & ES_READONLY) ? MF_ENABLED : MF_GRAYED));
	/* select all */
	EnableMenuItem_k(popup, 7, MF_BYPOSITION | (start || (end != get_text_length(es)) ? MF_ENABLED : MF_GRAYED));
	...
	if (!(es->flags & EF_FOCUSED))
            SetFocus_k(es->hwndSelf);

	TrackPopupMenu_k(popup, TPM_LEFTALIGN | TPM_RIGHTBUTTON, x, y, 0, es->hwndSelf, NULL);
	DestroyMenu_k(menu);
}
...
static INT EDIT_WM_GetText(const EDITSTATE *es, INT count, LPWSTR dst, BOOL unicode)
{
	...
	if (!WideCharToMultiByte_k(CP_ACP, 0, es->text, -1, textA, count, NULL, NULL))
...
static BOOL EDIT_CheckCombo(EDITSTATE *es, UINT msg, INT key)
{
   ...
   hCombo   = GetParent_k(es->hwndSelf);
   bDropped = TRUE;
   nEUI     = 0;
   ...
   {
      if (SendMessageW_k(hCombo, CB_GETEXTENDEDUI, 0, 0))
         nEUI = 1;

      if (msg == WM_KEYDOWN || nEUI)
          bDropped = (BOOL)SendMessageW_k(hCombo, CB_GETDROPPEDSTATE, 0, 0);
   }
   ...
   switch (msg)
   {
      case WM_KEYDOWN:
         if (!bDropped && nEUI && (key == VK_UP || key == VK_DOWN))
         {
            ...
            SendMessageW_k(hCombo, CB_SETEXTENDEDUI, FALSE, 0);
            key = VK_F4;
            nEUI = 2;
         }

         SendMessageW_k(hLBox, WM_KEYDOWN, key, 0);
         break;

      case WM_SYSKEYDOWN: /* Handle Alt+up/down arrows */
         if (nEUI)
            SendMessageW_k(hCombo, CB_SHOWDROPDOWN, !bDropped, 0);
         else
            SendMessageW_k(hLBox, WM_KEYDOWN, VK_F4, 0);
         break;
   }

   if(nEUI == 2)
      SendMessageW_k(hCombo, CB_SETEXTENDEDUI, TRUE, 0);
...
static LRESULT EDIT_WM_KeyDown(EDITSTATE *es, INT key)
{
	...
	if (GetKeyState_k(VK_MENU) & 0x8000)
		return 0;

	shift = GetKeyState_k(VK_SHIFT) & 0x8000;
	control = GetKeyState_k(VK_CONTROL) & 0x8000;
	...
	case VK_RETURN:
	    ...
	    if(!(es->style & ES_MULTILINE) || !(es->style & ES_WANTRETURN))
	    {
                ...
                if (control) break;
                dw = SendMessageW_k(es->hwndParent, DM_GETDEFID, 0, 0);
                if (HIWORD(dw) == DC_HASDEFID)
                {
                    HWND hwDefCtrl = GetDlgItem_k(es->hwndParent, LOWORD(dw));
                    if (hwDefCtrl)
                    {
                        SendMessageW_k(es->hwndParent, WM_NEXTDLGCTL, (WPARAM)hwDefCtrl, TRUE);
                        PostMessageW_k(hwDefCtrl, WM_KEYDOWN, VK_RETURN, 0);
                    }
                }
	    }
	    break;
        case VK_ESCAPE:
            if ((es->style & ES_MULTILINE) && EDIT_IsInsideDialog(es))
                PostMessageW_k(es->hwndParent, WM_CLOSE, 0, 0);
            break;
        case VK_TAB:
            if ((es->style & ES_MULTILINE) && EDIT_IsInsideDialog(es))
                SendMessageW_k(es->hwndParent, WM_NEXTDLGCTL, shift, 0);
            break;
	}
        return TRUE;
}
...
static LRESULT EDIT_WM_LButtonDblClk(EDITSTATE *es)
{
	...
	SetTimer_k(es->hwndSelf, 0, 100, NULL);
	return 0;
}
...
{
	if (es->bCaptureState) {
		KillTimer_k(es->hwndSelf, 0);
		if (GetCapture_k() == es->hwndSelf) ReleaseCapture_k();
	}
...
static LRESULT EDIT_WM_MouseMove(EDITSTATE *es, INT x, INT y)
{
	...
	INT prex, prey;
         ...
         */
        if (!es->bCaptureState || GetCapture_k() != es->hwndSelf)
		return 0;
...
static void EDIT_WM_Paint(EDITSTATE *es, HDC hdc)
{
	...
	BOOL rev = es->bEnableState &&
        ...
        dc = hdc ? hdc : BeginPaint_k(es->hwndSelf, &ps);

	/* The dc we use for calculating may not be the one we paint into.
	   This is the safest action. */
	EDIT_InvalidateUniscribeData(es);
	GetClientRect_k(es->hwndSelf, &rcClient);

	/* get the background brush */
	brush = EDIT_NotifyCtlColor(es, dc);

	/* paint the border and the background */
	IntersectClipRect_k(dc, rcClient.left, rcClient.top, rcClient.right, rcClient.bottom);
	
	if(es->style & WS_BORDER) {
		bw = GetSystemMetrics_k(SM_CXBORDER);
		bh = GetSystemMetrics_k(SM_CYBORDER);
		rc = rcClient;
		if(es->style & ES_MULTILINE) {
		...
		old_brush = SelectObject_k(dc, GetSysColorBrush_k(COLOR_WINDOWFRAME));
		PatBlt_k(dc, rc.left, rc.top, rc.right - rc.left, bh, PATCOPY);
		PatBlt_k(dc, rc.left, rc.top, bw, rc.bottom - rc.top, PATCOPY);
		PatBlt_k(dc, rc.left, rc.bottom - 1, rc.right - rc.left, -bw, PATCOPY);
		PatBlt_k(dc, rc.right - 1, rc.top, -bw, rc.bottom - rc.top, PATCOPY);
		SelectObject_k(dc, old_brush);
		
		/* Keep the border clean */
		IntersectClipRect_k(dc, rc.left+bw, rc.top+bh,
		    max(rc.right-bw, rc.left+bw), max(rc.bottom-bh, rc.top+bh));
	}
	
	GetClipBox_k(dc, &rc);
	FillRect_k(dc, &rc, brush);

	IntersectClipRect_k(dc, es->format_rect.left,
				es->format_rect.top,
				es->format_rect.right,
				es->format_rect.bottom);
	if (es->style & ES_MULTILINE) {
		rc = rcClient;
		IntersectClipRect_k(dc, rc.left, rc.top, rc.right, rc.bottom);
	}
	if (es->font)
		old_font = SelectObject_k(dc, es->font);

	if (!es->bEnableState)
		SetTextColor_k(dc, GetSysColor_k(COLOR_GRAYTEXT));
	GetClipBox_k(dc, &rcRgn);
	if (es->style & ES_MULTILINE) {
		INT vlc = get_vertical_line_count(es);
		for (i = es->y_offset ; i <= min(es->y_offset + vlc, es->y_offset + es->line_count - 1) ; i++) {
			...
			if (IntersectRect_k(&rc, &rcRgn, &rcLine))
...
static void EDIT_WM_SetFocus(EDITSTATE *es)
{
	es->flags |= EF_FOCUSED;
        ...
        if (!(es->style & ES_MULTILINE))
        {
            HDC hdc = GetDC_k(es->hwndSelf);
            EDIT_WM_Paint(es, hdc);
            ReleaseDC_k(es->hwndSelf, hdc);
        }

	CreateCaret_k(es->hwndSelf, 0, 1, es->line_height);
	EDIT_SetCaretPos(es, es->selection_end,
			 es->flags & EF_AFTER_WRAP);
	ShowCaret_k(es->hwndSelf);
...
static void EDIT_WM_SetFont(EDITSTATE *es, HFONT font, BOOL redraw)
{
	...
	es->font = font;
	EDIT_InvalidateUniscribeData(es);
	dc = GetDC_k(es->hwndSelf);
	if (font)
		old_font = SelectObject_k(dc, font);
	GetTextMetricsW_k(dc, &tm);
...
static void EDIT_WM_SetText(EDITSTATE *es, LPCWSTR text, BOOL unicode)
{
	...
	INT countW = MultiByteToWideChar_k(CP_ACP, 0, textA, -1, NULL, 0);
        textW = HeapAlloc_k(GetProcessHeap_k(), 0, countW * sizeof(WCHAR));
	if (textW)
	    MultiByteToWideChar_k(CP_ACP, 0, textA, -1, textW, countW);
	text = textW;
    }
	...
	if(!unicode)
	    HeapFree_k(GetProcessHeap_k(), 0, textW);
    } 
    else 
    {
	TRACE("<NULL>\n");
...
static LRESULT EDIT_WM_SysKeyDown(EDITSTATE *es, INT key, DWORD key_data)
{
	...
	return DefWindowProcW_k(es->hwndSelf, WM_SYSKEYDOWN, key, key_data);
}
...
static LRESULT EDIT_WM_HScroll(EDITSTATE *es, INT action, INT pos)
{
	...
	case SB_THUMBPOSITION:
		...
		es->flags &= ~EF_HSCROLL_TRACK;
		if(GetWindowLongW_k( es->hwndSelf, GWL_STYLE ) & WS_HSCROLL)
		    dx = pos - es->x_offset;
		else
		{
		    INT fw, new_x;
	...
	case EM_GETTHUMB: /* this one is used by NT notepad */
	{
		LRESULT ret;
		if(GetWindowLongW_k( es->hwndSelf, GWL_STYLE ) & WS_HSCROLL)
		    ret = GetScrollPos_k(es->hwndSelf, SB_HORZ);
		else
		{
		    /* Assume default scroll range 0-100 */
		    INT fw = es->format_rect.right - es->format_rect.left;
...
static LRESULT EDIT_WM_VScroll(EDITSTATE *es, INT action, INT pos)
{
	...
	{
		LRESULT ret;
		if(GetWindowLongW_k( es->hwndSelf, GWL_STYLE ) & WS_VSCROLL)
		    ret = GetScrollPos_k(es->hwndSelf, SB_VERT);
		else
		{
		    /* Assume default scroll range 0-100 */
		    INT vlc = get_vertical_line_count(es);
...
static void EDIT_GetCompositionStr(HIMC hIMC, LPARAM CompFlag, EDITSTATE *es)
{
    ...
    lpCompStr = HeapAlloc_k(GetProcessHeap_k(),0,buflen + sizeof(WCHAR));
    if (!lpCompStr)
    {
    ...
    if (CompFlag & GCS_COMPATTR)
    {
        ...
        {
            dwBufLenAttr ++;
            lpCompStrAttr = HeapAlloc_k(GetProcessHeap_k(),0,dwBufLenAttr+1);
            if (!lpCompStrAttr)
            {
                ERR("Unable to allocate IME Attribute String\n");
                HeapFree_k(GetProcessHeap_k(),0,lpCompStr);
                return;
            }
    ...
    HeapFree_k(GetProcessHeap_k(),0,lpCompStrAttr);
    HeapFree_k(GetProcessHeap_k(),0,lpCompStr);
}
...
static void EDIT_GetResultStr(HIMC hIMC, EDITSTATE *es)
{
    ...
    lpResultStr = HeapAlloc_k(GetProcessHeap_k(),0, buflen+sizeof(WCHAR));
    if (!lpResultStr)
    {
    ...
    HeapFree_k(GetProcessHeap_k(),0,lpResultStr);
}
...
static LRESULT EDIT_WM_NCCreate(HWND hwnd, LPCREATESTRUCTW lpcs, BOOL unicode)
{
	...
	if (!(es = HeapAlloc_k(GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(*es))))
		return FALSE;
        SetWindowLongPtrW_k( hwnd, 0, (LONG_PTR)es );
	...
	if (es->style & ES_COMBO)
	   es->hwndListBox = GetDlgItem_k(es->hwndParent, ID_CB_LISTBOX);
	...
	if(!(es->hloc32W = LocalAlloc_k(LMEM_MOVEABLE | LMEM_ZEROINIT, alloc_size)))
	    goto cleanup;
	es->buffer_size = LocalSize_k(es->hloc32W)/sizeof(WCHAR) - 1;

	if (!(es->undo_text = HeapAlloc_k(GetProcessHeap_k(), HEAP_ZERO_MEMORY, (es->buffer_size + 1) * sizeof(WCHAR))))
		goto cleanup;
	es->undo_buffer_size = es->buffer_size;

	if (es->style & ES_MULTILINE)
		if (!(es->first_line_def = HeapAlloc_k(GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(LINEDEF))))
			goto cleanup;
	es->line_count = 1;
	...
	if (lpcs->dwExStyle & WS_EX_CLIENTEDGE)
		...
		SetWindowLongW_k(hwnd, GWL_STYLE, es->style & ~WS_BORDER);

	return TRUE;

cleanup:
	SetWindowLongPtrW_k(es->hwndSelf, 0, 0);
	EDIT_InvalidateUniscribeData(es);
	HeapFree_k(GetProcessHeap_k(), 0, es->first_line_def);
	HeapFree_k(GetProcessHeap_k(), 0, es->undo_text);
	if (es->hloc32W) LocalFree_k(es->hloc32W);
	HeapFree_k(GetProcessHeap_k(), 0, es->logAttr);
	HeapFree_k(GetProcessHeap_k(), 0, es);
	return FALSE;
}
...
static LRESULT EDIT_WM_NCDestroy(EDITSTATE *es)
{
	...
	{
		pp = pc->next;
		HeapFree_k(GetProcessHeap_k(), 0, pc);
		pc = pp;
	}
...
static INLINE LRESULT DefWindowProcT(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, BOOL unicode)
{
	if(unicode)
		...
		return DefWindowProcA_k(hwnd, msg, wParam, lParam);
}
...
{
	EDITSTATE *es = (EDITSTATE *)GetWindowLongPtrW_k( hwnd, 0 );
	LRESULT result = 0;
	...
	case EM_GETRECT:
		if (lParam)
			CopyRect_k((LPRECT)lParam, &es->format_rect);
		break;
	...
	case 0x00ca:
		...
		result = DefWindowProcW_k(hwnd, msg, wParam, lParam);
		break;
	...
	case EM_REPLACESEL:
	{
		...
		else
		{
		    ...
		    INT countW = MultiByteToWideChar_k(CP_ACP, 0, textA, -1, NULL, 0);
		    if (!(textW = HeapAlloc_k(GetProcessHeap_k(), 0, countW * sizeof(WCHAR)))) break;
                    MultiByteToWideChar_k(CP_ACP, 0, textA, -1, textW, countW);
		}
	...
	case EM_SETPASSWORDCHAR:
	{
		...
		else
		{
		    ...
		    MultiByteToWideChar_k(CP_ACP, 0, &charA, 1, &charW, 1);
		}
	...
	case EM_SETREADONLY:
	{
		...
		if (wParam) {
                    SetWindowLongW_k( hwnd, GWL_STYLE,
                                    GetWindowLongW_k( hwnd, GWL_STYLE ) | ES_READONLY );
		...
		if (old_style ^ es->style)
		    InvalidateRect_k(es->hwndSelf, NULL, TRUE);
	...
	case EM_GETPASSWORDCHAR:
	{
		...
		else
		{
		    ...
		    CHAR charA = 0;
		    WideCharToMultiByte_k(CP_ACP, 0, &charW, 1, &charA, 1, NULL, NULL);
		    result = charA;
		}
		break;
	}
	...
	case WM_GETDLGCODE:
                    ...
                    if (((LPMSG)lParam)->message == WM_KEYDOWN)
                    {
                        ...
                        if (es->hwndListBox)
                        {
                            if (vk == VK_RETURN || vk == VK_ESCAPE)
                                if (SendMessageW_k(GetParent_k(hwnd), CB_GETDROPPEDSTATE, 0, 0))
        ...
        case WM_IME_CHAR:
            if (!unicode)
            {
                ...
                strng[0] = (CHAR)(wParam >> 8);
                strng[1] = wParam & 0xff;
                if (strng[0]) MultiByteToWideChar_k(CP_ACP, 0, strng, 2, &charW, 1);
                else MultiByteToWideChar_k(CP_ACP, 0, &strng[1], 1, &charW, 1);
		...
		else
		{
		    ...
		    MultiByteToWideChar_k(CP_ACP, 0, &charA, 1, &charW, 1);
                ...
                if (es->hwndListBox)
                {
                    ...
                    {
                        if (SendMessageW_k(GetParent_k(hwnd), CB_GETDROPPEDSTATE, 0, 0))
                            SendMessageW_k(GetParent_k(hwnd), WM_KEYDOWN, charW, 0);
                        break;
                    }
                }
		result = EDIT_WM_Char(es, charW);
	...
	case WM_CREATE:
		...
		else
		{
		    ...
		    if(nameA)
		    {
			INT countW = MultiByteToWideChar_k(CP_ACP, 0, nameA, -1, NULL, 0);
			if((nameW = HeapAlloc_k(GetProcessHeap_k(), 0, countW * sizeof(WCHAR))))
			    MultiByteToWideChar_k(CP_ACP, 0, nameA, -1, nameW, countW);
		    }
	...
	case WM_GETTEXTLENGTH:
                ...
                else result = WideCharToMultiByte_k( CP_ACP, 0, es->text, get_text_length(es),
                                                   NULL, 0, NULL, NULL );
		break;
        ...
        case WM_MOUSEWHEEL:
                {
                    ...
                    SystemParametersInfoW_k(SPI_GETWHEELSCROLLLINES,0, &pulScrollLines, 0);
                    ...
                    if (es->wheelDeltaRemainder && pulScrollLines)
                    {
                        ...
                        cLineScroll = (int)(pulScrollLines * (float)es->wheelDeltaRemainder / WHEEL_DELTA);
	...
	if (IsWindow_k(hwnd) && es) EDIT_UnlockBuffer(es, FALSE);
