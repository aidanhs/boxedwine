...
typedef struct
{
     ...
     * ShowWindow_k(SW_SHOWMAXIMIZED) request, therefore window is guaranteed to
...
static HWND MDI_GetChildByID(HWND hwnd, UINT id, MDICLIENTINFO *ci)
{
    ...
    {
        if (GetWindowLongPtrW_k( ci->child[i], GWLP_ID ) == id)
            return ci->child[i];
    }
    return 0;
}
...
static void MDI_PostUpdate(HWND hwnd, MDICLIENTINFO* ci, WORD recalc)
{
	...
	PostMessageA_k( hwnd, WM_MDICALCCHILDSCROLL, 0, 0);
    }
    ci->sbRecalc = recalc;
}
...
static MDICLIENTINFO *get_client_info( HWND client )
{
    ...
    if (win)
    {
        if (win == WND_DESKTOP)
        {
            if (IsWindow_k(client)) WARN( "client %p belongs to other process\n", client );
            return NULL;
        }
...
{
    if (GetClassLongW_k(hwnd, GCL_STYLE) & CS_NOCLOSE) return FALSE;

    if (!hSysMenu) hSysMenu = GetSystemMenu_k(hwnd, FALSE);
    if (hSysMenu)
    {
        UINT state = GetMenuState_k(hSysMenu, SC_CLOSE, MF_BYCOMMAND);
 ...
 */
static HWND MDI_GetWindow_k(MDICLIENTINFO *clientInfo, HWND hWnd, BOOL bNext,
                            DWORD dwStyleMask )
{
    int i;
    HWND *list;
    ...
    if (!(list = WIN_ListChildren( GetParent_k(hWnd) ))) return 0;
    i = 0;
    ...
    for ( ; list[i]; i++)
    {
        if (GetWindow_k( list[i], GW_OWNER )) continue;
        if ((GetWindowLongW_k( list[i], GWL_STYLE ) & dwStyleMask) != WS_VISIBLE) continue;
        last = list[i];
        if (bNext) goto found;
    }
...
void MDI_CalcDefaultChildPos( HWND hwndClient, INT total, LPPOINT lpPos, INT delta, UINT *id )
{
    INT  nstagger;
    RECT rect;
    INT spacing = GetSystemMetrics_k(SM_CYCAPTION) + GetSystemMetrics_k(SM_CYFRAME) - 1;
    ...
    GetClientRect_k( hwndClient, &rect );
 ...
 *            MDISetMenu
 */
static LRESULT MDISetMenu_k( HWND hwnd, HMENU hmenuFrame,
                           HMENU hmenuWindow)
{
    MDICLIENTINFO *ci;
    HWND hwndFrame = GetParent_k(hwnd);

    TRACE("%p, frame menu %p, window menu %p\n", hwnd, hmenuFrame, hmenuWindow);

    if (hmenuFrame && !IsMenu_k(hmenuFrame))
    {
	WARN("hmenuFrame is not a menu handle\n");
	return 0L;
    }

    if (hmenuWindow && !IsMenu_k(hmenuWindow))
    {
    ...
    if (hmenuFrame)
    {
        SetMenu_k(hwndFrame, hmenuFrame);
    ...
    {
        /* SetMenu_k() may already have been called, meaning that this window
         * already has its menu. But they may have done a SetMenu_k() on
         * an MDI window, and called MDISetMenu_k() after the fact, meaning
...
static LRESULT MDI_RefreshMenu(MDICLIENTINFO *ci)
{
     ...
     */
    count = GetMenuItemCount_k(ci->hWindowMenu);
    for (i = 0; i < count; i++)
    {
        MENUITEMINFOW mii;
        ...
        mii.fMask  = MIIM_TYPE;
        if (GetMenuItemInfoW_k(ci->hWindowMenu, i, TRUE, &mii))
        {
            if (mii.fType & MF_SEPARATOR)
            {
                ...
                mii.cbSize = sizeof(mii);
                mii.fMask  = MIIM_ID;
                if (GetMenuItemInfoW_k(ci->hWindowMenu, i + 1, TRUE, &mii))
                {
                    if (mii.wID == ci->idFirstChild)
                    {
                        TRACE("removing %u items including separator\n", count - i);
                        while (RemoveMenu_k(ci->hWindowMenu, i, MF_BYPOSITION))
                            /* nothing */;
    ...
    {
        if (GetWindowLongW_k(ci->child[i], GWL_STYLE) & WS_VISIBLE)
        {
            id = ci->idFirstChild + visible;

            if (visible == MDI_MOREWINDOWSLIMIT)
            {
                LoadStringW_k(user32_module, IDS_MDI_MOREWINDOWS, buf, sizeof(buf)/sizeof(WCHAR));
                AppendMenuW_k(ci->hWindowMenu, MF_STRING, id, buf);
                break;
            }

            if (!visible)
                /* Visio expects that separator has id 0 */
                AppendMenuW_k(ci->hWindowMenu, MF_SEPARATOR, 0, NULL);

            visible++;

            SetWindowLongPtrW_k(ci->child[i], GWLP_ID, id);

            buf[0] = '&';
            buf[1] = '0' + visible;
            buf[2] = ' ';
            InternalGetWindowText_k(ci->child[i], buf + 3, sizeof(buf)/sizeof(WCHAR) - 3);
            TRACE("Adding %p, id %u %s\n", ci->child[i], id, debugstr_w(buf));
            AppendMenuW_k(ci->hWindowMenu, MF_STRING, id, buf);

            if (ci->child[i] == ci->hwndActiveChild)
                CheckMenuItem_k(ci->hWindowMenu, id, MF_CHECKED);
        }
        else
...
static void MDI_ChildGetMinMaxInfo( HWND client, HWND hwnd, MINMAXINFO* lpMinMax )
{
    ...
    GetClientRect_k( client, &rect );
    AdjustWindowRectEx_k( &rect, GetWindowLongW_k( hwnd, GWL_STYLE ),
                        0, GetWindowLongW_k( hwnd, GWL_EXSTYLE ));
 ...
 *
 * Note: SetWindowPos_k sends WM_CHILDACTIVATE to the child window that is
 *       being activated
 */
static void MDI_SwitchActiveChild( MDICLIENTINFO *ci, HWND hwndTo, BOOL activate )
{
    ...
    if ( hwndTo != hwndPrev )
    {
        BOOL was_zoomed = IsZoomed_k(hwndPrev);

        if (was_zoomed)
        {
            /* restore old MDI child */
            SendMessageW_k( hwndPrev, WM_SETREDRAW, FALSE, 0 );
            ShowWindow_k( hwndPrev, SW_RESTORE );
            SendMessageW_k( hwndPrev, WM_SETREDRAW, TRUE, 0 );

            /* activate new MDI child */
            SetWindowPos_k( hwndTo, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE );
            /* maximize new MDI child */
            ShowWindow_k( hwndTo, SW_MAXIMIZE );
        }
        /* activate new MDI child */
        SetWindowPos_k( hwndTo, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | (activate ? 0 : SWP_NOACTIVATE) );
    }
}
                                ...
                                HWND child, BOOL flagDestroy )
{
    ...
    for (i = 0; i < ci->nActiveChildren; i++)
    {
        if (ci->child[i] == child)
        {
            HWND *new_child = HeapAlloc_k(GetProcessHeap_k(), 0, (ci->nActiveChildren - 1) * sizeof(HWND));
            ...
            HeapFree_k(GetProcessHeap_k(), 0, ci->child);
            ci->child = new_child;
    ...
    if (flagDestroy)
    {
        SendMessageW_k(client, WM_MDIREFRESHMENU, 0, 0);
        MDI_PostUpdate(GetParent_k(child), ci, SB_BOTH+1);
        DestroyWindow_k(child);
    }
...
static LONG MDI_ChildActivate( HWND client, HWND child )
{
    ...
    frame = GetParent_k(client);
    isActiveFrameWnd = (GetActiveWindow_k() == frame);
    prevActiveWnd = clientInfo->hwndActiveChild;

    /* deactivate prev. active child */
    if(prevActiveWnd)
    {
        SendMessageW_k( prevActiveWnd, WM_NCACTIVATE, FALSE, 0L );
        SendMessageW_k( prevActiveWnd, WM_MDIACTIVATE, (WPARAM)prevActiveWnd, (LPARAM)child);
    }
    ...
    if( isActiveFrameWnd )
    {
        SendMessageW_k( child, WM_NCACTIVATE, TRUE, 0L);
         ...
         */
        if (SetFocus_k(client) == client)
            SendMessageW_k( client, WM_SETFOCUS, (WPARAM)client, 0 );
    }

    SendMessageW_k( child, WM_MDIACTIVATE, (WPARAM)prevActiveWnd, (LPARAM)child );
    return TRUE;
}
...
{
 HDC 		hDCSrc  = CreateCompatibleDC_k(0);
 HDC		hDCDest	= CreateCompatibleDC_k(hDCSrc);
 HBITMAP	hbClose = LoadBitmapW_k(0, MAKEINTRESOURCEW(OBM_OLD_CLOSE) );
 HBITMAP	hbCopy;
 HBITMAP	hobjSrc, hobjDest;

 hobjSrc = SelectObject_k(hDCSrc, hbClose);
 hbCopy = CreateCompatibleBitmap_k(hDCSrc,GetSystemMetrics_k(SM_CXSIZE),GetSystemMetrics_k(SM_CYSIZE));
 hobjDest = SelectObject_k(hDCDest, hbCopy);

 BitBlt_k(hDCDest, 0, 0, GetSystemMetrics_k(SM_CXSIZE), GetSystemMetrics_k(SM_CYSIZE),
          hDCSrc, GetSystemMetrics_k(SM_CXSIZE), 0, SRCCOPY);

 SelectObject_k(hDCSrc, hobjSrc);
 DeleteObject_k(hbClose);
 DeleteDC_k(hDCSrc);

 hobjSrc = SelectObject_k( hDCDest, GetStockObject_k(BLACK_PEN) );

 MoveToEx_k( hDCDest, GetSystemMetrics_k(SM_CXSIZE) - 1, 0, NULL );
 LineTo_k( hDCDest, GetSystemMetrics_k(SM_CXSIZE) - 1, GetSystemMetrics_k(SM_CYSIZE) - 1);

 SelectObject_k(hDCDest, hobjSrc );
 SelectObject_k(hDCDest, hobjDest);
 DeleteDC_k(hDCDest);
...
static LONG MDICascade( HWND client, MDICLIENTINFO *ci )
{
    ...
    if (ci->hwndChildMaximized)
        SendMessageW_k(client, WM_MDIRESTORE, (WPARAM)ci->hwndChildMaximized, 0);
    ...
    {
        if (!IsWindowVisible_k( win_array[i] )) continue;
        if (GetWindow_k( win_array[i], GW_OWNER )) continue; /* skip owned windows */
        if (IsIconic_k( win_array[i] ))
        {
            has_icons = TRUE;
            continue;
        }
    ...
    if (total)
    {
        INT delta = 0, n = 0, i;
        POINT pos[2];
        if (has_icons) delta = GetSystemMetrics_k(SM_CYICONSPACING) + GetSystemMetrics_k(SM_CYICON);
        ...
        for (i = total - 1; i >= 0; i--)
        {
            ...
            style = GetWindowLongW_k(win_array[i], GWL_STYLE);

            if (!(style & WS_SIZEBOX)) posOptions |= SWP_NOSIZE;
            SetWindowPos_k( win_array[i], 0, pos[0].x, pos[0].y, pos[1].x, pos[1].y,
                           posOptions);
        }
    }
    HeapFree_k(GetProcessHeap_k(), 0, win_array );

    if (has_icons) ArrangeIconicWindows_k( client );
    return 0;
}
...
static void MDITile( HWND client, MDICLIENTINFO *ci, WPARAM wParam )
{
    ...
    {
        if (!IsWindowVisible_k( win_array[i] )) continue;
        if (GetWindow_k( win_array[i], GW_OWNER )) continue; /* skip owned windows (icon titles) */
        if (IsIconic_k( win_array[i] ))
        {
            has_icons = TRUE;
            continue;
        }
        if ((wParam & MDITILE_SKIPDISABLED) && !IsWindowEnabled_k( win_array[i] )) continue;
        win_array[total++] = win_array[i];
    }
    win_array[total] = 0;
    ...
    if (total)
    {
        ...
        if (has_icons)
        {
            y = rect.bottom - 2 * GetSystemMetrics_k(SM_CYICONSPACING) - GetSystemMetrics_k(SM_CYICON);
            rect.bottom = ( y - GetSystemMetrics_k(SM_CYICON) < rect.top )? rect.bottom: y;
        }
        ...
        for (x = i = 0, c = 1; c <= columns && *pWnd; c++)
        {
            ...
            {
                LONG posOptions = SWP_DRAWFRAME | SWP_NOACTIVATE | SWP_NOZORDER;
                LONG style = GetWindowLongW_k(win_array[i], GWL_STYLE);
                if (!(style & WS_SIZEBOX)) posOptions |= SWP_NOSIZE;

                SetWindowPos_k(*pWnd, 0, x, y, xsize, ysize, posOptions);
                y += ysize;
                pWnd++;
            }
            x += xsize;
...
static BOOL MDI_AugmentFrameMenu( HWND frame, HWND hChild )
{
    ...
    if (!(hSysPopup = GetSystemMenu_k(hChild, FALSE)))
    {
        TRACE("child %p doesn't have a system menu\n", hChild);
	return 0;
    }

    AppendMenuW_k(menu, MF_HELP | MF_BITMAP,
    ...
    hIcon = (HICON)SendMessageW_k(hChild, WM_GETICON, ICON_SMALL, 0);
    if (!hIcon)
        hIcon = (HICON)SendMessageW_k(hChild, WM_GETICON, ICON_BIG, 0);
    if (!hIcon)
        hIcon = LoadImageW_k(0, MAKEINTRESOURCEW(IDI_WINLOGO), IMAGE_ICON, GetSystemMetrics_k(SM_CXSMICON), GetSystemMetrics_k(SM_CYSMICON), LR_DEFAULTCOLOR);
    if (hIcon)
    {
      HDC hMemDC;
      HBITMAP hBitmap, hOldBitmap;
      HBRUSH hBrush;
      HDC hdc = GetDC_k(hChild);

      if (hdc)
      {
        int cx, cy;
        cx = GetSystemMetrics_k(SM_CXSMICON);
        cy = GetSystemMetrics_k(SM_CYSMICON);
        hMemDC = CreateCompatibleDC_k(hdc);
        hBitmap = CreateCompatibleBitmap_k(hdc, cx, cy);
        hOldBitmap = SelectObject_k(hMemDC, hBitmap);
        SetMapMode_k(hMemDC, MM_TEXT);
        hBrush = CreateSolidBrush_k(GetSysColor_k(COLOR_MENU));
        DrawIconEx_k(hMemDC, 0, 0, hIcon, cx, cy, 0, hBrush, DI_NORMAL);
        SelectObject_k (hMemDC, hOldBitmap);
        DeleteObject_k(hBrush);
        DeleteDC_k(hMemDC);
        ReleaseDC_k(hChild, hdc);
        hSysMenuBitmap = hBitmap;
      }
    }

    if( !InsertMenuA_k(menu,0,MF_BYPOSITION | MF_BITMAP | MF_POPUP,
                     (UINT_PTR)hSysPopup, (LPSTR)hSysMenuBitmap))
    {
        TRACE("not inserted\n");
	DestroyMenu_k(hSysPopup);
	return 0;
    }

    EnableMenuItem_k(hSysPopup, SC_SIZE, MF_BYCOMMAND | MF_GRAYED);
    EnableMenuItem_k(hSysPopup, SC_MOVE, MF_BYCOMMAND | MF_GRAYED);
    EnableMenuItem_k(hSysPopup, SC_MAXIMIZE, MF_BYCOMMAND | MF_GRAYED);
    SetMenuDefaultItem_k(hSysPopup, SC_CLOSE, FALSE);
...
static BOOL MDI_RestoreFrameMenu( HWND frame, HWND hChild )
{
    ...
    nItems = GetMenuItemCount_k(menu) - 1;
    iId = GetMenuItemID_k(menu, nItems);
    ...
    GetMenuItemInfoW_k(menu,
		     0,
		     TRUE,
		     &menuInfo);

    RemoveMenu_k(menu,0,MF_BYPOSITION);

    if ( (menuInfo.fType & MFT_BITMAP)           &&
	 (LOWORD(menuInfo.dwTypeData)!=0)        &&
	 (LOWORD(menuInfo.dwTypeData)!=HBITMAP_16(hBmpClose)) )
    {
        DeleteObject_k(HBITMAP_32(LOWORD(menuInfo.dwTypeData)));
    }

    /* close */
    DeleteMenu_k(menu, SC_CLOSE, MF_BYCOMMAND);
    /* restore */
    DeleteMenu_k(menu, SC_RESTORE, MF_BYCOMMAND);
    /* minimize */
    DeleteMenu_k(menu, SC_MINIMIZE, MF_BYCOMMAND);
...
static void MDI_UpdateFrameText( HWND frame, HWND hClient, BOOL repaint, LPCWSTR lpTitle )
{
    ...
    {
        GetWindowTextW_k( frame, lpBuffer, sizeof(lpBuffer)/sizeof(WCHAR) );
        lpTitle = lpBuffer;
    }
    ...
    if (lpTitle)
    {
	HeapFree_k(GetProcessHeap_k(), 0, ci->frameTitle );
	if ((ci->frameTitle = HeapAlloc_k(GetProcessHeap_k(), 0, (strlenW(lpTitle)+1)*sizeof(WCHAR))))
	...
	if (ci->hwndChildMaximized)
	{
		...
		strcatW( lpBuffer, lpBracket );
                if (GetWindowTextW_k( ci->hwndActiveChild, lpBuffer + i_frame_text_length + 4,
    ...
    DefWindowProcW_k( frame, WM_SETTEXT, 0, (LPARAM)lpBuffer );

    if (repaint)
        SetWindowPos_k( frame, 0,0,0,0,0, SWP_FRAMECHANGED |
...
LRESULT MDIClientWndProc_common( HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam, BOOL unicode )
{
    ...
    if (!(ci = get_client_info( hwnd )))
    {
        ...
        return unicode ? DefWindowProcW_k( hwnd, message, wParam, lParam ) :
                         DefWindowProcA_k( hwnd, message, wParam, lParam );
    }
	...
	ci->frameTitle		= NULL;
	ci->mdiFlags		= 0;
        ci->hFrameMenu = GetMenu_k(cs->hwndParent);
	...
	if (!hBmpClose) hBmpClose = CreateMDIMenuBitmap();
      ...
      {
          if( ci->hwndChildMaximized )
              MDI_RestoreFrameMenu(GetParent_k(hwnd), ci->hwndChildMaximized);
      ...
      case WM_MDIACTIVATE:
      {
	    ...
	    SetWindowPos_k((HWND)wParam, 0,0,0,0,0, SWP_NOSIZE | SWP_NOMOVE);
        return 0;
      }
      ...
      case WM_MDICREATE:
        if (lParam)
        {
            ...
            if (unicode)
            {
                ...
                child = CreateWindowExW_k(WS_EX_MDICHILD, csW->szClass,
            ...
            else
            {
                ...
                child = CreateWindowExA_k(WS_EX_MDICHILD, csA->szClass,
      ...
      case WM_MDIGETACTIVE:
          if (lParam) *(BOOL *)lParam = IsZoomed_k(ci->hwndActiveChild);
          return (LRESULT)ci->hwndActiveChild;

      case WM_MDIICONARRANGE:
	ci->mdiFlags |= MDIF_NEEDUPDATE;
        ArrangeIconicWindows_k( hwnd );
	ci->sbRecalc = SB_BOTH+1;
        SendMessageW_k( hwnd, WM_MDICALCCHILDSCROLL, 0, 0 );
        return 0;

      case WM_MDIMAXIMIZE:
	ShowWindow_k( (HWND)wParam, SW_MAXIMIZE );
        return 0;

      case WM_MDINEXT: /* lParam != 0 means previous window */
      {
        HWND hwnd = wParam ? WIN_GetFullHandle((HWND)wParam) : ci->hwndActiveChild;
        HWND next = MDI_GetWindow_k( ci, hwnd, !lParam, 0 );
        MDI_SwitchActiveChild( ci, next, TRUE );
        if(!lParam)
            SetWindowPos_k(hwnd, HWND_BOTTOM, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE);
	break;
      }

      case WM_MDIRESTORE:
        ShowWindow_k( (HWND)wParam, SW_SHOWNORMAL );
        return 0;

      case WM_MDISETMENU:
          return MDISetMenu_k( hwnd, (HMENU)wParam, (HMENU)lParam );
      ...
      case WM_MDITILE:
	ci->mdiFlags |= MDIF_NEEDUPDATE;
        ShowScrollBar_k( hwnd, SB_BOTH, FALSE );
      ...
      case WM_HSCROLL:
	ci->mdiFlags |= MDIF_NEEDUPDATE;
        ScrollChildren_k( hwnd, message, wParam, lParam );
	ci->mdiFlags &= ~MDIF_NEEDUPDATE;
        return 0;

      case WM_SETFOCUS:
          if (ci->hwndActiveChild && !IsIconic_k( ci->hwndActiveChild ))
              SetFocus_k( ci->hwndActiveChild );
          return 0;

      case WM_NCACTIVATE:
        if( ci->hwndActiveChild )
            SendMessageW_k(ci->hwndActiveChild, message, wParam, lParam);
	break;
      ...
      case WM_PARENTNOTIFY:
        ...
        {
        case WM_CREATE:
            if (GetWindowLongW_k((HWND)lParam, GWL_EXSTYLE) & WS_EX_MDICHILD)
            {
                ci->nTotalCreated++;
                ci->nActiveChildren++;

                if (!ci->child)
                    ci->child = HeapAlloc_k(GetProcessHeap_k(), 0, sizeof(HWND));
                else
                    ci->child = HeapReAlloc_k(GetProcessHeap_k(), 0, ci->child, sizeof(HWND) * ci->nActiveChildren);
        ...
        case WM_LBUTTONDOWN:
            ...
            child = ChildWindowFromPoint_k(hwnd, pt);

            TRACE("notification from %p (%i,%i)\n",child,pt.x,pt.y);

            if( child && child != hwnd && child != ci->hwndActiveChild )
                SetWindowPos_k(child, 0,0,0,0,0, SWP_NOSIZE | SWP_NOMOVE );
            break;
            }
      ...
      case WM_SIZE:
        if( ci->hwndActiveChild && IsZoomed_k(ci->hwndActiveChild) )
	{
	    RECT	rect;
	    ...
	    AdjustWindowRectEx_k(&rect, GetWindowLongA_k(ci->hwndActiveChild, GWL_STYLE),
                               0, GetWindowLongA_k(ci->hwndActiveChild, GWL_EXSTYLE) );
	    MoveWindow_k(ci->hwndActiveChild, rect.left, rect.top,
	...
	{
            CalcChildScroll_k(hwnd, ci->sbRecalc-1);
	    ci->sbRecalc = 0;
	    ci->mdiFlags &= ~MDIF_NEEDUPDATE;
	}
        return 0;
 ...
 */
LRESULT WINAPI DefFrameProcA_k( HWND hwnd, HWND hwndMDIClient,
                                UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
        case WM_SETTEXT:
            {
                DWORD len = MultiByteToWideChar_k( CP_ACP, 0, (LPSTR)lParam, -1, NULL, 0 );
                LPWSTR text = HeapAlloc_k(GetProcessHeap_k(), 0, len * sizeof(WCHAR) );
                MultiByteToWideChar_k( CP_ACP, 0, (LPSTR)lParam, -1, text, len );
        ...
        case WM_SETFOCUS:
        case WM_SIZE:
            return DefFrameProcW_k( hwnd, hwndMDIClient, message, wParam, lParam );
        }
    }
    return DefWindowProcA_k(hwnd, message, wParam, lParam);
}
 ...
 */
LRESULT WINAPI DefFrameProcW_k( HWND hwnd, HWND hwndMDIClient,
                                UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
        case WM_COMMAND:
            {
                ...
                if (id <  ci->idFirstChild || id >= ci->idFirstChild + ci->nActiveChildren)
                {
                    ...
                    case SC_PREVWINDOW:
                    case SC_RESTORE:
                        return SendMessageW_k( ci->hwndChildMaximized, WM_SYSCOMMAND,
                                             wParam, lParam);
                    }
                }
                else
                {
                    ...
                    if( childHwnd )
                        SendMessageW_k( hwndMDIClient, WM_MDIACTIVATE, (WPARAM)childHwnd, 0 );
                }
            }
            break;

        case WM_NCACTIVATE:
	    SendMessageW_k(hwndMDIClient, message, wParam, lParam);
	    break;
        ...
        case WM_SETFOCUS:
	    SetFocus_k(hwndMDIClient);
	    break;

        case WM_SIZE:
            MoveWindow_k(hwndMDIClient, 0, 0, LOWORD(lParam), HIWORD(lParam), TRUE);
            break;
        ...
        case WM_NEXTMENU:
            {
                ...
                if (!IsIconic_k(hwnd) && ci->hwndActiveChild && !IsZoomed_k(ci->hwndActiveChild))
                {
                    /* control menu is between the frame system menu and
                     * the first entry of menu bar */
                    WND *wndPtr = WIN_GetPtr(hwnd);

                    if( (wParam == VK_LEFT && GetMenu_k(hwnd) == next_menu->hmenuIn) ||
                        (wParam == VK_RIGHT && GetSubMenu_k(wndPtr->hSysMenu, 0) == next_menu->hmenuIn) )
                    {
                        wndPtr = WIN_GetPtr(ci->hwndActiveChild);
                        next_menu->hmenuNext = GetSubMenu_k(wndPtr->hSysMenu, 0);
 ...
 */
LRESULT WINAPI DefMDIChildProcA_k( HWND hwnd, UINT message,
                                   WPARAM wParam, LPARAM lParam )
{
    HWND client = GetParent_k(hwnd);
    MDICLIENTINFO *ci = get_client_info( client );

    TRACE("%p %04x (%s) %08lx %08lx\n", hwnd, message, SPY_GetMsgName(message, hwnd), wParam, lParam);

    hwnd = WIN_GetFullHandle( hwnd );
    if (!ci) return DefWindowProcA_k( hwnd, message, wParam, lParam );
	...
	if( ci->hwndChildMaximized == hwnd )
	    MDI_UpdateFrameText( GetParent_k(client), client, TRUE, NULL );
        return 1; /* success. FIXME: check text length */
    ...
    case WM_SYSCHAR:
    case WM_DESTROY:
        return DefMDIChildProcW_k( hwnd, message, wParam, lParam );
    }
    return DefWindowProcA_k(hwnd, message, wParam, lParam);
}
 ...
 */
LRESULT WINAPI DefMDIChildProcW_k( HWND hwnd, UINT message,
                                   WPARAM wParam, LPARAM lParam )
{
    HWND client = GetParent_k(hwnd);
    MDICLIENTINFO *ci = get_client_info( client );
    ...
    hwnd = WIN_GetFullHandle( hwnd );
    if (!ci) return DefWindowProcW_k( hwnd, message, wParam, lParam );

    switch (message)
    {
    case WM_SETTEXT:
        DefWindowProcW_k(hwnd, message, wParam, lParam);
        if( ci->hwndChildMaximized == hwnd )
            MDI_UpdateFrameText( GetParent_k(client), client, TRUE, NULL );
        return 1; /* success. FIXME: check text length */
    ...
    case WM_CLOSE:
        SendMessageW_k( client, WM_MDIDESTROY, (WPARAM)hwnd, 0 );
        return 0;
    ...
    case WM_SYSCOMMAND:
        ...
        case SC_MAXIMIZE:
            if (ci->hwndChildMaximized == hwnd)
                return SendMessageW_k( GetParent_k(client), message, wParam, lParam);
            break;
        case SC_NEXTWINDOW:
            SendMessageW_k( client, WM_MDINEXT, (WPARAM)ci->hwndActiveChild, 0);
            return 0;
        case SC_PREVWINDOW:
            SendMessageW_k( client, WM_MDINEXT, (WPARAM)ci->hwndActiveChild, 1);
            return 0;
        }
        break;
    ...
    case WM_SIZE:
        ...
        if( wParam == SIZE_MAXIMIZED )
        {
            ...
            if( hMaxChild)
            {
                SendMessageW_k( hMaxChild, WM_SETREDRAW, FALSE, 0 );

                MDI_RestoreFrameMenu( GetParent_k(client), hMaxChild );
                ShowWindow_k( hMaxChild, SW_SHOWNOACTIVATE );

                SendMessageW_k( hMaxChild, WM_SETREDRAW, TRUE, 0 );
            }
        ...
        {
            HWND switchTo = MDI_GetWindow_k( ci, hwnd, TRUE, WS_MINIMIZE );

            if (!switchTo) switchTo = hwnd;
            SendMessageW_k( switchTo, WM_CHILDACTIVATE, 0, 0 );
	}
    ...
    case WM_NEXTMENU:
        {
            MDINEXTMENU *next_menu = (MDINEXTMENU *)lParam;
            HWND parent = GetParent_k(client);
            ...
            {
                next_menu->hmenuNext = GetMenu_k(parent);
            }
    ...
    case WM_SYSCHAR:
        if (wParam == '-')
        {
            SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_KEYMENU, VK_SPACE);
            return 0;
        }
        break;
 ...
 *    Failure: NULL
 */
HWND WINAPI CreateMDIWindowA_k(
    ...
    LPARAM lParam)         /* [in] Application-defined value */
{
    ...
    return CreateWindowExA_k(WS_EX_MDICHILD, lpClassName, lpWindowName,
 ...
 *    Failure: NULL
 */
HWND WINAPI CreateMDIWindowW_k(
    ...
    LPARAM lParam)         /* [in] Application-defined value */
{
    ...
    return CreateWindowExW_k(WS_EX_MDICHILD, lpClassName, lpWindowName,
                           dwStyle, X, Y, nWidth, nHeight, hWndParent,
                           0, hInstance, (LPVOID)lParam);
}
 ...
 */
BOOL WINAPI TranslateMDISysAccel_k( HWND hwndClient, LPMSG msg )
{
    if (msg->message == WM_KEYDOWN || msg->message == WM_SYSKEYDOWN)
    {
        ...
        if (!ci || !IsWindowEnabled_k(ci->hwndActiveChild)) return 0;

        /* translate if the Ctrl key is down and Alt not. */

        if( (GetKeyState_k(VK_CONTROL) & 0x8000) && !(GetKeyState_k(VK_MENU) & 0x8000))
        {
            switch( msg->wParam )
            {
            case VK_F6:
            case VK_TAB:
                wParam = ( GetKeyState_k(VK_SHIFT) & 0x8000 ) ? SC_NEXTWINDOW : SC_PREVWINDOW;
                break;
            case VK_F4:
            case VK_RBUTTON:
            ...
            TRACE("wParam = %04lx\n", wParam);
            SendMessageW_k(ci->hwndActiveChild, WM_SYSCOMMAND, wParam, msg->wParam);
            return 1;
        }
    }
    return 0; /* failure */
 ...
 */
void WINAPI CalcChildScroll_k( HWND hwnd, INT scroll )
{
    SCROLLINFO info;
    RECT childRect, clientRect;
    HWND *list;

    GetClientRect_k( hwnd, &clientRect );
    SetRectEmpty_k( &childRect );
    ...
    if ((list = WIN_ListChildren( hwnd )))
    {
        ...
        for (i = 0; list[i]; i++)
        {
            DWORD style = GetWindowLongW_k( list[i], GWL_STYLE );
            ...
            if (style & WS_VISIBLE)
            {
                ...
                UnionRect_k( &childRect, &rect, &childRect );
            }
        }
        HeapFree_k(GetProcessHeap_k(), 0, list );
    }
    UnionRect_k( &childRect, &clientRect, &childRect );
	...
	case SB_HORZ:
			...
			SetScrollInfo_k(hwnd, SB_HORZ, &info, TRUE);
	...
	case SB_VERT:
			...
			SetScrollInfo_k(hwnd, SB_VERT, &info, TRUE);
			break;
    }
}
 ...
 */
void WINAPI ScrollChildren_k(HWND hWnd, UINT uMsg, WPARAM wParam,
                             LPARAM lParam)
{
    INT newPos = -1;
    ...
    {
    case WM_HSCROLL:
	GetScrollRange_k(hWnd,SB_HORZ,&minPos,&maxPos);
	curPos = GetScrollPos_k(hWnd,SB_HORZ);
	length = (rect.right - rect.left) / 2;
	shift = GetSystemMetrics_k(SM_CYHSCROLL);
        break;
    case WM_VSCROLL:
	GetScrollRange_k(hWnd,SB_VERT,&minPos,&maxPos);
	curPos = GetScrollPos_k(hWnd,SB_VERT);
	length = (rect.bottom - rect.top) / 2;
	shift = GetSystemMetrics_k(SM_CXVSCROLL);
        break;
    default:
        return;
    }
	...
	case SB_BOTTOM:
			...
			break;
	case SB_ENDSCROLL:
			CalcChildScroll_k(hWnd,(uMsg == WM_VSCROLL)?SB_VERT:SB_HORZ);
			return;
    }
    ...
    SetScrollPos_k(hWnd, (uMsg == WM_VSCROLL)?SB_VERT:SB_HORZ , newPos, TRUE);

    if( uMsg == WM_VSCROLL )
	ScrollWindowEx_k(hWnd ,0 ,curPos - newPos, NULL, NULL, 0, NULL,
			SW_INVALIDATE | SW_ERASE | SW_SCROLLCHILDREN );
    else
	ScrollWindowEx_k(hWnd ,curPos - newPos, 0, NULL, NULL, 0, NULL,
			SW_INVALIDATE | SW_ERASE | SW_SCROLLCHILDREN );
}
 ...
 *    Failure: 0
 */
WORD WINAPI CascadeWindows_k(HWND hwndParent, UINT wFlags, const RECT *lpRect,
 ...
 */
WORD WINAPI CascadeChildWindows_k( HWND parent, UINT flags )
{
    return CascadeWindows_k( parent, flags, NULL, 0, NULL );
}
 ...
 *    Failure: 0
 */
WORD WINAPI TileWindows_k(HWND hwndParent, UINT wFlags, const RECT *lpRect,
	     UINT cKids, const HWND *lpKids)
{
    FIXME("(%p,0x%08x,...,%u,...): stub\n", hwndParent, wFlags, cKids);
    return 0;
 ...
 */
WORD WINAPI TileChildWindows_k( HWND parent, UINT flags )
{
    return TileWindows_k( parent, flags, NULL, 0, NULL );
}
...
static INT_PTR WINAPI MDI_MoreWindowsDlgProc (HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    switch (iMsg)
    {
       case WM_INITDIALOG:
       {
           ...
           HWND hListBox = GetDlgItem_k(hDlg, MDI_IDC_LISTBOX);

           for (i = 0; i < ci->nActiveChildren; i++)
           {
               WCHAR buffer[MDI_MAXTITLELENGTH];

               if (!InternalGetWindowText_k( ci->child[i], buffer, sizeof(buffer)/sizeof(WCHAR) ))
                   continue;
               SendMessageW_k(hListBox, LB_ADDSTRING, 0, (LPARAM)buffer );
               SendMessageW_k(hListBox, LB_SETITEMDATA, i, (LPARAM)ci->child[i] );
               length = strlenW(buffer);  /* FIXME: should use GetTextExtentPoint */
               if (length > widest)
                   widest = length;
           }
           /* Make sure the horizontal scrollbar scrolls ok */
           SendMessageW_k(hListBox, LB_SETHORIZONTALEXTENT, widest * 6, 0);

           /* Set the current selection */
           SendMessageW_k(hListBox, LB_SETCURSEL, MDI_MOREWINDOWSLIMIT, 0);
           return TRUE;
       }
       ...
       case WM_COMMAND:
           switch (LOWORD(wParam))
           {
                ...
                case IDOK:
                {
                     ...
                     */
                    HWND hListBox     = GetDlgItem_k(hDlg, MDI_IDC_LISTBOX);
                    UINT index        = SendMessageW_k(hListBox, LB_GETCURSEL, 0, 0);
                    LRESULT res = SendMessageW_k(hListBox, LB_GETITEMDATA, index, 0);
                    EndDialog_k(hDlg, res);
                    return TRUE;
                }
                case IDCANCEL:
                    EndDialog_k(hDlg, 0);
...
static HWND MDI_MoreWindowsDialog(HWND hwnd)
{
    ...
    hRes = FindResourceA_k(user32_module, "MDI_MOREWINDOWS", (LPSTR)RT_DIALOG);

    if (hRes == 0)
        return 0;

    hDlgTmpl = LoadResource_k(user32_module, hRes );

    if (hDlgTmpl == 0)
        return 0;

    template = LockResource_k( hDlgTmpl );

    if (template == 0)
        return 0;

    return (HWND) DialogBoxIndirectParamA_k(user32_module, template, hwnd,
