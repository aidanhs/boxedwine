...
static WINDOWPROC winproc_array[MAX_WINPROCS] =
{
    ...
    { DefWindowProcA_k, DefWindowProcW_k },        /* WINPROC_DEFWND */
    { DefDlgProcA_k, DefDlgProcW_k },              /* WINPROC_DIALOG */
    ...
    { NULL, DesktopWndProc_k },                  /* WINPROC_DESKTOP */
    { NULL, IconTitleWndProc_k },                /* WINPROC_ICONTITLE */
    { NULL, PopupMenuWndProc_k },                /* WINPROC_MENU */
    { NULL, MessageWndProc_k },                  /* WINPROC_MESSAGE */
};
...
static INLINE void *get_buffer( void *static_buffer, size_t size, size_t need )
{
    ...
    return HeapAlloc_k( GetProcessHeap_k(), 0, need );
}

static INLINE void free_buffer( void *static_buffer, void *buffer )
{
    if (buffer != static_buffer) HeapFree_k( GetProcessHeap_k(), 0, buffer );
}
...
static INLINE WINDOWPROC *alloc_winproc( WNDPROC func, BOOL unicode )
{
    ...
    EnterCriticalSection_k( &winproc_cs );
    ...
    LeaveCriticalSection_k( &winproc_cs );
    return proc;
}
...
static WPARAM map_wparam_char_WtoA( WPARAM wParam, DWORD len )
{
    ...
    len = WideCharToMultiByte_k( cp, 0, &wch, 1, (LPSTR)ch, len, NULL, NULL );
    if (len == 2)
...
static LRESULT call_dialog_proc( HWND hwnd, UINT msg, WPARAM wp, LPARAM lp, LRESULT *result, void *arg )
{
    ...
    *result = GetWindowLongPtrW_k( hwnd, DWLP_MSGRESULT );
                              ...
                              LPARAM lParam, LRESULT *result, void *arg, enum wm_char_mapping mapping )
{
    ...
    case WM_CREATE:
        {
            ...
            if (!IS_INTRESOURCE(csA->lpszClass))
            {
                ...
                RtlMultiByteToUnicodeSize_k( &class_lenW, csA->lpszClass, class_lenA );
            }
            if (!IS_INTRESOURCE(csA->lpszName))
            {
                name_lenA = strlen(csA->lpszName) + 1;
                RtlMultiByteToUnicodeSize_k( &name_lenW, csA->lpszName, name_lenA );
            }
            ...
            {
                csW.lpszClass = ptr;
                RtlMultiByteToUnicodeN_k( ptr, class_lenW, NULL, csA->lpszClass, class_lenA );
            }
            if (name_lenW)
            {
                csW.lpszName = ptr + class_lenW/sizeof(WCHAR);
                RtlMultiByteToUnicodeN_k( ptr + class_lenW/sizeof(WCHAR), name_lenW, NULL,
    ...
    case WM_MDICREATE:
        {
            ...
            if (!IS_INTRESOURCE(csA->szTitle))
            {
                ...
                RtlMultiByteToUnicodeSize_k( &title_lenW, csA->szTitle, title_lenA );
            }
            if (!IS_INTRESOURCE(csA->szClass))
            {
                class_lenA = strlen(csA->szClass) + 1;
                RtlMultiByteToUnicodeSize_k( &class_lenW, csA->szClass, class_lenA );
            }
            ...
            {
                csW.szTitle = ptr;
                RtlMultiByteToUnicodeN_k( ptr, title_lenW, NULL, csA->szTitle, title_lenA );
            }
            if (class_lenW)
            {
                csW.szClass = ptr + title_lenW/sizeof(WCHAR);
                RtlMultiByteToUnicodeN_k( ptr + title_lenW/sizeof(WCHAR), class_lenW, NULL,
    ...
    case WM_ASKCBFORMATNAME:
        {
            ...
            if (wParam)
            {
                len = 0;
                if (*result)
                    RtlUnicodeToMultiByteN_k( str, wParam - 1, &len, ptr, strlenW(ptr) * sizeof(WCHAR) );
                str[len] = 0;
                *result = len;
            }
            free_buffer( buffer, ptr );
    ...
    case EM_REPLACESEL:
        ...
        else
        {
            ...
            RtlMultiByteToUnicodeSize_k( &lenW, strA, lenA );
            if ((ptr = get_buffer( buffer, sizeof(buffer), lenW )))
            {
                RtlMultiByteToUnicodeN_k( ptr, lenW, NULL, strA, lenA );
                ret = callback( hwnd, msg, wParam, (LPARAM)ptr, result, arg );
                free_buffer( buffer, ptr );
            }
        }
    ...
    case CB_GETLBTEXT:
        if (lParam && WINPROC_TestLBForStr( hwnd, msg ))
        {
            ...
            {
                DWORD len;
                RtlUnicodeToMultiByteN_k( (LPSTR)lParam, ~0u, &len,
    ...
    case EM_GETLINE:
        {
            ...
            *((WORD *)ptr) = len;   /* store the length */
            ...
            {
                DWORD reslen;
                RtlUnicodeToMultiByteN_k( (LPSTR)lParam, len, &reslen, ptr, *result * sizeof(WCHAR) );
    ...
    case LB_GETTEXTLEN:
        ...
        if (*result >= 0)
        {
            ...
            if (callback == call_window_proc)  /* FIXME: hack */
                ...
                tmp = SendMessageW_k( hwnd, msgGetText, wp, (LPARAM)ptr );
            RtlUnicodeToMultiByteSize_k( &len, ptr, tmp * sizeof(WCHAR) );
            *result = len;
            free_buffer( buffer, ptr );
        }
        break;
                                     ...
                                     LPARAM lParam, LRESULT *result, void *arg )
{
    ...
    case WM_CREATE:
        {
            ...
            if (!IS_INTRESOURCE(csW->lpszClass))
            {
                ...
                RtlUnicodeToMultiByteSize_k(&class_lenA, csW->lpszClass, class_lenW);
            }
            if (!IS_INTRESOURCE(csW->lpszName))
            {
                name_lenW = (strlenW(csW->lpszName) + 1) * sizeof(WCHAR);
                RtlUnicodeToMultiByteSize_k(&name_lenA, csW->lpszName, name_lenW);
            }
            ...
            if (class_lenA)
            {
                RtlUnicodeToMultiByteN_k(cls, class_lenA, NULL, csW->lpszClass, class_lenW);
                csA.lpszClass = cls;
            }
            if (name_lenA)
            {
                char *name = cls + class_lenA;
                RtlUnicodeToMultiByteN_k(name, name_lenA, NULL, csW->lpszName, name_lenW);
                csA.lpszName = name;
            }
    ...
    case WM_ASKCBFORMATNAME:
        {
            ...
            if (len)
            {
                if (*result)
                {
                    RtlMultiByteToUnicodeN_k( (LPWSTR)lParam, wParam*sizeof(WCHAR), &len, ptr, strlen(ptr)+1 );
    ...
    case EM_REPLACESEL:
        ...
        else
        {
            ...
            RtlUnicodeToMultiByteSize_k( &lenA, strW, lenW );
            if ((ptr = get_buffer( buffer, sizeof(buffer), lenA )))
            {
                RtlUnicodeToMultiByteN_k( ptr, lenA, NULL, strW, lenW );
                ret = callback( hwnd, msg, wParam, (LPARAM)ptr, result, arg );
                free_buffer( buffer, ptr );
            }
        }
    ...
    case WM_MDICREATE:
        {
            ...
            if (!IS_INTRESOURCE(csW->szTitle))
            {
                ...
                RtlUnicodeToMultiByteSize_k( &title_lenA, csW->szTitle, title_lenW );
            }
            if (!IS_INTRESOURCE(csW->szClass))
            {
                class_lenW = (strlenW(csW->szClass) + 1) * sizeof(WCHAR);
                RtlUnicodeToMultiByteSize_k( &class_lenA, csW->szClass, class_lenW );
            }
            ...
            if (title_lenA)
            {
                RtlUnicodeToMultiByteN_k( ptr, title_lenA, NULL, csW->szTitle, title_lenW );
                csA.szTitle = ptr;
            }
            if (class_lenA)
            {
                RtlUnicodeToMultiByteN_k( ptr + title_lenA, class_lenA, NULL, csW->szClass, class_lenW );
    ...
    case CB_GETLBTEXT:
        if (lParam && WINPROC_TestLBForStr( hwnd, msg ))
        {
            ...
            {
                DWORD len;
                RtlMultiByteToUnicodeN_k( (LPWSTR)lParam, ~0u, &len, buffer, strlen(buffer) + 1 );
    ...
    case EM_GETLINE:
        {
            ...
            *((WORD *)ptr) = len * 2;   /* store the length */
            ...
            {
                DWORD reslen;
                RtlMultiByteToUnicodeN_k( (LPWSTR)lParam, len*sizeof(WCHAR), &reslen, ptr, *result );
    ...
    case WM_CHAR:
        {
            ...
            char ch[2];
            DWORD cp = get_input_codepage();
            DWORD len = WideCharToMultiByte_k( cp, 0, &wch, 1, ch, 2, NULL, NULL );
                          ...
                          LRESULT *result, BOOL unicode, enum wm_char_mapping mapping )
{
    ...
    if (wndPtr == WND_DESKTOP) return FALSE;
    if (wndPtr->tid != GetCurrentThreadId_k())
    {
        return FALSE;
    }
    func = wndPtr->winproc;
 ...
 *   ECMA-234, Win32
 */
LRESULT WINAPI CallWindowProcA_k(
 ...
 * See CallWindowProcA.
 */
LRESULT WINAPI CallWindowProcW_k( WNDPROC func, HWND hwnd, UINT msg,
                                  WPARAM wParam, LPARAM lParam )
{
    WINDOWPROC *proc;
    LRESULT result;
...
INT_PTR WINPROC_CallDlgProcA( DLGPROC func, HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    ...
    else if (proc == WINPROC_PROC16)
    {
        ...
        SetWindowLongPtrW_k( hwnd, DWLP_MSGRESULT, result );
    }
    else if (proc->procW)
    {
...
static DWORD wait_message( DWORD count, CONST HANDLE *handles, DWORD timeout, DWORD mask, DWORD flags )
{
    ...
    if (ret == WAIT_TIMEOUT && !count && !timeout) NtYieldExecution_k();
 ...
 */
void WINAPI UserRegisterWowHandlers_k( const struct wow_handlers16 *new, struct wow_handlers32 *orig )
{
