...
typedef struct {
    ...
    INT      xTab;		/* X position of text after Tab */
...
static HMENU top_popup_hmenu;
  ...
  /* Flag set by EndMenu_k() to force an exit from menu tracking */
...
static HFONT get_menu_font( BOOL bold )
{
    ...
    if (!ret)
    {
        ...
        SystemParametersInfoW_k(SPI_GETNONCLIENTMETRICS, sizeof(NONCLIENTMETRICSW), &ncm, 0);
        ...
        if (!(ret = CreateFontIndirectW_k( &ncm.lfMenuFont ))) return 0;
        prev = InterlockedCompareExchangePointer_k( (void **)(bold ? &hMenuFontBold : &hMenuFont),
                                                  ret, NULL );
        if (prev)
        {
...
static HBITMAP get_arrow_bitmap(void)
{
    ...
    if (!arrow_bitmap) arrow_bitmap = LoadBitmapW_k(0, MAKEINTRESOURCEW(OBM_MNARROW));
    return arrow_bitmap;
}
...
static HBITMAP get_down_arrow_bitmap(void)
{
    ...
    if (!arrow_bitmap) arrow_bitmap = LoadBitmapW_k(0, MAKEINTRESOURCEW(OBM_DNARROW));
    return arrow_bitmap;
}
...
static HBITMAP get_down_arrow_inactive_bitmap(void)
{
    ...
    if (!arrow_bitmap) arrow_bitmap = LoadBitmapW_k(0, MAKEINTRESOURCEW(OBM_DNARROWI));
    return arrow_bitmap;
}
...
static HBITMAP get_up_arrow_bitmap(void)
{
    ...
    if (!arrow_bitmap) arrow_bitmap = LoadBitmapW_k(0, MAKEINTRESOURCEW(OBM_UPARROW));
    return arrow_bitmap;
}
...
static HBITMAP get_up_arrow_inactive_bitmap(void)
{
    ...
    if (!arrow_bitmap) arrow_bitmap = LoadBitmapW_k(0, MAKEINTRESOURCEW(OBM_UPARROWI));
    return arrow_bitmap;
}
...
static HMENU MENU_CopySysPopup(BOOL mdi)
{
    ...
    HMENU hMenu = LoadMenuW_k(user32_module, (mdi ? sysmenumdiW : sysmenuW));
    ...
    if( hMenu ) {
        ...
        SetMenuInfo_k( hMenu, &minfo);
        miteminfo.cbSize = sizeof( MENUITEMINFOW);
        miteminfo.fMask = MIIM_BITMAP;
        miteminfo.hbmpItem = HBMMENU_POPUP_CLOSE;
        SetMenuItemInfoW_k( hMenu, SC_CLOSE, FALSE, &miteminfo);
        miteminfo.hbmpItem = HBMMENU_POPUP_RESTORE;
        SetMenuItemInfoW_k( hMenu, SC_RESTORE, FALSE, &miteminfo);
        miteminfo.hbmpItem = HBMMENU_POPUP_MAXIMIZE;
        SetMenuItemInfoW_k( hMenu, SC_MAXIMIZE, FALSE, &miteminfo);
        miteminfo.hbmpItem = HBMMENU_POPUP_MINIMIZE;
        SetMenuItemInfoW_k( hMenu, SC_MINIMIZE, FALSE, &miteminfo);
	SetMenuDefaultItem_k(hMenu, SC_CLOSE, FALSE);
    }
    else
...
static HMENU MENU_GetSysMenu( HWND hWnd, HMENU hPopupMenu )
{
	...
	if (!hPopupMenu)
        {
            if (GetWindowLongW_k(hWnd, GWL_EXSTYLE) & WS_EX_MDICHILD)
	...
	if (hPopupMenu)
	{
            if (GetClassLongW_k(hWnd, GCL_STYLE) & CS_NOCLOSE)
                DeleteMenu_k(hPopupMenu, SC_CLOSE, MF_BYCOMMAND);

	    InsertMenuW_k( hMenu, -1, MF_SYSMENU | MF_POPUP | MF_BYPOSITION,
...
static void MENU_InitSysMenuPopup( HMENU hmenu, DWORD style, DWORD clsStyle )
{
    ...
    EnableMenuItem_k( hmenu, SC_SIZE, (gray ? MF_GRAYED : MF_ENABLED) );
    gray = ((style & WS_MAXIMIZE) != 0);
    EnableMenuItem_k( hmenu, SC_MOVE, (gray ? MF_GRAYED : MF_ENABLED) );
    gray = !(style & WS_MINIMIZEBOX) || (style & WS_MINIMIZE);
    EnableMenuItem_k( hmenu, SC_MINIMIZE, (gray ? MF_GRAYED : MF_ENABLED) );
    gray = !(style & WS_MAXIMIZEBOX) || (style & WS_MAXIMIZE);
    EnableMenuItem_k( hmenu, SC_MAXIMIZE, (gray ? MF_GRAYED : MF_ENABLED) );
    gray = !(style & (WS_MAXIMIZE | WS_MINIMIZE));
    EnableMenuItem_k( hmenu, SC_RESTORE, (gray ? MF_GRAYED : MF_ENABLED) );
    gray = (clsStyle & CS_NOCLOSE) != 0;

    /* The menu item must keep its state if it's disabled */
    if(gray)
	EnableMenuItem_k( hmenu, SC_CLOSE, MF_GRAYED);
}
...
{
    /* delete text */
    HeapFree_k(GetProcessHeap_k(), 0, item->text );
}
...
MENU_AdjustMenuItemRect(const POPUPMENU *menu, LPRECT rect)
{
    if (menu->bScrolling)
    {
        ...
        GetObjectW_k(get_up_arrow_bitmap(), sizeof(bmp), &bmp);
					...
					POINT pt, UINT *pos )
{
    ...
    if (!GetWindowRect_k(menu->hWnd, &rect)) return NULL;
    if (GetWindowLongW_k( menu->hWnd, GWL_EXSTYLE ) & WS_EX_LAYOUTRTL) pt.x = rect.right - 1 - pt.x;
    else pt.x -= rect.left;
    pt.y -= rect.top;
    item = menu->items;
    for (i = 0; i < menu->nItems; i++, item++)
                                ...
                                WCHAR key, BOOL forceMenuChar )
{
    ...
    if (!IsMenu_k( hmenu )) hmenu = GetSubMenu_k( get_win_sys_menu(hwndOwner), 0);
	...
	{
	     UINT i;
	     BOOL cjk = GetSystemMetrics_k( SM_DBCSENABLED );
	...
	menuchar = SendMessageW_k( hwndOwner, WM_MENUCHAR,
                                    ...
                                    HWND hwndOwner)
{
    ...
    case (INT_PTR)HBMMENU_CALLBACK:
        {
            ...
            SendMessageW_k( hwndOwner, WM_MEASUREITEM, 0, (LPARAM)&measItem);
    ...
    case (INT_PTR)HBMMENU_MBAR_CLOSE_D:
        size->cx = GetSystemMetrics_k( SM_CYMENU ) - 4;
        size->cy = size->cx;
        return;
    ...
    case (INT_PTR)HBMMENU_POPUP_MINIMIZE:
        size->cx = GetSystemMetrics_k( SM_CXMENUSIZE);
        size->cy = GetSystemMetrics_k( SM_CYMENUSIZE);
        return;
    }
    if (GetObjectW_k(bmp, sizeof(bm), &bm ))
    {
        size->cx = bm.bmWidth;
        size->cy = bm.bmHeight;
    }
                    ...
                    HMENU hmenu, HWND hwndOwner, UINT odaction, BOOL menuBar)
{
	...
	switch((INT_PTR)hbmToDraw)
        ...
        case (INT_PTR)HBMMENU_SYSTEM:
            ...
            {
                bmp = (HBITMAP)lpitem->dwItemData;
                if (!GetObjectW_k( bmp, sizeof(bm), &bm )) return;
            }
            else
            {
                static HBITMAP hBmpSysMenu;

                if (!hBmpSysMenu) hBmpSysMenu = LoadBitmapW_k(0, MAKEINTRESOURCEW(OBM_CLOSE));
                bmp = hBmpSysMenu;
                if (!GetObjectW_k( bmp, sizeof(bm), &bm )) return;
        ...
        case (INT_PTR)HBMMENU_CALLBACK:
            {
                ...
                SendMessageW_k( hwndOwner, WM_DRAWITEM, 0, (LPARAM)&drawItem);
                return;
            }
            break;
        case (INT_PTR)HBMMENU_POPUP_CLOSE:
        ...
        if (bmchr)
        {
            ...
            hfont = CreateFontIndirectW_k( &logfont);
            hfontsav = SelectObject_k(hdc, hfont);
            TextOutW_k( hdc,  rect->left, rect->top + 2, &bmchr, 1);
            SelectObject_k(hdc, hfontsav);
        ...
        else
        {
            ...
            DrawFrameControl_k( hdc, &r, DFC_CAPTION, flags );
        }
        return;
    }

    if (!bmp || !GetObjectW_k( bmp, sizeof(bm), &bm )) return;

 got_bitmap:
    hdcMem = CreateCompatibleDC_k( hdc );
    SelectObject_k( hdcMem, bmp );
    ...
    if ((lpitem->fState & MF_HILITE) && lpitem->hbmpItem)
        SetBkColor_k(hdc, GetSysColor_k(COLOR_HIGHLIGHT));
    BitBlt_k( hdc, left, top, w, h, hdcMem, bmp_xoffset, 0, rop );
    DeleteDC_k( hdcMem );
}
...
{
    WCHAR *p;
    UINT check_bitmap_width = GetSystemMetrics_k( SM_CXMENUCHECK );
    UINT arrow_bitmap_width;
    BITMAP bm;
    INT itemheight;

    TRACE("dc=%p owner=%p (%d,%d)\n", hdc, hwndOwner, orgX, orgY);
    //debug_print_menuitem("MENU_CalcItemSize: menuitem:", lpitem, (menuBar ? " (MenuBar)" : ""));

    GetObjectW_k( get_arrow_bitmap(), sizeof(bm), &bm );
    arrow_bitmap_width = bm.bmWidth;

    /* not done in Menu_Init: GetDialogBaseUnits_k() breaks there */
    if( !menucharsize.cx ) {
        menucharsize.cx = GdiGetCharDimensions_k( hdc, NULL, &menucharsize.cy );
        /* Win95/98/ME will use menucharsize.cy here. Testing is possible
         * but it is unlikely an application will depend on that */
        ODitemheight = HIWORD( GetDialogBaseUnits_k());
    }

    SetRect_k( &lpitem->rect, orgX, orgY, orgX, orgY );
    ...
    if (lpitem->fType & MF_OWNERDRAW)
    {
        ...
        mis.itemWidth  = 0;
        SendMessageW_k( hwndOwner, WM_MEASUREITEM, 0, (LPARAM)&mis );
        ...
        if (menuBar) {
            ...
            lpitem->rect.bottom += GetSystemMetrics_k(SM_CYMENUSIZE);
        } else
    ...
    {
        lpitem->rect.bottom += GetSystemMetrics_k( SM_CYMENUSIZE)/2;
        if( !menuBar)
    ...
    if (!menuBar) {
        if (lpitem->hbmpItem) {
            ...
            lpitem->bmpsize = size;
            lppop->textOffset = max( lppop->textOffset, (WORD)size.cx);
	...
	if ( lpitem->fState & MFS_DEFAULT ) {
	     hfontOld = SelectObject_k( hdc, get_menu_font(TRUE) );
	}
        if (menuBar) {
            txtheight = DrawTextW_k( hdc, lpitem->text, -1, &rc,
                    DT_SINGLELINE|DT_CALCRECT); 
            lpitem->rect.right  += rc.right - rc.left;
            itemheight = max( max( itemheight, txtheight),
                    GetSystemMetrics_k( SM_CYMENU) - 1);
            ...
            if ((p = strchrW( lpitem->text, '\t' )) != NULL) {
                ...
                txtheight = DrawTextW_k( hdc, lpitem->text, n, &rc,
                        DT_SINGLELINE|DT_CALCRECT);
                txtwidth = rc.right - rc.left;
                p += 1; /* advance past the Tab */
                /* get text size after the tab */
                tmpheight = DrawTextW_k( hdc, p, -1, &tmprc,
                        DT_SINGLELINE|DT_CALCRECT);
	...
	if (hfontOld) SelectObject_k(hdc, hfontOld);
    } else if( menuBar) {
        itemheight = max( itemheight, GetSystemMetrics_k(SM_CYMENU)-1);
    }
 ...
 */
static UINT MENU_GetMaxPopupHeight(const POPUPMENU *lppop)
{
    if (lppop->cyMax)
        return lppop->cyMax;
    return GetSystemMetrics_k(SM_CYSCREEN) - GetSystemMetrics_k(SM_CYBORDER);
}
...
static void MENU_PopupMenuCalcSize( LPPOPUPMENU lppop )
{
    ...
    SelectObject_k( hdc, get_menu_font(FALSE));
                                  ...
                                  LPPOPUPMENU lppop, HWND hwndOwner )
{
    ...
    lppop->Width  = (WORD)(lprect->right - lprect->left);
    lppop->Height = 0;
    maxY = lprect->top+1;
    start = 0;
    helpPos = ~0U;
    ...
    lprect->bottom = maxY;
    lppop->Height = (WORD)(lprect->bottom - lprect->top);
 ...
 * Draw scroll arrows.
 */
static void MENU_DrawScrollArrows(const POPUPMENU *lppop, HDC hdc)
{
    HDC hdcMem = CreateCompatibleDC_k(hdc);
    HBITMAP hOrigBitmap;
    UINT arrow_bitmap_width, arrow_bitmap_height;
    BITMAP bmp;
    RECT rect;

    GetObjectW_k(get_down_arrow_bitmap(), sizeof(bmp), &bmp);
    arrow_bitmap_width = bmp.bmWidth;
    arrow_bitmap_height = bmp.bmHeight;

    
    if (lppop->nScrollPos)
        hOrigBitmap = SelectObject_k(hdcMem, get_up_arrow_bitmap());
    else
        hOrigBitmap = SelectObject_k(hdcMem, get_up_arrow_inactive_bitmap());
    rect.left = 0;
    rect.top = 0;
    rect.right = lppop->Width;
    rect.bottom = arrow_bitmap_height;
    FillRect_k(hdc, &rect, GetSysColorBrush_k(COLOR_MENU));
    BitBlt_k(hdc, (lppop->Width - arrow_bitmap_width) / 2, 0,
    ...
    if (lppop->nScrollPos < lppop->nTotalHeight - (MENU_GetMaxPopupHeight(lppop) - 2 * arrow_bitmap_height))
        SelectObject_k(hdcMem, get_down_arrow_bitmap());
    else
        SelectObject_k(hdcMem, get_down_arrow_inactive_bitmap());
    BitBlt_k(hdc, (lppop->Width - arrow_bitmap_width) / 2,
           lppop->Height - arrow_bitmap_height,
           arrow_bitmap_width, arrow_bitmap_height, hdcMem, 0, 0, SRCCOPY);
    SelectObject_k(hdcMem, hOrigBitmap);
    DeleteDC_k(hdcMem);
}
        ...
        UINT arrow_bitmap_height)
{
    ...
    hOrigBitmap = SelectObject_k( hdcMem, get_arrow_bitmap() );
    BitBlt_k( hdc, rect.right - arrow_bitmap_width - 1,
			       ...
			       UINT height, BOOL menuBar, UINT odaction )
{
    ...
    if (!menuBar) {
        BITMAP bmp;
        GetObjectW_k( get_arrow_bitmap(), sizeof(bmp), &bmp );
        arrow_bitmap_width = bmp.bmWidth;
        arrow_bitmap_height = bmp.bmHeight;
    }
    ...
    SystemParametersInfoW_k (SPI_GETFLATMENU, 0, &flat_menu, 0);
    ...
    if (lpitem->fState & MF_HILITE)
    {
	    ...
	    SetTextColor_k(hdc, GetSysColor_k(COLOR_MENUTEXT));
            SetBkColor_k(hdc, GetSysColor_k(COLOR_MENU));
	} else {
	    if(lpitem->fState & MF_GRAYED)
		SetTextColor_k(hdc, GetSysColor_k(COLOR_GRAYTEXT));
	    else
		SetTextColor_k(hdc, GetSysColor_k(COLOR_HIGHLIGHTTEXT));
            SetBkColor_k(hdc, GetSysColor_k(COLOR_HIGHLIGHT));
	}
    }
    else
	...
	if (lpitem->fState & MF_GRAYED)
	    ...
	    SetTextColor_k( hdc, GetSysColor_k( COLOR_MENUTEXT ) );
	SetBkColor_k( hdc, GetSysColor_k( bkgnd ) );
    }
	...
	dis.CtlID     = 0;
        ...
        dis.hDC        = hdc;
        dis.rcItem     = rect;
        //TRACE("Ownerdraw: owner=%p itemID=%d, itemState=%d, itemAction=%d, " "hwndItem=%p, hdc=%p, rcItem=%s\n", hwndOwner, dis.itemID, dis.itemState, dis.itemAction, dis.hwndItem, dis.hDC, wine_dbgstr_rect( &dis.rcItem));
        SendMessageW_k( hwndOwner, WM_DRAWITEM, 0, (LPARAM)&dis );
    ...
    if (lpitem->fState & MF_HILITE)
    {
        if (flat_menu)
        {
            InflateRect_k(&rect, -1, -1);
            FillRect_k(hdc, &rect, GetSysColorBrush_k(COLOR_MENUHILIGHT));
            InflateRect_k(&rect, 1, 1);
            FrameRect_k(hdc, &rect, GetSysColorBrush_k(COLOR_HIGHLIGHT));
        }
        else
        {
            if(menuBar)
                DrawEdge_k(hdc, &rect, BDR_SUNKENOUTER, BF_RECT);
            else
                FillRect_k(hdc, &rect, GetSysColorBrush_k(COLOR_HIGHLIGHT));
        }
    }
    else
        FillRect_k( hdc, &rect, GetSysColorBrush_k(bkgnd) );
    ...
    if (!menuBar && (lpitem->fType & MF_MENUBARBREAK))
    {
        ...
        if (flat_menu)
        {
            oldPen = SelectObject_k( hdc, SYSCOLOR_GetPen(COLOR_BTNSHADOW) );
            MoveToEx_k( hdc, rc.left, rc.top, NULL );
            LineTo_k( hdc, rc.left, rc.bottom );
            SelectObject_k( hdc, oldPen );
        }
        else
            DrawEdge_k(hdc, &rc, EDGE_ETCHED, BF_LEFT);
    }
    ...
    if (lpitem->fType & MF_SEPARATOR)
    {
        ...
        if (flat_menu)
        {
            oldPen = SelectObject_k( hdc, SYSCOLOR_GetPen(COLOR_BTNSHADOW) );
            MoveToEx_k( hdc, rc.left, rc.top, NULL );
            LineTo_k( hdc, rc.right, rc.top );
            SelectObject_k( hdc, oldPen );
        }
        else
            DrawEdge_k(hdc, &rc, EDGE_ETCHED, BF_TOP);
        return;
    }

	/* helper lines for debugging */
/*	FrameRect_k(hdc, &rect, GetStockObject_k(BLACK_BRUSH));
	SelectObject_k( hdc, SYSCOLOR_GetPen(COLOR_WINDOWFRAME) );
	MoveToEx_k( hdc, rect.left, (rect.top + rect.bottom)/2, NULL );
	LineTo_k( hdc, rect.right, (rect.top + rect.bottom)/2 );
*/
    ...
    if (lpitem->hbmpItem) {
        ...
        if( menuBar) {
            ...
            bmprc.left = 4 + GetSystemMetrics_k(SM_CXMENUCHECK);
    ...
    if (!menuBar)
    {
        ...
        int checked = FALSE;
        UINT check_bitmap_width = GetSystemMetrics_k( SM_CXMENUCHECK );
        UINT check_bitmap_height = GetSystemMetrics_k( SM_CYMENUCHECK );
        /* Draw the check mark
         *
         * FIXME:
        ...
        if( !(menu->dwStyle & MNS_NOCHECK)) {
            ...
            if (bm)  /* we have a custom bitmap */
            {
                ...
                SelectObject_k( hdcMem, bm );
                BitBlt_k( hdc, rc.left, (y - check_bitmap_height) / 2,
            ...
            {
                RECT r;
                HBITMAP bm = CreateBitmap_k( check_bitmap_width,
                        check_bitmap_height, 1, 1, NULL );
                HDC hdcMem = CreateCompatibleDC_k( hdc );

                SelectObject_k( hdcMem, bm );
                SetRect_k( &r, 0, 0, check_bitmap_width, check_bitmap_height);
                DrawFrameControl_k( hdcMem, &r, DFC_MENU,
                        (lpitem->fType & MFT_RADIOCHECK) ?
                        DFCS_MENUBULLET : DFCS_MENUCHECK );
                BitBlt_k( hdc, rc.left, (y - r.bottom) / 2, r.right, r.bottom,
                        hdcMem, 0, 0, SRCCOPY );
                DeleteDC_k( hdcMem );
                DeleteObject_k( bm );
                checked = TRUE;
        ...
        if( lpitem->hbmpItem &&
            ...
            SetViewportOrgEx_k( hdc, rect.left, rect.top, &origorg);
            MENU_DrawBitmapItem(hdc, lpitem, &bmprc, hmenu, hwndOwner,
                    odaction, FALSE);
            SetViewportOrgEx_k( hdc, origorg.x, origorg.y, NULL);
        }
	/* Draw the popup-menu arrow */
        if (lpitem->fType & MF_POPUP)
            draw_popup_arrow( hdc, rect, arrow_bitmap_width,
	...
	if(lpitem->fState & MF_GRAYED)
	{
		...
		SetTextColor_k(hdc, RGB(0xff, 0xff, 0xff));
		DrawTextW_k( hdc, lpitem->text, i, &rect, uFormat );
		--rect.left; --rect.top; --rect.right; --rect.bottom;
	    }
	    SetTextColor_k(hdc, RGB(0x80, 0x80, 0x80));
	}
	...
	if (!menuBar && lpitem->text[i])  /* There's a tab or flush-right char */
	{
		...
		if (!(lpitem->fState & MF_HILITE) )
		{
		    ++rect.left; ++rect.top; ++rect.right; ++rect.bottom;
		    SetTextColor_k(hdc, RGB(0xff, 0xff, 0xff));
		    DrawTextW_k( hdc, lpitem->text + i + 1, -1, &rect, uFormat );
		    --rect.left; --rect.top; --rect.right; --rect.bottom;
		}
		SetTextColor_k(hdc, RGB(0x80, 0x80, 0x80));
	    }
...
static void MENU_DrawPopupMenu( HWND hwnd, HDC hdc, HMENU hmenu )
{
    ...
    if((hPrevBrush = SelectObject_k( hdc, GetSysColorBrush_k(COLOR_MENU) ))
        && (SelectObject_k( hdc, get_menu_font(FALSE))))
    {
	HPEN hPrevPen;

	Rectangle_k( hdc, rect.left, rect.top, rect.right, rect.bottom );

	hPrevPen = SelectObject_k( hdc, GetStockObject_k( NULL_PEN ) );
	if( hPrevPen )
	{
	    POPUPMENU *menu;
	    BOOL flat_menu = FALSE;
	    ...
	    SystemParametersInfoW_k (SPI_GETFLATMENU, 0, &flat_menu, 0);
	    if (flat_menu)
		FrameRect_k(hdc, &rect, GetSysColorBrush_k(COLOR_BTNSHADOW));
	    else
		DrawEdge_k(hdc, &rect, EDGE_RAISED, BF_RECT);
                         ...
                         BOOL suppress_draw)
{
    LPPOPUPMENU lppop;
    HFONT hfontOld = 0;
    HMENU hMenu = GetMenu_k(hwnd);

    lppop = MENU_GetMenu( hMenu );
    if (lppop == NULL || lprect == NULL)
    {
        return GetSystemMetrics_k(SM_CYMENU);
    }

    if (suppress_draw)
    {
	hfontOld = SelectObject_k( hDC, get_menu_font(FALSE));
                              ...
                              INT x, INT y, INT xanchor, INT yanchor )
{
    ...
    width = menu->Width + GetSystemMetrics_k(SM_CXBORDER);
    height = menu->Height + GetSystemMetrics_k(SM_CYBORDER);

    /* FIXME: should use item rect */
    pt.x = x;
    pt.y = y;
    monitor = MonitorFromPoint_k( pt, MONITOR_DEFAULTTONEAREST );
    info.cbSize = sizeof(info);
    GetMonitorInfoW_k( monitor, &info );
    ...
    menu->hWnd = CreateWindowExW_k( ex_style, (LPCWSTR)POPUPMENU_CLASS_ATOM, NULL,
                                WS_POPUP, x, y, width, height,
                                hwndOwner, 0, (HINSTANCE)GetWindowLongPtrW_k(hwndOwner, GWLP_HINSTANCE),
                                (LPVOID)hmenu );
    ...
    SetWindowPos_k( menu->hWnd, HWND_TOPMOST, 0, 0, 0, 0,
                  SWP_SHOWWINDOW | SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE );
    UpdateWindow_k( menu->hWnd );
    return TRUE;
}
 ...
 */
static void MENU_EnsureMenuItemVisible(LPPOPUPMENU lppop, UINT wIndex, HDC hdc)
{
    if (lppop->bScrolling)
    {
        ...
        nMaxHeight -= GetSystemMetrics_k(SM_CYBORDER) + 2 * arrow_bitmap_height;
        if ((UINT)item->rect.bottom > lppop->nScrollPos + nMaxHeight)
        {
            
            lppop->nScrollPos = item->rect.bottom - nMaxHeight;
            ScrollWindow_k(lppop->hWnd, 0, nOldPos - lppop->nScrollPos, &rc, &rc);
            MENU_DrawScrollArrows(lppop, hdc);
        }
        else if ((UINT)item->rect.top - MENU_TOP_MARGIN < lppop->nScrollPos)
        {
                             ...
                             BOOL sendMenuSelect, HMENU topmenu )
{
    ...
    if (lppop->wFlags & MF_POPUP) hdc = GetDC_k( lppop->hWnd );
    else hdc = GetDCEx_k( lppop->hWnd, 0, DCX_CACHE | DCX_WINDOW);
    if (!top_popup) {
    ...
    if (lppop->FocusedItem != NO_SELECTED_ITEM)
    {
	    ...
	    SendMessageW_k( hwndOwner, WM_MENUSELECT,
        ...
        if(topmenu){
            ...
            if((pos=MENU_FindSubMenu(&topmenu, hmenu))!=NO_SELECTED_ITEM){
                ...
                SendMessageW_k( hwndOwner, WM_MENUSELECT, MAKEWPARAM(pos,
                         ip->fType | ip->fState |
                         (ptm->wFlags & MF_SYSMENU)), (LPARAM)topmenu);
            }
        }
    }
    ReleaseDC_k( lppop->hWnd, hdc );
}
...
static void MENU_MoveSelection( HWND hwndOwner, HMENU hmenu, INT offset )
{
	...
	for (i = menu->FocusedItem + offset ; i >= 0 && i < (INT)menu->nItems
					    ; i += offset)
    ...
    for ( i = (offset > 0) ? 0 : (INT)menu->nItems - 1;
		  i >= 0 && i < (INT)menu->nItems ; i += offset)
	if (!(menu->items[i].fType & MF_SEPARATOR))
	{
	    MENU_SelectItem( hwndOwner, hmenu, i, TRUE, 0 );
	    return;
...
static MENUITEM *MENU_InsertItem( HMENU hMenu, UINT pos, UINT flags )
{
    ...
    newItems = HeapAlloc_k(GetProcessHeap_k(), 0, sizeof(MENUITEM) * (menu->nItems+1) );
    if (!newItems)
    {
	...
	if (pos < menu->nItems) memcpy( &newItems[pos+1], &menu->items[pos],
        ...
        HeapFree_k(GetProcessHeap_k(), 0, menu->items );
    }
    menu->items = newItems;
    menu->nItems++;
...
static LPCSTR MENU_ParseResource( LPCSTR res, HMENU hMenu )
{
    ...
    do
    {
        ...
        if (flags & MF_POPUP)
        {
            HMENU hSubMenu = CreatePopupMenu_k();
            if (!hSubMenu) return NULL;
            if (!(res = MENU_ParseResource( res, hSubMenu ))) return NULL;
            AppendMenuW_k( hMenu, flags, (UINT_PTR)hSubMenu, str );
        }
        else  /* Not a popup */
        {
            AppendMenuW_k( hMenu, flags, id, *str ? str : NULL );
        }
    } while (!end_flag);
    return res;
}
...
static LPCSTR MENUEX_ParseResource( LPCSTR res, HMENU hMenu)
{
	...
	if (resinfo & 1) {	/* Pop-up? */
	    ...
	    res += sizeof(DWORD);
	    mii.hSubMenu = CreatePopupMenu_k();
	    if (!mii.hSubMenu)
		return NULL;
	    if (!(res = MENUEX_ParseResource(res, mii.hSubMenu))) {
		DestroyMenu_k(mii.hSubMenu);
                return NULL;
	    }
	...
	InsertMenuItemW_k(hMenu, -1, MF_BYPOSITION, &mii);
                                ...
                                BOOL sendMenuSelect, UINT wFlags )
{
	...
	MENU_SelectItem( hwndOwner, hsubmenu, NO_SELECTED_ITEM, sendMenuSelect, 0 );
        DestroyWindow_k( submenu->hWnd );
        submenu->hWnd = 0;

        if (!(wFlags & TPM_NONOTIFY))
           SendMessageW_k( hwndOwner, WM_UNINITMENUPOPUP, (WPARAM)hsubmenu,
                                  ...
                                  BOOL selectFirst, UINT wFlags )
{
    ...
    if (!(wFlags & TPM_NONOTIFY))
       SendMessageW_k( hwndOwner, WM_INITMENUPOPUP, (WPARAM)item->hSubMenu,
    ...
    {
        if (menu->wFlags & MF_POPUP) hdc = GetDC_k( menu->hWnd );
        else hdc = GetDCEx_k( menu->hWnd, 0, DCX_CACHE | DCX_WINDOW);
	...
	MENU_InitSysMenuPopup(item->hSubMenu,
                              GetWindowLongW_k( menu->hWnd, GWL_STYLE ),
                              GetClassLongW_k( menu->hWnd, GCL_STYLE));

	NC_GetSysPopupPos( menu->hWnd, &rect );
	if (wFlags & TPM_LAYOUTRTL) rect.left = rect.right;
	rect.top = rect.bottom;
	rect.right = GetSystemMetrics_k(SM_CXSIZE);
        rect.bottom = GetSystemMetrics_k(SM_CYSIZE);
    }
    else
    {
        GetWindowRect_k( menu->hWnd, &rect );
	    ...
	    /* The first item in the popup menu has to be at the
	       same y position as the focused menu item */
            if (wFlags & TPM_LAYOUTRTL)
                rect.left += GetSystemMetrics_k(SM_CXBORDER);
	    else
                rect.left += rc.right - GetSystemMetrics_k(SM_CXBORDER);
	    rect.top += rc.top - MENU_TOP_MARGIN;
	    rect.right = rc.left - rc.right + GetSystemMetrics_k(SM_CXBORDER);
	    rect.bottom = rc.top - rc.bottom - MENU_TOP_MARGIN
	                  - MENU_BOTTOM_MARGIN - GetSystemMetrics_k(SM_CYBORDER);
	}
	else
	{
            if (wFlags & TPM_LAYOUTRTL)
...
void MENU_EndMenu( HWND hwnd )
{
    ...
    if (menu && (hwnd == menu->hWnd || hwnd == menu->hwndOwner)) EndMenu_k();
}
...
static INT MENU_ExecFocusedItem( MTRACKER* pmt, HMENU hMenu, UINT wFlags )
{
	...
	if (!(item->fState & (MF_GRAYED | MF_DISABLED)) && !(item->fType & MF_SEPARATOR))
	{
		...
		if( menu->wFlags & MF_SYSMENU )
		    PostMessageW_k( pmt->hOwnerWnd, WM_SYSCOMMAND, item->wID,
		...
		else
		{
                    ...
                    if (dwStyle & MNS_NOTIFYBYPOS)
                        PostMessageW_k( pmt->hOwnerWnd, WM_MENUCOMMAND, menu->FocusedItem,
                                      (LPARAM)hMenu);
                    else
                        PostMessageW_k( pmt->hOwnerWnd, WM_COMMAND, item->wID, 0 );
		}
	    }
	    return item->wID;
	}
...
static LRESULT MENU_DoNextMenu( MTRACKER* pmt, UINT vk, UINT wFlags )
{
	...
	UINT  id = 0;
        ...
        next_menu.hmenuIn = (IS_SYSTEM_MENU(menu)) ? GetSubMenu_k(pmt->hTopMenu,0) : pmt->hTopMenu;
        next_menu.hmenuNext = 0;
        next_menu.hwndNext = 0;
        SendMessageW_k( pmt->hOwnerWnd, WM_NEXTMENU, vk, (LPARAM)&next_menu );
	...
	{
            DWORD style = GetWindowLongW_k( pmt->hOwnerWnd, GWL_STYLE );
	    hNewWnd = pmt->hOwnerWnd;
	    if( IS_SYSTEM_MENU(menu) )
	    {
		/* switch to the menu bar */

                if(style & WS_CHILD || !(hNewMenu = GetMenu_k(hNewWnd))) return FALSE;
	...
	else    /* application returned a new menu to switch to */
	{
	    ...
	    if( IsMenu_k(hNewMenu) && IsWindow_k(hNewWnd) )
	    {
                DWORD style = GetWindowLongW_k( hNewWnd, GWL_STYLE );

		if (style & WS_SYSMENU &&
		    GetSubMenu_k(get_win_sys_menu(hNewWnd), 0) == hNewMenu )
		{
	            /* get the real system menu */
		    hNewMenu =  get_win_sys_menu(hNewWnd);
		}
	        else if (style & WS_CHILD || GetMenu_k(hNewWnd) != hNewMenu )
		{
...
static BOOL MENU_SuspendPopup( MTRACKER* pmt, UINT16 uMsg )
{
    ...
    PeekMessageW_k( &msg, 0, uMsg, uMsg, PM_NOYIELD | PM_REMOVE);
    pmt->trackFlags |= TF_SKIPREMOVE;

    switch( uMsg )
    {
	case WM_KEYDOWN:
	     PeekMessageW_k( &msg, 0, 0, 0, PM_NOYIELD | PM_NOREMOVE);
	     if( msg.message == WM_KEYUP || msg.message == WM_PAINT )
	     {
		 PeekMessageW_k( &msg, 0, 0, 0, PM_NOYIELD | PM_REMOVE);
	         PeekMessageW_k( &msg, 0, 0, 0, PM_NOYIELD | PM_NOREMOVE);
                            ...
                            HWND hwnd, const RECT *lprect )
{
    ...
    if (!(menu = MENU_GetMenu( hmenu )))
    {
        ...
        SetLastError_k(ERROR_INVALID_MENU_HANDLE);
        return FALSE;
    }
	 ...
	 * clear that menu loop is not over yet. */
        ...
        for (;;)
        {
            if (PeekMessageW_k( &msg, 0, 0, 0, PM_NOREMOVE ))
            {
                if (!CallMsgFilterW_k( &msg, MSGF_MENU )) break;
                /* remove the message from the queue */
                PeekMessageW_k( &msg, 0, msg.message, msg.message, PM_REMOVE );
            }
            else
            {
                if (!enterIdleSent)
                {
                    ...
                    enterIdleSent = TRUE;
                    SendMessageW_k( mt.hOwnerWnd, WM_ENTERIDLE, MSGF_MENU, (LPARAM)win );
                }
                WaitMessage_k();
            }
        }

	/* check if EndMenu_k() tried to cancel us, by posting this message */
	...
	else if ((msg.message >= WM_KEYFIRST) && (msg.message <= WM_KEYLAST))
	{
	    ...
	    case WM_SYSCHAR:
		{
		    ...
		    else if (pos == (UINT)-1) MessageBeep_k(0);
		    else
		    {
    ...
    if( IsMenu_k( mt.hTopMenu ) )
    {
	menu = MENU_GetMenu( mt.hTopMenu );

        if( IsWindow_k( mt.hOwnerWnd ) )
        {
	    MENU_HideSubPopups( mt.hOwnerWnd, mt.hTopMenu, FALSE, wFlags );

	    if (menu && (menu->wFlags & MF_POPUP))
	    {
                DestroyWindow_k( menu->hWnd );
                menu->hWnd = 0;

                if (!(wFlags & TPM_NONOTIFY))
                   SendMessageW_k( mt.hOwnerWnd, WM_UNINITMENUPOPUP, (WPARAM)mt.hTopMenu,
                                 MAKELPARAM(0, IS_SYSTEM_MENU(menu)) );
	    }
	    MENU_SelectItem( mt.hOwnerWnd, mt.hTopMenu, NO_SELECTED_ITEM, FALSE, 0 );
	    SendMessageW_k( mt.hOwnerWnd, WM_MENUSELECT, MAKEWPARAM(0,0xffff), 0 );
        }
...
static BOOL MENU_InitTracking(HWND hWnd, HMENU hMenu, BOOL bPopup, UINT wFlags)
{
    ...
    if (!(wFlags & TPM_NONOTIFY))
       SendMessageW_k( hWnd, WM_ENTERMENULOOP, bPopup, 0 );

    SendMessageW_k( hWnd, WM_SETCURSOR, (WPARAM)hWnd, HTCAPTION );

    if (!(wFlags & TPM_NONOTIFY))
    {
       SendMessageW_k( hWnd, WM_INITMENU, (WPARAM)hMenu, 0 );
...
static BOOL MENU_ExitTracking(HWND hWnd, BOOL bPopup)
{
    ...
    SendMessageW_k( hWnd, WM_EXITMENULOOP, bPopup, 0 );
    ShowCaret_k(0);
    top_popup = 0;
...
{
    HMENU hMenu = (ht == HTSYSMENU) ? get_win_sys_menu( hWnd ) : GetMenu_k( hWnd );
    UINT wFlags = TPM_BUTTONDOWN | TPM_LEFTALIGN | TPM_LEFTBUTTON;

    //TRACE("wnd=%p ht=0x%04x %s\n", hWnd, ht, wine_dbgstr_point( &pt));

    if (GetWindowLongW_k( hWnd, GWL_EXSTYLE ) & WS_EX_LAYOUTRTL) wFlags |= TPM_LAYOUTRTL;
    if (IsMenu_k(hMenu))
    {
	MENU_InitTracking( hWnd, hMenu, FALSE, wFlags );

        /* fetch the window menu again, it may have changed */
        hMenu = (ht == HTSYSMENU) ? get_win_sys_menu( hWnd ) : GetMenu_k( hWnd );
...
void MENU_TrackKbdMenuBar( HWND hwnd, UINT wParam, WCHAR wChar)
{
    ...
    while (!WIN_ALLOWED_MENU(GetWindowLongW_k( hwnd, GWL_STYLE )))
        if (!(hwnd = GetAncestor_k( hwnd, GA_PARENT ))) return;

    /* check if we have to track a system menu */

    hTrackMenu = GetMenu_k( hwnd );
    if (!hTrackMenu || IsIconic_k(hwnd) || wChar == ' ' )
    {
        if (!(GetWindowLongW_k( hwnd, GWL_STYLE ) & WS_SYSMENU)) return;
    ...
    if (!IsMenu_k( hTrackMenu )) return;

    MENU_InitTracking( hwnd, hTrackMenu, FALSE, wFlags );

    /* fetch the window menu again, it may have changed */
    hTrackMenu = (wParam & HTSYSMENU) ? get_win_sys_menu( hwnd ) : GetMenu_k( hwnd );
    ...
    if( wChar && wChar != ' ' )
    {
        ...
        if ( uItem >= (UINT)(-2) )
        {
            if( uItem == (UINT)(-1) ) MessageBeep_k(0);
    ...
    if (!(wParam & HTSYSMENU) || wChar == ' ')
    {
        if( uItem == NO_SELECTED_ITEM )
            ...
            PostMessageW_k( hwnd, WM_KEYDOWN, VK_RETURN, 0 );
    }
 ...
 */
BOOL WINAPI TrackPopupMenuEx_k( HMENU hMenu, UINT wFlags, INT x, INT y,
                              HWND hWnd, LPTPMPARAMS lpTpm )
{
    POPUPMENU *menu;
    BOOL ret = FALSE;

    // TRACE("hmenu %p flags %04x (%d,%d) hwnd %p lpTpm %p rect %s\n", hMenu, wFlags, x, y, hWnd, lpTpm, lpTpm ? wine_dbgstr_rect( &lpTpm->rcExclude) : "-" );
    ...
    if (IsWindow_k(menu->hWnd))
    {
        SetLastError_k( ERROR_POPUP_ALREADY_ACTIVE );
        return FALSE;
    }
    ...
    /* Send WM_INITMENUPOPUP message only if TPM_NONOTIFY flag is not specified */
    if (!(wFlags & TPM_NONOTIFY))
        SendMessageW_k( hWnd, WM_INITMENUPOPUP, (WPARAM)hMenu, 0);
 ...
 *
 */
BOOL WINAPI TrackPopupMenu_k( HMENU hMenu, UINT wFlags, INT x, INT y,
                            INT nReserved, HWND hWnd, const RECT *lpRect )
{
    return TrackPopupMenuEx_k( hMenu, wFlags, x, y, hWnd, NULL);
}
 ...
 */
LRESULT WINAPI PopupMenuWndProc_k( HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_CREATE:
	{
	    ...
	    SetWindowLongPtrW_k( hwnd, 0, (LONG_PTR)cs->lpCreateParams );
            return 0;
	}
	...
	{
	    PAINTSTRUCT ps;
	    BeginPaint_k( hwnd, &ps );
	    MENU_DrawPopupMenu( hwnd, ps.hdc,
                                (HMENU)GetWindowLongPtrW_k( hwnd, 0 ) );
	    EndPaint_k( hwnd, &ps );
            return 0;
	}
	...
	if( wParam )
	{
            if (!GetWindowLongPtrW_k( hwnd, 0 )) ERR("no menu to display\n");
	}
	else
            SetWindowLongPtrW_k( hwnd, 0, 0 );
	break;

    case MM_SETMENUHANDLE:
        SetWindowLongPtrW_k( hwnd, 0, wParam );
        break;

    case MM_GETMENUHANDLE:
    case MN_GETHMENU:
        return GetWindowLongPtrW_k( hwnd, 0 );

    default:
        return DefWindowProcW_k( hwnd, message, wParam, lParam );
    }
    return 0;
}
                              ...
                              INT orgX, INT orgY )
{
    ...
    if (!(lppop = MENU_GetMenu( GetMenu_k(hwnd) ))) return 0;

    hdc = GetDCEx_k( hwnd, 0, DCX_CACHE | DCX_WINDOW );
    SelectObject_k( hdc, get_menu_font(FALSE));
    SetRect_k(&rectBar, orgX, orgY, orgX+menubarWidth, orgY+GetSystemMetrics_k(SM_CYMENU));
 ...
 *         ChangeMenuA    (USER32.@)
 */
BOOL WINAPI ChangeMenuA_k( HMENU hMenu, UINT pos, LPCSTR data,
                             UINT id, UINT flags )
{
    TRACE("menu=%p pos=%d data=%p id=%08x flags=%08x\n", hMenu, pos, data, id, flags );
    if (flags & MF_APPEND) return AppendMenuA_k( hMenu, flags & ~MF_APPEND,
                                                 id, data );
    if (flags & MF_DELETE) return DeleteMenu_k(hMenu, pos, flags & ~MF_DELETE);
    if (flags & MF_CHANGE) return ModifyMenuA_k(hMenu, pos, flags & ~MF_CHANGE,
                                                id, data );
    if (flags & MF_REMOVE) return RemoveMenu_k( hMenu,
                                              flags & MF_BYPOSITION ? pos : id,
                                              flags & ~MF_REMOVE );
    /* Default: MF_INSERT */
    return InsertMenuA_k( hMenu, pos, flags, id, data );
}
 ...
 *         ChangeMenuW    (USER32.@)
 */
BOOL WINAPI ChangeMenuW_k( HMENU hMenu, UINT pos, LPCWSTR data,
                             UINT id, UINT flags )
{
    TRACE("menu=%p pos=%d data=%p id=%08x flags=%08x\n", hMenu, pos, data, id, flags );
    if (flags & MF_APPEND) return AppendMenuW_k( hMenu, flags & ~MF_APPEND,
                                                 id, data );
    if (flags & MF_DELETE) return DeleteMenu_k(hMenu, pos, flags & ~MF_DELETE);
    if (flags & MF_CHANGE) return ModifyMenuW_k(hMenu, pos, flags & ~MF_CHANGE,
                                                id, data );
    if (flags & MF_REMOVE) return RemoveMenu_k( hMenu,
                                              flags & MF_BYPOSITION ? pos : id,
                                              flags & ~MF_REMOVE );
    /* Default: MF_INSERT */
    return InsertMenuW_k( hMenu, pos, flags, id, data );
}
 ...
 */
DWORD WINAPI CheckMenuItem_k( HMENU hMenu, UINT id, UINT flags )
{
    MENUITEM *item;
    DWORD ret;
 ...
 */
BOOL WINAPI EnableMenuItem_k( HMENU hMenu, UINT wItemID, UINT wFlags )
{
    UINT    oldflags;
    MENUITEM *item;
    POPUPMENU *menu;
	...
	if (menu->hSysMenuOwner != 0)
	{
		...
		return (UINT)-1;
            ...
            rc.bottom = 0;
            RedrawWindow_k(parentMenu->hWnd, &rc, 0, RDW_FRAME | RDW_INVALIDATE | RDW_NOCHILDREN);
	}
    }
...
INT WINAPI GetMenuStringA_k(
    ...
    if (!(item = MENU_FindItem( &hMenu, &wItemID, wFlags ))) {
        SetLastError_k( ERROR_MENU_ITEM_NOT_FOUND);
        return 0;
    }
    if (!item->text) return 0;
    if (!str || !nMaxSiz) return strlenW(item->text);
    if (!WideCharToMultiByte_k( CP_ACP, 0, item->text, -1, str, nMaxSiz, NULL, NULL ))
        str[nMaxSiz-1] = 0;
    TRACE("returning %s\n", debugstr_a(str));
    return strlen(str);
}
 ...
 */
INT WINAPI GetMenuStringW_k( HMENU hMenu, UINT wItemID,
 ...
 */
BOOL WINAPI HiliteMenuItem_k( HWND hWnd, HMENU hMenu, UINT wItemID,
                                UINT wHilite )
{
    LPPOPUPMENU menu;
 ...
 *         GetMenuState    (USER32.@)
 */
UINT WINAPI GetMenuState_k( HMENU hMenu, UINT wItemID, UINT wFlags )
{
    MENUITEM *item;
 ...
 */
INT WINAPI GetMenuItemCount_k( HMENU hMenu )
{
 ...
 */
UINT WINAPI GetMenuItemID_k( HMENU hMenu, INT nPos )
{
    MENUITEM * lpmi;
...
{
    memset( pmii, 0, sizeof( MENUITEMINFOW));
 ...
 *         InsertMenuW    (USER32.@)
 */
BOOL WINAPI InsertMenuW_k( HMENU hMenu, UINT pos, UINT flags,
 ...
 *         InsertMenuA    (USER32.@)
 */
BOOL WINAPI InsertMenuA_k( HMENU hMenu, UINT pos, UINT flags,
                         UINT_PTR id, LPCSTR str )
{
    ...
    if (IS_STRING_ITEM(flags) && str)
    {
        INT len = MultiByteToWideChar_k( CP_ACP, 0, str, -1, NULL, 0 );
        LPWSTR newstr = HeapAlloc_k(GetProcessHeap_k(), 0, len * sizeof(WCHAR) );
        if (newstr)
        {
            MultiByteToWideChar_k( CP_ACP, 0, str, -1, newstr, len );
            ret = InsertMenuW_k( hMenu, pos, flags, id, newstr );
            HeapFree_k(GetProcessHeap_k(), 0, newstr );
        }
        return ret;
    }
    else return InsertMenuW_k( hMenu, pos, flags, id, (LPCWSTR)str );
}
 ...
 *         AppendMenuA    (USER32.@)
 */
BOOL WINAPI AppendMenuA_k( HMENU hMenu, UINT flags,
                         UINT_PTR id, LPCSTR data )
{
    return InsertMenuA_k( hMenu, -1, flags | MF_BYPOSITION, id, data );
}
 ...
 *         AppendMenuW    (USER32.@)
 */
BOOL WINAPI AppendMenuW_k( HMENU hMenu, UINT flags,
                         UINT_PTR id, LPCWSTR data )
{
    return InsertMenuW_k( hMenu, -1, flags | MF_BYPOSITION, id, data );
}
 ...
 *         RemoveMenu    (USER32.@)
 */
BOOL WINAPI RemoveMenu_k( HMENU hMenu, UINT nPos, UINT wFlags )
{
    LPPOPUPMENU	menu;
    MENUITEM *item;
    ...
    else
    {
        ...
        menu->items = HeapReAlloc_k(GetProcessHeap_k(), 0, menu->items, menu->nItems * sizeof(MENUITEM) );
    }
    return TRUE;
}
 ...
 *         DeleteMenu    (USER32.@)
 */
BOOL WINAPI DeleteMenu_k( HMENU hMenu, UINT nPos, UINT wFlags )
{
    MENUITEM *item = MENU_FindItem( &hMenu, &nPos, wFlags );
    if (!item) return FALSE;
    if (item->fType & MF_POPUP) DestroyMenu_k( item->hSubMenu );
      /* nPos is now the position of the item */
    RemoveMenu_k( hMenu, nPos, wFlags | MF_BYPOSITION );
    return TRUE;
}
 ...
 *         ModifyMenuW    (USER32.@)
 */
BOOL WINAPI ModifyMenuW_k( HMENU hMenu, UINT pos, UINT flags,
                         UINT_PTR id, LPCWSTR str )
{
    MENUITEM *item;
    MENUITEMINFOW mii;
 ...
 *         ModifyMenuA    (USER32.@)
 */
BOOL WINAPI ModifyMenuA_k( HMENU hMenu, UINT pos, UINT flags,
                         UINT_PTR id, LPCSTR str )
{
    BOOL ret = FALSE;
    ...
    if (IS_STRING_ITEM(flags) && str)
    {
        ...
        {
            MultiByteToWideChar_k( CP_ACP, 0, str, -1, newstr, len );
            ret = ModifyMenuW_k( hMenu, pos, flags, id, newstr );
            HeapFree_k(GetProcessHeap_k(), 0, newstr );
        }
        return ret;
    }
    else return ModifyMenuW_k( hMenu, pos, flags, id, (LPCWSTR)str );
}
 ...
 */
HMENU WINAPI CreatePopupMenu_k(void)
{
    HMENU hmenu;
    POPUPMENU *menu;

    if (!(hmenu = CreateMenu_k())) return 0;
    menu = MENU_GetMenu( hmenu );
 ...
 */
DWORD WINAPI GetMenuCheckMarkDimensions_k(void)
{
    return MAKELONG( GetSystemMetrics_k(SM_CXMENUCHECK), GetSystemMetrics_k(SM_CYMENUCHECK) );
}
 ...
 */
BOOL WINAPI SetMenuItemBitmaps_k( HMENU hMenu, UINT nPos, UINT wFlags,
 ...
 *         CreateMenu    (USER32.@)
 */
HMENU WINAPI CreateMenu_k(void)
{
    HMENU hMenu;
    LPPOPUPMENU menu;

    if (!(menu = HeapAlloc_k(GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(*menu) ))) return 0;
    menu->FocusedItem = NO_SELECTED_ITEM;
    menu->bTimeToHide = FALSE;

    if (!(hMenu = alloc_user_handle( &menu->obj, USER_MENU ))) HeapFree_k(GetProcessHeap_k(), 0, menu );
 ...
 *         DestroyMenu    (USER32.@)
 */
BOOL WINAPI DestroyMenu_k( HMENU hMenu )
{
    LPPOPUPMENU lppop;
    ...
    if (lppop->items) /* recursively destroy submenus */
    {
        ...
        HeapFree_k(GetProcessHeap_k(), 0, lppop->items );
    }
    HeapFree_k(GetProcessHeap_k(), 0, lppop );
    return TRUE;
}
 ...
 */
HMENU WINAPI GetSystemMenu_k( HWND hWnd, BOOL bRevert )
{
    ...
    //{
     //   if (IsWindow_k( hWnd )) FIXME( "not supported on other process window %p\n", hWnd );
    //}
    else if (wndPtr)
    {
	if (wndPtr->hSysMenu && bRevert)
	{
            DestroyMenu_k(wndPtr->hSysMenu);
            wndPtr->hSysMenu = 0;
	}
	...
	if( wndPtr->hSysMenu )
        {
	    POPUPMENU *menu;
	    retvalue = GetSubMenu_k(wndPtr->hSysMenu, 0);
 ...
 */
BOOL WINAPI SetSystemMenu_k( HWND hwnd, HMENU hMenu )
{
    WND *wndPtr = WIN_GetPtr( hwnd );

    if (wndPtr && wndPtr != WND_DESKTOP)
    {
	if (wndPtr->hSysMenu) DestroyMenu_k( wndPtr->hSysMenu );
 ...
 *         GetMenu    (USER32.@)
 */
HMENU WINAPI GetMenu_k( HWND hWnd )
{
    HMENU retvalue = (HMENU)GetWindowLongPtrW_k( hWnd, GWLP_ID );
 ...
 */
BOOL WINAPI GetMenuBarInfo_k( HWND hwnd, LONG idObject, LONG idItem, PMENUBARINFO pmbi )
{
    POPUPMENU *menu;
    HMENU hmenu = NULL;
    ATOM class_atom;
    ...
    {
    case OBJID_CLIENT:
        class_atom = (ATOM)GetClassLongW_k(hwnd, GCW_ATOM);
        if (!class_atom)
            return FALSE;
        if (class_atom != (ATOM)POPUPMENU_CLASS_ATOM)
        {
        ...
        hmenu = (HMENU)GetWindowLongPtrW_k(hwnd, 0);
        break;
    case OBJID_MENU:
        hmenu = GetMenu_k(hwnd);
        break;
    case OBJID_SYSMENU:
        hmenu = GetSystemMenu_k(hwnd, FALSE);
        break;
    default:
        return FALSE;
    }
    ...
    if (!menu)
        return FALSE;
    if (idItem < 0 || idItem > (LONG)menu->nItems)
        return FALSE;

    if (!menu->Height)
    {
        SetRectEmpty_k(&pmbi->rcBar);
    }
    else if (idItem == 0)
    {
        GetMenuItemRect_k(hwnd, hmenu, 0, &pmbi->rcBar);
        pmbi->rcBar.right = pmbi->rcBar.left + menu->Width;
        pmbi->rcBar.bottom = pmbi->rcBar.top + menu->Height;
    }
    else
    {
        GetMenuItemRect_k(hwnd, hmenu, idItem - 1, &pmbi->rcBar);
    }
...
BOOL MENU_SetMenu( HWND hWnd, HMENU hMenu )
{
    ...
    if (hMenu && !IsMenu_k(hMenu))
    {
        WARN("hMenu %p is not a menu handle\n", hMenu);
        return FALSE;
    }
    if (!WIN_ALLOWED_MENU(GetWindowLongW_k( hWnd, GWL_STYLE )))
        return FALSE;

    hWnd = WIN_GetFullHandle( hWnd );
    if (GetCapture_k() == hWnd)
    ...
    SetWindowLongPtrW_k( hWnd, GWLP_ID, (LONG_PTR)hMenu );
    return TRUE;
}
 ...
 *         SetMenu    (USER32.@)
 */
BOOL WINAPI SetMenu_k( HWND hWnd, HMENU hMenu )
{   
    if(!MENU_SetMenu(hWnd, hMenu))
        return FALSE;
 
    SetWindowPos_k( hWnd, 0, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE |
 ...
 *         GetSubMenu    (USER32.@)
 */
HMENU WINAPI GetSubMenu_k( HMENU hMenu, INT nPos )
{
    MENUITEM * lpmi;
 ...
 *         DrawMenuBar    (USER32.@)
 */
BOOL WINAPI DrawMenuBar_k( HWND hWnd )
{
    LPPOPUPMENU lppop;
    HMENU hMenu = GetMenu_k(hWnd);
 ...
 */
DWORD WINAPI DrawMenuBarTemp_k(HWND hwnd, HDC hDC, LPRECT lprect, HMENU hMenu, HFONT hFont)
{
    LPPOPUPMENU lppop;
    UINT i,retvalue;
    HFONT hfontOld = 0;
    ...
    if (lppop == NULL || lprect == NULL)
    {
        retvalue = GetSystemMetrics_k(SM_CYMENU);
        goto END;
    }

    TRACE("(%p, %p, %p, %p, %p)\n", hwnd, hDC, lprect, hMenu, hFont);

    hfontOld = SelectObject_k( hDC, hFont);
    ...
    FillRect_k(hDC, lprect, GetSysColorBrush_k(flat_menu ? COLOR_MENUBAR : COLOR_MENU) );

    SelectObject_k( hDC, SYSCOLOR_GetPen(COLOR_3DFACE));
    MoveToEx_k( hDC, lprect->left, lprect->bottom, NULL );
    LineTo_k( hDC, lprect->right, lprect->bottom );
...
BOOL WINAPI EndMenu_k(void)
{
	...
	/* don't end up with an orphaned menu */
        PostMessageW_k( top_popup, WM_CANCELMODE, 0, 0);
    }
    return fEndMenu;
}
 ...
 *        LoadMenuA   (USER32.@)
 */
HMENU WINAPI LoadMenuA_k( HINSTANCE instance, LPCSTR name )
{
    HRSRC hrsrc = FindResourceA_k( instance, name, (LPSTR)RT_MENU );
    if (!hrsrc) return 0;
    return LoadMenuIndirectA_k( LoadResource_k( instance, hrsrc ));
}
 ...
 *        LoadMenuW   (USER32.@)
 */
HMENU WINAPI LoadMenuW_k( HINSTANCE instance, LPCWSTR name )
{
    HRSRC hrsrc = FindResourceW_k( instance, name, (LPWSTR)RT_MENU );
    if (!hrsrc) return 0;
    return LoadMenuIndirectW_k( LoadResource_k( instance, hrsrc ));
}
 ...
 */
HMENU WINAPI LoadMenuIndirectW_k( LPCVOID template )
{
    HMENU hMenu;
    WORD version, offset;
    LPCSTR p = template;
 ...
 */
HMENU WINAPI LoadMenuIndirectA_k( LPCVOID template )
{
    return LoadMenuIndirectW_k( template );
}
 ...
 *		IsMenu    (USER32.@)
 */
BOOL WINAPI IsMenu_k(HMENU hmenu)
{
    LPPOPUPMENU menu = MENU_GetMenu(hmenu);
					...
					LPMENUITEMINFOW lpmii, BOOL unicode)
{
    ...
    if ((lpmii->fMask & (MIIM_TYPE|MIIM_STRING))) {
         if( !menu->text ) {
            ...
            {
                len = WideCharToMultiByte_k( CP_ACP, 0, menu->text, -1, NULL,
                        0, NULL, NULL ) - 1;
                if(lpmii->dwTypeData && lpmii->cch)
                    if (!WideCharToMultiByte_k( CP_ACP, 0, menu->text, -1,
 ...
 */
BOOL WINAPI GetMenuItemInfoA_k( HMENU hmenu, UINT item, BOOL bypos,
 ...
 */
BOOL WINAPI GetMenuItemInfoW_k( HMENU hmenu, UINT item, BOOL bypos,
...
static INLINE void set_menu_item_text( MENUITEM *menu, LPCWSTR text, BOOL unicode )
{
    ...
    {
        if ((menu->text = HeapAlloc_k(GetProcessHeap_k(), 0, (strlenW(text)+1) * sizeof(WCHAR) )))
    ...
    else
    {
        ...
        int len = MultiByteToWideChar_k( CP_ACP, 0, str, -1, NULL, 0 );
        if ((menu->text = HeapAlloc_k(GetProcessHeap_k(), 0, len * sizeof(WCHAR) )))
            MultiByteToWideChar_k( CP_ACP, 0, str, -1, menu->text, len );
    }
}
				       ...
				       BOOL unicode)
{
    ...
    if (lpmii->fMask & MIIM_STRING ) {
        ...
        HeapFree_k(GetProcessHeap_k(), 0, menu->text);
                                              ...
                                              MENUITEMINFOW *pmii_out )
{
    ...
    if( pmii_out->fMask & MIIM_TYPE){
        ...
        if( IS_STRING_ITEM(pmii_out->fType)){
            ...
            pmii_out->hbmpItem = (HANDLE)(LOWORD(pmii_out->dwTypeData));
        }
    }
    return TRUE;
}
 ...
 */
BOOL WINAPI SetMenuItemInfoA_k(HMENU hmenu, UINT item, BOOL bypos,
 ...
 */
BOOL WINAPI SetMenuItemInfoW_k(HMENU hmenu, UINT item, BOOL bypos,
                                 const MENUITEMINFOW *lpmii)
{
    MENUITEMINFOW mii;
 ...
 *
 */
BOOL WINAPI SetMenuDefaultItem_k(HMENU hmenu, UINT uItem, UINT bypos)
{
	UINT i;
	POPUPMENU *menu;
	MENUITEM *item;
 ...
 */
UINT WINAPI GetMenuDefaultItem_k(HMENU hmenu, UINT bypos, UINT flags)
{
	POPUPMENU *menu;
	MENUITEM * item;
	UINT i = 0;
	...
	{
	    UINT ret;
	    ret = GetMenuDefaultItem_k( item->hSubMenu, bypos, flags );
	    if ( -1 != ret ) return ret;
 ...
 */
BOOL WINAPI InsertMenuItemA_k(HMENU hMenu, UINT uItem, BOOL bypos,
                                const MENUITEMINFOA *lpmii)
{
    MENUITEM *item;
    MENUITEMINFOW mii;
 ...
 */
BOOL WINAPI InsertMenuItemW_k(HMENU hMenu, UINT uItem, BOOL bypos,
                                const MENUITEMINFOW *lpmii)
{
    MENUITEM *item;
    MENUITEMINFOW mii;
...
BOOL WINAPI CheckMenuRadioItem_k(HMENU hMenu,
 ...
 *
 */
BOOL WINAPI GetMenuItemRect_k(HWND hwnd, HMENU hMenu, UINT uItem,
				 LPRECT rect)
{
     POPUPMENU *itemMenu;
     MENUITEM *item;
     ...
     *rect = item->rect;
     ...
     MapWindowPoints_k(referenceHwnd, 0, (LPPOINT)rect, 2);
 ...
 */
static BOOL menu_SetMenuInfo_k( HMENU hMenu, LPCMENUINFO lpmi)
{
    POPUPMENU *menu;
    if( !(menu = MENU_GetMenu(hMenu))) return FALSE;
    ...
    if( lpmi->fMask & MIM_APPLYTOSUBMENUS) {
        ...
        for( i = menu->nItems; i; i--, item++)
            if( item->fType & MF_POPUP)
                menu_SetMenuInfo_k( item->hSubMenu, lpmi);
    }
    return TRUE;
}

BOOL WINAPI SetMenuInfo_k(HMENU hMenu, LPCMENUINFO lpmi)
{
    TRACE("(%p %p)\n", hMenu, lpmi);
    if( lpmi && (lpmi->cbSize == sizeof( MENUINFO)) && (menu_SetMenuInfo_k( hMenu, lpmi))) {
	if( lpmi->fMask & MIM_STYLE) {
 ...
 *
 */
BOOL WINAPI GetMenuInfo_k(HMENU hMenu, LPMENUINFO lpmi)
{   POPUPMENU *menu;
 ...
 */
BOOL WINAPI SetMenuContextHelpId_k( HMENU hMenu, DWORD dwContextHelpID)
{
    LPPOPUPMENU menu;
 ...
 */
DWORD WINAPI GetMenuContextHelpId_k( HMENU hMenu )
{
    LPPOPUPMENU menu;
 ...
 */
INT WINAPI MenuItemFromPoint_k(HWND hWnd, HMENU hMenu, POINT ptScreen)
{
    POPUPMENU *menu = MENU_GetMenu(hMenu);
    UINT pos;
                                   ...
                                   BYTE fVirt, WORD key, WORD cmd )
{
    ...
    if (GetKeyState_k(VK_CONTROL) & 0x8000) mask |= FCONTROL;
    if (GetKeyState_k(VK_MENU) & 0x8000) mask |= FALT;
    if (GetKeyState_k(VK_SHIFT) & 0x8000) mask |= FSHIFT;
 ...
 found:
    ...
    else
    {
        ...
        hMenu = (GetWindowLongW_k( hWnd, GWL_STYLE ) & WS_CHILD) ? 0 : GetMenu_k(hWnd);
        ...
        if(MENU_FindItem(&hSubMenu, &nPos, MF_BYCOMMAND))
        {
            if (GetCapture_k())
                mesg = 2;
            if (!IsWindowEnabled_k(hWnd))
                mesg = 3;
            else
            {
                SendMessageW_k(hWnd, WM_INITMENU, (WPARAM)hSysMenu, 0L);
                if(hSubMenu != hSysMenu)
                {
                    nPos = MENU_FindSubMenu(&hSysMenu, hSubMenu);
                    TRACE_(accel)("hSysMenu = %p, hSubMenu = %p, nPos = %d\n", hSysMenu, hSubMenu, nPos);
                    SendMessageW_k(hWnd, WM_INITMENUPOPUP, (WPARAM)hSubMenu, MAKELPARAM(nPos, TRUE));
                }
                uSysStat = GetMenuState_k(GetSubMenu_k(hSysMenu, 0), cmd, MF_BYCOMMAND);
            }
        }
        else /* 2. in the window's menu */
        {
            ...
            if(MENU_FindItem(&hSubMenu, &nPos, MF_BYCOMMAND))
            {
                ...
                {
                    SendMessageW_k(hWnd, WM_INITMENU, (WPARAM)hMenu, 0L);
                    if(hSubMenu != hMenu)
                    {
                        nPos = MENU_FindSubMenu(&hMenu, hSubMenu);
                        TRACE_(accel)("hMenu = %p, hSubMenu = %p, nPos = %d\n", hMenu, hSubMenu, nPos);
                        SendMessageW_k(hWnd, WM_INITMENUPOPUP, (WPARAM)hSubMenu, MAKELPARAM(nPos, FALSE));
                    }
                    uStat = GetMenuState_k(hMenu, cmd, MF_BYCOMMAND);
                }
            }
        }
    ...
    if( mesg==WM_COMMAND )
    {
        ...
        SendMessageW_k(hWnd, mesg, 0x10000 | cmd, 0L);
    }
    else if( mesg==WM_SYSCOMMAND )
    {
        TRACE_(accel)(", sending WM_SYSCOMMAND, wParam=%0x\n", cmd);
        SendMessageW_k(hWnd, mesg, cmd, 0x00010000L);
    }
    else
    {
 ...
 */
INT WINAPI TranslateAcceleratorA_k( HWND hWnd, HACCEL hAccel, LPMSG msg )
{
    switch (msg->message)
    {
    case WM_KEYDOWN:
    case WM_SYSKEYDOWN:
        return TranslateAcceleratorW_k( hWnd, hAccel, msg );

    case WM_CHAR:
    case WM_SYSCHAR:
        {
            MSG msgW = *msg;
            char ch = (char)LOWORD(msg->wParam);
            WCHAR wch;
            MultiByteToWideChar_k(CP_ACP, 0, &ch, 1, &wch, 1);
            msgW.wParam = MAKEWPARAM(wch, HIWORD(msg->wParam));
            return TranslateAcceleratorW_k( hWnd, hAccel, &msgW );
        }
 ...
 */
INT WINAPI TranslateAcceleratorW_k( HWND hWnd, HACCEL hAccel, LPMSG msg )
{
    ...
    if (!(count = CopyAcceleratorTableW_k( hAccel, NULL, 0 ))) return 0;
    if (count > sizeof(data)/sizeof(data[0]))
    {
        if (!(ptr = HeapAlloc_k(GetProcessHeap_k(), 0, count * sizeof(*ptr) ))) return 0;
    }
    count = CopyAcceleratorTableW_k( hAccel, ptr, count );
    for (i = 0; i < count; i++)
    {
    ...
    if (ptr != data) HeapFree_k(GetProcessHeap_k(), 0, ptr );
    return (i < count);
}
