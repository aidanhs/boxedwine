...
#define SEND_NOTIFICATION(descr,code) \
    (SendMessageW_k( (descr)->owner, WM_COMMAND, \
     MAKEWPARAM( GetWindowLongPtrW_k((descr->self),GWLP_ID), (code)), (LPARAM)(descr->self) ))
    ...
    */
    ...
    if (descr->style & LBS_MULTICOLUMN)
    {
        ...
        if (descr->style & WS_HSCROLL)
            SetScrollInfo_k( descr->self, SB_HORZ, &info, TRUE );
        info.nMax = 0;
        info.fMask = SIF_RANGE;
        if (descr->style & WS_VSCROLL)
            SetScrollInfo_k( descr->self, SB_VERT, &info, TRUE );
    }
    else
    {
        info.nMin  = 0;
...
static LRESULT LISTBOX_SetTopItem( LB_DESCR *descr, INT index, BOOL scroll )
{
    ...
    if (scroll)
    {
        ...
        ScrollWindowEx_k( descr->self, 0, diff, NULL, NULL, 0, NULL,
                        SW_INVALIDATE | SW_ERASE | SW_SCROLLCHILDREN );
    }
    if (!scroll) InvalidateRect_k( descr->self, NULL, TRUE );
...
static void LISTBOX_UpdatePage( LB_DESCR *descr )
{
    ...
    if (descr->style & LBS_MULTICOLUMN)
        InvalidateRect_k( descr->self, NULL, TRUE );
...
static void LISTBOX_UpdateSize( LB_DESCR *descr )
{
    ...
    GetClientRect_k( descr->self, &rect );
    ...
    if (!(descr->style & LBS_NOINTEGRALHEIGHT) && !(descr->style & LBS_OWNERDRAWVARIABLE))
    {
        ...
        GetWindowRect_k( descr->self, &rect );
        ...
        if ((descr->height > descr->item_height) && remaining)
        {
            ...
            SetWindowPos_k( descr->self, 0, 0, 0, rect.right - rect.left,
    ...
    {
        InvalidateRect_k( descr->self, &rect, FALSE );
    }
}
...
static LRESULT LISTBOX_GetItemRect( const LB_DESCR *descr, INT index, RECT *rect )
{
    ...
    SetRect_k( rect, 0, 0, descr->width, descr->height );
    if (descr->style & LBS_MULTICOLUMN)
    {
			       ...
			       INT index, UINT action, BOOL ignoreFocus )
{
	...
	if (!item)
	{
        ...
        GetClientRect_k(descr->self, &r);
        hrgn = set_control_clipping( hdc, &r );

        dis.CtlType      = ODT_LISTBOX;
        dis.CtlID        = GetWindowLongPtrW_k( descr->self, GWLP_ID );
        ...
        if (!IsWindowEnabled_k(descr->self)) dis.itemState |= ODS_DISABLED;
        dis.itemData     = item->data;
        dis.rcItem       = *rect;
        //TRACE("[%p]: drawitem %d (%s) action=%02x state=%02x rect=%s\n", descr->self, index, debugstr_w(item->str), action, dis.itemState, wine_dbgstr_rect(rect) );
        SendMessageW_k(descr->owner, WM_DRAWITEM, dis.CtlID, (LPARAM)&dis);
    ...
    else
    {
        ...
        {
            oldBk = SetBkColor_k( hdc, GetSysColor_k( COLOR_HIGHLIGHT ) );
            oldText = SetTextColor_k( hdc, GetSysColor_k(COLOR_HIGHLIGHTTEXT));
        }

        //TRACE("[%p]: painting %d (%s) action=%02x rect=%s\n", descr->self, index, item ? debugstr_w(item->str) : "", action, wine_dbgstr_rect(rect) );
        if (!item)
            ExtTextOutW_k( hdc, rect->left + 1, rect->top,
        ...
        else
	{
	    /* Output empty string to paint background in the full width. */
            ExtTextOutW_k( hdc, rect->left + 1, rect->top,
                         ETO_OPAQUE | ETO_CLIPPED, rect, NULL, 0, NULL );
            TabbedTextOutW_k( hdc, rect->left + 1 , rect->top,
                            item->str, strlenW(item->str),
                            descr->nb_tabs, descr->tabs, 0);
	}
        if (item && item->selected)
        {
            SetBkColor_k( hdc, oldBk );
            SetTextColor_k( hdc, oldText );
        }
        if (!ignoreFocus && (descr->focus_item == index) &&
            (descr->caret_on) &&
            (descr->in_focus)) DrawFocusRect_k( hdc, rect );
    }
}
...
static void LISTBOX_RepaintItem( LB_DESCR *descr, INT index, UINT action )
{
    ...
    if (!IsWindowVisible_k(descr->self)) return;
    ...
    if (!(hdc = GetDCEx_k( descr->self, 0, DCX_CACHE ))) return;
    if (descr->font) oldFont = SelectObject_k( hdc, descr->font );
    hbrush = (HBRUSH)SendMessageW_k( descr->owner, WM_CTLCOLORLISTBOX,
				   (WPARAM)hdc, (LPARAM)descr->self );
    if (hbrush) oldBrush = SelectObject_k( hdc, hbrush );
    if (!IsWindowEnabled_k(descr->self))
        SetTextColor_k( hdc, GetSysColor_k( COLOR_GRAYTEXT ) );
    SetWindowOrgEx_k( hdc, descr->horz_pos, 0, NULL );
    LISTBOX_PaintItem( descr, hdc, &rect, index, action, TRUE );
    if (oldFont) SelectObject_k( hdc, oldFont );
    if (oldBrush) SelectObject_k( hdc, oldBrush );
    ReleaseDC_k( descr->self, hdc );
}
...
static LRESULT LISTBOX_InitStorage( LB_DESCR *descr, INT nb_items )
{
    ...
    if (descr->items) {
        nb_items += descr->items_size / sizeof(*item);
	    item = HeapReAlloc_k(GetProcessHeap_k(), 0, descr->items, nb_items * sizeof(LB_ITEMDATA));
    }
    else {
	    item = HeapAlloc_k(GetProcessHeap_k(), 0, nb_items * sizeof(LB_ITEMDATA));
    }
    ...
    descr->items = item;
    descr->items_size = nb_items * sizeof(LB_ITEMDATA);
    return LB_OKAY;
}
...
static BOOL LISTBOX_SetTabStops( LB_DESCR *descr, INT count, LPINT tabs )
{
    ...
    {
        SetLastError_k(ERROR_LB_WITHOUT_TABSTOPS);
        return FALSE;
    }

    HeapFree_k(GetProcessHeap_k(), 0, descr->tabs );
    if (!(descr->nb_tabs = count))
    {
        descr->tabs = NULL;
        return TRUE;
    }
    if (!(descr->tabs = HeapAlloc_k(GetProcessHeap_k(), 0,
    ...
    for (i = 0; i < descr->nb_tabs; i++)
        descr->tabs[i] = MulDiv_k(descr->tabs[i], descr->avg_char_width, 4);
...
static LRESULT LISTBOX_GetText( LB_DESCR *descr, INT index, LPWSTR buffer, BOOL unicode )
{
    ...
    if (HAS_STRINGS(descr))
    {
        if (!buffer)
        {
            ...
            if( unicode )
                return len;
            return WideCharToMultiByte_k( CP_ACP, 0, descr->items[index].str, len,
                                        NULL, 0, NULL, NULL );
        }
	...
	TRACE("index %d (0x%04x) %s\n", index, index, debugstr_w(descr->items[index].str));
        ...
        //__TRY  /* hide a Delphi bug that passes a read-only buffer */
        {
            ...
            {
                len = WideCharToMultiByte_k(CP_ACP, 0, descr->items[index].str, -1,
...
{
    INT ret = CompareStringW_k( lcid, NORM_IGNORECASE, str1, -1, str2, -1 );
...
static INT LISTBOX_FindStringPos( LB_DESCR *descr, LPCWSTR str, BOOL exact )
{
    ...
    while (min != max)
    {
        ...
        else
        {
            ...
            UINT id = (UINT)GetWindowLongPtrW_k( descr->self, GWLP_ID );
            ...
            res = SendMessageW_k( descr->owner, WM_COMPAREITEM, id, (LPARAM)&cis );
        }
        if (!res) return index;
        if (res < 0) max = index;
        else min = index + 1;
...
static LRESULT LISTBOX_Paint( LB_DESCR *descr, HDC hdc )
{
    ...
    hbrush = (HBRUSH)SendMessageW_k( descr->owner, WM_CTLCOLORLISTBOX,
			   	   (WPARAM)hdc, (LPARAM)descr->self );
    if (hbrush) oldBrush = SelectObject_k( hdc, hbrush );
    if (!IsWindowEnabled_k(descr->self)) SetTextColor_k( hdc, GetSysColor_k( COLOR_GRAYTEXT ) );
        ...
        (descr->in_focus))
    {
        ...
        rect.bottom = rect.top + descr->item_height;
        ExtTextOutW_k( hdc, 0, 0, ETO_OPAQUE | ETO_CLIPPED,
                     &rect, NULL, 0, NULL );
...
static void LISTBOX_InvalidateItems( LB_DESCR *descr, INT index )
{
    ...
    if (LISTBOX_GetItemRect( descr, index, &rect ) == 1)
    {
        ...
        rect.bottom = descr->height;
        InvalidateRect_k( descr->self, &rect, TRUE );
        if (descr->style & LBS_MULTICOLUMN)
        {
...
static LRESULT LISTBOX_SetItemHeight( LB_DESCR *descr, INT index, INT height, BOOL repaint )
{
    ...
    else if (height != descr->item_height)
    {
        ...
        LISTBOX_UpdateScroll( descr );
	if (repaint)
	    InvalidateRect_k( descr->self, 0, TRUE );
    }
    return LB_OKAY;
}
...
static void LISTBOX_SetHorizontalPos( LB_DESCR *descr, INT pos )
{
    ...
    if (abs(diff) < descr->width)
    {
        ...
        if (LISTBOX_GetItemRect( descr, descr->focus_item, &rect ) == 1)
            InvalidateRect_k( descr->self, &rect, TRUE );
        ScrollWindowEx_k( descr->self, diff, 0, NULL, NULL, 0, NULL,
                          SW_INVALIDATE | SW_ERASE | SW_SCROLLCHILDREN );
    }
    else
        InvalidateRect_k( descr->self, NULL, TRUE );
...
static INT LISTBOX_SetFont( LB_DESCR *descr, HFONT font )
{
    ...
    if (!(hdc = GetDCEx_k( descr->self, 0, DCX_CACHE )))
    {
        ERR("unable to get DC.\n" );
        return 16;
    }
    if (font) oldFont = SelectObject_k( hdc, font );
    GetTextExtentPointA_k( hdc, alphabet, 52, &sz);
    if (oldFont) SelectObject_k( hdc, oldFont );
    ReleaseDC_k( descr->self, hdc );
                                   ...
                                   LPWSTR str, ULONG_PTR data )
{
    ...
    else max_items = descr->items_size;
    if (descr->nb_items == max_items)
    {
        /* We need to grow the array */
        max_items += LB_ARRAY_GRANULARITY;
	    if (descr->items)
    	    item = HeapReAlloc_k(GetProcessHeap_k(), 0, descr->items, max_items * sizeof(LB_ITEMDATA) );
	    else
	        item = HeapAlloc_k(GetProcessHeap_k(), 0, max_items * sizeof(LB_ITEMDATA) );
        if (!item)
        {
            SEND_NOTIFICATION( descr, LBN_ERRSPACE );
            return LB_ERRSPACE;
        ...
        descr->items = item;
        descr->items_size = max_items * sizeof(LB_ITEMDATA);
    }    
    ...
    if (descr->style & LBS_OWNERDRAWVARIABLE)
    {
        ...
        SendMessageW_k( descr->owner, WM_MEASUREITEM, id, (LPARAM)&mis );
...
static LRESULT LISTBOX_InsertString( LB_DESCR *descr, INT index, LPCWSTR str )
{
    ...
    if (HAS_STRINGS(descr))
    {
        ...
        if (!(new_str = HeapAlloc_k(GetProcessHeap_k(), 0, (strlenW(str) + 1) * sizeof(WCHAR) )))
        {
            SEND_NOTIFICATION( descr, LBN_ERRSPACE );
            return LB_ERRSPACE;
        }
    ...
    {
        HeapFree_k(GetProcessHeap_k(), 0, new_str );
        return ret;
    }
...
static void LISTBOX_DeleteItem( LB_DESCR *descr, INT index )
{
    ...
    if (!descr->nb_items)
        SendMessageW_k( descr->self, LB_RESETCONTENT, 0, 0 );
    ...
    if (IS_OWNERDRAW(descr) || item_data)
    {
        ...
        SendMessageW_k( descr->owner, WM_DELETEITEM, id, (LPARAM)&dis );
    }
    if (HAS_STRINGS(descr))
        HeapFree_k(GetProcessHeap_k(), 0, item_str );
}
...
static LRESULT LISTBOX_RemoveItem( LB_DESCR *descr, INT index )
{
    ...
    max_items = descr->items_size / sizeof(LB_ITEMDATA);
...
static void LISTBOX_ResetContent( LB_DESCR *descr )
{
    ...
    HeapFree_k(GetProcessHeap_k(), 0, descr->items );
    descr->nb_items      = 0;
    descr->top_item      = 0;
    descr->selected_item = -1;
    descr->focus_item    = 0;
...
static LRESULT LISTBOX_SetCount( LB_DESCR *descr, INT count )
{
    ...
    if (HAS_STRINGS(descr))
    {
        SetLastError_k(ERROR_SETCOUNT_ON_BAD_LB);
        return LB_ERR;
    }
                                  ...
                                  LPCWSTR filespec, BOOL long_names )
{
    ...
    if (attrib != (DDL_DRIVES | DDL_EXCLUSIVE)) {
        /* scan directory */
        if ((handle = FindFirstFileW_k(filespec, &entry)) == INVALID_HANDLE_VALUE)
        {
	     int le = GetLastError_k();
...
#undef ATTRIBS
                ...
                if (!long_names) CharLowerW_k( buffer );
            ...
            } while (FindNextFileW_k( handle, &entry ));
            FindClose_k( handle );
        }
    }
    if (ret >= 0)
    {
        ...
        if (attrib & DDL_DRIVES)
        {
            ...
            {
                if (GetDriveTypeW_k(root) <= DRIVE_NO_ROOT_DIR) continue;
...
static LRESULT LISTBOX_HandleVScroll( LB_DESCR *descr, WORD scrollReq, WORD pos )
{
    ...
    case SB_THUMBTRACK:
        ...
        GetScrollInfo_k( descr->self, SB_VERT, &info );
...
static LRESULT LISTBOX_HandleHScroll( LB_DESCR *descr, WORD scrollReq, WORD pos )
{
    ...
    else if (descr->horz_extent)
    {
        ...
        case SB_THUMBTRACK:
            info.cbSize = sizeof(info);
            info.fMask = SIF_TRACKPOS;
            GetScrollInfo_k( descr->self, SB_HORZ, &info );
...
static LRESULT LISTBOX_HandleMouseWheel(LB_DESCR *descr, SHORT delta )
{
    ...
    if (descr->wheel_remain && pulScrollLines)
    {
        ...
        cLineScroll = (int)(pulScrollLines * (float)descr->wheel_remain / WHEEL_DELTA);
...
static LRESULT LISTBOX_HandleLButtonDown( LB_DESCR *descr, DWORD keys, INT x, INT y )
{
    ...
    if (!descr->in_focus)
    {
        if( !descr->lphc ) SetFocus_k( descr->self );
        else SetFocus_k( (descr->lphc->hWndEdit) ? descr->lphc->hWndEdit : descr->lphc->self );
    }
    ...
    if (!descr->lphc)
    {
        if (descr->style & LBS_NOTIFY )
            SendMessageW_k( descr->owner, WM_LBTRACKPOINT, index,
                            MAKELPARAM( x, y ) );
    }

    descr->captured = TRUE;
    SetCapture_k( descr->self );
    ...
    if (!descr->lphc)
    {
        if (GetWindowLongW_k( descr->self, GWL_EXSTYLE ) & WS_EX_DRAGDETECT)
        {
            POINT pt;
	    ...
	    pt.y = y;
            ...
            if (DragDetect_k( descr->self, pt ))
                SendMessageW_k( descr->owner, WM_BEGINDRAG, 0, 0 );
        }
    }
    return 0;
}
...
static LRESULT LISTBOX_HandleLButtonDownCombo( LB_DESCR *descr, UINT msg, DWORD keys, INT x, INT y)
{
    ...
    GetClientRect_k(descr->self, &clientRect);

    if(PtInRect_k(&clientRect, mousePos))
    {
    ...
    else
    {
        ...
        hWndOldCapture = GetCapture_k();
        ReleaseCapture_k();
        GetWindowRect_k(descr->self, &screenRect);
        ClientToScreen_k(descr->self, &screenMousePos);

        if(!PtInRect_k(&screenRect, screenMousePos))
        {
        ...
        else
        {
            ...
            INT nHitTestType=0;
            LONG style = GetWindowLongW_k( descr->self, GWL_STYLE );
            /* Check Vertical scroll bar */
            if (style & WS_VSCROLL)
            {
                clientRect.right += GetSystemMetrics_k(SM_CXVSCROLL);
                if (PtInRect_k( &clientRect, mousePos ))
            ...
            if (style & WS_HSCROLL)
            {
                clientRect.bottom += GetSystemMetrics_k(SM_CYHSCROLL);
                if (PtInRect_k( &clientRect, mousePos ))
...
static LRESULT LISTBOX_HandleLButtonUp( LB_DESCR *descr )
{
    if (LISTBOX_Timer != LB_TIMER_NONE)
        KillSystemTimer_k( descr->self, LB_TIMER_ID );
    LISTBOX_Timer = LB_TIMER_NONE;
    if (descr->captured)
    {
        descr->captured = FALSE;
        if (GetCapture_k() == descr->self) ReleaseCapture_k();
                                     ...
                                     INT x, INT y )
{
    ...
    if (dir != LB_TIMER_NONE)
        SetSystemTimer_k( descr->self, LB_TIMER_ID, LB_SCROLL_TIMEOUT, NULL);
...
static LRESULT LISTBOX_HandleKeyDown( LB_DESCR *descr, DWORD key )
{
    ...
    {
        caret = SendMessageW_k( descr->owner, WM_VKEYTOITEM,
    ...
    if (caret >= 0)
    {
        ...
        if (descr->style & LBS_NOTIFY)
        {
            if (descr->lphc && IsWindowVisible_k( descr->self ))
            {
...
static LRESULT LISTBOX_HandleChar( LB_DESCR *descr, WCHAR charW )
{
    ...
    if (descr->style & LBS_WANTKEYBOARDINPUT)
    {
        caret = SendMessageW_k( descr->owner, WM_CHARTOITEM,
...
static BOOL LISTBOX_Create( HWND hwnd, LPHEADCOMBO lphc )
{
    ...
    if (!(descr = HeapAlloc_k(GetProcessHeap_k(), 0, sizeof(*descr) )))
        return FALSE;

    GetClientRect_k( hwnd, &rect );
    descr->self          = hwnd;
    descr->owner         = GetParent_k( descr->self );
    descr->style         = GetWindowLongW_k( descr->self, GWL_STYLE );
    descr->width         = rect.right - rect.left;
    descr->height        = rect.bottom - rect.top;
    descr->items         = NULL;
    descr->nb_items      = 0;
    ...
    descr->captured      = FALSE;
    descr->font          = 0;
    descr->locale        = GetUserDefaultLCID_k();
    descr->lphc		 = lphc;
    ...
    SetWindowLongPtrW_k( descr->self, 0, (LONG_PTR)descr );
...
static BOOL LISTBOX_Destroy( LB_DESCR *descr )
{
    ...
    SetWindowLongPtrW_k( descr->self, 0, 0 );
    HeapFree_k(GetProcessHeap_k(), 0, descr );
    return TRUE;
}
...
{
    LB_DESCR *descr = (LB_DESCR *)GetWindowLongPtrW_k( hwnd, 0 );
    LPHEADCOMBO lphc = 0;
    LRESULT ret;
    ...
    if (!descr)
    {
	    ...
	    CREATESTRUCTW *lpcs = (CREATESTRUCTW *)lParam;
            ...
            TRACE("creating hwnd %p descr %p\n", hwnd, (void *)GetWindowLongPtrW_k( hwnd, 0 ) );
            return 0;
        }
        /* Ignore all other messages before we get a WM_CREATE */
        return unicode ? DefWindowProcW_k( hwnd, msg, wParam, lParam ) :
                         DefWindowProcA_k( hwnd, msg, wParam, lParam );
    }
    ...
    case LB_ADDSTRING:
    {
        ...
        else
        {
            ...
            if((textW = HeapAlloc_k(GetProcessHeap_k(), 0, countW * sizeof(WCHAR))))
    ...
    case LB_GETTEXTLEN:
        ...
        return WideCharToMultiByte_k( CP_ACP, 0, descr->items[wParam].str,
    ...
    {
        LCID ret;
        if (!IsValidLocale_k((LCID)wParam, LCID_INSTALLED))
            return LB_ERR;
        ret = descr->locale;
    ...
    case WM_PAINT:
        {
            PAINTSTRUCT ps;
            HDC hdc = ( wParam ) ? ((HDC)wParam) :  BeginPaint_k( descr->self, &ps );
            ret = LISTBOX_Paint( descr, hdc );
            if( !wParam ) EndPaint_k( descr->self, &ps );
        }
        return ret;
    case WM_SIZE:
    ...
    case WM_SETFONT:
        ...
        if (lParam) InvalidateRect_k( descr->self, 0, TRUE );
        return 0;
    case WM_SETFOCUS:
        descr->in_focus = TRUE;
        descr->caret_on = TRUE;
    ...
    case WM_MOUSEWHEEL:
        if (wParam & (MK_SHIFT | MK_CONTROL))
            return DefWindowProcW_k( descr->self, msg, wParam, lParam );
    ...
    case WM_MOUSEMOVE:
        if ( lphc && ((lphc->dwStyle & CBS_DROPDOWNLIST) != CBS_SIMPLE) )
        {
             ...
             */
            if (GetClientRect_k(descr->self, &clientRect) && PtInRect_k( &clientRect, mousePos ))
                descr->captured = TRUE;
        ...
        else if (GetCapture_k() == descr->self)
        {
	...
	if (lphc)
	{
             ...
             */
            if ( (lParam == (LPARAM)-1) || (!PtInRect_k( &clientRect, mousePos )) )
    ...
    case WM_ERASEBKGND:
        ...
        {
            RECT rect;
            HBRUSH hbrush = (HBRUSH)SendMessageW_k( descr->owner, WM_CTLCOLORLISTBOX,
                                              wParam, (LPARAM)descr->self );
	    TRACE("hbrush = %p\n", hbrush);
	    if(!hbrush)
		hbrush = GetSysColorBrush_k(COLOR_WINDOW);
	    if(hbrush)
	    {
		GetClientRect_k(descr->self, &rect);
		FillRect_k((HDC)wParam, &rect, hbrush);
	    }
        }
        return 1;
    case WM_DROPFILES:
        if( lphc ) return 0;
        return unicode ? SendMessageW_k( descr->owner, msg, wParam, lParam ) :
                         SendMessageA_k( descr->owner, msg, wParam, lParam );
...
{
    TRACE("%p\n", hwnd);
    return SendMessageW_k(hwnd, LB_GETLISTBOXINFO, 0, 0);
}
