                            ...
                            int *len_before, int *len_ellip)
{
    ...
    if (*len_str > 0 &&
        GetTextExtentExPointW_k(hdc, str, *len_str, width, NULL, NULL, size) &&
        size->cx > width)
    {
        for (lo = 0, hi = *len_str; lo < hi; )
        {
            mid = (lo + hi) / 2;
            if (!GetTextExtentExPointW_k(hdc, str, mid, width, NULL, NULL, size))
                break;
            if (size->cx > width)
                hi = mid;
            else
    ...
    for ( ; ; )
    {
        ...
        if (!GetTextExtentExPointW_k(hdc, str, *len_str + len_ellipsis, width,
                                ...
                                WCHAR *modstr, ellipsis_data *pellip)
{
    ...
    len_ellipsis = strlenW (ELLIPSISW);
    if (!max_len) return;
    if (len_ellipsis >= (int)max_len) len_ellipsis = (int)max_len - 1;
                            ...
                            unsigned int *chars_used, SIZE *size)
{
    ...
    if (word_fits)
    {
        ...
        if (!(format & (DT_RIGHT | DT_CENTER)) || *p != SPACE)
            p++;
        next_is_space = (p - str) < (int)*len_str && *p == SPACE;
        *len_str = p - str;
        /* and if the next character is a space then discard it. */
        *chars_used = *len_str;
        if (next_is_space)
    ...
    GetTextExtentExPointW_k(hdc, str, *len_str, 0, NULL, NULL, size);
}
                          ...
                          const ellipsis_data *pe)
{
    ...
    for (i = 0; i < n; i++, str++)
    {
        ...
        if (*str == PREFIX || *str == ALPHA_PREFIX)
        {
            ...
            if (!ns) break;
            if (*str != PREFIX)
                result = ((INT)i < pe->before || pe->under == 0) ? (INT)i : (INT)i - pe->under + pe->len;
                                 ...
                                 ellipsis_data *pellip)
{
    ...
    *pprefix_offset = -1;
    ...
    while (*count)
    {
        ...
        GetTextExtentExPointW_k(hdc, dest + seg_j, j_in_seg, max_seg_width, &num_fit, NULL, &size);
...
static void TEXT_DrawUnderscore (HDC hdc, int x, int y, const WCHAR *str, int offset, const RECT *rect)
{
    ...
    GetTextExtentPointW_k (hdc, str, offset, &size);
    prefix_x = x + size.cx;
    GetTextExtentPointW_k (hdc, str, offset+1, &size);
    ...
    hpen = CreatePen_k(PS_SOLID, 1, GetTextColor_k(hdc));
    oldPen = SelectObject_k(hdc, hpen);
    MoveToEx_k(hdc, prefix_x, y, NULL);
    LineTo_k(hdc, prefix_end, y);
...
INT WINAPI DrawTextExW_k( HDC hdc, LPWSTR str, INT i_count,
                        LPRECT rect, UINT flags, LPDRAWTEXTPARAMS dtp )
{
    ...
    if (GetGraphicsMode_k(hdc) == GM_COMPATIBLE)
    {
        ...
        GetWindowExtEx_k(hdc, &window_ext);
        GetViewportExtEx_k(hdc, &viewport_ext);
    ...
    if (flags & DT_MODIFYSTRING)
    {
        ...
        retstr = HeapAlloc_k(GetProcessHeap_k(), 0, size_retstr);
        if (!retstr) return 0;
	...
	if (!(flags & DT_CALCRECT))
	{
            ...
            while (len)
            {
                ...
                if ((flags & DT_EXPANDTABS))
                {
                    ...
                    len_seg = p - str;
                    if (len_seg != len && !GetTextExtentPointW_k(hdc, str, len_seg, &size))
                    {
                        HeapFree_k(GetProcessHeap_k(), 0, retstr);
                        return 0;
                    }
                }
                else
 ...
 * string we return.
 */
INT WINAPI DrawTextExA_k( HDC hdc, LPSTR str, INT count,
                        LPRECT rect, UINT flags, LPDRAWTEXTPARAMS dtp )
{
   WCHAR *wstr;
   WCHAR *p;
   ...
   wcount = MultiByteToWideChar_k( cp, 0, str, count, NULL, 0 );
   wmax = wcount;
   amax = count;
   if (flags & DT_MODIFYSTRING)
   {
   ...
   wstr = HeapAlloc_k(GetProcessHeap_k(), 0, wmax * sizeof(WCHAR));
   if (wstr)
   {
       MultiByteToWideChar_k( cp, 0, str, count, wstr, wcount );
       if (flags & DT_MODIFYSTRING)
            ...
            */
       ret = DrawTextExW_k( hdc, wstr, wcount, rect, flags, dtp );
       if (flags & DT_MODIFYSTRING)
       {
            ...
            WideCharToMultiByte_k( cp, 0, wstr, wcount, str, amax, NULL, NULL );
       }
 ...
 *           DrawTextW    (USER32.@)
 */
INT WINAPI DrawTextW_k( HDC hdc, LPCWSTR str, INT count, LPRECT rect, UINT flags )
{
    DRAWTEXTPARAMS dtp;
    ...
    return DrawTextExW_k(hdc, (LPWSTR)str, count, rect, flags, &dtp);
}

/***********************************************************************
 *           DrawTextA    (USER32.@)
 */
INT WINAPI DrawTextA_k( HDC hdc, LPCSTR str, INT count, LPRECT rect, UINT flags )
{
    DRAWTEXTPARAMS dtp;
    ...
    return DrawTextExA_k( hdc, (LPSTR)str, count, rect, flags, &dtp );
}
                            ...
                            INT x, INT y, INT cx, INT cy )
{
    ...
    if(!hdc) return FALSE;
    if (!(memdc = CreateCompatibleDC_k(hdc))) return FALSE;

    hbm = CreateBitmap_k(cx, cy, 1, 1, NULL);
    hbmsave = SelectObject_k(memdc, hbm);
    hbsave = SelectObject_k( memdc, GetStockObject_k(BLACK_BRUSH) );
    PatBlt_k( memdc, 0, 0, cx, cy, PATCOPY );
    SelectObject_k( memdc, hbsave );
    SetTextColor_k(memdc, RGB(255, 255, 255));
    SetBkColor_k(memdc, RGB(0, 0, 0));
    hfsave = SelectObject_k(memdc, GetCurrentObject_k(hdc, OBJ_FONT));

    retval = fn(memdc, lp, slen);
    SelectObject_k(memdc, hfsave);
...
*/
...
#endif
    {
        ...
        PatBlt_k(memdc, 0, 0, cx, cy, 0x000A0329);
        SelectObject_k(memdc, hbsave);
    }

    if(hb) hbsave = SelectObject_k(hdc, hb);
    fg = SetTextColor_k(hdc, RGB(0, 0, 0));
    bg = SetBkColor_k(hdc, RGB(255, 255, 255));
    BitBlt_k(hdc, x, y, cx, cy, memdc, 0, 0, 0x00E20746);
    SetTextColor_k(hdc, fg);
    SetBkColor_k(hdc, bg);
    if(hb) SelectObject_k(hdc, hbsave);
 ...
 *           GrayStringA   (USER32.@)
 */
BOOL WINAPI GrayStringA_k( HDC hdc, HBRUSH hbr, GRAYSTRINGPROC gsprc,
                         ...
                         INT cx, INT cy )
{
    ...
    {
        SIZE s;
        GetTextExtentPoint32A_k( hdc, (LPCSTR)lParam, cch, &s );
        if (cx == 0) cx = s.cx;
        if (cy == 0) cy = s.cy;
    }
 ...
 *           GrayStringW   (USER32.@)
 */
BOOL WINAPI GrayStringW_k( HDC hdc, HBRUSH hbr, GRAYSTRINGPROC gsprc,
                         LPARAM lParam, INT cch, INT x, INT y,
                         INT cx, INT cy )
{
    ...
    {
        SIZE s;
        GetTextExtentPoint32W_k( hdc, (LPCWSTR)lParam, cch, &s );
        if (cx == 0) cx = s.cx;
        if (cy == 0) cy = s.cy;
    }
                                ...
                                BOOL fDisplayText )
{
    ...
    while (count > 0)
    {
        ...
        GetTextExtentPointW_k( hdc, lpstr + i, j - i , &extent );
        ...
        if (fDisplayText)
        {
            ...
            ExtTextOutW_k( hdc, x0, y, GetBkMode_k(hdc) == OPAQUE ? ETO_OPAQUE : 0,
                         &r, lpstr + i, j - i, NULL );
        }
        count -= j;
        lpstr += j;
 ...
 * See TabbedTextOutW.
 */
LONG WINAPI TabbedTextOutA_k( HDC hdc, INT x, INT y, LPCSTR lpstr, INT count,
                            INT cTabStops, const INT *lpTabPos, INT nTabOrg )
{
    LONG ret;
    DWORD len = MultiByteToWideChar_k( CP_ACP, 0, lpstr, count, NULL, 0 );
    LPWSTR strW = HeapAlloc_k( GetProcessHeap_k(), 0, len * sizeof(WCHAR) );
    if (!strW) return 0;
    MultiByteToWideChar_k( CP_ACP, 0, lpstr, count, strW, len );
    ret = TabbedTextOutW_k( hdc, x, y, strW, len, cTabStops, lpTabPos, nTabOrg );
 ...
 */
LONG WINAPI TabbedTextOutW_k( HDC hdc, INT x, INT y, LPCWSTR str, INT count,
                            INT cTabStops, const INT *lpTabPos, INT nTabOrg )
{
 ...
 */
DWORD WINAPI GetTabbedTextExtentA_k( HDC hdc, LPCSTR lpstr, INT count,
                                   INT cTabStops, const INT *lpTabPos )
{
    ...
    if (!strW) return 0;
    MultiByteToWideChar_k( CP_ACP, 0, lpstr, count, strW, len );
    ret = GetTabbedTextExtentW_k( hdc, strW, len, cTabStops, lpTabPos );
    HeapFree_k( GetProcessHeap_k(), 0, strW );
    return ret;
}
 ...
 */
DWORD WINAPI GetTabbedTextExtentW_k( HDC hdc, LPCWSTR lpstr, INT count,
                                   INT cTabStops, const INT *lpTabPos )
{
