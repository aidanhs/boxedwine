...
static HICON STATIC_SetIcon( HWND hwnd, HICON hicon, DWORD style )
{
    ...
    prevIcon = (HICON)SetWindowLongPtrW_k( hwnd, HICON_GWL_OFFSET, (LONG_PTR)hicon );
    if (hicon && !(style & SS_CENTERIMAGE) && !(style & SS_REALSIZECONTROL))
    {
        ...
        if ((style & SS_RIGHTJUST) != 0)
        {
            ...
            SetWindowPos_k( hwnd, 0, wr.right - info->nWidth, wr.bottom - info->nHeight,
                          info->nWidth, info->nHeight, SWP_NOACTIVATE | SWP_NOZORDER );
        }
        else */
        {
            SetWindowPos_k( hwnd, 0, 0, 0, size.cx, size.cy, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOZORDER );
        }
    }
    return prevIcon;
}
...
static HBITMAP STATIC_SetBitmap( HWND hwnd, HBITMAP hBitmap, DWORD style )
{
    ...
    if (hBitmap && GetObjectType_k(hBitmap) != OBJ_BITMAP) {
        WARN("hBitmap != 0, but it's not a bitmap\n");
        return 0;
    }
    hOldBitmap = (HBITMAP)SetWindowLongPtrW_k( hwnd, HICON_GWL_OFFSET, (LONG_PTR)hBitmap );
    if (hBitmap && !(style & SS_CENTERIMAGE) && !(style & SS_REALSIZECONTROL))
    {
        BITMAP bm;
        GetObjectW_k(hBitmap, sizeof(bm), &bm);
        /* Windows currently doesn't implement SS_RIGHTJUST */
        /*
        if ((style & SS_RIGHTJUST) != 0)
        {
            RECT wr;
            GetWindowRect(hwnd, &wr);
            SetWindowPos_k( hwnd, 0, wr.right - bm.bmWidth, wr.bottom - bm.bmHeight,
                          bm.bmWidth, bm.bmHeight, SWP_NOACTIVATE | SWP_NOZORDER );
        }
        else */
        {
            SetWindowPos_k( hwnd, 0, 0, 0, bm.bmWidth, bm.bmHeight,
...
static HENHMETAFILE STATIC_SetEnhMetaFile( HWND hwnd, HENHMETAFILE hEnhMetaFile, DWORD style )
{
    ...
    if (hEnhMetaFile && GetObjectType_k(hEnhMetaFile) != OBJ_ENHMETAFILE) {
        WARN("hEnhMetaFile != 0, but it's not an enhanced metafile\n");
        return 0;
    }
    return (HENHMETAFILE)SetWindowLongPtrW_k( hwnd, HICON_GWL_OFFSET, (LONG_PTR)hEnhMetaFile );
}
...
static HANDLE STATIC_GetImage( HWND hwnd, WPARAM wParam, DWORD style )
{
    ...
    return (HANDLE)GetWindowLongPtrW_k( hwnd, HICON_GWL_OFFSET );
}
...
static HICON STATIC_LoadIconA( HINSTANCE hInstance, LPCSTR name, DWORD style )
{
    ...
    if (hInstance && ((ULONG_PTR)hInstance >> 16))
    {
        if ((style & SS_REALSIZEIMAGE) != 0)
            hicon = LoadImageA_k(hInstance, name, IMAGE_ICON, 0, 0, LR_SHARED);
        else
        {
            hicon = LoadIconA_k( hInstance, name );
            if (!hicon) hicon = LoadCursorA_k( hInstance, name );
        }
    }
    if (!hicon) hicon = LoadIconA_k( 0, name );
...
static HICON STATIC_LoadIconW( HINSTANCE hInstance, LPCWSTR name, DWORD style )
{
    ...
    {
        if ((style & SS_REALSIZEIMAGE) != 0)
            hicon = LoadImageW_k(hInstance, name, IMAGE_ICON, 0, 0, LR_SHARED);
        else
        {
            hicon = LoadIconW_k( hInstance, name );
            if (!hicon) hicon = LoadCursorW_k( hInstance, name );
        }
    }
    if (!hicon) hicon = LoadIconW_k( 0, name );
    /* Windows doesn't try to load a standard cursor,
       probably because most IDs for standard cursors conflict
       with the IDs for standard icons anyway */
    return hicon;
...
static VOID STATIC_TryPaintFcn(HWND hwnd, LONG full_style)
{
    ...
    if (!IsRectEmpty_k(&rc) && IsWindowVisible_k(hwnd) && staticPaintFunc[style])
    {
	HDC hdc;
        HRGN hrgn;
...
static HBRUSH STATIC_SendWmCtlColorStatic(HWND hwnd, HDC hdc)
{
    ...
    hBrush = (HBRUSH) SendMessageW_k( parent,
...
{
    color_3ddkshadow  = GetSysColor_k(COLOR_3DDKSHADOW);
    color_3dshadow    = GetSysColor_k(COLOR_3DSHADOW);
    color_3dhighlight = GetSysColor_k(COLOR_3DHIGHLIGHT);
}
...
{
    LRESULT lResult = 0;
    LONG full_style = GetWindowLongW_k( hwnd, GWL_STYLE );
    LONG style = full_style & SS_TYPEMASK;
 ...
 */
        ...
        else return unicode ? DefWindowProcW_k(hwnd, uMsg, wParam, lParam) :
        ...
        if (full_style & SS_NOTIFY) {
            if (wParam) {
                SendMessageW_k( GetParent_k(hwnd), WM_COMMAND,
                              MAKEWPARAM( GetWindowLongPtrW_k(hwnd,GWLP_ID), STN_ENABLE ), (LPARAM)hwnd);
            }
            else {
                SendMessageW_k( GetParent_k(hwnd), WM_COMMAND,
                              MAKEWPARAM( GetWindowLongPtrW_k(hwnd,GWLP_ID), STN_DISABLE ), (LPARAM)hwnd);
            }
        }
        break;
    ...
    case WM_NCCREATE:
        {
            ...
            if (full_style & SS_SUNKEN)
                SetWindowLongW_k( hwnd, GWL_EXSTYLE,
                                GetWindowLongW_k( hwnd, GWL_EXSTYLE ) | WS_EX_STATICEDGE );
            ...
            case SS_BITMAP:
                if ((ULONG_PTR)cs->hInstance >> 16)
                {
                    HBITMAP hBitmap;
                    if (unicode || IS_INTRESOURCE(cs->lpszName))
                        hBitmap = LoadBitmapW_k(cs->hInstance, cs->lpszName);
                    else
                        hBitmap = LoadBitmapA_k(cs->hInstance, (LPCSTR)cs->lpszName);
    ...
    case WM_SETTEXT:
	    ...
	    {
	        if(unicode)
		     lResult = DefWindowProcW_k( hwnd, uMsg, wParam, lParam );
                else
                    lResult = DefWindowProcA_k( hwnd, uMsg, wParam, lParam );
	        STATIC_TryPaintFcn( hwnd, full_style );
	    }
	}
        break;
    ...
    case WM_SETFONT:
        if (hasTextStyle( full_style ))
        {
            SetWindowLongPtrW_k( hwnd, HFONT_GWL_OFFSET, wParam );
            if (LOWORD(lParam))
                RedrawWindow_k( hwnd, NULL, 0, RDW_INVALIDATE | RDW_ERASE | RDW_UPDATENOW | RDW_ALLCHILDREN );
        }
        break;

    case WM_GETFONT:
        return GetWindowLongPtrW_k( hwnd, HFONT_GWL_OFFSET );
    ...
    case WM_NCLBUTTONDOWN:
        if (full_style & SS_NOTIFY)
            SendMessageW_k( GetParent_k(hwnd), WM_COMMAND,
                          MAKEWPARAM( GetWindowLongPtrW_k(hwnd,GWLP_ID), STN_CLICKED ), (LPARAM)hwnd);
        return 0;
    ...
    case WM_NCLBUTTONDBLCLK:
        if (full_style & SS_NOTIFY)
            SendMessageW_k( GetParent_k(hwnd), WM_COMMAND,
                          MAKEWPARAM( GetWindowLongPtrW_k(hwnd,GWLP_ID), STN_DBLCLK ), (LPARAM)hwnd);
        return 0;
...
static void STATIC_PaintOwnerDrawfn( HWND hwnd, HDC hdc, DWORD style )
{
  ...
  dis.itemState  = IsWindowEnabled_k(hwnd) ? 0 : ODS_DISABLED;
  dis.hwndItem   = hwnd;
  dis.hDC        = hdc;
  dis.itemData   = 0;
  GetClientRect_k( hwnd, &dis.rcItem );

  font = (HFONT)GetWindowLongPtrW_k( hwnd, HFONT_GWL_OFFSET );
  if (font) oldFont = SelectObject_k( hdc, font );
  SendMessageW_k( GetParent_k(hwnd), WM_CTLCOLORSTATIC, (WPARAM)hdc, (LPARAM)hwnd );
  SendMessageW_k( GetParent_k(hwnd), WM_DRAWITEM, id, (LPARAM)&dis );
  if (font) SelectObject_k( hdc, oldFont );
}
...
static void STATIC_PaintTextfn( HWND hwnd, HDC hdc, DWORD style )
{
    ...
    if (GetWindowLongW_k( hwnd, GWL_EXSTYLE ) & WS_EX_RIGHT)
    ...
    if ((hFont = (HFONT)GetWindowLongPtrW_k( hwnd, HFONT_GWL_OFFSET )))
    ...
    if ((style & SS_TYPEMASK) != SS_SIMPLE)
    {
        ...
        if (!IsWindowEnabled_k(hwnd)) SetTextColor_k(hdc, GetSysColor_k(COLOR_GRAYTEXT));
    }

    buf_size = 256;
    if (!(text = HeapAlloc_k( GetProcessHeap_k(), 0, buf_size * sizeof(WCHAR) )))
        goto no_TextOut;

    while ((len = InternalGetWindowText_k( hwnd, text, buf_size )) == buf_size - 1)
    {
        buf_size *= 2;
        if (!(text = HeapReAlloc_k( GetProcessHeap_k(), 0, text, buf_size * sizeof(WCHAR) )))
            goto no_TextOut;
    }
    ...
    if (((style & SS_TYPEMASK) == SS_SIMPLE) && (style & SS_NOPREFIX))
    {
        ...
        ExtTextOutW_k( hdc, rc.left, rc.top, ETO_CLIPPED | ETO_OPAQUE,
                     &rc, text, len, NULL );
    }
    else
    {
        DrawTextW_k( hdc, text, -1, &rc, format );
    }
...
static void STATIC_PaintRectfn( HWND hwnd, HDC hdc, DWORD style )
{
    ...
    {
    case SS_BLACKRECT:
	hBrush = CreateSolidBrush_k(color_3ddkshadow);
        FillRect_k( hdc, &rc, hBrush );
	break;
    case SS_GRAYRECT:
	hBrush = CreateSolidBrush_k(color_3dshadow);
        FillRect_k( hdc, &rc, hBrush );
	break;
    case SS_WHITERECT:
	hBrush = CreateSolidBrush_k(color_3dhighlight);
        FillRect_k( hdc, &rc, hBrush );
	break;
    case SS_BLACKFRAME:
	hBrush = CreateSolidBrush_k(color_3ddkshadow);
        FrameRect_k( hdc, &rc, hBrush );
	break;
    case SS_GRAYFRAME:
	hBrush = CreateSolidBrush_k(color_3dshadow);
        FrameRect_k( hdc, &rc, hBrush );
...
static void STATIC_PaintIconfn( HWND hwnd, HDC hdc, DWORD style )
{
    ...
    GetClientRect_k( hwnd, &rc );
    hbrush = STATIC_SendWmCtlColorStatic(hwnd, hdc);
    hIcon = (HICON)GetWindowLongPtrW_k( hwnd, HICON_GWL_OFFSET );
    if (!hIcon || !get_icon_size( hIcon, &size ))
    {
        FillRect_k(hdc, &rc, hbrush);
    }
    else
    {
        if (style & SS_CENTERIMAGE)
        {
            ...
            iconRect = rc;
        FillRect_k( hdc, &rc, hbrush );
        DrawIconEx_k( hdc, iconRect.left, iconRect.top, hIcon, iconRect.right - iconRect.left,
...
static void STATIC_PaintBitmapfn(HWND hwnd, HDC hdc, DWORD style )
{
    ...
    if ((hBitmap = (HBITMAP)GetWindowLongPtrW_k( hwnd, HICON_GWL_OFFSET ))
         && (GetObjectType_k(hBitmap) == OBJ_BITMAP)
         && (hMemDC = CreateCompatibleDC_k( hdc )))
    {
        ...
        GetObjectW_k(hBitmap, sizeof(bm), &bm);
        oldbitmap = SelectObject_k(hMemDC, hBitmap);

        /* Set the background color for monochrome bitmaps
           to the color of the background brush */
        if (GetObjectW_k( hbrush, sizeof(brush), &brush ))
        {
            if (brush.lbStyle == BS_SOLID)
                SetBkColor_k(hdc, brush.lbColor);
        }
        GetClientRect_k(hwnd, &rcClient);
        if (style & SS_CENTERIMAGE)
        {
            INT x, y;
            x = (rcClient.right - rcClient.left)/2 - bm.bmWidth/2;
            y = (rcClient.bottom - rcClient.top)/2 - bm.bmHeight/2;
            FillRect_k( hdc, &rcClient, hbrush );
            BitBlt_k(hdc, x, y, bm.bmWidth, bm.bmHeight, hMemDC, 0, 0,
                   SRCCOPY);
        }
        else
        {
            StretchBlt_k(hdc, 0, 0, rcClient.right - rcClient.left,
                       rcClient.bottom - rcClient.top, hMemDC,
                       0, 0, bm.bmWidth, bm.bmHeight, SRCCOPY);
        }
        SelectObject_k(hMemDC, oldbitmap);
        DeleteDC_k(hMemDC);
    }
}
...
static void STATIC_PaintEnhMetafn(HWND hwnd, HDC hdc, DWORD style )
{
    ...
    hbrush = STATIC_SendWmCtlColorStatic(hwnd, hdc);
    FillRect_k(hdc, &rc, hbrush);
    if ((hEnhMetaFile = (HENHMETAFILE)GetWindowLongPtrW_k( hwnd, HICON_GWL_OFFSET )))
    {
        /* The control's current font is not selected into the
           device context! */
        if (GetObjectType_k(hEnhMetaFile) == OBJ_ENHMETAFILE)
            PlayEnhMetaFile_k(hdc, hEnhMetaFile, &rc);
    }
}
...
static void STATIC_PaintEtchedfn( HWND hwnd, HDC hdc, DWORD style )
{
    ...
    {
	case SS_ETCHEDHORZ:
	    DrawEdge_k(hdc,&rc,EDGE_ETCHED,BF_TOP|BF_BOTTOM);
	    break;
	case SS_ETCHEDVERT:
	    DrawEdge_k(hdc,&rc,EDGE_ETCHED,BF_LEFT|BF_RIGHT);
	    break;
	case SS_ETCHEDFRAME:
	    DrawEdge_k (hdc, &rc, EDGE_ETCHED, BF_RECT);
	    break;
    }
}
