...
static HICON alloc_icon_handle( BOOL is_ani, UINT num_steps )
{
    ...
    obj = HeapAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY, icon_size );
...
static BOOL free_icon_handle( HICON handle )
{
    ...
    else if (obj)
    {
        ...
        if (!obj->is_ani)
        {
            ...
            if (frame->alpha) DeleteObject_k( frame->alpha );
            if (frame->color) DeleteObject_k( frame->color );
            DeleteObject_k( frame->mask );
        ...
        if (!IS_INTRESOURCE( obj->resname )) HeapFree_k( GetProcessHeap_k(), 0, obj->resname );
...
static const void *map_fileW( LPCWSTR name, LPDWORD filesize )
{
    ...
    hFile = CreateFileW_k( name, GENERIC_READ, FILE_SHARE_READ, NULL,
                         OPEN_EXISTING, FILE_FLAG_RANDOM_ACCESS, 0 );
    if (hFile != INVALID_HANDLE_VALUE)
    {
        hMapping = CreateFileMappingW_k( hFile, NULL, PAGE_READONLY, 0, 0, NULL );
        if (hMapping)
        {
            ptr = MapViewOfFile_k( hMapping, FILE_MAP_READ, 0, 0, 0 );
            CloseHandle_k( hMapping );
            if (filesize)
                *filesize = GetFileSize_k( hFile, NULL );
        }
        CloseHandle_k( hFile );
    }
    return ptr;
...
static HBITMAP copy_bitmap( HBITMAP bitmap )
{
    ...
    if (!bitmap) return 0;
    if (!GetObjectW_k( bitmap, sizeof(bmp), &bmp )) return 0;

    if ((src = CreateCompatibleDC_k( 0 )) && (dst = CreateCompatibleDC_k( 0 )))
    {
        SelectObject_k( src, bitmap );
        if ((new_bitmap = CreateCompatibleBitmap_k( src, bmp.bmWidth, bmp.bmHeight )))
        {
            SelectObject_k( dst, new_bitmap );
            BitBlt_k( dst, 0, 0, bmp.bmWidth, bmp.bmHeight, src, 0, 0, SRCCOPY );
        }
    }
    DeleteDC_k( dst );
    DeleteDC_k( src );
                                    ...
                                    int width, int height, int depth, UINT loadflags )
{
    ...
    {
        if (!width) width = GetSystemMetrics_k( SM_CXICON );
        if (!height) height = GetSystemMetrics_k( SM_CYICON );
    }
                                        ...
                                        int *width, int *height, int *bits )
{
    ...
    if ( resdir->idCount <= n )
        return FALSE;
    if ((const char *)&resdir->idEntries[n + 1] - (const char *)dir > (INT)size)
        return FALSE;
                                      ...
                                      int width, int height, int depth, UINT loadflags )
{
    ...
    if (loadflags & LR_DEFAULTSIZE)
    {
        if (!width) width = GetSystemMetrics_k( SM_CXCURSOR );
        if (!height) height = GetSystemMetrics_k( SM_CYCURSOR );
    }
    else if (!width && !height)
    {
        /* use the first entry */
                                     ...
                                     int *width, int *height, int *bits )
{
    ...
    if ( filedir->idCount <= n )
        return FALSE;
    if ((const char *)&filedir->idEntries[n + 1] - (const char *)dir > (INT)size)
        return FALSE;
    entry = &filedir->idEntries[n];
    info = (const BITMAPINFOHEADER *)((const char *)dir + entry->dwDIBOffset);
    if ((const char *)(info + 1) - (const char *)dir > (INT)size) return FALSE;
    *width = entry->bWidth;
                                    ...
                                    const BITMAPINFO *src_info, const void *color_bits )
{
    ...
    if (!GetObjectW_k( color, sizeof(bm), &bm )) return 0;
    if (bm.bmBitsPixel != 32) return 0;

    if (!(hdc = CreateCompatibleDC_k( 0 ))) return 0;
    if (!(info = HeapAlloc_k( GetProcessHeap_k(), 0, FIELD_OFFSET( BITMAPINFO, bmiColors[256] )))) goto done;
    ...
    if (!(alpha = CreateDIBSection_k( hdc, info, DIB_RGB_COLORS, &bits, NULL, 0 ))) goto done;

    if (src_info)
    {
        SelectObject_k( hdc, alpha );
        StretchDIBits_k( hdc, 0, 0, bm.bmWidth, bm.bmHeight,
    ...
    {
        GetDIBits_k( hdc, color, 0, bm.bmHeight, bits, info, DIB_RGB_COLORS );
        if (!bmi_has_alpha( info, bits ))
        {
            DeleteObject_k( alpha );
            alpha = 0;
            goto done;
        }
    }
    ...
    DeleteDC_k( hdc );
    HeapFree_k( GetProcessHeap_k(), 0, info );
    return alpha;
}
                                   ...
                                   UINT cFlag )
{
    ...
    {
        if (!width) width = GetSystemMetrics_k( bIcon ? SM_CXICON : SM_CXCURSOR );
        if (!height) height = GetSystemMetrics_k( bIcon ? SM_CYICON : SM_CYCURSOR );
    }
    else
    {
    ...
    if (!screen_dc) screen_dc = CreateDCW_k( DISPLAYW, NULL, NULL, NULL );
    if (!screen_dc) return 0;

    if (!(bmi_copy = HeapAlloc_k(GetProcessHeap_k(), 0, max( size, (DWORD)(FIELD_OFFSET( BITMAPINFO, bmiColors[2] ))))))
        return 0;
    if (!(hdc = CreateCompatibleDC_k( 0 ))) goto done;
    ...
    {
        if (!(mask = CreateBitmap_k( width, height * 2, 1, 1, NULL ))) goto done;
        color = 0;

        /* copy color data into second half of mask bitmap */
        SelectObject_k( hdc, mask );
        StretchDIBits_k( hdc, 0, height, width, height,
                       0, 0, bmi_copy->bmiHeader.biWidth, bmi_copy->bmiHeader.biHeight,
                       color_bits, bmi_copy, DIB_RGB_COLORS, SRCCOPY );
    }
    else
    {
        if (!(mask = CreateBitmap_k( width, height, 1, 1, NULL ))) goto done;
        if (!(color = CreateBitmap_k( width, height, GetDeviceCaps_k( screen_dc, PLANES ),
                                     GetDeviceCaps_k( screen_dc, BITSPIXEL ), NULL )))
        {
            DeleteObject_k( mask );
            goto done;
        }
        SelectObject_k( hdc, color );
        StretchDIBits_k( hdc, 0, 0, width, height,
                       0, 0, bmi_copy->bmiHeader.biWidth, bmi_copy->bmiHeader.biHeight,
                       color_bits, bmi_copy, DIB_RGB_COLORS, SRCCOPY );
    ...
    DeleteDC_k( hdc );
    HeapFree_k( GetProcessHeap_k(), 0, bmi_copy );
    ...
    if (hObj)
    {
        ...
        {
            info->resname = HeapAlloc_k(GetProcessHeap_k(), 0, (strlenW(resname) + 1) * sizeof(WCHAR) );
                                             ...
                                             INT depth, BOOL is_icon, UINT loadflags )
{
    ...
    if (!cursor) return 0;
    frames = HeapAlloc_k(GetProcessHeap_k(), 0, sizeof(DWORD)*header.num_frames );
    if (!frames)
    {
    ...
    for (i=0; i<header.num_frames; i++)
    {
        ...
        frames[i] = NULL;
        if ((INT)entry->dwDIBOffset < bits + bits_size - icon_data)
        {
        ...
        if (!frames[i])
        {
            ...
            if (i == 0)
            {
                ...
                free_icon_handle( cursor );
                HeapFree_k( GetProcessHeap_k(), 0, frames );
                return 0;
            }
            break;
        }
 ...
 */
HICON WINAPI CreateIconFromResourceEx_k( LPBYTE bits, UINT cbSize,
 ...
 */
HICON WINAPI CreateIconFromResource_k( LPBYTE bits, UINT cbSize,
                                           BOOL bIcon, DWORD dwVersion)
{
    return CreateIconFromResourceEx_k( bits, cbSize, bIcon, dwVersion, 0,0,0);
}
                             ...
                             BOOL fCursor, UINT loadflags)
{
    ...
    if ( filesize < (DWORD)FIELD_OFFSET( CURSORICONFILEDIR, idEntries[dir->idCount] ))
        goto end;
                             ...
                             BOOL fCursor, UINT loadflags)
{
    ...
    if (!(hRsrc = FindResourceW_k( hInstance, name,
                                 (LPWSTR)(fCursor ? RT_GROUP_CURSOR : RT_GROUP_ICON) )))
    {
        /* try animated resource */
        if (!(hRsrc = FindResourceW_k( hInstance, name,
                                    (LPWSTR)(fCursor ? RT_ANICURSOR : RT_ANIICON) ))) return 0;
        if (!(handle = LoadResource_k( hInstance, hRsrc ))) return 0;
        bits = LockResource_k( handle );
        return CURSORICON_CreateIconFromANI( bits, SizeofResource_k( hInstance, handle ),
    ...
    if (!(handle = LoadResource_k( hInstance, hRsrc ))) return 0;
    if (!(dir = LockResource_k( handle ))) return 0;
    size = SizeofResource_k( hInstance, hRsrc );
    if (fCursor)
    ...
    FreeResource_k( handle );

    /* Load the resource */

    if (!(hRsrc = FindResourceW_k(hInstance,MAKEINTRESOURCEW(wResId),
 ...
 *		CreateCursor (USER32.@)
 */
HCURSOR WINAPI CreateCursor_k( HINSTANCE hInstance,
                                 ...
                                 LPCVOID lpANDbits, LPCVOID lpXORbits )
{
    ...
    info.hbmMask = CreateBitmap_k( nWidth, nHeight, 1, 1, lpANDbits );
    info.hbmColor = CreateBitmap_k( nWidth, nHeight, 1, 1, lpXORbits );
    hCursor = CreateIconIndirect_k( &info );
    DeleteObject_k( info.hbmMask );
    DeleteObject_k( info.hbmColor );
    return hCursor;
}
    ...
    LPCVOID   lpXORbits)  /* [in] the icon's 'color' bitmap */
{
    ...
    iinfo.hbmMask = CreateBitmap_k( nWidth, nHeight, 1, 1, lpANDbits );
    iinfo.hbmColor = CreateBitmap_k( nWidth, nHeight, bPlanes, bBitsPixel, lpXORbits );

    hIcon = CreateIconIndirect_k( &iinfo );

    DeleteObject_k( iinfo.hbmMask );
    DeleteObject_k( iinfo.hbmColor );
 ...
 *		CopyIcon (USER32.@)
 */
HICON WINAPI CopyIcon_k( HICON hIcon )
{
    ...
    {
        SetLastError_k( ERROR_INVALID_CURSOR_HANDLE );
        return 0;
    }
 ...
 *		DestroyIcon (USER32.@)
 */
BOOL WINAPI DestroyIcon_k( HICON hIcon )
{
    BOOL ret = FALSE;
 ...
 */
BOOL WINAPI DestroyCursor_k( HCURSOR hCursor )
{
    return DestroyIcon_k( hCursor );
}

/***********************************************************************
 *		DrawIcon (USER32.@)
 */
BOOL WINAPI DrawIcon_k( HDC hdc, INT x, INT y, HICON hIcon )
{
    return DrawIconEx_k( hdc, x, y, hIcon, 0, 0, 0, 0, DI_NORMAL | DI_COMPAT | DI_DEFAULTSIZE );
}
 ...
 */
HCURSOR WINAPI SetCursor_k( HCURSOR hCursor /* [in] Handle of cursor to show */ )
{
    struct cursoricon_object *obj;
    HCURSOR hOldCursor;
    struct thread_input *input = currentThread()->input;

    hOldCursor = (HCURSOR)input->cursor;
    input->cursor = (unsigned int)hCursor;

    USER_Driver->pSetCursor( input->cursor_count >= 0 ? hCursor : 0 );
 ...
 *		ShowCursor (USER32.@)
 */
INT WINAPI ShowCursor_k( BOOL bShow )
{
    int increment = bShow ? 1 : -1;
    int count;
    struct thread_input *input = currentThread()->input;

    count = input->cursor_count;
    input->cursor_count+=increment;

    TRACE("%d, count=%d\n", bShow, count );

    if (bShow && !count) USER_Driver->pSetCursor( (HCURSOR)input->cursor );
 ...
 *		GetCursor (USER32.@)
 */
HCURSOR WINAPI GetCursor_k(void)
{
    return (HCURSOR)currentThread()->input->cursor;
}
 ...
 *		ClipCursor (USER32.@)
 */
BOOL WINAPI ClipCursor_k( const RECT *rect )
{
 ...
 */
BOOL WINAPI GetClipCursor_k( RECT *rect )
{
    rect->left   = 0;
    rect->top    = 0;
 ...
 */
BOOL WINAPI SetSystemCursor_k(HCURSOR hcur, DWORD id)
{
    FIXME("(%p,%08x),stub!\n",  hcur, id);
    return TRUE;
}
 ...
 */
INT WINAPI LookupIconIdFromDirectoryEx_k( LPBYTE xdir, BOOL bIcon,
             INT width, INT height, UINT cFlag )
{
    ...
    if( dir && !dir->idReserved && (dir->idType & 3) )
    {
        ...
        const HDC hdc = GetDC_k(0);
        const int depth = (cFlag & LR_MONOCHROME) ?
            1 : GetDeviceCaps_k(hdc, BITSPIXEL);
        ReleaseDC_k(0, hdc);
 ...
 */
INT WINAPI LookupIconIdFromDirectory_k( LPBYTE dir, BOOL bIcon )
{
    return LookupIconIdFromDirectoryEx_k( dir, bIcon, 0, 0, bIcon ? 0 : LR_MONOCHROME );
}

/***********************************************************************
 *              LoadCursorW (USER32.@)
 */
HCURSOR WINAPI LoadCursorW_k(HINSTANCE hInstance, LPCWSTR name)
{
    TRACE("%p, %s\n", hInstance, debugstr_w(name));

    return LoadImageW_k( hInstance, name, IMAGE_CURSOR, 0, 0,
 ...
 *		LoadCursorA (USER32.@)
 */
HCURSOR WINAPI LoadCursorA_k(HINSTANCE hInstance, LPCSTR name)
{
    TRACE("%p, %s\n", hInstance, debugstr_a(name));

    return LoadImageA_k( hInstance, name, IMAGE_CURSOR, 0, 0,
                       LR_SHARED | LR_DEFAULTSIZE );
}
 ...
 */
HCURSOR WINAPI LoadCursorFromFileW_k (LPCWSTR name)
{
    TRACE("%s\n", debugstr_w(name));

    return LoadImageW_k( 0, name, IMAGE_CURSOR, 0, 0,
 ...
 */
HCURSOR WINAPI LoadCursorFromFileA_k (LPCSTR name)
{
    TRACE("%s\n", debugstr_a(name));

    return LoadImageA_k( 0, name, IMAGE_CURSOR, 0, 0,
                       LR_LOADFROMFILE | LR_DEFAULTSIZE );
}
 ...
 *		LoadIconW (USER32.@)
 */
HICON WINAPI LoadIconW_k(HINSTANCE hInstance, LPCWSTR name)
{
    TRACE("%p, %s\n", hInstance, debugstr_w(name));

    return LoadImageW_k( hInstance, name, IMAGE_ICON, 0, 0,
                       LR_SHARED | LR_DEFAULTSIZE );
}
 ...
 *              LoadIconA (USER32.@)
 */
HICON WINAPI LoadIconA_k(HINSTANCE hInstance, LPCSTR name)
{
    TRACE("%p, %s\n", hInstance, debugstr_a(name));

    return LoadImageA_k( hInstance, name, IMAGE_ICON, 0, 0,
                       LR_SHARED | LR_DEFAULTSIZE );
}
 ...
 */
HCURSOR WINAPI GetCursorFrameInfo_k(HCURSOR hCursor, DWORD reserved, DWORD istep, DWORD *rate_jiffies, DWORD *num_steps)
{
    struct cursoricon_object *ptr;
    HCURSOR ret = 0;
    UINT icon_steps;
 ...
 *              GetIconInfo (USER32.@)
 */
BOOL WINAPI GetIconInfo_k(HICON hIcon, PICONINFO iconinfo)
{
    ICONINFOEXW infoW;

    infoW.cbSize = sizeof(infoW);
    if (!GetIconInfoExW_k( hIcon, &infoW )) return FALSE;
 ...
 */
BOOL WINAPI GetIconInfoExA_k( HICON icon, ICONINFOEXA *info )
{
    ICONINFOEXW infoW;
    ...
    infoW.cbSize = sizeof(infoW);
    if (!GetIconInfoExW_k( icon, &infoW )) return FALSE;
    ...
    WideCharToMultiByte_k( CP_ACP, 0, infoW.szModName, -1, info->szModName, MAX_PATH, NULL, NULL );
    WideCharToMultiByte_k( CP_ACP, 0, infoW.szResName, -1, info->szResName, MAX_PATH, NULL, NULL );
    return TRUE;
}
 ...
 */
BOOL WINAPI GetIconInfoExW_k( HICON icon, ICONINFOEXW *info )
{
    struct cursoricon_frame *frame;
    struct cursoricon_object *ptr;
    HMODULE module;
    ...
    release_icon_ptr( icon, ptr );
    if (ret && module) GetModuleFileNameW_k( module, info->szModName, MAX_PATH );
    return ret;
}
...
{
    HDC hdc = CreateCompatibleDC_k( 0 );

    if (!SelectObject_k( hdc, src ))  /* do it the hard way */
    {
        BITMAPINFO *info;
        void *bits;

        if (!(info = HeapAlloc_k(GetProcessHeap_k(), 0, FIELD_OFFSET( BITMAPINFO, bmiColors[256] )))) return;
        info->bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
        info->bmiHeader.biWidth = width;
        info->bmiHeader.biHeight = height;
        info->bmiHeader.biPlanes = GetDeviceCaps_k( hdc_dst, PLANES );
        info->bmiHeader.biBitCount = GetDeviceCaps_k( hdc_dst, BITSPIXEL );
        info->bmiHeader.biCompression = BI_RGB;
        ...
        info->bmiHeader.biClrUsed = 0;
        info->bmiHeader.biClrImportant = 0;
        bits = HeapAlloc_k(GetProcessHeap_k(), 0, info->bmiHeader.biSizeImage );
        if (bits && GetDIBits_k( hdc, src, 0, height, bits, info, DIB_RGB_COLORS ))
            StretchDIBits_k( hdc_dst, dst_x, dst_y, dst_width, dst_height,
                           0, 0, width, height, bits, info, DIB_RGB_COLORS, SRCCOPY );

        HeapFree_k( GetProcessHeap_k(), 0, bits );
        HeapFree_k( GetProcessHeap_k(), 0, info );
    }
    else StretchBlt_k( hdc_dst, dst_x, dst_y, dst_width, dst_height, hdc, 0, 0, width, height, SRCCOPY );
 ...
 */
HICON WINAPI CreateIconIndirect_k(PICONINFO iconinfo)
{
    ...
    GetObjectW_k( iconinfo->hbmMask, sizeof(bmpAnd), &bmpAnd );
    ...
    {
        GetObjectW_k( iconinfo->hbmColor, sizeof(bmpXor), &bmpXor );
        ...
        {
            color = CreateCompatibleBitmap_k( screen_dc, width, height );
            mask = CreateBitmap_k( width, height, 1, 1, NULL );
        }
        else mask = CreateBitmap_k( width, height * 2, 1, 1, NULL );
    }
    else
    {
 ...
 *    Failure: FALSE
 */
BOOL WINAPI DrawIconEx_k( HDC hdc, INT x0, INT y0, HICON hIcon,
                            ...
                            HBRUSH hbr, UINT flags )
{
    ...
    if (!(hMemDC = CreateCompatibleDC_k( hdc )))
    {
        release_icon_frame( ptr, istep, frame );
        release_icon_ptr( hIcon, ptr );
        return FALSE;
    ...
    if (cxWidth == 0)
    {
        if (flags & DI_DEFAULTSIZE)
            cxWidth = GetSystemMetrics_k(SM_CXICON);
        else
            cxWidth = frame->width;
    }
    if (cyWidth == 0)
    {
        if (flags & DI_DEFAULTSIZE)
            cyWidth = GetSystemMetrics_k(SM_CYICON);
        else
            cyWidth = frame->height;
    }

    DoOffscreen = (GetObjectType_k( hbr ) == OBJ_BRUSH);
    ...
    if (DoOffscreen) {
        ...
        if (!(hdc_dest = CreateCompatibleDC_k(hdc))) goto failed;
        if (!(hB_off = CreateCompatibleBitmap_k(hdc, cxWidth, cyWidth)))
        {
            DeleteDC_k( hdc_dest );
            goto failed;
        }
        SelectObject_k(hdc_dest, hB_off);
        FillRect_k(hdc_dest, &r, hbr);
        x = y = 0;
    }
    else
    {
    ...
    nStretchMode = SetStretchBltMode_k(hdc, STRETCH_DELETESCANS);

    oldFg = SetTextColor_k( hdc, RGB(0,0,0) );
    oldBg = SetBkColor_k( hdc, RGB(255,255,255) );

    if (frame->alpha && (flags & DI_IMAGE))
    {
        BOOL alpha_blend = TRUE;

        if (GetObjectType_k( hdc_dest ) == OBJ_MEMDC)
        {
            BITMAP bm;
            HBITMAP bmp = GetCurrentObject_k( hdc_dest, OBJ_BITMAP );
            alpha_blend = GetObjectW_k( bmp, sizeof(bm), &bm ) && bm.bmBitsPixel > 8;
        }
        if (alpha_blend)
        {
            BLENDFUNCTION pixelblend = { AC_SRC_OVER, 0, 255, AC_SRC_ALPHA };
            SelectObject_k( hMemDC, frame->alpha );
            if (GdiAlphaBlend_k( hdc_dest, x, y, cxWidth, cyWidth, hMemDC,
    ...
    if (flags & DI_MASK)
    {
        ...
        SelectObject_k( hMemDC, frame->mask );
        StretchBlt_k( hdc_dest, x, y, cxWidth, cyWidth,
    ...
    if (flags & DI_IMAGE)
    {
        if (frame->color)
        {
            ...
            SelectObject_k( hMemDC, frame->color );
            StretchBlt_k( hdc_dest, x, y, cxWidth, cyWidth,
                        hMemDC, 0, 0, frame->width, frame->height, rop );
        }
        else
    ...
    if (DoOffscreen) BitBlt_k( hdc, x0, y0, cxWidth, cyWidth, hdc_dest, 0, 0, SRCCOPY );

    SetTextColor_k( hdc, oldFg );
    SetBkColor_k( hdc, oldBg );
    SetStretchBltMode_k(hdc, nStretchMode);
    result = TRUE;
    if (hdc_dest != hdc) DeleteDC_k( hdc_dest );
    if (hB_off) DeleteObject_k(hB_off);
failed:
    DeleteDC_k( hMemDC );
    release_icon_frame( ptr, istep, frame );
    release_icon_ptr( hIcon, ptr );
...
static void DIB_FixColorsToLoadflags(BITMAPINFO * bmi, UINT loadflags, BYTE pix)
{
    ...
    c_W = GetSysColor_k(COLOR_WINDOW);
    c_S = GetSysColor_k(COLOR_3DSHADOW);
    c_F = GetSysColor_k(COLOR_3DFACE);
    c_L = GetSysColor_k(COLOR_3DLIGHT);
                            ...
                            INT desiredx, INT desiredy, UINT loadflags )
{
    ...
    if (!(loadflags & LR_LOADFROMFILE))
    {
        ...
        if (!(hRsrc = FindResourceW_k( instance, name, (LPWSTR)RT_BITMAP ))) return 0;
        if (!(handle = LoadResource_k( instance, hRsrc ))) return 0;

        if ((info = LockResource_k( handle )) == NULL) return 0;
    }
    else
    {
    ...
    fix_info = HeapAlloc_k(GetProcessHeap_k(), 0, size);
    scaled_info = HeapAlloc_k(GetProcessHeap_k(), 0, size);
    ...
    if (!screen_dc) screen_dc = CreateDCW_k( DISPLAYW, NULL, NULL, NULL );
    if (!(screen_mem_dc = CreateCompatibleDC_k( screen_dc ))) goto end;
    ...
    if (loadflags & LR_CREATEDIBSECTION)
    {
        ...
        hbitmap = CreateDIBSection_k(screen_dc, scaled_info, DIB_RGB_COLORS, NULL, 0, 0);
    }
    else
    {
        if (is_dib_monochrome(fix_info))
            hbitmap = CreateBitmap_k(new_width, new_height, 1, 1, NULL);
        else
            hbitmap = CreateCompatibleBitmap_k(screen_dc, new_width, new_height);        
    }

    orig_bm = SelectObject_k(screen_mem_dc, hbitmap);
    StretchDIBits_k(screen_mem_dc, 0, 0, new_width, new_height, 0, 0, width, height, bits, fix_info, DIB_RGB_COLORS, SRCCOPY);
    SelectObject_k(screen_mem_dc, orig_bm);

end:
    if (screen_mem_dc) DeleteDC_k(screen_mem_dc);
    HeapFree_k(GetProcessHeap_k(), 0, scaled_info);
    HeapFree_k(GetProcessHeap_k(), 0, fix_info);
    if (loadflags & LR_LOADFROMFILE) UnmapViewOfFile_k( ptr );
 ...
 * See LoadImageW.
 */
HANDLE WINAPI LoadImageA_k( HINSTANCE hinst, LPCSTR name, UINT type,
                              INT desiredx, INT desiredy, UINT loadflags)
{
    HANDLE res;
    LPWSTR u_name;
    ...
    if (IS_INTRESOURCE(name))
        return LoadImageW_k(hinst, (LPCWSTR)name, type, desiredx, desiredy, loadflags);

     {
        DWORD len = MultiByteToWideChar_k( CP_ACP, 0, name, -1, NULL, 0 );
        u_name = HeapAlloc_k(GetProcessHeap_k(), 0, len * sizeof(WCHAR) );
        MultiByteToWideChar_k( CP_ACP, 0, name, -1, u_name, len );
    }    
    
    res = LoadImageW_k(hinst, u_name, type, desiredx, desiredy, loadflags);
    HeapFree_k(GetProcessHeap_k(), 0, u_name);
    return res;
}
 ...
 */
HANDLE WINAPI LoadImageW_k( HINSTANCE hinst, LPCWSTR name, UINT type,
                INT desiredx, INT desiredy, UINT loadflags )
{
    int depth;
    ...
    case IMAGE_CURSOR:
        ...
        {
            if (!screen_dc) screen_dc = CreateDCW_k( DISPLAYW, NULL, NULL, NULL );
            if (screen_dc) depth = GetDeviceCaps_k( screen_dc, BITSPIXEL );
        }
 ...
 */
HANDLE WINAPI CopyImage_k( HANDLE hnd, UINT type, INT desiredx,
                             INT desiredy, UINT flags )
{
    ...
    switch (type)
    {
        case IMAGE_BITMAP:
        {
            ...
            objSize = GetObjectW_k( hnd, sizeof(ds), &ds );
            if (!objSize) return 0;
            ...
            bi = HeapAlloc_k(GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(BITMAPINFOHEADER) + 256 * sizeof(RGBQUAD));
            if (!bi) return 0;
            ...
            if (flags & LR_CREATEDIBSECTION)
            {
                ...
                void * bits;
                HDC dc = CreateCompatibleDC_k(NULL);
                ...
                GetDIBits_k(dc, hnd, 0, ds.dsBm.bmHeight, NULL, bi, DIB_RGB_COLORS);

                res = CreateDIBSection_k(dc, bi, DIB_RGB_COLORS, &bits, NULL, 0);
                DeleteDC_k(dc);
            }
            else
            {
                ...
                if (monochrome)
                {
                    res = CreateBitmap_k(desiredx, desiredy, 1, 1, NULL);
                }
                else
                {
                    HDC screenDC = GetDC_k(NULL);
                    res = CreateCompatibleBitmap_k(screenDC, desiredx, desiredy);
                    ReleaseDC_k(NULL, screenDC);
                }
            }
            ...
            if (res)
            {
                ...
                {
                    HDC screenDC = GetDC_k(NULL);
                    int screen_depth = GetDeviceCaps_k(screenDC, BITSPIXEL);
                    ReleaseDC_k(NULL, screenDC);
                ...
                if (copyContents)
                {
                    ...
                    /* Fill in biSizeImage */
                    GetDIBits_k(dc, hnd, 0, ds.dsBm.bmHeight, NULL, bi, DIB_RGB_COLORS);
                    bits = HeapAlloc_k(GetProcessHeap_k(), HEAP_ZERO_MEMORY, bi->bmiHeader.biSizeImage);
                    ...
                    if (bits)
                    {
                        ...
                        GetDIBits_k(dc, hnd, 0, ds.dsBm.bmHeight, bits, bi, DIB_RGB_COLORS);

                        /* Copy it to the destination bitmap */
                        oldBmp = SelectObject_k(dc, res);
                        StretchDIBits_k(dc, 0, 0, desiredx, desiredy,
                                      0, 0, ds.dsBm.bmWidth, ds.dsBm.bmHeight,
                                      bits, bi, DIB_RGB_COLORS, SRCCOPY);
                        SelectObject_k(dc, oldBmp);
            ...
            HeapFree_k(GetProcessHeap_k(), 0, bi);
            return res;
        }
        case IMAGE_ICON:
        case IMAGE_CURSOR:
        {
            ...
            HICON res = 0;
            int depth = (flags & LR_MONOCHROME) ? 1 : GetDeviceCaps_k( screen_dc, BITSPIXEL );

            if (flags & LR_DEFAULTSIZE)
            {
                if (!desiredx) desiredx = GetSystemMetrics_k( type == IMAGE_ICON ? SM_CXICON : SM_CXCURSOR );
                if (!desiredy) desiredy = GetSystemMetrics_k( type == IMAGE_ICON ? SM_CYICON : SM_CYCURSOR );
            }
            ...
            if (icon->rsrc && (flags & LR_COPYFROMRESOURCE))
                ...
                res = CopyIcon_k( hnd ); /* FIXME: change size if necessary */
            release_icon_ptr( hnd, icon );

            if (res && (flags & LR_COPYDELETEORG)) DeleteObject_k( hnd );
            return res;
        }
    }
    return 0;
 ...
 * See LoadBitmapW.
 */
HBITMAP WINAPI LoadBitmapA_k( HINSTANCE instance, LPCSTR name )
{
    return LoadImageA_k( instance, name, IMAGE_BITMAP, 0, 0, 0 );
}
