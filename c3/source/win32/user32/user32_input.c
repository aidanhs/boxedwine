...
static WORD get_key_state(void)
{
    ...
    if (GetSystemMetrics_k( SM_SWAPBUTTON ))
    {
        if (GetAsyncKeyState_k(VK_RBUTTON) & 0x80) ret |= MK_LBUTTON;
        if (GetAsyncKeyState_k(VK_LBUTTON) & 0x80) ret |= MK_RBUTTON;
    }
    else
    {
        if (GetAsyncKeyState_k(VK_LBUTTON) & 0x80) ret |= MK_LBUTTON;
        if (GetAsyncKeyState_k(VK_RBUTTON) & 0x80) ret |= MK_RBUTTON;
    }
    if (GetAsyncKeyState_k(VK_MBUTTON) & 0x80)  ret |= MK_MBUTTON;
    if (GetAsyncKeyState_k(VK_SHIFT) & 0x80)    ret |= MK_SHIFT;
    if (GetAsyncKeyState_k(VK_CONTROL) & 0x80)  ret |= MK_CONTROL;
    if (GetAsyncKeyState_k(VK_XBUTTON1) & 0x80) ret |= MK_XBUTTON1;
    if (GetAsyncKeyState_k(VK_XBUTTON2) & 0x80) ret |= MK_XBUTTON2;
    return ret;
}
...
BOOL set_capture_window( HWND hwnd, UINT gui_flags, HWND *prev_ret )
{
    ...
    if (input->menu_owner && !(gui_flags & GUI_INMENUMODE)) {
        SetLastError_k(ERROR_ACCESS_DENIED);
        return FALSE;
    }
    if (wnd && wnd->tid!=thread->id) {
        SetLastError_k(ERROR_ACCESS_DENIED);
        return FALSE;
    }
    ...
    if (gui_flags & GUI_INMENUMODE) 
        input->menu_owner = hwnd;
    else
        input->menu_owner = 0;
    if (gui_flags & GUI_INMOVESIZE) 
        input->move_size = hwnd;
    else
        input->move_size = 0;
    ...
    if (previous && previous != hwnd)
        SendMessageW_k( previous, WM_CAPTURECHANGED, 0, (LPARAM)hwnd );
...
static void update_mouse_coords( INPUT *input )
{
    ...
    {
        input->mi.dx = (input->mi.dx * GetSystemMetrics_k( SM_CXSCREEN )) >> 16;
        input->mi.dy = (input->mi.dy * GetSystemMetrics_k( SM_CYSCREEN )) >> 16;
    }
    else
    {
        int accel[3];
        ...
        SystemParametersInfoW_k(SPI_GETMOUSE, 0, accel, 0);
 ...
 *		SendInput  (USER32.@)
 */
UINT WINAPI SendInput_k( UINT count, LPINPUT inputs, int size )
{
    UINT i;
    NTSTATUS status;
                         ...
                         DWORD dwFlags, ULONG_PTR dwExtraInfo )
{
    ...
    SendInput_k( 1, &input, sizeof(input) );
}
 ...
 *		GetCursorPos (USER32.@)
 */
BOOL WINAPI GetCursorPos_k( POINT *pt )
{
 ...
 */
BOOL WINAPI GetCursorInfo_k( PCURSORINFO pci )
{
    struct thread_input* input = currentThread()->input;

    if (!pci) return 0;

    pci->hCursor = (HCURSOR)input->cursor;
    pci->flags = (input->cursor_count >= 0) ? CURSOR_SHOWING : 0;
    GetCursorPos_k(&pci->ptScreenPos);
    return TRUE;
}
 ...
 *		SetCursorPos (USER32.@)
 */
BOOL WINAPI SetCursorPos_k( INT x, INT y )
{
    USER_Driver->pSetCursorPos( x, y );
    return TRUE;
}
 ...
 *		SetCapture (USER32.@)
 */
HWND WINAPI SetCapture_k( HWND hwnd )
{
    HWND previous = 0;
 ...
 */
BOOL WINAPI ReleaseCapture_k(void)
{
 ...
 *		GetCapture (USER32.@)
 */
HWND WINAPI GetCapture_k(void)
{
    return currentThread()->input->capture;
}
 ...
 * been pressed.
 */
SHORT WINAPI GetAsyncKeyState_k( INT key )
{
    if (key < 0 || key >= 256) return 0;

    check_for_events( QS_INPUT );

    return USER_Driver->pGetAsyncKeyState( key );
}
 ...
 */
DWORD WINAPI GetQueueStatus_k( UINT flags )
{
    struct KThread* thread = currentThread();
    DWORD changed_bits;
    DWORD wake_bits;
    ...
    get_queue_status(TRUE, & wake_bits, & changed_bits);
 ...
 */
BOOL WINAPI GetInputState_k(void)
{
    DWORD changed_bits;
    DWORD wake_bits;

    check_for_events( QS_INPUT );
    get_queue_status(FALSE, & wake_bits, & changed_bits);
    return wake_bits & (QS_KEY | QS_MOUSEBUTTON);
}

extern DWORD last_input_time;

/******************************************************************
 *              GetLastInputInfo (USER32.@)
 */
BOOL WINAPI GetLastInputInfo_k(PLASTINPUTINFO plii)
{
    TRACE("%p\n", plii);
...
*/
UINT WINAPI GetRawInputDeviceList_k(RAWINPUTDEVICELIST *devices, UINT *device_count, UINT size)
{
...
*/
BOOL WINAPI RegisterRawInputDevices_k(RAWINPUTDEVICE *devices, UINT device_count, UINT size)
{
    /*
...
*/
UINT WINAPI GetRawInputData_k(HRAWINPUT rawinput, UINT command, void *data, UINT *data_size, UINT header_size)
{
...
*/
UINT WINAPI GetRawInputBuffer_k(PRAWINPUT pData, PUINT pcbSize, UINT cbSizeHeader)
{
...
*/
UINT WINAPI GetRawInputDeviceInfoA_k(HANDLE device, UINT command, void *data, UINT *data_size)
{
    UINT ret;

    TRACE("device %p, command %u, data %p, data_size %p.\n", device, command, data, data_size);

    ret = GetRawInputDeviceInfoW_k(device, command, data, data_size);
    if (command == RIDI_DEVICENAME && ret && ret != ~0U)
        ret = WideCharToMultiByte_k(CP_ACP, 0, data, -1, data, *data_size, NULL, NULL);
...
*/
UINT WINAPI GetRawInputDeviceInfoW_k(HANDLE device, UINT command, void *data, UINT *data_size)
{
...
*/
UINT WINAPI GetRegisteredRawInputDevices_k(PRAWINPUTDEVICE pRawInputDevices, PUINT puiNumDevices, UINT cbSize)
{
...
*/
LRESULT WINAPI DefRawInputProc_k(PRAWINPUT *paRawInput, INT nInput, UINT cbSizeHeader)
{
...
BOOL WINAPI AttachThreadInput_k( DWORD from, DWORD to, BOOL attach )
{
    return attach_thread_input_queue(attach, from, to)?TRUE:FALSE;
}
 ...
 */
SHORT WINAPI GetKeyState_k(INT vkey)
{
    return currentThread()->input->keystate[vkey & 0xFF];
}
 ...
 */
BOOL WINAPI GetKeyboardState_k( LPBYTE state )
{
    memcpy(state, currentThread()->input->keystate, 256);
    return TRUE;
}
 ...
 */
BOOL WINAPI SetKeyboardState_k( LPBYTE state )
{
    memcpy(currentThread()->input->keystate, state, 256);
    return TRUE;
}
 ...
 */
SHORT WINAPI VkKeyScanA_k(CHAR cChar)
{
    WCHAR wChar;

    if (IsDBCSLeadByte_k(cChar)) return -1;

    MultiByteToWideChar_k(CP_ACP, 0, &cChar, 1, &wChar, 1);
    return VkKeyScanW_k(wChar);
}

/******************************************************************************
 *		VkKeyScanW (USER32.@)
 */
SHORT WINAPI VkKeyScanW_k(WCHAR cChar)
{
    return VkKeyScanExW_k(cChar, GetKeyboardLayout_k(0));
}

/**********************************************************************
 *		VkKeyScanExA (USER32.@)
 */
WORD WINAPI VkKeyScanExA_k(CHAR cChar, HKL dwhkl)
{
    WCHAR wChar;
    ...
    MultiByteToWideChar_k(CP_ACP, 0, &cChar, 1, &wChar, 1);
    return VkKeyScanExW_k(wChar, dwhkl);
}

/******************************************************************************
 *		VkKeyScanExW (USER32.@)
 */
WORD WINAPI VkKeyScanExW_k(WCHAR cChar, HKL dwhkl)
{
 ...
 *		OemKeyScan (USER32.@)
 */
DWORD WINAPI OemKeyScan_k(WORD wOemChar)
{
    return wOemChar;
}
 ...
 */
INT WINAPI GetKeyboardType_k(INT nTypeFlag)
{
    TRACE_(keyboard)("(%d)\n", nTypeFlag);
    switch(nTypeFlag)
    {
 ...
 */
UINT WINAPI MapVirtualKeyA_k(UINT code, UINT maptype)
{
    return MapVirtualKeyExA_k( code, maptype, GetKeyboardLayout_k(0) );
}

/******************************************************************************
 *		MapVirtualKeyW (USER32.@)
 */
UINT WINAPI MapVirtualKeyW_k(UINT code, UINT maptype)
{
    return MapVirtualKeyExW_k(code, maptype, GetKeyboardLayout_k(0));
}

/******************************************************************************
 *		MapVirtualKeyExA (USER32.@)
 */
UINT WINAPI MapVirtualKeyExA_k(UINT code, UINT maptype, HKL hkl)
{
    UINT ret;

    ret = MapVirtualKeyExW_k( code, maptype, hkl );
    if (maptype == MAPVK_VK_TO_CHAR)
    {
        BYTE ch = 0;
        WCHAR wch = ret;

        WideCharToMultiByte_k( CP_ACP, 0, &wch, 1, (LPSTR)&ch, 1, NULL, NULL );
        ret = ch;
    }
    return ret;
}
 ...
 */
UINT WINAPI MapVirtualKeyExW_k(UINT code, UINT maptype, HKL hkl)
{
 ...
 */
UINT WINAPI GetKBCodePage_k(void)
{
    return GetOEMCP_k();
}
 ...
 */
HKL WINAPI GetKeyboardLayout_k(DWORD thread_id)
{
 ...
 */
BOOL WINAPI GetKeyboardLayoutNameA_k(LPSTR pszKLID)
{
    WCHAR buf[KL_NAMELENGTH];

    if (GetKeyboardLayoutNameW_k(buf))
        return WideCharToMultiByte_k( CP_ACP, 0, buf, -1, pszKLID, KL_NAMELENGTH, NULL, NULL ) != 0;
    return FALSE;
}
 ...
 */
BOOL WINAPI GetKeyboardLayoutNameW_k(LPWSTR pwszKLID)
{
 ...
 */
INT WINAPI GetKeyNameTextA_k(LONG lParam, LPSTR lpBuffer, INT nSize)
{
    WCHAR buf[256];
    INT ret;

    if (!nSize || !GetKeyNameTextW_k(lParam, buf, 256))
    {
        lpBuffer[0] = 0;
        return 0;
    }
    ret = WideCharToMultiByte_k(CP_ACP, 0, buf, -1, lpBuffer, nSize, NULL, NULL);
    if (!ret && nSize)
    {
        ret = nSize - 1;
        lpBuffer[ret] = 0;
 ...
 */
INT WINAPI GetKeyNameTextW_k(LONG lParam, LPWSTR lpBuffer, INT nSize)
{
 ...
 *		ToUnicode (USER32.@)
 */
INT WINAPI ToUnicode_k(UINT virtKey, UINT scanCode, const BYTE *lpKeyState,
		     LPWSTR lpwStr, int size, UINT flags)
{
    return ToUnicodeEx_k(virtKey, scanCode, lpKeyState, lpwStr, size, flags, GetKeyboardLayout_k(0));
}

/****************************************************************************
 *		ToUnicodeEx (USER32.@)
 */
INT WINAPI ToUnicodeEx_k(UINT virtKey, UINT scanCode, const BYTE *lpKeyState,
 ...
 *		ToAscii (USER32.@)
 */
INT WINAPI ToAscii_k( UINT virtKey, UINT scanCode, const BYTE *lpKeyState,
                    LPWORD lpChar, UINT flags )
{
    return ToAsciiEx_k(virtKey, scanCode, lpKeyState, lpChar, flags, GetKeyboardLayout_k(0));
}

/****************************************************************************
 *		ToAsciiEx (USER32.@)
 */
INT WINAPI ToAsciiEx_k( UINT virtKey, UINT scanCode, const BYTE *lpKeyState,
                      LPWORD lpChar, UINT flags, HKL dwhkl )
{
    WCHAR uni_chars[2];
    INT ret, n_ret;

    ret = ToUnicodeEx_k(virtKey, scanCode, lpKeyState, uni_chars, 2, flags, dwhkl);
    if (ret < 0) n_ret = 1; /* FIXME: make ToUnicode return 2 for dead chars */
    else n_ret = ret;
    WideCharToMultiByte_k(CP_ACP, 0, uni_chars, n_ret, (LPSTR)lpChar, 2, NULL, NULL);
    return ret;
}
 ...
 */
HKL WINAPI ActivateKeyboardLayout_k(HKL hLayout, UINT flags)
{
    TRACE_(keyboard)("(%p, %d)\n", hLayout, flags);
 ...
 *		BlockInput (USER32.@)
 */
BOOL WINAPI BlockInput_k(BOOL fBlockIt)
{
 ...
 */
UINT WINAPI GetKeyboardLayoutList_k(INT nBuff, HKL *layouts)
{
    FIXME("GetKeyboardLayoutList not implemented");
    return 0;
}
 ...
 */
BOOL WINAPI RegisterHotKey_k(HWND hwnd,INT id,UINT modifiers,UINT vk)
{
    FIXME("RegisterHotKey not implemented");
    return FALSE;
}
 ...
 */
BOOL WINAPI UnregisterHotKey_k(HWND hwnd,INT id)
{
    FIXME("UnregisterHotKey not implemented");
    return FALSE;
}
 ...
 */
HKL WINAPI LoadKeyboardLayoutW_k(LPCWSTR pwszKLID, UINT Flags)
{
 ...
 */
HKL WINAPI LoadKeyboardLayoutA_k(LPCSTR pwszKLID, UINT Flags)
{
    HKL ret;
    UNICODE_STRING pwszKLIDW;

    if (pwszKLID) RtlCreateUnicodeStringFromAsciiz_k(&pwszKLIDW, pwszKLID);
    else pwszKLIDW.Buffer = NULL;

    ret = LoadKeyboardLayoutW_k(pwszKLIDW.Buffer, Flags);
    RtlFreeUnicodeString_k(&pwszKLIDW);
    return ret;
}
 ...
 */
BOOL WINAPI UnloadKeyboardLayout_k(HKL hkl)
{
    TRACE_(keyboard)("(%p)\n", hkl);
...
static void check_mouse_leave(HWND hwnd, int hittest)
{
    if (tracking_info.tme.hwndTrack != hwnd)
    {
        if (tracking_info.tme.dwFlags & TME_NONCLIENT)
            PostMessageW_k(tracking_info.tme.hwndTrack, WM_NCMOUSELEAVE, 0, 0);
        else
            PostMessageW_k(tracking_info.tme.hwndTrack, WM_MOUSELEAVE, 0, 0);
                                         ...
                                         DWORD dwTime)
{
    ...
    SystemParametersInfoW_k(SPI_GETMOUSEHOVERWIDTH, 0, &hoverwidth, 0);
    SystemParametersInfoW_k(SPI_GETMOUSEHOVERHEIGHT, 0, &hoverheight, 0);

    //TRACE("tracked pos %s, current pos %s, hover width %d, hover height %d\n", wine_dbgstr_point(&tracking_info.pos), wine_dbgstr_point(&pos), hoverwidth, hoverheight);
    ...
    if (tracking_info.tme.dwFlags & TME_HOVER)
    {
        ...
        else
        {
            if (hittest == HTCLIENT)
            {
                ScreenToClient_k(hwnd, &pos);
                //TRACE("client cursor pos %s\n", wine_dbgstr_point(&pos));

                PostMessageW_k(tracking_info.tme.hwndTrack, WM_MOUSEHOVER,
                             get_key_state(), MAKELPARAM( pos.x, pos.y ));
            }
            else
            {
                if (tracking_info.tme.dwFlags & TME_NONCLIENT)
                    PostMessageW_k(tracking_info.tme.hwndTrack, WM_NCMOUSEHOVER,
    ...
    {
        KillSystemTimer_k(tracking_info.tme.hwndTrack, timer);
        timer = 0;
...
BOOL WINAPI TrackMouseEvent_k (TRACKMOUSEEVENT *ptme)
{
    HWND hwnd;
    POINT pos;
    DWORD hover_time;
    ...
    if (hover_time == HOVER_DEFAULT || hover_time == 0)
        SystemParametersInfoW_k(SPI_GETMOUSEHOVERTIME, 0, &hover_time, 0);
    ...
    if (ptme->dwFlags & TME_CANCEL)
    {
        ...
        if (ptme->hwndTrack == hwnd)
        {
            ...
            timer = SetSystemTimer_k(tracking_info.tme.hwndTrack, (UINT_PTR)&tracking_info.tme, hover_time, TrackMouseEventProc);
        }
    }
 ...
 *     Failure: -1
 */
int WINAPI GetMouseMovePointsEx_k(UINT size, LPMOUSEMOVEPOINT ptin, LPMOUSEMOVEPOINT ptout, int count, DWORD res) {
    ...
    SetLastError_k(ERROR_POINT_NOT_FOUND);
    return -1;
}
