...
static BYTE * ICO_GetIconDirectory( LPBYTE peimage, LPicoICONDIR* lplpiID, ULONG *uSize )
{
	...
	if( (lpID = HeapAlloc_k(GetProcessHeap_k(),0, *uSize) ))
	{
	  /* copy the header */
	...
	UINT flags)
{
	...
	TRACE("%s, %d, %d %p 0x%08x\n", debugstr_w(lpszExeFileName), nIconIndex, nIcons, pIconId, flags);
        ...
        dwSearchReturn = SearchPathW_k(NULL, lpszExeFileName, NULL, sizeof(szExePath) / sizeof(szExePath[0]), szExePath, NULL);
	...
	hFile = CreateFileW_k(szExePath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, 0);
	if (hFile == INVALID_HANDLE_VALUE) return ret;
	fsizel = GetFileSize_k(hFile,&fsizeh);

	/* Map the file */
	fmapping = CreateFileMappingW_k(hFile, NULL, PAGE_READONLY | SEC_COMMIT, 0, 0, NULL);
	CloseHandle_k(hFile);
	if (!fmapping)
	{
	...
	if (!(peimage = MapViewOfFile_k(fmapping, FILE_MAP_READ, 0, 0, 0)))
	{
          WARN("MapViewOfFile error %d\n", GetLastError_k() );
	  CloseHandle_k(fmapping);
	  return 0xFFFFFFFF;
	}
	CloseHandle_k(fmapping);
	  ...
	  if ((pIconStorage && pIconDir) || lpiID)	  /* load resources and create icons */
	  {
	    if (nIcons == 0)
	    {
	      ret = iconDirCount;
	        ...
	        HeapFree_k(GetProcessHeap_k(), 0, pCIDir);
	    }
	    else if (nIconIndex < iconDirCount)
	    {
	      UINT16   i, icon;
	      if (nIcons > (UINT)(iconDirCount - nIconIndex))
	      ...
	      for (i = 0; i < nIcons; i++)
	      {
	        ...
	        pIconId[i] = LookupIconIdFromDirectoryEx_k(pCIDir, TRUE, cx1, cy1, flags);
                if (cx2 && cy2) pIconId[++i] = LookupIconIdFromDirectoryEx_k(pCIDir, TRUE,  cx2, cy2, flags);
	      }
              if (lpiID)	/* *.ico file, deallocate heap pointer*/
	        HeapFree_k(GetProcessHeap_k(), 0, pCIDir);
	      ...
	      for (icon = 0; icon < nIcons; icon++)
	      {
	        ...
	        if (pCIDir)
                {
	          RetPtr[icon] = CreateIconFromResourceEx_k(pCIDir, uSize, TRUE, 0x00030000,
                                                                 cx1, cy1, flags);
                  if (cx2 && cy2)
                      RetPtr[++icon] = CreateIconFromResourceEx_k(pCIDir, uSize, TRUE, 0x00030000,
                                                                       cx2, cy2, flags);
...
/* exe/dll */
	else if( sig == IMAGE_NT_SIGNATURE )
	{
        ...
        rootresdir = RtlImageDirectoryEntryToData_k((HMODULE)peimage, FALSE, IMAGE_DIRECTORY_ENTRY_RESOURCE, &size);
        if (!rootresdir)
        {
	  ...
	  for (i=0; i < nIcons; i++,xresent++)
	  {
        ...
        igdata = RtlImageRvaToVa_k(RtlImageNtHeader_k((HMODULE)peimage), (HMODULE)peimage, igdataent->OffsetToData, NULL);
        if (!igdata)
	    {
	      FIXME("no matching real address for icongroup!\n");
	      goto end;	/* failure */
	    }
	    pIconId[i] = LookupIconIdFromDirectoryEx_k(igdata, TRUE, cx1, cy1, flags);
            if (cx2 && cy2) pIconId[++i] = LookupIconIdFromDirectoryEx_k(igdata, TRUE, cx2, cy2, flags);
	  }
	  ...
	  for (i=0; i<nIcons; i++)
	  {
        ...
        idata = RtlImageRvaToVa_k(RtlImageNtHeader_k((HMODULE)peimage), (HMODULE)peimage, idataent->OffsetToData, NULL);
        if (!idata)
	    {
	    ...
	    RetPtr[i] = CreateIconFromResourceEx_k(idata, idataent->Size, TRUE, 0x00030000, cx1, cy1, flags);
            if (cx2 && cy2)
                RetPtr[++i] = CreateIconFromResourceEx_k(idata, idataent->Size, TRUE, 0x00030000, cx2, cy2, flags);
	  }
	  ret = i;	/* return number of retrieved icons */
	}			/* if(sig == IMAGE_NT_SIGNATURE) */

end:
	UnmapViewOfFile_k(peimage);	/* success */
	return ret;
}
...
UINT WINAPI PrivateExtractIconsW_k (
	LPCWSTR lpwstrFile,
	int nIndex,
	int sizeX,
	int sizeY,
...
UINT WINAPI PrivateExtractIconsA_k (
	LPCSTR lpstrFile,
	int nIndex,
	int sizeX,
	int sizeY,
...
{
    UINT ret;
    INT len = MultiByteToWideChar_k(CP_ACP, 0, lpstrFile, -1, NULL, 0);
    LPWSTR lpwstrFile = HeapAlloc_k(GetProcessHeap_k(), 0, len * sizeof(WCHAR));

    MultiByteToWideChar_k(CP_ACP, 0, lpstrFile, -1, lpwstrFile, len);
    ret = PrivateExtractIconsW_k(lpwstrFile, nIndex, sizeX, sizeY, phicon, piconid, nIcons, flags);

    HeapFree_k(GetProcessHeap_k(), 0, lpwstrFile);
    return ret;
}
 ...
 */
UINT WINAPI PrivateExtractIconExW_k (
	LPCWSTR lpwstrFile,
	int nIndex,
	HICON * phIconLarge,
	HICON * phIconSmall,
	UINT nIcons )
{
	...
	{
	  HICON hIcon[2];
	  cxicon = GetSystemMetrics_k(SM_CXICON);
	  cyicon = GetSystemMetrics_k(SM_CYICON);
	  cxsmicon = GetSystemMetrics_k(SM_CXSMICON);
	  cysmicon = GetSystemMetrics_k(SM_CYSMICON);
 ...
 */
UINT WINAPI PrivateExtractIconExA_k (
	LPCSTR lpstrFile,
	int nIndex,
	HICON * phIconLarge,
	HICON * phIconSmall,
	UINT nIcons )
{
	...
	MultiByteToWideChar_k(CP_ACP, 0, lpstrFile, -1, lpwstrFile, len);
	ret = PrivateExtractIconExW_k(lpwstrFile, nIndex, phIconLarge, phIconSmall, nIcons);
	HeapFree_k(GetProcessHeap_k(), 0, lpwstrFile);
	return ret;
}
