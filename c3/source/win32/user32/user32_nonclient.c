...
#define HAS_MENU(hwnd,style)  ((((style) & (WS_CHILD | WS_POPUP)) != WS_CHILD) && GetMenu_k(hwnd))
...
static void NC_AdjustRectOuter (LPRECT rect, DWORD style, BOOL menu, DWORD exStyle)
{
    ...
    if ((style & WS_THICKFRAME) && !(exStyle & WS_EX_DLGMODALFRAME))
        adjust +=  ( GetSystemMetrics_k(SM_CXFRAME)
                   - GetSystemMetrics_k(SM_CXDLGFRAME)); /* The resize border */
    if ((style & (WS_BORDER|WS_DLGFRAME)) ||
        (exStyle & WS_EX_DLGMODALFRAME))
        adjust++; /* The other border */

    InflateRect_k (rect, adjust, adjust);

    if ((style & WS_CAPTION) == WS_CAPTION)
    {
        if (exStyle & WS_EX_TOOLWINDOW)
            rect->top -= GetSystemMetrics_k(SM_CYSMCAPTION);
        else
            rect->top -= GetSystemMetrics_k(SM_CYCAPTION);
    }
    if (menu) rect->top -= GetSystemMetrics_k(SM_CYMENU);
}
...
static void NC_AdjustRectInner (LPRECT rect, DWORD style, DWORD exStyle)
{
    if (exStyle & WS_EX_CLIENTEDGE)
        InflateRect_k(rect, GetSystemMetrics_k(SM_CXEDGE), GetSystemMetrics_k(SM_CYEDGE));

    if (style & WS_VSCROLL)
    {
        if((exStyle & WS_EX_LEFTSCROLLBAR) != 0)
            rect->left  -= GetSystemMetrics_k(SM_CXVSCROLL);
        else
            rect->right += GetSystemMetrics_k(SM_CXVSCROLL);
    }
    if (style & WS_HSCROLL) rect->bottom += GetSystemMetrics_k(SM_CYHSCROLL);
}
...
static HICON NC_IconForWindow( HWND hwnd )
{
    ...
    if (!hIcon) hIcon = (HICON) GetClassLongPtrW_k( hwnd, GCLP_HICONSM );
    if (!hIcon) hIcon = (HICON) GetClassLongPtrW_k( hwnd, GCLP_HICON );

    /* If there is no hIcon specified and this is a modal dialog,
     * get the default one.
     */
    if (!hIcon && (GetWindowLongW_k( hwnd, GWL_STYLE ) & DS_MODALFRAME))
        hIcon = LoadImageW_k(0, (LPCWSTR)IDI_WINLOGO, IMAGE_ICON, 0, 0, LR_DEFAULTCOLOR);
    return hIcon;
}
                               ...
                               BOOL active, BOOL gradient)
{
    if (gradient)
    {
        ...
        DWORD colLeft = 
            GetSysColor_k(active ? COLOR_ACTIVECAPTION : COLOR_INACTIVECAPTION);
        DWORD colRight = 
            GetSysColor_k(active ? COLOR_GRADIENTACTIVECAPTION 
                                : COLOR_GRADIENTINACTIVECAPTION);
        int buttonsAreaSize = GetSystemMetrics_k(SM_CYCAPTION) - 1;
        ...
        if ((dwStyle & WS_SYSMENU)
            ...
            buttonsAreaSize += 2 * (GetSystemMetrics_k(SM_CXSIZE) + 1);

        /* area behind icon; solid filled with left color */
        vertices[0].x = rect->left;
        vertices[0].y = rect->top;
        if (dwStyle & WS_SYSMENU)
            vertices[1].x = min (rect->left + GetSystemMetrics_k(SM_CXSMICON), rect->right);
        else
        ...
        GdiGradientFill_k(hdc, vertices, 4, mesh, 3, GRADIENT_FILL_RECT_H);
    }
    else
        FillRect_k(hdc, rect, GetSysColorBrush_k(active ?
...
BOOL WINAPI DrawCaption_k(HWND hwnd, HDC hdc, const RECT *lpRect, UINT uFlags)
{
    return DrawCaptionTempW_k(hwnd, hdc, lpRect, 0, 0, NULL, uFlags & 0x103F);
}
 ...
 */
BOOL WINAPI DrawCaptionTempA_k(HWND hwnd, HDC hdc, const RECT *rect, HFONT hFont,
                              HICON hIcon, LPCSTR str, UINT uFlags)
{
    ...
    if (!(uFlags & DC_TEXT) || !str)
        return DrawCaptionTempW_k( hwnd, hdc, rect, hFont, hIcon, NULL, uFlags );

    len = MultiByteToWideChar_k( CP_ACP, 0, str, -1, NULL, 0 );
    if ((strW = HeapAlloc_k( GetProcessHeap_k(), 0, len * sizeof(WCHAR) )))
    {
        MultiByteToWideChar_k( CP_ACP, 0, str, -1, strW, len );
        ret = DrawCaptionTempW_k(hwnd, hdc, rect, hFont, hIcon, strW, uFlags);
 ...
 */
BOOL WINAPI DrawCaptionTempW_k(HWND hwnd, HDC hdc, const RECT *rect, HFONT hFont,
                              HICON hIcon, LPCWSTR str, UINT uFlags)
{
    ...
    if (uFlags & DC_INBUTTON) {
        FillRect_k(hdc, &rc, GetSysColorBrush_k(COLOR_3DFACE));

        if (uFlags & DC_ACTIVE) {
            HBRUSH hbr = SelectObject_k(hdc, getSYSCOLOR_55AABrush());
            PatBlt_k(hdc, rc.left, rc.top,
                      rc.right-rc.left, rc.bottom-rc.top, 0xFA0089);
            SelectObject_k(hdc, hbr);
        }
    }
    else {
    ...
    if ((uFlags & DC_ICON) && !(uFlags & DC_SMALLCAP)) {
        ...
        pt.x = rc.left + 2;
        pt.y = (rc.bottom + rc.top - GetSystemMetrics_k(SM_CYSMICON)) / 2;

        if (!hIcon) hIcon = NC_IconForWindow(hwnd);
        DrawIconEx_k(hdc, pt.x, pt.y, hIcon, GetSystemMetrics_k(SM_CXSMICON),
                    GetSystemMetrics_k(SM_CYSMICON), 0, 0, DI_NORMAL);
    ...
    if (uFlags & DC_TEXT) {
        ...
        if (uFlags & DC_INBUTTON)
            SetTextColor_k(hdc, GetSysColor_k(COLOR_BTNTEXT));
        else if (uFlags & DC_ACTIVE)
            SetTextColor_k(hdc, GetSysColor_k(COLOR_CAPTIONTEXT));
        else
            SetTextColor_k(hdc, GetSysColor_k(COLOR_INACTIVECAPTIONTEXT));

        SetBkMode_k(hdc, TRANSPARENT);

        if (hFont)
            hOldFont = SelectObject_k(hdc, hFont);
        else {
            NONCLIENTMETRICSW nclm;
            HFONT hNewFont;
            nclm.cbSize = sizeof(NONCLIENTMETRICSW);
            SystemParametersInfoW_k(SPI_GETNONCLIENTMETRICS, 0, &nclm, 0);
            hNewFont = CreateFontIndirectW_k((uFlags & DC_SMALLCAP) ? &nclm.lfSmCaptionFont : &nclm.lfCaptionFont);
            hOldFont = SelectObject_k(hdc, hNewFont);
        }

        if (str)
            DrawTextW_k(hdc, str, -1, &rc, DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX | DT_LEFT);
        else {
            WCHAR szText[128];
            INT nLen;
            nLen = GetWindowTextW_k(hwnd, szText, 128);
            DrawTextW_k(hdc, szText, nLen, &rc, DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX | DT_LEFT);
        }

        if (hFont)
            SelectObject_k(hdc, hOldFont);
        else
            DeleteObject_k(SelectObject_k(hdc, hOldFont));
    }
 ...
 */
BOOL WINAPI AdjustWindowRect_k( LPRECT rect, DWORD style, BOOL menu )
{
    return AdjustWindowRectEx_k( rect, style, menu, 0 );
}
 ...
 */
BOOL WINAPI AdjustWindowRectEx_k( LPRECT rect, DWORD style, BOOL menu, DWORD exStyle )
{
...
LRESULT NC_HandleNCCalcSize( HWND hwnd, WPARAM wparam, RECT *winRect )
{
    ...
    LRESULT result = 0;
    LONG cls_style = GetClassLongW_k(hwnd, GCL_STYLE);
    ...
    if (!(style & WS_ICONIC))
    {
        ...
        if (((style & (WS_CHILD | WS_POPUP)) != WS_CHILD) && GetMenu_k(hwnd))
        {
        ...
        if( exStyle & WS_EX_CLIENTEDGE)
            if( winRect->right - winRect->left > 2 * GetSystemMetrics_k(SM_CXEDGE) &&
                   winRect->bottom - winRect->top > 2 * GetSystemMetrics_k(SM_CYEDGE))
                InflateRect_k( winRect, - GetSystemMetrics_k(SM_CXEDGE),
                        - GetSystemMetrics_k(SM_CYEDGE));

        if (style & WS_VSCROLL)
            if (winRect->right - winRect->left >= GetSystemMetrics_k(SM_CXVSCROLL))
            {
                /* rectangle is in screen coords when wparam is false */
                if (!wparam && (exStyle & WS_EX_LAYOUTRTL)) exStyle ^= WS_EX_LEFTSCROLLBAR;

                if((exStyle & WS_EX_LEFTSCROLLBAR) != 0)
                    winRect->left  += GetSystemMetrics_k(SM_CXVSCROLL);
                else
                    winRect->right -= GetSystemMetrics_k(SM_CXVSCROLL);
            }

        if (style & WS_HSCROLL)
            if( winRect->bottom - winRect->top > GetSystemMetrics_k(SM_CYHSCROLL))
                    winRect->bottom -= GetSystemMetrics_k(SM_CYHSCROLL);
                              ...
                              DWORD style, DWORD ex_style )
{
    ...
    {
        InflateRect_k( rect, -GetSystemMetrics_k(SM_CXFRAME), -GetSystemMetrics_k(SM_CYFRAME) );
    }
    else if (HAS_DLGFRAME( style, ex_style ))
    {
        InflateRect_k( rect, -GetSystemMetrics_k(SM_CXDLGFRAME), -GetSystemMetrics_k(SM_CYDLGFRAME));
    }
    else if (HAS_THINFRAME( style ))
    {
        InflateRect_k( rect, -GetSystemMetrics_k(SM_CXBORDER), -GetSystemMetrics_k(SM_CYBORDER) );
    }
...
LRESULT NC_HandleNCHitTest( HWND hwnd, POINT pt )
{
    ...
    if (!PtInRect_k( &rect, pt )) return HTNOWHERE;

    style = GetWindowLongW_k( hwnd, GWL_STYLE );
    ex_style = GetWindowLongW_k( hwnd, GWL_EXSTYLE );
    if (style & WS_MINIMIZE) return HTCAPTION;

    if (PtInRect_k( &rcClient, pt )) return HTCLIENT;
    ...
    if (HAS_THICKFRAME( style, ex_style ))
    {
        ...
        if (!PtInRect_k( &rect, pt ))
        {
            ...
            if (pt.y < rect.top)
            {
                if (pt.x < rect.left+GetSystemMetrics_k(SM_CXSIZE)) return HTTOPLEFT;
                if (pt.x >= rect.right-GetSystemMetrics_k(SM_CXSIZE)) return HTTOPRIGHT;
                return HTTOP;
            }
            /* Check bottom sizing border */
            if (pt.y >= rect.bottom)
            {
                if (pt.x < rect.left+GetSystemMetrics_k(SM_CXSIZE)) return HTBOTTOMLEFT;
                if (pt.x >= rect.right-GetSystemMetrics_k(SM_CXSIZE)) return HTBOTTOMRIGHT;
                return HTBOTTOM;
            }
            /* Check left sizing border */
            if (pt.x < rect.left)
            {
                if (pt.y < rect.top+GetSystemMetrics_k(SM_CYSIZE)) return HTTOPLEFT;
                if (pt.y >= rect.bottom-GetSystemMetrics_k(SM_CYSIZE)) return HTBOTTOMLEFT;
                return HTLEFT;
            }
            /* Check right sizing border */
            if (pt.x >= rect.right)
            {
                if (pt.y < rect.top+GetSystemMetrics_k(SM_CYSIZE)) return HTTOPRIGHT;
                if (pt.y >= rect.bottom-GetSystemMetrics_k(SM_CYSIZE)) return HTBOTTOMRIGHT;
                return HTRIGHT;
            }
        }
    }
    else  /* No thick frame */
    {
        if (HAS_DLGFRAME( style, ex_style ))
            ...
            InflateRect_k(&rect, -GetSystemMetrics_k(SM_CXBORDER), -GetSystemMetrics_k(SM_CYBORDER));
        if (!PtInRect_k( &rect, pt )) return HTBORDER;
    }
    ...
    if ((style & WS_CAPTION) == WS_CAPTION)
    {
        if (ex_style & WS_EX_TOOLWINDOW)
            rect.top += GetSystemMetrics_k(SM_CYSMCAPTION) - 1;
        else
            rect.top += GetSystemMetrics_k(SM_CYCAPTION) - 1;
        if (!PtInRect_k( &rect, pt ))
        {
            ...
            if (ex_style & WS_EX_LAYOUTRTL)
            {
                ...
                {
                    rect.right -= GetSystemMetrics_k(SM_CYCAPTION) - 1;
                ...
                if (style & WS_SYSMENU)
                {
                    rect.left += GetSystemMetrics_k(SM_CYCAPTION);
                ...
                {
                    rect.left += GetSystemMetrics_k(SM_CXSIZE);
            ...
            else
            {
                ...
                if ((style & WS_SYSMENU) && !(ex_style & WS_EX_TOOLWINDOW) && NC_IconForWindow(hwnd))
                {
                    rect.left += GetSystemMetrics_k(SM_CYCAPTION) - 1;
                ...
                if (style & WS_SYSMENU)
                {
                    rect.right -= GetSystemMetrics_k(SM_CYCAPTION);
    ...
    if (style & WS_VSCROLL)
    {
        if((ex_style & WS_EX_LEFTSCROLLBAR) != 0)
            rcClient.left -= GetSystemMetrics_k(SM_CXVSCROLL);
        else
            rcClient.right += GetSystemMetrics_k(SM_CXVSCROLL);
        if (PtInRect_k( &rcClient, pt )) return HTVSCROLL;
    }
    ...
    if (style & WS_HSCROLL)
    {
        rcClient.bottom += GetSystemMetrics_k(SM_CYHSCROLL);
        if (PtInRect_k( &rcClient, pt ))
        {
            /* Check size box */
            if ((style & WS_VSCROLL) &&
                ((((ex_style & WS_EX_LEFTSCROLLBAR) != 0) && (pt.x <= rcClient.left + GetSystemMetrics_k(SM_CXVSCROLL))) ||
                (((ex_style & WS_EX_LEFTSCROLLBAR) == 0) && (pt.x >= rcClient.right - GetSystemMetrics_k(SM_CXVSCROLL)))))
                return HTSIZE;
            return HTHSCROLL;
        }
    }
...
BOOL NC_DrawSysButton (HWND hwnd, HDC hdc, BOOL down)
{
    ...
    if (hIcon)
    {
        ...
        POINT pt;
        DWORD style = GetWindowLongW_k( hwnd, GWL_STYLE );
        DWORD ex_style = GetWindowLongW_k( hwnd, GWL_EXSTYLE );

        NC_GetInsideRect( hwnd, COORDS_WINDOW, &rect, style, ex_style );
        pt.x = rect.left + 2;
        pt.y = (rect.top + GetSystemMetrics_k(SM_CYCAPTION) - GetSystemMetrics_k(SM_CYSMICON)) / 2;
        DrawIconEx_k(hdc, pt.x, pt.y, hIcon,
...
static void NC_DrawCloseButton (HWND hwnd, HDC hdc, BOOL down, BOOL bGrayed)
{
    ...
    if (ex_style & WS_EX_TOOLWINDOW)
    {
        ...
        INT iCaptionHeight = GetSystemMetrics_k(SM_CYSMCAPTION);
    ...
    {
        rect.left = rect.right - GetSystemMetrics_k(SM_CXSIZE);
        rect.bottom = rect.top + GetSystemMetrics_k(SM_CYSIZE) - 2;
        rect.top += 2;
        rect.right -= 2;
    }
    DrawFrameControl_k( hdc, &rect, DFC_CAPTION,
                      (DFCS_CAPTIONCLOSE |
                       (down ? DFCS_PUSHED : 0) |
...
static void NC_DrawMaxButton(HWND hwnd,HDC hdc,BOOL down, BOOL bGrayed)
{
    ...
    DrawFrameControl_k( hdc, &rect, DFC_CAPTION, flags );
}
...
static void  NC_DrawMinButton(HWND hwnd,HDC hdc,BOOL down, BOOL bGrayed)
{
    ...
    if (style & (WS_MAXIMIZEBOX|WS_MINIMIZEBOX))
        rect.right -= GetSystemMetrics_k(SM_CXSIZE) - 2;
    rect.left = rect.right - GetSystemMetrics_k(SM_CXSIZE);
    rect.bottom = rect.top + GetSystemMetrics_k(SM_CYSIZE) - 2;
    rect.top += 2;
    rect.right -= 2;
...
static void  NC_DrawFrame( HDC  hdc, RECT  *rect, BOOL  active, DWORD style, DWORD exStyle)
{
    ...
    {
        width = GetSystemMetrics_k(SM_CXFRAME) - GetSystemMetrics_k(SM_CXDLGFRAME);
        height = GetSystemMetrics_k(SM_CYFRAME) - GetSystemMetrics_k(SM_CYDLGFRAME);

        SelectObject_k( hdc, GetSysColorBrush_k(active ? COLOR_ACTIVEBORDER :
        ...
        PatBlt_k( hdc, rect->left, rect->bottom - 1,
                  rect->right - rect->left, -height, PATCOPY );
        PatBlt_k( hdc, rect->right - 1, rect->top,
                  -width, rect->bottom - rect->top, PATCOPY );

        InflateRect_k( rect, -width, -height );
    }
    ...
    if ((style & (WS_BORDER|WS_DLGFRAME)) ||
        (exStyle & WS_EX_DLGMODALFRAME))
    {
        width = GetSystemMetrics_k(SM_CXDLGFRAME) - GetSystemMetrics_k(SM_CXEDGE);
        height = GetSystemMetrics_k(SM_CYDLGFRAME) - GetSystemMetrics_k(SM_CYEDGE);
        /* This should give a value of 1 that should also work for a border */

        SelectObject_k( hdc, GetSysColorBrush_k(
                             ...
                             DWORD  exStyle, BOOL active )
{
    ...
    hPrevPen = SelectObject_k( hdc, SYSCOLOR_GetPen(
                     ((exStyle & (WS_EX_STATICEDGE|WS_EX_CLIENTEDGE|
                                 WS_EX_DLGMODALFRAME)) == WS_EX_STATICEDGE) ?
                      COLOR_WINDOWFRAME : COLOR_3DFACE) );
    MoveToEx_k( hdc, r.left, r.bottom - 1, NULL );
    LineTo_k( hdc, r.right, r.bottom - 1 );
    SelectObject_k( hdc, hPrevPen );
    r.bottom--;

    SystemParametersInfoW_k(SPI_GETGRADIENTCAPTIONS, 0, &gradient, 0);
    NC_DrawCaptionBar (hdc, &r, style, active, gradient);

    if ((style & WS_SYSMENU) && !(exStyle & WS_EX_TOOLWINDOW)) {
        if (NC_DrawSysButton (hwnd, hdc, FALSE))
            r.left += GetSystemMetrics_k(SM_CXSMICON) + 2;
    }
    ...
    if (style & WS_SYSMENU)
    {
        ...
        /* Go get the sysmenu */
        hSysMenu = GetSystemMenu_k(hwnd, FALSE);
        state = GetMenuState_k(hSysMenu, SC_CLOSE, MF_BYCOMMAND);
        ...
        r.right -= GetSystemMetrics_k(SM_CYCAPTION) - 1;
        ...
        if ((style & WS_MAXIMIZEBOX) || (style & WS_MINIMIZEBOX))
        {
            ...
            r.right -= GetSystemMetrics_k(SM_CXSIZE) + 1;
    ...
    if (GetWindowTextW_k( hwnd, buffer, sizeof(buffer)/sizeof(WCHAR) ))
    {
        NONCLIENTMETRICSW nclm;
        ...
        SystemParametersInfoW_k(SPI_GETNONCLIENTMETRICS, 0, &nclm, 0);
        if (exStyle & WS_EX_TOOLWINDOW)
            hFont = CreateFontIndirectW_k(&nclm.lfSmCaptionFont);
        else
            hFont = CreateFontIndirectW_k(&nclm.lfCaptionFont);
        hOldFont = SelectObject_k(hdc, hFont);
        if (active) SetTextColor_k( hdc, GetSysColor_k( COLOR_CAPTIONTEXT ) );
        else SetTextColor_k( hdc, GetSysColor_k( COLOR_INACTIVECAPTIONTEXT ) );
        SetBkMode_k( hdc, TRANSPARENT );
        r.left += 2;
        DrawTextW_k( hdc, buffer, -1, &r,
...
static void  NC_DoNCPaint( HWND  hwnd, HRGN  clip, BOOL  suppress_menupaint )
{
    ...
    hrgn = CreateRectRgnIndirect_k( &rectClient );

    if (clip > (HRGN)1)
    {
        CombineRgn_k( hrgn, clip, hrgn, RGN_DIFF );
        hdc = GetDCEx_k( hwnd, hrgn, DCX_USESTYLE | DCX_WINDOW | DCX_INTERSECTRGN );
    }
    else
    {
        hdc = GetDCEx_k( hwnd, hrgn, DCX_USESTYLE | DCX_WINDOW | DCX_EXCLUDERGN );
    }

    if (!hdc) return;

    WIN_GetRectangles( hwnd, COORDS_WINDOW, &rect, NULL );
    GetClipBox_k( hdc, &rectClip );

    SelectObject_k( hdc, SYSCOLOR_GetPen(COLOR_WINDOWFRAME) );

    if (HAS_STATICOUTERFRAME(dwStyle, dwExStyle)) {
        DrawEdge_k(hdc, &rect, BDR_SUNKENOUTER, BF_RECT | BF_ADJUST);
    }
    else if (HAS_BIGFRAME( dwStyle, dwExStyle)) {
        DrawEdge_k(hdc, &rect, EDGE_RAISED, BF_RECT | BF_ADJUST);
    }
    ...
    if ((dwStyle & WS_CAPTION) == WS_CAPTION)
    {
        ...
        if (dwExStyle & WS_EX_TOOLWINDOW) {
            r.bottom = rect.top + GetSystemMetrics_k(SM_CYSMCAPTION);
            rect.top += GetSystemMetrics_k(SM_CYSMCAPTION);
        }
        else {
            r.bottom = rect.top + GetSystemMetrics_k(SM_CYCAPTION);
            rect.top += GetSystemMetrics_k(SM_CYCAPTION);
        }
        if( IntersectRect_k( &rfuzz, &r, &rectClip ) )
    ...
    {
	RECT r = rect;
	r.bottom = rect.top + GetSystemMetrics_k(SM_CYMENU);
	...
	DrawEdge_k(hdc, &rect, EDGE_SUNKEN, BF_RECT | BF_ADJUST);
    ...
    if ((dwStyle & WS_VSCROLL) && (dwStyle & WS_HSCROLL))
    {
        ...
        if((dwExStyle & WS_EX_LEFTSCROLLBAR) != 0)
            r.right = r.left + GetSystemMetrics_k(SM_CXVSCROLL) + 1;
        else
            r.left = r.right - GetSystemMetrics_k(SM_CXVSCROLL) + 1;
        r.top  = r.bottom - GetSystemMetrics_k(SM_CYHSCROLL) + 1;
        FillRect_k( hdc, &r,  GetSysColorBrush_k(COLOR_SCROLLBAR) );
    }
...
{
    DWORD dwStyle = GetWindowLongW_k( hwnd, GWL_STYLE );
 ...
 */
LRESULT NC_HandleSetCursor_k( HWND hwnd, WPARAM wParam, LPARAM lParam )
{
    ...
    case HTCLIENT:
        {
            HCURSOR hCursor = (HCURSOR)GetClassLongPtrW_k(hwnd, GCLP_HCURSOR);
            if(hCursor) {
                SetCursor_k(hCursor);
                return TRUE;
            }
    ...
    case HTLEFT:
    case HTRIGHT:
        return (LRESULT)SetCursor_k( LoadCursorA_k( 0, (LPSTR)IDC_SIZEWE ) );

    case HTTOP:
    case HTBOTTOM:
        return (LRESULT)SetCursor_k( LoadCursorA_k( 0, (LPSTR)IDC_SIZENS ) );

    case HTTOPLEFT:
    case HTBOTTOMRIGHT:
        return (LRESULT)SetCursor_k( LoadCursorA_k( 0, (LPSTR)IDC_SIZENWSE ) );

    case HTTOPRIGHT:
    case HTBOTTOMLEFT:
        return (LRESULT)SetCursor_k( LoadCursorA_k( 0, (LPSTR)IDC_SIZENESW ) );
    }

    /* Default cursor: arrow */
    return (LRESULT)SetCursor_k( LoadCursorA_k( 0, (LPSTR)IDC_ARROW ) );
}
...
{
    if (IsIconic_k(hwnd)) GetWindowRect_k( hwnd, rect );
    else
    {
        DWORD style = GetWindowLongW_k( hwnd, GWL_STYLE );
        DWORD ex_style = GetWindowLongW_k( hwnd, GWL_EXSTYLE );
        ...
        rect->right = rect->left + GetSystemMetrics_k(SM_CYCAPTION) - 1;
        rect->bottom = rect->top + GetSystemMetrics_k(SM_CYCAPTION) - 1;
        MapWindowPoints_k( hwnd, 0, (POINT *)rect, 2 );
    }
}
...
{
    MSG msg;
    HDC hdc = GetWindowDC_k( hwnd );
    BOOL pressed = TRUE;
    UINT state;
    DWORD wndStyle = GetWindowLongW_k( hwnd, GWL_STYLE);
    HMENU hSysMenu = GetSystemMenu_k(hwnd, FALSE);
    ...
    if (wParam == HTMINBUTTON)
    {
        ...
        state = GetMenuState_k(hSysMenu, SC_MINIMIZE, MF_BYCOMMAND);
    ...
    else
    {
        ...
        state = GetMenuState_k(hSysMenu, SC_MAXIMIZE, MF_BYCOMMAND);
    ...
    while(1)
    {
        ...
        if (!GetMessageW_k( &msg, 0, WM_MOUSEFIRST, WM_MOUSELAST )) break;
        if (CallMsgFilterW_k( &msg, MSGF_MAX )) continue;
    ...
    if (wParam == HTMINBUTTON)
        SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_MINIMIZE, MAKELONG(msg.pt.x,msg.pt.y) );
    else
        SendMessageW_k( hwnd, WM_SYSCOMMAND,
                      IsZoomed_k(hwnd) ? SC_RESTORE:SC_MAXIMIZE, MAKELONG(msg.pt.x,msg.pt.y) );
}
...
static void NC_TrackCloseButton (HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_CLOSE, lParam );
}
...
LRESULT NC_HandleNCLButtonDown( HWND hwnd, WPARAM wParam, LPARAM lParam )
{
    ...
    case HTCAPTION:
        {
            ...
            if (FOCUS_MouseActivate( top ) || (GetActiveWindow_k() == top))
                SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_MOVE + HTCAPTION, lParam );
            break;
        }
    ...
    case HTSYSMENU:
         if( style & WS_SYSMENU )
         {
             ...
             SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_MOUSEMENU + HTSYSMENU, lParam );
         }
         break;

    case HTMENU:
        SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_MOUSEMENU, lParam );
        break;

    case HTHSCROLL:
        SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_HSCROLL + HTHSCROLL, lParam );
        break;

    case HTVSCROLL:
        SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_VSCROLL + HTVSCROLL, lParam );
        break;

    case HTMINBUTTON:
    case HTMAXBUTTON:
        NC_TrackMinMaxBox( hwnd, (WORD)wParam );
        break;
    ...
    case HTBOTTOMRIGHT:
         ...
         */
        SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_SIZE + wParam - (HTLEFT-WMSZ_LEFT), lParam);
        break;
...
LRESULT NC_HandleNCRButtonDown( HWND hwnd, WPARAM wParam, LPARAM lParam )
{
    ...
    case HTCAPTION:
    case HTSYSMENU:
        if (!GetSystemMenu_k( hwnd, FALSE )) break;
        ...
        if (hittest == HTCAPTION || hittest == HTSYSMENU)
            SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_MOUSEMENU + HTSYSMENU, msg.lParam );
        break;
    }
    return 0;
}
...
LRESULT NC_HandleNCLButtonDblClk( HWND hwnd, WPARAM wParam, LPARAM lParam )
{
    ...
    if (IsIconic_k(hwnd))
    {
        SendMessageW_k( hwnd, WM_SYSCOMMAND, SC_RESTORE, lParam );
        return 0;
    }
    ...
    case HTCAPTION:
        ...
        if (GetWindowLongW_k( hwnd, GWL_STYLE ) & WS_MAXIMIZEBOX)
            SendMessageW_k( hwnd, WM_SYSCOMMAND,
                          IsZoomed_k(hwnd) ? SC_RESTORE : SC_MAXIMIZE, lParam );
        break;
...
LRESULT NC_HandleSysCommand( HWND hwnd, WPARAM wParam, LPARAM lParam )
{
    ...
    if (!IsWindowEnabled_k( hwnd )) return 0;
    ...
    case SC_MINIMIZE:
        if (hwnd == GetActiveWindow_k())
            ShowOwnedPopups_k(hwnd,FALSE);
        ShowWindow_k( hwnd, SW_MINIMIZE );
        break;

    case SC_MAXIMIZE:
        if (IsIconic_k(hwnd) && hwnd == GetActiveWindow_k())
            ShowOwnedPopups_k(hwnd,TRUE);
        ShowWindow_k( hwnd, SW_MAXIMIZE );
        break;

    case SC_RESTORE:
        if (IsIconic_k(hwnd) && hwnd == GetActiveWindow_k())
            ShowOwnedPopups_k(hwnd,TRUE);
        ShowWindow_k( hwnd, SW_RESTORE );
        break;

    case SC_CLOSE:
        return SendMessageW_k( hwnd, WM_CLOSE, 0, 0 );
    ...
    case SC_TASKLIST:
        WinExec_k( "taskman.exe", SW_SHOWNORMAL );
        break;

    case SC_SCREENSAVE:
        if (wParam == SC_ABOUTWINE)
        {
            HMODULE hmodule = LoadLibraryA_k( "shell32.dll" );
            if (hmodule)
            {
                BOOL (WINAPI *aboutproc)(HWND, LPCSTR, LPCSTR, HICON);

                aboutproc = (void *)GetProcAddress_k( hmodule, "ShellAboutA" );
 ...
 */
BOOL WINAPI GetTitleBarInfo_k(HWND hwnd, PTITLEBARINFO tbi) {
    DWORD dwStyle;
    DWORD dwExStyle;
    ...
    dwStyle = GetWindowLongW_k(hwnd, GWL_STYLE);
    dwExStyle = GetWindowLongW_k(hwnd, GWL_EXSTYLE);
    NC_GetInsideRect(hwnd, COORDS_SCREEN, &tbi->rcTitleBar, dwStyle, dwExStyle);

    tbi->rcTitleBar.bottom = tbi->rcTitleBar.top;
    if(dwExStyle & WS_EX_TOOLWINDOW)
        tbi->rcTitleBar.bottom += GetSystemMetrics_k(SM_CYSMCAPTION);
    else {
        tbi->rcTitleBar.bottom += GetSystemMetrics_k(SM_CYCAPTION);
        tbi->rcTitleBar.left += GetSystemMetrics_k(SM_CXSIZE);
    }

    memset(tbi->rgstate, 0, sizeof(tbi->rgstate));
