...
#define CB_NOTIFY( lphc, code ) \
    (SendMessageW_k((lphc)->owner, WM_COMMAND, \
                  MAKEWPARAM(GetWindowLongPtrW_k((lphc)->self,GWLP_ID), (code)), (LPARAM)(lphc)->self))

#define CB_DISABLED( lphc )   (!IsWindowEnabled_k((lphc)->self))
#define CB_OWNERDRAWN( lphc ) ((lphc)->dwStyle & (CBS_OWNERDRAWFIXED | CBS_OWNERDRAWVARIABLE))
#define CB_HASSTRINGS( lphc ) ((lphc)->dwStyle & CBS_HASSTRINGS)
#define CB_HWND( lphc )       ((lphc)->self)
#define CB_GETTYPE( lphc )    ((lphc)->dwStyle & (CBS_DROPDOWNLIST))

#define ISWIN31 (LOWORD(GetVersion_k()) == 0x0a03)
...
static BOOL COMBO_Init(void)
{
  ...
  if( (hDC = CreateCompatibleDC_k(0)) )
  {
    BOOL	bRet = FALSE;
    if( (hComboBmp = LoadBitmapW_k(0, MAKEINTRESOURCEW(OBM_COMBO))) )
    {
      BITMAP      bm;
      HBITMAP     hPrevB;
      RECT        r;

      GetObjectW_k( hComboBmp, sizeof(bm), &bm );
      CBitHeight = bm.bmHeight;
      CBitWidth  = bm.bmWidth;

      TRACE("combo bitmap [%i,%i]\n", CBitWidth, CBitHeight );

      hPrevB = SelectObject_k( hDC, hComboBmp);
      SetRect_k( &r, 0, 0, CBitWidth, CBitHeight );
      InvertRect_k( hDC, &r );
      SelectObject_k( hDC, hPrevB );
      bRet = TRUE;
    }
    DeleteDC_k( hDC );
    return bRet;
...
static LRESULT COMBO_NCCreate(HWND hwnd, LONG style)
{
    ...
    if (COMBO_Init() && (lphc = HeapAlloc_k(GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(HEADCOMBO))) )
    {
        lphc->self = hwnd;
        SetWindowLongPtrW_k( hwnd, 0, (LONG_PTR)lphc );

       /* some braindead apps do try to use scrollbar/border flags */

	lphc->dwStyle = style & ~(WS_BORDER | WS_HSCROLL | WS_VSCROLL);
        SetWindowLongW_k( hwnd, GWL_STYLE, style & ~(WS_BORDER | WS_HSCROLL | WS_VSCROLL) );

	/*
	 * We also have to remove the client edge style to make sure
	 * we don't end-up with a non client area.
	 */
        SetWindowLongW_k( hwnd, GWL_EXSTYLE,
                        GetWindowLongW_k( hwnd, GWL_EXSTYLE ) & ~WS_EX_CLIENTEDGE );

	if( !(style & (CBS_OWNERDRAWFIXED | CBS_OWNERDRAWVARIABLE)) )
              lphc->dwStyle |= CBS_HASSTRINGS;
	if( !(GetWindowLongW_k( hwnd, GWL_EXSTYLE ) & WS_EX_NOPARENTNOTIFY) )
...
static LRESULT COMBO_NCDestroy( LPHEADCOMBO lphc )
{
   ...
   if( lphc )
   {
       ...
       if( (CB_GETTYPE(lphc) != CBS_SIMPLE) && lphc->hWndLBox )
   	   DestroyWindow_k( lphc->hWndLBox );

       SetWindowLongPtrW_k( lphc->self, 0, 0 );
       HeapFree_k( GetProcessHeap_k(), 0, lphc );
   }
   return 0;
}
  ...
  LPHEADCOMBO lphc)
{
  ...
  else
  {
    ...
    if (lphc->hFont)
      hPrevFont = SelectObject_k( hDC, lphc->hFont );
       ...
       (lphc->wState & CBF_MEASUREITEM) )
  {
    ...
    UINT id = (UINT)GetWindowLongPtrW_k( lphc->self, GWLP_ID );
    ...
    SendMessageW_k(lphc->owner, WM_MEASUREITEM, id, (LPARAM)&measureItem);
  ...
  LPHEADCOMBO lphc)
{
  ...
  GetWindowRect_k(lphc->self, &windowRect);
  ...
  LPRECT      lprLB)
{
   ...
   */
  GetClientRect_k(hwnd, lprEdit);

  /*
   * Remove the borders
   */
  InflateRect_k(lprEdit, -COMBO_XBORDERSIZE(), -COMBO_YBORDERSIZE());
  ...
  else
  {
     ...
     */
    lprButton->left = lprButton->right - GetSystemMetrics_k(SM_CXVSCROLL);
  ...
  {
    InflateRect_k(lprEdit, -EDIT_CONTROL_PADDING(), -EDIT_CONTROL_PADDING());
  }
...
static void CBGetDroppedControlRect( LPHEADCOMBO lphc, LPRECT lpRect)
{
    ...
    GetWindowRect_k(lphc->self, lpRect);
                             ...
                             BOOL unicode )
{
  ...
  if( lphc->owner || !(style & WS_VISIBLE) )
  {
       ...
       * recalculated.
       */
      GetClientRect_k( hwnd, &lphc->droppedRect );
	...
	if( CB_GETTYPE(lphc) == CBS_DROPDOWN )
	  lphc->droppedRect.left += COMBO_EDITBUTTONSPACE();
        ...
        MapWindowPoints_k( hwnd, 0, (LPPOINT)&lphc->droppedRect, 2 );
      }
	...
	lbeExStyle |= WS_EX_CLIENTEDGE;
      ...
      if (unicode)
          lphc->hWndLBox = CreateWindowExW_k(lbeExStyle, clbName, NULL, lbeStyle,
                                           lphc->droppedRect.left,
                                           lphc->droppedRect.top,
                                           lphc->droppedRect.right - lphc->droppedRect.left,
                                           lphc->droppedRect.bottom - lphc->droppedRect.top,
                                           hwnd, (HMENU)ID_CB_LISTBOX,
                                           (HINSTANCE)GetWindowLongPtrW_k( hwnd, GWLP_HINSTANCE ), lphc );
      else
          lphc->hWndLBox = CreateWindowExA_k(lbeExStyle, "ComboLBox", NULL, lbeStyle,
                                           lphc->droppedRect.left,
                                           lphc->droppedRect.top,
                                           lphc->droppedRect.right - lphc->droppedRect.left,
                                           lphc->droppedRect.bottom - lphc->droppedRect.top,
	  ...
	  if( lphc->wState & CBF_EDIT )
	  {
		  ...
		  lbeStyle |= ES_UPPERCASE;
              ...
              if (!IsWindowEnabled_k(hwnd)) lbeStyle |= WS_DISABLED;

              if (unicode)
                  lphc->hWndEdit = CreateWindowExW_k(0, editName, NULL, lbeStyle,
                                                   lphc->textRect.left, lphc->textRect.top,
                                                   lphc->textRect.right - lphc->textRect.left,
                                                   lphc->textRect.bottom - lphc->textRect.top,
                                                   hwnd, (HMENU)ID_CB_EDIT,
                                                   (HINSTANCE)GetWindowLongPtrW_k( hwnd, GWLP_HINSTANCE ), NULL );
              else
                  lphc->hWndEdit = CreateWindowExA_k(0, "Edit", NULL, lbeStyle,
                                                   lphc->textRect.left, lphc->textRect.top,
                                                   lphc->textRect.right - lphc->textRect.left,
                                                   lphc->textRect.bottom - lphc->textRect.top,
                                                   hwnd, (HMENU)ID_CB_EDIT,
          ...
          if( bEdit )
	  {
	    if( CB_GETTYPE(lphc) != CBS_SIMPLE )
	    {
	      ...
	      SetParent_k(lphc->hWndLBox, HWND_DESKTOP);
              /*
...
static void CBPaintButton( LPHEADCOMBO lphc, HDC hdc, RECT rectButton)
{
    ...
    DrawFrameControl_k(hdc, &rectButton, DFC_SCROLL, buttonState);
}
  ...
  RECT        rectEdit)
{
   ...
   if( (id = SendMessageW_k(lphc->hWndLBox, LB_GETCURSEL, 0, 0) ) != LB_ERR )
   {
        size = SendMessageW_k(lphc->hWndLBox, LB_GETTEXTLEN, id, 0);
	if (size == LB_ERR)
	  FIXME("LB_ERR probably not handled yet\n");
        if( (pText = HeapAlloc_k( GetProcessHeap_k(), 0, (size + 1) * sizeof(WCHAR))) )
	{
            /* size from LB_GETTEXTLEN may be too large, from LB_GETTEXT is accurate */
           size=SendMessageW_k(lphc->hWndLBox, LB_GETTEXT, id, (LPARAM)pText);
   ...
   if( lphc->wState & CBF_EDIT )
   {
	...
	if( CB_HASSTRINGS(lphc) ) SetWindowTextW_k( lphc->hWndEdit, pText ? pText : empty_stringW );
	if( lphc->wState & CBF_FOCUSED )
           SendMessageW_k(lphc->hWndEdit, EM_SETSEL, 0, -1);
   }
   else /* paint text field ourselves */
   {
     UINT	itemState = ODS_COMBOBOXEDIT;
     HFONT	hPrevFont = (lphc->hFont) ? SelectObject_k(hdc, lphc->hFont) : 0;

     /*
      * Give ourselves some space.
      */
     InflateRect_k( &rectEdit, -1, -1 );

     if( CB_OWNERDRAWN(lphc) )
     {
       DRAWITEMSTRUCT dis;
       HRGN           clipRegion;
       UINT ctlid = (UINT)GetWindowLongPtrW_k( lphc->self, GWLP_ID );
       ...
       if (!IsWindowEnabled_k(lphc->self)) itemState |= ODS_DISABLED;
       ...
       dis.hDC		= hdc;
       dis.rcItem	= rectEdit;
       dis.itemData     = SendMessageW_k(lphc->hWndLBox, LB_GETITEMDATA, id, 0);
	...
	*/
       ...
       SendMessageW_k(lphc->owner, WM_DRAWITEM, ctlid, (LPARAM)&dis );

       SelectClipRgn_k( hdc, clipRegion );
       if (clipRegion) DeleteObject_k( clipRegion );
     }
     else
     {
       static const WCHAR empty_stringW[] = { 0 };
	   ...
	   /* highlight */
	   FillRect_k( hdc, &rectEdit, GetSysColorBrush_k(COLOR_HIGHLIGHT) );
	 ...
	 DrawFocusRect_k( hdc, &rectEdit );
     }

     if( hPrevFont )
       SelectObject_k(hdc, hPrevFont );
   }
   HeapFree_k( GetProcessHeap_k(), 0, pText );
}
  ...
  HDC             hdc)
{
  ...
  {
    CopyRect_k(&clientRect, &lphc->textRect);

    InflateRect_k(&clientRect, EDIT_CONTROL_PADDING(), EDIT_CONTROL_PADDING());
    InflateRect_k(&clientRect, COMBO_XBORDERSIZE(), COMBO_YBORDERSIZE());
  }

  DrawEdge_k(hdc, &clientRect, EDGE_SUNKEN, BF_RECT);
}
  ...
  HDC         hDC)
{
  ...
  if (CB_DISABLED(lphc))
  {
    hBkgBrush = (HBRUSH)SendMessageW_k(lphc->owner, WM_CTLCOLORSTATIC,
  ...
  else
  {
      ...
      hBkgBrush = (HBRUSH)SendMessageW_k(lphc->owner, WM_CTLCOLOREDIT,
				       (WPARAM)hDC, (LPARAM)lphc->self );
  }
   ...
   */
  if( !hBkgBrush )
    hBkgBrush = GetSysColorBrush_k(COLOR_WINDOW);
...
static LRESULT COMBO_Paint(LPHEADCOMBO lphc, HDC hParamDC)
{
  ...
  if( hDC && !(lphc->wState & CBF_NOREDRAW) )
  {
      ...
      hPrevBrush = SelectObject_k( hDC, hBkgBrush );
      if (!(lphc->wState & CBF_EDIT))
        FillRect_k(hDC, &lphc->textRect, hBkgBrush);
      ...
      if( !IsRectEmpty_k(&lphc->buttonRect) )
      {
	CBPaintButton(lphc, hDC, lphc->buttonRect);
      ...
      if (CB_GETTYPE(lphc) != CBS_DROPDOWNLIST)
      {
          ...
          InflateRect_k(&rPadEdit, EDIT_CONTROL_PADDING(), EDIT_CONTROL_PADDING());

          FrameRect_k( hDC, &rPadEdit, GetSysColorBrush_k(COLOR_WINDOW) );
      }
...
static INT CBUpdateLBox( LPHEADCOMBO lphc, BOOL bSelect )
{
   ...
   idx = LB_ERR;
   length = SendMessageW_k( lphc->hWndEdit, WM_GETTEXTLENGTH, 0, 0 );

   if( length > 0 )
       pText = HeapAlloc_k( GetProcessHeap_k(), 0, (length + 1) * sizeof(WCHAR));

   TRACE("\t edit text length %i\n", length );

   if( pText )
   {
       GetWindowTextW_k( lphc->hWndEdit, pText, length + 1);
       idx = SendMessageW_k(lphc->hWndLBox, LB_FINDSTRING, -1, (LPARAM)pText);
       HeapFree_k( GetProcessHeap_k(), 0, pText );
   }

   SendMessageW_k(lphc->hWndLBox, LB_SETCURSEL, bSelect ? idx : -1, 0);

   /* probably superfluous but Windows sends this too */
   SendMessageW_k(lphc->hWndLBox, LB_SETCARETINDEX, idx < 0 ? 0 : idx, 0);
   SendMessageW_k(lphc->hWndLBox, LB_SETTOPINDEX, idx < 0 ? 0 : idx, 0);
...
static void CBUpdateEdit( LPHEADCOMBO lphc , INT index )
{
   ...
   {
       length = SendMessageW_k(lphc->hWndLBox, LB_GETTEXTLEN, index, 0);
       if( length != LB_ERR)
       {
	   if( (pText = HeapAlloc_k( GetProcessHeap_k(), 0, (length + 1) * sizeof(WCHAR))) )
	   {
               SendMessageW_k(lphc->hWndLBox, LB_GETTEXT, index, (LPARAM)pText);
	   }
       }
   }
   ...
   if( CB_HASSTRINGS(lphc) )
   {
      ...
      SendMessageW_k(lphc->hWndEdit, WM_SETTEXT, 0, pText ? (LPARAM)pText : (LPARAM)empty_stringW);
      lphc->wState &= ~(CBF_NOEDITNOTIFY | CBF_NOLBSELECT);
   }
...
static void CBDropDown( LPHEADCOMBO lphc )
{
   ...
   {
       lphc->droppedIndex = SendMessageW_k(lphc->hWndLBox, LB_GETCURSEL, 0, 0);

       SendMessageW_k(lphc->hWndLBox, LB_SETTOPINDEX,
                    lphc->droppedIndex == LB_ERR ? 0 : lphc->droppedIndex, 0);
       SendMessageW_k(lphc->hWndLBox, LB_CARETON, 0, 0);
   }

   /* now set popup position */
   GetWindowRect_k( lphc->self, &rect );
  ...
  /* if listbox length has been set directly by its handle */
   GetWindowRect_k(lphc->hWndLBox, &r);
   if (nDroppedHeight < r.bottom - r.top)
       nDroppedHeight = r.bottom - r.top;
   nItems = (int)SendMessageW_k(lphc->hWndLBox, LB_GETCOUNT, 0, 0);
   ...
   if (nItems > 0)
   {
      ...
      nIHeight = (int)SendMessageW_k(lphc->hWndLBox, LB_GETITEMHEIGHT, 0, 0);
   ...
   SetWindowPos_k( lphc->hWndLBox, HWND_TOP, r.left, r.top, r.right - r.left, r.bottom - r.top,
                 SWP_NOACTIVATE | SWP_SHOWWINDOW );


   if( !(lphc->wState & CBF_NOREDRAW) )
     RedrawWindow_k( lphc->self, NULL, 0, RDW_INVALIDATE |
			   RDW_ERASE | RDW_UPDATENOW | RDW_NOCHILDREN );

   EnableWindow_k( lphc->hWndLBox, TRUE );
   if (GetCapture_k() != lphc->self)
      SetCapture_k(lphc->hWndLBox);
}
...
static void CBRollUp( LPHEADCOMBO lphc, BOOL ok, BOOL bButton )
{
   ...
   if( IsWindow_k( hWnd ) && CB_GETTYPE(lphc) != CBS_SIMPLE )
   {
	   ...
	   lphc->wState &= ~CBF_DROPPED;
	   ShowWindow_k( lphc->hWndLBox, SW_HIDE );

           if(GetCapture_k() == lphc->hWndLBox)
           {
               ReleaseCapture_k();
           }
	   ...
	   if( bButton && !(lphc->wState & CBF_NOREDRAW) )
	       RedrawWindow_k( hWnd, &rect, 0, RDW_INVALIDATE |
			       RDW_ERASE | RDW_UPDATENOW | RDW_NOCHILDREN );
...
static void CBRepaintButton( LPHEADCOMBO lphc )
   {
  InvalidateRect_k(lphc->self, &lphc->buttonRect, TRUE);
  UpdateWindow_k(lphc->self);
}
...
static void COMBO_SetFocus( LPHEADCOMBO lphc )
{
   if( !(lphc->wState & CBF_FOCUSED) )
   {
       ...
       if( !(lphc->wState & CBF_EDIT) )
	 InvalidateRect_k(lphc->self, &lphc->textRect, TRUE);
...
static void COMBO_KillFocus( LPHEADCOMBO lphc )
{
   ...
   if( lphc->wState & CBF_FOCUSED )
   {
       ...
       {
           if( CB_GETTYPE(lphc) == CBS_DROPDOWNLIST )
               SendMessageW_k(lphc->hWndLBox, LB_CARETOFF, 0, 0);
   ...
   else if( lphc->hWndLBox == hWnd )
   {
	   ...
	   case LBN_SELCANCEL:
		...
		if( HIWORD(wParam) == LBN_SELCHANGE)
		{
		   if( lphc->wState & CBF_EDIT )
		   {
		       INT index = SendMessageW_k(lphc->hWndLBox, LB_GETCURSEL, 0, 0);
		       lphc->wState |= CBF_NOLBSELECT;
...
{
   HWND hWnd = lphc->self;
   UINT id = (UINT)GetWindowLongPtrW_k( hWnd, GWLP_ID );
   ...
   return SendMessageW_k(lphc->owner, msg, id, lParam);
}
...
static LRESULT COMBO_GetTextW( LPHEADCOMBO lphc, INT count, LPWSTR buf )
{
    ...
    if( lphc->wState & CBF_EDIT )
        return SendMessageW_k( lphc->hWndEdit, WM_GETTEXT, count, (LPARAM)buf );
    ...
    if( lphc->hWndLBox )
    {
        INT idx = SendMessageW_k(lphc->hWndLBox, LB_GETCURSEL, 0, 0);
        if (idx == LB_ERR) goto error;
        length = SendMessageW_k(lphc->hWndLBox, LB_GETTEXTLEN, idx, 0 );
        if (length == LB_ERR) goto error;

        /* 'length' is without the terminating character */
        if (length >= count)
        {
            LPWSTR lpBuffer = HeapAlloc_k(GetProcessHeap_k(), 0, (length + 1) * sizeof(WCHAR));
            if (!lpBuffer) goto error;
            length = SendMessageW_k(lphc->hWndLBox, LB_GETTEXT, idx, (LPARAM)lpBuffer);
            ...
            HeapFree_k( GetProcessHeap_k(), 0, lpBuffer );
        }
        else length = SendMessageW_k(lphc->hWndLBox, LB_GETTEXT, idx, (LPARAM)buf);
...
static LRESULT COMBO_GetTextA( LPHEADCOMBO lphc, INT count, LPSTR buf )
{
    ...
    if( lphc->wState & CBF_EDIT )
        return SendMessageA_k( lphc->hWndEdit, WM_GETTEXT, count, (LPARAM)buf );
    ...
    if( lphc->hWndLBox )
    {
        INT idx = SendMessageW_k(lphc->hWndLBox, LB_GETCURSEL, 0, 0);
        if (idx == LB_ERR) goto error;
        length = SendMessageA_k(lphc->hWndLBox, LB_GETTEXTLEN, idx, 0 );
        if (length == LB_ERR) goto error;

        /* 'length' is without the terminating character */
        if (length >= count)
        {
            LPSTR lpBuffer = HeapAlloc_k(GetProcessHeap_k(), 0, (length + 1) );
            if (!lpBuffer) goto error;
            length = SendMessageA_k(lphc->hWndLBox, LB_GETTEXT, idx, (LPARAM)lpBuffer);
        ...
        else length = SendMessageA_k(lphc->hWndLBox, LB_GETTEXT, idx, (LPARAM)buf);
  ...
  BOOL        bRedraw)
{
   ...
   if( bDrop )
   {
       ...
       if( bRedraw && !(lphc->wState & CBF_NOREDRAW) )
           RedrawWindow_k( lphc->self, NULL, 0,
...
static void COMBO_Size( LPHEADCOMBO lphc )
{
  ...
  if( CB_GETTYPE(lphc) != CBS_SIMPLE )
  {
     ...
     */
    if( curComboHeight != newComboHeight )
      SetWindowPos_k(lphc->self, 0, 0, 0, curComboWidth, newComboHeight,
...
static void COMBO_Font( LPHEADCOMBO lphc, HFONT hFont, BOOL bRedraw )
{
   ...
   */
  if( lphc->wState & CBF_EDIT )
      SendMessageW_k(lphc->hWndEdit, WM_SETFONT, (WPARAM)hFont, bRedraw);
  SendMessageW_k(lphc->hWndLBox, WM_SETFONT, (WPARAM)hFont, bRedraw);
...
static LRESULT COMBO_SetItemHeight( LPHEADCOMBO lphc, INT index, INT height )
{
   ...
   if( index == -1 ) /* set text field height */
   {
	 ...
	 else
	 {
       ...
       lRet = SendMessageW_k(lphc->hWndLBox, LB_SETITEMHEIGHT, index, height);
   return lRet;
}
...
{
   INT index = unicode ? SendMessageW_k(lphc->hWndLBox, LB_SELECTSTRING, start, pText) :
                         SendMessageA_k(lphc->hWndLBox, LB_SELECTSTRING, start, pText);
   if( index >= 0 )
   {
     if( lphc->wState & CBF_EDIT )
       CBUpdateEdit( lphc, index );
...
static void COMBO_LButtonDown( LPHEADCOMBO lphc, LPARAM lParam )
{
   ...
   bButton = PtInRect_k(&lphc->buttonRect, pt);
...
static void COMBO_MouseMove( LPHEADCOMBO lphc, WPARAM wParam, LPARAM lParam )
{
   ...
   GetClientRect_k( lphc->hWndLBox, &lbRect );
   MapWindowPoints_k( lphc->self, lphc->hWndLBox, &pt, 1 );
   if( PtInRect_k(&lbRect, pt) )
   {
       lphc->wState &= ~CBF_CAPTURE;
       ReleaseCapture_k();
       ...
       SendMessageW_k(lphc->hWndLBox, WM_LBUTTONDOWN, wParam, lParam);
   }
}
...
static char *strdupA(LPCSTR str)
{
    ...
    len = strlen(str);
    ret = HeapAlloc_k(GetProcessHeap_k(), 0, len + 1);
...
{
      LPHEADCOMBO lphc = (LPHEADCOMBO)GetWindowLongPtrW_k( hwnd, 0 );
	...
	case WM_CLEAR:
                ...
                {
                    int j = SendMessageW_k(lphc->hWndLBox, LB_GETCURSEL, 0, 0);
                    if (j == -1) return 0;
                    return unicode ? SendMessageW_k(lphc->hWndLBox, LB_GETTEXTLEN, j, 0) :
                                     SendMessageA_k(lphc->hWndLBox, LB_GETTEXTLEN, j, 0);
                }
		else if( lphc->wState & CBF_EDIT )
		{
		    LRESULT ret;
		    lphc->wState |= CBF_NOEDITNOTIFY;
		    ret = unicode ? SendMessageW_k(lphc->hWndEdit, message, wParam, lParam) :
				    SendMessageA_k(lphc->hWndEdit, message, wParam, lParam);
		    lphc->wState &= ~CBF_NOEDITNOTIFY;
		    return ret;
		}
		else return CB_ERR;
	...
	case WM_COPY:
		if( lphc->wState & CBF_EDIT )
		{
		    return unicode ? SendMessageW_k(lphc->hWndEdit, message, wParam, lParam) :
				     SendMessageA_k(lphc->hWndEdit, message, wParam, lParam);
		}
		else return  CB_ERR;
	...
	case WM_ENABLE:
		if( lphc->wState & CBF_EDIT )
		    EnableWindow_k( lphc->hWndEdit, (BOOL)wParam );
		EnableWindow_k( lphc->hWndLBox, (BOOL)wParam );

		/* Force the control to repaint when the enabled state changes. */
		InvalidateRect_k(lphc->self, NULL, TRUE);
		return  TRUE;
	case WM_SETREDRAW:
		if( wParam )
		...
		if( lphc->wState & CBF_EDIT )
		    SendMessageW_k(lphc->hWndEdit, message, wParam, lParam);
		SendMessageW_k(lphc->hWndLBox, message, wParam, lParam);
		return  0;
	case WM_SYSKEYDOWN:
	...
	case WM_IME_CHAR:
	{
		...
		return unicode ? SendMessageW_k(hwndTarget, message, wParam, lParam) :
				 SendMessageA_k(hwndTarget, message, wParam, lParam);
	}
	case WM_LBUTTONDOWN:
		if( !(lphc->wState & CBF_FOCUSED) ) SetFocus_k( lphc->self );
	...
	case WM_MOUSEMOVE:
        ...
        case WM_MOUSEWHEEL:
                ...
                if (GET_WHEEL_DELTA_WPARAM(wParam) > 0) return SendMessageW_k(hwnd, WM_KEYDOWN, VK_UP, 0);
                if (GET_WHEEL_DELTA_WPARAM(wParam) < 0) return SendMessageW_k(hwnd, WM_KEYDOWN, VK_DOWN, 0);
                return TRUE;
	...
	case CB_ADDSTRING:
		if( unicode )
                {
                    if( lphc->dwStyle & CBS_LOWERCASE )
                        CharLowerW_k((LPWSTR)lParam);
                    else if( lphc->dwStyle & CBS_UPPERCASE )
                        CharUpperW_k((LPWSTR)lParam);
                    return SendMessageW_k(lphc->hWndLBox, LB_ADDSTRING, 0, lParam);
                }
                        ...
                        the string if converting case */
                {
                    ...
                    ret = SendMessageA_k(lphc->hWndLBox, LB_ADDSTRING, 0, string ? (LPARAM)string : lParam);
                    HeapFree_k(GetProcessHeap_k(), 0, string);
                    return ret;
                }
		...
		if( unicode )
                {
                    if( lphc->dwStyle & CBS_LOWERCASE )
                        ...
                        CharUpperW_k((LPWSTR)lParam);
                    return SendMessageW_k(lphc->hWndLBox, LB_INSERTSTRING, wParam, lParam);
                }
                else
                {
                    if( lphc->dwStyle & CBS_LOWERCASE )
                        CharLowerA_k((LPSTR)lParam);
                    else if( lphc->dwStyle & CBS_UPPERCASE )
                        CharUpperA_k((LPSTR)lParam);

                    return SendMessageA_k(lphc->hWndLBox, LB_INSERTSTRING, wParam, lParam);
                }
	case CB_DELETESTRING:
		return unicode ? SendMessageW_k(lphc->hWndLBox, LB_DELETESTRING, wParam, 0) :
				 SendMessageA_k(lphc->hWndLBox, LB_DELETESTRING, wParam, 0);
	case CB_SELECTSTRING:
		return COMBO_SelectString(lphc, (INT)wParam, lParam, unicode);
	case CB_FINDSTRING:
		return unicode ? SendMessageW_k(lphc->hWndLBox, LB_FINDSTRING, wParam, lParam) :
				 SendMessageA_k(lphc->hWndLBox, LB_FINDSTRING, wParam, lParam);
	case CB_FINDSTRINGEXACT:
		return unicode ? SendMessageW_k(lphc->hWndLBox, LB_FINDSTRINGEXACT, wParam, lParam) :
				 SendMessageA_k(lphc->hWndLBox, LB_FINDSTRINGEXACT, wParam, lParam);
	case CB_SETITEMHEIGHT:
		return  COMBO_SetItemHeight( lphc, (INT)wParam, (INT)lParam);
	case CB_GETITEMHEIGHT:
		if( (INT)wParam >= 0 )	/* listbox item */
                    return SendMessageW_k(lphc->hWndLBox, LB_GETITEMHEIGHT, wParam, 0);
                return  CBGetTextAreaHeight(hwnd, lphc);
	case CB_RESETCONTENT:
		SendMessageW_k(lphc->hWndLBox, LB_RESETCONTENT, 0, 0);
                if( (lphc->wState & CBF_EDIT) && CB_HASSTRINGS(lphc) )
		{
		    static const WCHAR empty_stringW[] = { 0 };
                    SendMessageW_k(lphc->hWndEdit, WM_SETTEXT, 0, (LPARAM)empty_stringW);
		}
                else
                    InvalidateRect_k(lphc->self, NULL, TRUE);
		return  TRUE;
	case CB_INITSTORAGE:
		return SendMessageW_k(lphc->hWndLBox, LB_INITSTORAGE, wParam, lParam);
	case CB_GETHORIZONTALEXTENT:
		return SendMessageW_k(lphc->hWndLBox, LB_GETHORIZONTALEXTENT, 0, 0);
	case CB_SETHORIZONTALEXTENT:
		return SendMessageW_k(lphc->hWndLBox, LB_SETHORIZONTALEXTENT, wParam, 0);
	case CB_GETTOPINDEX:
		return SendMessageW_k(lphc->hWndLBox, LB_GETTOPINDEX, 0, 0);
	case CB_GETLOCALE:
		return SendMessageW_k(lphc->hWndLBox, LB_GETLOCALE, 0, 0);
	case CB_SETLOCALE:
		return SendMessageW_k(lphc->hWndLBox, LB_SETLOCALE, wParam, 0);
	...
	case CB_DIR:
		return unicode ? SendMessageW_k(lphc->hWndLBox, LB_DIR, wParam, lParam) :
				 SendMessageA_k(lphc->hWndLBox, LB_DIR, wParam, lParam);
	...
	case CB_SHOWDROPDOWN:
		...
		return  TRUE;
	case CB_GETCOUNT:
		return SendMessageW_k(lphc->hWndLBox, LB_GETCOUNT, 0, 0);
	case CB_GETCURSEL:
		return SendMessageW_k(lphc->hWndLBox, LB_GETCURSEL, 0, 0);
	case CB_SETCURSEL:
		lParam = SendMessageW_k(lphc->hWndLBox, LB_SETCURSEL, wParam, 0);
	        if( lParam >= 0 )
	            SendMessageW_k(lphc->hWndLBox, LB_SETTOPINDEX, wParam, 0);
		...
		lphc->wState &= ~CBF_SELCHANGE;
	        return  lParam;
	case CB_GETLBTEXT:
		return unicode ? SendMessageW_k(lphc->hWndLBox, LB_GETTEXT, wParam, lParam) :
				 SendMessageA_k(lphc->hWndLBox, LB_GETTEXT, wParam, lParam);
	case CB_GETLBTEXTLEN:
                return unicode ? SendMessageW_k(lphc->hWndLBox, LB_GETTEXTLEN, wParam, 0) :
                                 SendMessageA_k(lphc->hWndLBox, LB_GETTEXTLEN, wParam, 0);
	case CB_GETITEMDATA:
		return SendMessageW_k(lphc->hWndLBox, LB_GETITEMDATA, wParam, 0);
	case CB_SETITEMDATA:
		return SendMessageW_k(lphc->hWndLBox, LB_SETITEMDATA, wParam, lParam);
	case CB_GETEDITSEL:
		/* Edit checks passed parameters itself */
		if( lphc->wState & CBF_EDIT )
		    return SendMessageW_k(lphc->hWndEdit, EM_GETSEL, wParam, lParam);
		return  CB_ERR;
	case CB_SETEDITSEL:
		if( lphc->wState & CBF_EDIT )
                    return SendMessageW_k(lphc->hWndEdit, EM_SETSEL,
	...
	case CB_LIMITTEXT:
		if( lphc->wState & CBF_EDIT )
			return SendMessageW_k(lphc->hWndEdit, EM_LIMITTEXT, wParam, lParam);
		return  TRUE;
	default:
		if (message >= WM_USER)
...
{
    TRACE("(%p, %p)\n", hwndCombo, pcbi);
    return SendMessageW_k(hwndCombo, CB_GETCOMBOBOXINFO, 0, (LPARAM)pcbi);
}
