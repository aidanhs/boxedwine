...
{
    if (SendMessageW_k( hwndCtrl, WM_GETDLGCODE, 0, 0 ) & DLGC_HASSETSEL)
        SendMessageW_k( hwndCtrl, EM_SETSEL, 0, -1 );
    SetFocus_k( hwndCtrl );
}
...
static void DEFDLG_SaveFocus( HWND hwnd )
{
    ...
    if (!hwndFocus || !IsChild_k( hwnd, hwndFocus )) return;
...
static void DEFDLG_RestoreFocus( HWND hwnd, BOOL justActivate )
{
    ...
    if (!IsWindow_k(infoPtr->hwndFocus) || infoPtr->hwndFocus == hwnd) {
        ...
        infoPtr->hwndFocus = GetNextDlgTabItem_k( hwnd, 0, FALSE );
        /* If there are no WS_TABSTOP controls, set focus to the first visible,
           non-disabled control in the dialog */
        if (!infoPtr->hwndFocus) infoPtr->hwndFocus = GetNextDlgGroupItem_k( hwnd, 0, FALSE );
        if (!IsWindow_k( infoPtr->hwndFocus )) return;
    }
    if (justActivate)
        SetFocus_k( infoPtr->hwndFocus );
    else
...
static HWND DEFDLG_FindDefButton( HWND hwndDlg )
{
    ...
    hwndChild = GetWindow_k( hwndDlg, GW_CHILD );
    while (hwndChild)
    {
        if (SendMessageW_k( hwndChild, WM_GETDLGCODE, 0, 0 ) & DLGC_DEFPUSHBUTTON)
            break;

        /* Recurse into WS_EX_CONTROLPARENT controls */
        if (GetWindowLongW_k( hwndChild, GWL_EXSTYLE ) & WS_EX_CONTROLPARENT)
        {
            LONG dsStyle = GetWindowLongW_k( hwndChild, GWL_STYLE );
...
static BOOL DEFDLG_SetDefId( HWND hwndDlg, DIALOGINFO *dlgInfo, WPARAM wParam)
{
    ...
    HWND hwndOld, hwndNew = GetDlgItem_k(hwndDlg, wParam);
    INT old_id = dlgInfo->idResult;

    dlgInfo->idResult = wParam;
    if (hwndNew &&
        !((dlgcode=SendMessageW_k(hwndNew, WM_GETDLGCODE, 0, 0 ))
    ...
    hwndOld = GetDlgItem_k( hwndDlg, old_id );
    if (!hwndOld || !(SendMessageW_k( hwndOld, WM_GETDLGCODE, 0, 0) & DLGC_DEFPUSHBUTTON))
        hwndOld = DEFDLG_FindDefButton( hwndDlg );
    if (hwndOld && hwndOld != hwndNew)
        SendMessageW_k( hwndOld, BM_SETSTYLE, BS_PUSHBUTTON, TRUE );

    if (hwndNew)
    {
        if(dlgcode & DLGC_UNDEFPUSHBUTTON)
            SendMessageW_k( hwndNew, BM_SETSTYLE, BS_DEFPUSHBUTTON, TRUE );
    }
    return TRUE;
}
...
{
    DWORD dlgcode=0; /* initialize just to avoid a warning */
    HWND hwndOld = GetDlgItem_k( hwndDlg, dlgInfo->idResult );
                            ...
                            LPARAM lParam, DIALOGINFO *dlgInfo )
{
    switch(msg)
    {
        case WM_ERASEBKGND:
        {
            HBRUSH brush = (HBRUSH)SendMessageW_k( hwnd, WM_CTLCOLORDLG, wParam, (LPARAM)hwnd );
            if (!brush) brush = (HBRUSH)DefWindowProcW_k( hwnd, WM_CTLCOLORDLG, wParam, (LPARAM)hwnd );
            if (brush)
            {
                RECT rect;
                HDC hdc = (HDC)wParam;
        ...
        case WM_NCDESTROY:
            if (dlgInfo)
            {
                ...
                if (dlgInfo->hUserFont) DeleteObject_k( dlgInfo->hUserFont );
                if (dlgInfo->hMenu) DestroyMenu_k( dlgInfo->hMenu );
                HeapFree_k( GetProcessHeap_k(), 0, dlgInfo );
        ...
        case DM_GETDEFID:
            if (dlgInfo && !(dlgInfo->flags & DF_END))
            {
                ...
                if ((hwndDefId = DEFDLG_FindDefButton( hwnd )))
                    return MAKELONG( GetDlgCtrlID_k( hwndDefId ), DC_HASDEFID);
            }
            return 0;
        ...
        case WM_NEXTDLGCTL:
            if (dlgInfo)
            {
                ...
                if (!lParam)
                    hwndDest = GetNextDlgTabItem_k(hwnd, GetFocus_k(), wParam);
        ...
        case WM_NCLBUTTONDOWN:
            {
                ...
                if (hwndFocus)
                {
                    ...
                    if (!SendMessageW_k( hwndFocus, CB_SHOWDROPDOWN, FALSE, 0 ))
                        SendMessageW_k( GetParent_k(hwndFocus), CB_SHOWDROPDOWN, FALSE, 0 );
                }
            }
            return DefWindowProcA_k( hwnd, msg, wParam, lParam );
        ...
        case WM_CLOSE:
            PostMessageA_k( hwnd, WM_COMMAND, MAKEWPARAM(IDCANCEL, BN_CLICKED),
                            (LPARAM)GetDlgItem_k( hwnd, IDCANCEL ) );
            return 0;
    }
    return 0;
}
...
DIALOGINFO *DIALOG_get_info( HWND hwnd, BOOL create )
{
    ...
    if (!dlgInfo && create)
    {
        if (!(dlgInfo = HeapAlloc_k( GetProcessHeap_k(), 0, sizeof(*dlgInfo) )))
            goto out;
        dlgInfo->hwndFocus   = 0;
        dlgInfo->hUserFont   = 0;
        dlgInfo->hMenu       = 0;
 ...
 *              DefDlgProcA (USER32.@)
 */
LRESULT WINAPI DefDlgProcA_k( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    DIALOGINFO *dlgInfo;
    DLGPROC dlgproc;
    LRESULT result = 0;
    ...
    return GetWindowLongPtrW_k( hwnd, DWLP_MSGRESULT );
}
 ...
 *              DefDlgProcW (USER32.@)
 */
LRESULT WINAPI DefDlgProcW_k( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    DIALOGINFO *dlgInfo;
    DLGPROC dlgproc;
    LRESULT result = 0;
