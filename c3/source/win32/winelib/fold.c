...
static __inline WCHAR to_unicode_digit( WCHAR ch )
{
    extern const WCHAR wine_digitmap[];
    return ch + wine_digitmap[wine_digitmap[ch >> 8] + (ch & 0xff)];
}

static __inline WCHAR to_unicode_native( WCHAR ch )
{
...
static __inline int get_ligature_len( WCHAR wc )
{
...
static __inline const WCHAR* get_ligature( WCHAR wc )
{
