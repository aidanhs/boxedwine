...
{
    return InterlockedIncrement_k( &surface->ref );
}

static INLINE ULONG window_surface_release( struct window_surface *surface )
{
    ULONG ret = InterlockedDecrement_k( &surface->ref );
...
WINGDIAPI DWORD_PTR WINAPI GetDCHook_k(HDC,DCHOOKPROC*);
WINGDIAPI BOOL      WINAPI SetDCHook_k(HDC,DCHOOKPROC,DWORD_PTR);
WINGDIAPI WORD      WINAPI SetHookFlags_k(HDC,WORD);
