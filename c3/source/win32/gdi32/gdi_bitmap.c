 ...
 *    Failure: 0
 */
HBITMAP WINAPI CreateBitmap_k( INT width, INT height, UINT planes,
                             UINT bpp, LPCVOID bits )
{
    ...
    return CreateBitmapIndirect_k( &bm );
}
 ...
 *    Failure: 0
 */
HBITMAP WINAPI CreateCompatibleBitmap_k( HDC hdc, INT width, INT height)
{
    ...
    if (GetObjectType_k( hdc ) != OBJ_MEMDC)
        return CreateBitmap_k( width, height,
                             GetDeviceCaps_k(hdc, PLANES), GetDeviceCaps_k(hdc, BITSPIXEL), NULL );

    switch (GetObjectW_k( GetCurrentObject_k( hdc, OBJ_BITMAP ), sizeof(dib), &dib ))
    {
    case sizeof(BITMAP): /* A device-dependent bitmap is selected in the DC */
        return CreateBitmap_k( width, height, dib.dsBm.bmPlanes, dib.dsBm.bmBitsPixel, NULL );
    ...
    case sizeof(DIBSECTION): /* A DIB section is selected in the DC */
        ...
        if (dib.dsBmih.biCompression == BI_BITFIELDS)  /* copy the color masks */
            ...
            GetDIBColorTable_k(hdc, 0, 256, bi->bmiColors);
        return CreateDIBSection_k( hdc, bi, DIB_RGB_COLORS, NULL, NULL, 0 );
 ...
 */
HBITMAP WINAPI CreateBitmapIndirect_k( const BITMAP *bmp )
{
    BITMAP bm;
    BITMAPOBJ *bmpobj;
    HBITMAP hbitmap;
    ...
    {
        return GetStockObject_k( DEFAULT_BITMAP );
    }
    else
    {
        if (bm.bmHeight < 0)
    ...
    if (!(bmpobj = HeapAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(*bmpobj) )))
    {
        SetLastError_k( ERROR_NOT_ENOUGH_MEMORY );
        return 0;
    }
    ...
    {
        HeapFree_k( GetProcessHeap_k(), 0, bmpobj );
        return 0;
    }

    if (bm.bmBits)
        SetBitmapBits_k( hbitmap, bm.bmHeight * bm.bmWidthBytes, bm.bmBits );
    ...
    LPCVOID bits)      /* [in] Address of array with bitmap bits */
{
    ...
    if (count % src_stride)
    {
        ...
        clip = CreateRectRgn_k( src.visrect.left, src.visrect.top,
                              src.visrect.right, src.visrect.bottom - 1 );
        last_row = CreateRectRgn_k( src.visrect.left, src.visrect.bottom - 1,
                                  src.visrect.left + extra_pixels, src.visrect.bottom );
        CombineRgn_k( clip, clip, last_row, RGN_OR );
        DeleteObject_k( last_row );
    }
    ...
    {
        if (!(src_bits.ptr = HeapAlloc_k( GetProcessHeap_k(), 0, dst.height * dst_stride )))
        {
            GDI_ReleaseObj( hbitmap );
            return 0;
        }
...
static HGDIOBJ BITMAP_SelectObject( HGDIOBJ handle, HDC hdc )
{
    ...
    if (handle != GetStockObject_k(DEFAULT_BITMAP) && GDI_get_ref_count( handle ))
    {
    ...
    if (bitmap->dib.dsBm.bmBitsPixel != 1 &&
        bitmap->dib.dsBm.bmBitsPixel != GetDeviceCaps_k( hdc, BITSPIXEL ))
    {
...
static BOOL BITMAP_DeleteObject( HGDIOBJ handle )
{
    ...
    if (!bmp) return FALSE;
    HeapFree_k( GetProcessHeap_k(), 0, bmp->dib.dsBm.bmBits );
 ...
 *    Failure: NULL
 */
HBITMAP WINAPI CreateDiscardableBitmap_k(
    HDC hdc,    /* [in] Handle to device context */
    INT width,  /* [in] Bitmap width */
    INT height) /* [in] Bitmap height */
{
    return CreateCompatibleBitmap_k( hdc, width, height );
}
 ...
 *    Failure: FALSE
 */
BOOL WINAPI GetBitmapDimensionEx_k(
    HBITMAP hbitmap, /* [in]  Handle to bitmap */
 ...
 *    Failure: FALSE
 */
BOOL WINAPI SetBitmapDimensionEx_k(
    HBITMAP hbitmap, /* [in]  Handle to bitmap */
