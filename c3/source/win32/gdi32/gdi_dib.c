...
static BOOL is_valid_dib_format( const BITMAPINFOHEADER *info, BOOL allow_compression )
{
    ...
    if ((LONG)(UINT_MAX / info->biBitCount) < info->biWidth) return FALSE;
    if ((LONG)(UINT_MAX / get_dib_stride( info->biWidth, info->biBitCount )) < abs( info->biHeight )) return FALSE;
...
static int fill_color_table_from_palette( BITMAPINFO *info, HDC hdc )
{
    ...
    HPALETTE palette = GetCurrentObject_k( hdc, OBJ_PAL );
    ...
    if (!GetPaletteEntries_k( palette, 0, colors, palEntry ))
        return 0;
...
BOOL fill_color_table_from_pal_colors( BITMAPINFO *info, HDC hdc )
{
    ...
    if (!(palette = GetCurrentObject_k( hdc, OBJ_PAL ))) return FALSE;
    if (!(count = GetPaletteEntries_k( palette, 0, colors, entries ))) return FALSE;
...
static BOOL build_rle_bitmap( const BITMAPINFO *info, struct gdi_image_bits *bits, HRGN *clip )
{
    ...
    out_bits = HeapAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY, get_dib_image_size( info ) );
    if (!out_bits) goto fail;

    if (clip)
    {
        *clip = CreateRectRgn_k( 0, 0, 0, 0 );
        run   = CreateRectRgn_k( 0, 0, 0, 0 );
    ...
    while (i < info->bmiHeader.biSizeImage - 1)
    {
        ...
        else
        {
            if (data < 3)
            {
                if(left != right && clip)
                {
                    SetRectRgn_k( run, left, y, right, y + 1 );
                    CombineRgn_k( *clip, run, *clip, RGN_OR );
                }
                switch (data)
                {
                case 0: /* eol */
...
fail:
    if (run) DeleteObject_k( run );
    if (clip && *clip) DeleteObject_k( *clip );
    HeapFree_k( GetProcessHeap_k(), 0, out_bits );
    return FALSE;
}
                           ...
                           BITMAPINFO *src_info, UINT coloruse, DWORD rop )
{
    ...
    if (clip) OffsetRgn_k( clip, dst.x - src.x, dst.y - src.y );
    ...
    if (err == ERROR_BAD_FORMAT)
    {
        ...
        {
            COLORREF color = GetBkColor_k( dev->hdc );
    ...
    if (err == ERROR_TRANSFORM_NOT_SUPPORTED)
    {
        ...
        err = stretch_bits( src_info, &src, dst_info, &dst, &src_bits, GetStretchBltMode_k( dev->hdc ) );
 ...
 *           StretchDIBits   (GDI32.@)
 */
INT WINAPI StretchDIBits_k(HDC hdc, INT xDst, INT yDst, INT widthDst, INT heightDst,
                         INT xSrc, INT ySrc, INT widthSrc, INT heightSrc, const void *bits,
 ...
 *    Failure: 0
 */
INT WINAPI SetDIBits_k( HDC hdc, HBITMAP hbitmap, UINT startscan,
		      UINT lines, LPCVOID bits, const BITMAPINFO *info,
		      UINT coloruse )
{
    BITMAPOBJ *bitmap;
    char src_bmibuf[sizeof(BITMAPINFO)+sizeof(RGBQUAD)*256];
    BITMAPINFO *src_info = (BITMAPINFO *)src_bmibuf;
    char dst_bmibuf[sizeof(BITMAPINFO)+sizeof(RGBQUAD)*256];
    ...
    {
        src_to_dst_offset = -(INT)startscan;
        lines = min( lines, src.visrect.bottom - startscan );
        if ((LONG)lines < src.visrect.bottom) src.visrect.top = src.visrect.bottom - lines;
    }
    else
    {
        src_to_dst_offset = src.visrect.bottom - lines - startscan;
        /* Unlike the bottom-up case, Windows doesn't limit lines. */
        if ((LONG)lines < src.visrect.bottom) src.visrect.bottom = lines;
    }
                               ...
                               const void *bits, BITMAPINFO *src_info, UINT coloruse )
{
    ...
    else
    {
        ...
        if (src.y > 0)
        {
            if (!top_down)
            {
                ...
                if (src.y >= (int)lines) return 0;
                lines -= src.y;
                src.y = 0;
            }
            else if (src.y >= (int)lines) return lines;
        }
        src_info->bmiHeader.biHeight = top_down ? -(INT)lines : lines;
    }
    ...
    pt.x = x_dst;
    pt.y = y_dst;
    LPtoDP_k( dev->hdc, &pt, 1 );
    dst.x = pt.x;
    dst.y = pt.y;
    dst.width = cx;
    dst.height = cy;
    if (GetLayout_k( dev->hdc ) & LAYOUT_RTL) dst.x -= cx - 1;
 ...
 */
INT WINAPI SetDIBitsToDevice_k(HDC hdc, INT xDest, INT yDest, DWORD cx,
 ...
 */
UINT WINAPI SetDIBColorTable_k( HDC hdc, UINT startpos, UINT entries, CONST RGBQUAD *colors )
{
    DC * dc;
    UINT result = 0;
    BITMAPOBJ * bitmap;
    ...
    if ((bitmap = GDI_GetObjPtr( dc->hBitmap, OBJ_BITMAP )))
    {
        ...
        {
            SetTextColor_k( hdc, dc->textColor );
            SetBkColor_k( hdc, dc->backgroundColor );
            SelectObject_k( hdc, dc->hPen );
            SelectObject_k( hdc, dc->hBrush );
        }
    }
    release_dc_ptr( dc );
    return result;
 ...
 */
UINT WINAPI GetDIBColorTable_k( HDC hdc, UINT startpos, UINT entries, RGBQUAD *colors )
{
    DC * dc;
    BITMAPOBJ *bitmap;
    UINT result = 0;
...
static int fill_query_info( BITMAPINFO *info, BITMAPOBJ *bmp )
{
    ...
    if ( info->bmiHeader.biSize == sizeof(BITMAPCOREHEADER) )
    {
        ...
        coreheader->bcWidth    = (WORD)header.biWidth;
        coreheader->bcHeight   = (WORD)header.biHeight;
...
BITMAPINFO *copy_packed_dib( const BITMAPINFO *src_info, UINT usage )
{
    ...
    if ((ret = HeapAlloc_k( GetProcessHeap_k(), 0, info_size + info->bmiHeader.biSizeImage )))
    {
    ...
    UINT coloruse)   /* [in]  RGB or palette index */
{
    ...
    if (lines == 0 || (LONG)startscan >= dst.visrect.bottom)
        bits = NULL;
    ...
    if (bits)
    {
        ...
        {
            dst_to_src_offset = -(INT)startscan;
            lines = min( lines, dst.visrect.bottom - startscan );
            if ((LONG)lines < dst.visrect.bottom) dst.visrect.top = dst.visrect.bottom - lines;
        }
        else
        {
            ...
            if ((LONG)lines < dst.visrect.bottom) dst.visrect.bottom = lines;
        }
        ...
        if (dst_info->bmiHeader.biHeight > 0)
        {
            ...
            {
                int pad_lines = min( dst_info->bmiHeader.biHeight - dst.visrect.bottom, (LONG)lines );
        ...
        {
            if (dst.visrect.bottom < (LONG)lines)
            {
    ...
    if (coloruse == DIB_PAL_COLORS)
    {
        ...
        for (i = 0; i < (int)dst_info->bmiHeader.biClrUsed; i++, index++)
            *index = i;
    }
 ...
 */
HBITMAP WINAPI CreateDIBitmap_k( HDC hdc, const BITMAPINFOHEADER *header,
                            ...
                            UINT coloruse )
{
    ...
    if (hdc == NULL)
        handle = CreateBitmap_k( info.biWidth, height, 1, 1, NULL );
    else
        handle = CreateCompatibleBitmap_k( hdc, info.biWidth, height );

    if (handle)
    {
        if (init & CBM_INIT)
        {
            if (SetDIBits_k( hdc, handle, 0, height, bits, data, coloruse ) == 0)
            {
 ...
 */
HBITMAP WINAPI CreateDIBSection_k(HDC hdc, CONST BITMAPINFO *bmi, UINT usage,
                                VOID **bits, HANDLE section, DWORD offset)
{
    ...
    if (!(bmp = HeapAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(*bmp) ))) return 0;
    ...
    if (info->bmiHeader.biBitCount <= 8)  /* build the color table */
    {
        ...
        if (!(bmp->color_table = HeapAlloc_k( GetProcessHeap_k(), 0,
...
error:
    if (section) UnmapViewOfFile_k( mapBits );
    else VirtualFree_k( bmp->dib.dsBm.bmBits, 0, MEM_RELEASE );
    HeapFree_k( GetProcessHeap_k(), 0, bmp->color_table );
    HeapFree_k( GetProcessHeap_k(), 0, bmp );
    return 0;
}
...
static BOOL DIB_DeleteObject( HGDIOBJ handle )
{
    ...
    if (bmp->dib.dshSection)
    {
        SYSTEM_INFO SystemInfo;
        GetSystemInfo_k( &SystemInfo );
        UnmapViewOfFile_k( (char *)bmp->dib.dsBm.bmBits -
