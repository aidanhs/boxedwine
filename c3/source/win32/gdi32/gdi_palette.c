...
HPALETTE PALETTE_Init(void)
{
    ...
    char buffer[sizeof(LOGPALETTE)+sizeof(PALETTEENTRY)*20];
 ...
 *    Failure: NULL
 */
HPALETTE WINAPI CreatePalette_k(
    const LOGPALETTE* palette) /* [in] Pointer to logical color palette */
{
    ...
    if (!(palettePtr = HeapAlloc_k( GetProcessHeap_k(), 0, sizeof(*palettePtr) ))) return 0;
    ...
    if (!(palettePtr->entries = HeapAlloc_k( GetProcessHeap_k(), 0, size )))
    {
        HeapFree_k( GetProcessHeap_k(), 0, palettePtr );
        return 0;
    }
    memcpy( palettePtr->entries, palette->palPalEntry, size );
    if (!(hpalette = alloc_gdi_handle( palettePtr, OBJ_PAL, &palette_funcs )))
    {
        HeapFree_k( GetProcessHeap_k(), 0, palettePtr->entries );
        HeapFree_k( GetProcessHeap_k(), 0, palettePtr );
    }
    TRACE("   returning %p\n", hpalette);
    return hpalette;
 ...
 */
HPALETTE WINAPI CreateHalftonePalette_k(
    HDC hdc) /* [in] Handle to device context */
{
    const RGBQUAD *entries = get_default_color_table( 8 );
    char buffer[sizeof(LOGPALETTE)+sizeof(PALETTEENTRY)*256];
 ...
 *    Failure: 0
 */
UINT WINAPI GetPaletteEntries_k(
 ...
 *    Failure: 0
 */
UINT WINAPI SetPaletteEntries_k(
    HPALETTE hpalette,    /* [in] Handle of logical palette */
    ...
    const PALETTEENTRY *entries) /* [in] Address of array of structures */
{
    ...
    if (hpalette == GetStockObject_k(DEFAULT_PALETTE)) return 0;
    palPtr = GDI_GetObjPtr( hpalette, OBJ_PAL );
    if (!palPtr) return 0;
    ...
    UINT cEntries) /* [in] Number of entries in logical palette */
{
    ...
    if (!(entries = HeapReAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY,
    ...
    const PALETTEENTRY* PaletteColors) /* [in] Pointer to first replacement */
{
    ...
    if( hPal != GetStockObject_k(DEFAULT_PALETTE) )
    {
        PALETTEOBJ * palPtr;
        UINT pal_entries;
 ...
 */
UINT WINAPI GetSystemPaletteUse_k(
    HDC hdc) /* [in] Handle of device context */
{
    return currentProcess()->SystemPaletteUse;
}
 ...
 *    Failure: 0
 */
UINT WINAPI GetSystemPaletteEntries_k(
    HDC hdc,              /* [in]  Handle of device context */
 ...
 *    Failure: CLR_INVALID
 */
UINT WINAPI GetNearestPaletteIndex_k(
...
COLORREF nulldrv_GetNearestColor( PHYSDEV dev, COLORREF color )
{
    ...
    if (!(GetDeviceCaps_k( dev->hdc, RASTERCAPS ) & RC_PALETTE)) return color;
    ...
    if (spec_type == 1 || spec_type == 2)
    {
        ...
        UINT index;
        PALETTEENTRY entry;
        HPALETTE hpal = GetCurrentObject_k( dev->hdc, OBJ_PAL );

        if (!hpal) hpal = GetStockObject_k( DEFAULT_PALETTE );
        if (spec_type == 2) /* PALETTERGB */
            index = GetNearestPaletteIndex_k( hpal, color );
        else  /* PALETTEINDEX */
            index = LOWORD(color);

        if (!GetPaletteEntries_k( hpal, index, 1, &entry ))
        {
            WARN("RGB(%x) : idx %d is out of bounds, assuming NULL\n", color, index );
            if (!GetPaletteEntries_k( hpal, 0, 1, &entry )) return CLR_INVALID;
        }
 ...
 *    Failure: CLR_INVALID
 */
COLORREF WINAPI GetNearestColor_k(
    HDC hdc,      /* [in] Handle of device context */
    COLORREF color) /* [in] Color to be matched */
{
...
static BOOL PALETTE_UnrealizeObject( HGDIOBJ handle )
{
    ...
    if (InterlockedCompareExchangePointer_k( (void **)&currentProcess()->hLastRealizedPalette, 0, handle ) == handle)
...
static BOOL PALETTE_DeleteObject( HGDIOBJ handle )
{
    ...
    HeapFree_k( GetProcessHeap_k(), 0, obj->entries );
 ...
 */
HPALETTE WINAPI GDISelectPalette_k( HDC hdc, HPALETTE hpal, WORD wBkg)
{
    HPALETTE ret = 0;
    DC *dc;
    ...
    hpal = get_full_gdi_handle( hpal );
    if (GetObjectType_k(hpal) != OBJ_PAL)
    {
    ...
    if ((dc = get_dc_ptr( hdc )))
    {
        ...
        {
            dc->hPalette = hpal;
            if (!wBkg) currentProcess()->hPrimaryPalette = (U32)hpal;
        }
        else ret = 0;
        release_dc_ptr( dc );
    }
...
UINT WINAPI GDIRealizePalette( HDC hdc )
{
    ...
    if( dc->hPalette == GetStockObject_k( DEFAULT_PALETTE ))
    {
        PHYSDEV physdev = GET_DC_PHYSDEV( dc, pRealizeDefaultPalette );
        realized = physdev->funcs->pRealizeDefaultPalette( physdev );
    }
    else if (InterlockedExchangePointer_k( (void **)&process->hLastRealizedPalette, dc->hPalette ) != dc->hPalette)
    {
 ...
 *    Failure: FALSE
 */
BOOL WINAPI UpdateColors_k(
    HDC hDC) /* [in] Handle of device context */
{
    int size = GetDeviceCaps_k( hDC, SIZEPALETTE );
    HWND hWnd;
    if (!size) return 0;

    hWnd = WindowFromDC_k( hDC );
    ...
    if (hWnd && size)
    {
        RedrawWindow_k( hWnd, NULL, 0, RDW_INVALIDATE );
    }
    return 0x666;
}
 ...
 */
BOOL WINAPI SetMagicColors_k(HDC hdc, ULONG u1, ULONG u2)
{
