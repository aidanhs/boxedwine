                                ...
                                DC *dc_src, struct bitblt_coords *src )
{
    ...
    LPtoDP_k( dc_dst->hSelf, (POINT *)&rect, 2 );
    dst->x      = rect.left;
    dst->y      = rect.top;
    ...
    LPtoDP_k( dc_src->hSelf, (POINT *)&rect, 2 );
    src->x      = rect.left;
    src->y      = rect.top;
...
{
    HeapFree_k( GetProcessHeap_k(), 0, bits->ptr );
}
                    ...
                    BITMAPINFO *dst_info, struct gdi_image_bits *bits )
{
    ...
    if (!(ptr = HeapAlloc_k( GetProcessHeap_k(), 0, dst_info->bmiHeader.biSizeImage )))
                         ...
                         struct gdi_image_bits *dst_bits, struct bitblt_coords *dst, BLENDFUNCTION blend )
{
    if (!dst_bits->is_copy)
    {
        ...
        void *ptr = HeapAlloc_k( GetProcessHeap_k(), 0, size );
...
{
    COLORREF color = GetBkColor_k( hdc );
                         ...
                         PHYSDEV src_dev, struct bitblt_coords *src, DWORD rop )
{
    ...
    char src_buffer[sizeof(BITMAPINFO)+sizeof(RGBQUAD)*256];
    char dst_buffer[sizeof(BITMAPINFO)+sizeof(RGBQUAD)*256];
        ...
        ((src->width != dst->width) || (src->height != dst->height)))
    {
        ...
        err = stretch_bits( src_info, src, dst_info, dst, &bits, GetStretchBltMode_k( dst_dev->hdc ));
                           ...
                           void * grad_array, ULONG ngrad, ULONG mode )
{
    ...
    if (!(pts = HeapAlloc_k( GetProcessHeap_k(), 0, nvert * sizeof(*pts) ))) return FALSE;
    for (i = 0; i < nvert; i++)
    {
        pts[i].x = vert_array[i].x;
        pts[i].y = vert_array[i].y;
    }
    LPtoDP_k( dev->hdc, pts, nvert );
    ...
    if (!(bits.ptr = HeapAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY, info->bmiHeader.biSizeImage )))
        goto done;
    bits.is_copy = TRUE;
    bits.free = free_heap_bits;
...
COLORREF nulldrv_GetPixel( PHYSDEV dev, INT x, INT y )
{
    ...
    src.visrect.left = x;
    src.visrect.top  = y;
    LPtoDP_k( dev->hdc, (POINT *)&src.visrect, 1 );
 ...
 *           PatBlt    (GDI32.@)
 */
BOOL WINAPI PatBlt_k( HDC hdc, INT left, INT top, INT width, INT height, DWORD rop)
{
    DC * dc;
    BOOL ret = FALSE;
 ...
 *           BitBlt    (GDI32.@)
 */
BOOL WINAPI BitBlt_k( HDC hdcDst, INT xDst, INT yDst, INT width,
                    INT height, HDC hdcSrc, INT xSrc, INT ySrc, DWORD rop )
{
    if (!rop_uses_src( rop )) return PatBlt_k( hdcDst, xDst, yDst, width, height, rop );
    else return StretchBlt_k( hdcDst, xDst, yDst, width, height,
 ...
 *           StretchBlt    (GDI32.@)
 */
BOOL WINAPI StretchBlt_k( HDC hdcDst, INT xDst, INT yDst, INT widthDst, INT heightDst,
                        HDC hdcSrc, INT xSrc, INT ySrc, INT widthSrc, INT heightSrc, DWORD rop )
{
    BOOL ret = FALSE;
    DC *dcDst, *dcSrc;

    if (!rop_uses_src( rop )) return PatBlt_k( hdcDst, xDst, yDst, widthDst, heightDst, rop );
 ...
 *           MaskBlt [GDI32.@]
 */
BOOL WINAPI MaskBlt_k(HDC hdcDest, INT nXDest, INT nYDest,
			...
			INT xMask, INT yMask, DWORD dwRop)
{
    ...
    if (!hbmMask)
	return BitBlt_k(hdcDest, nXDest, nYDest, nWidth, nHeight, hdcSrc, nXSrc, nYSrc, FRGND_ROP3(dwRop));

    hbrMask = CreatePatternBrush_k(hbmMask);
    hbrDst = SelectObject_k(hdcDest, GetStockObject_k(NULL_BRUSH));

    /* make bitmap */
    hDC1 = CreateCompatibleDC_k(hdcDest);
    hBitmap1 = CreateCompatibleBitmap_k(hdcDest, nWidth, nHeight);
    hOldBitmap1 = SelectObject_k(hDC1, hBitmap1);

    /* draw using bkgnd rop */
    BitBlt_k(hDC1, 0, 0, nWidth, nHeight, hdcDest, nXDest, nYDest, SRCCOPY);
    hbrTmp = SelectObject_k(hDC1, hbrDst);
    BitBlt_k(hDC1, 0, 0, nWidth, nHeight, hdcSrc, nXSrc, nYSrc, BKGND_ROP3(dwRop));
    SelectObject_k(hDC1, hbrTmp);

    /* make bitmap */
    hDC2 = CreateCompatibleDC_k(hdcDest);
    hBitmap2 = CreateCompatibleBitmap_k(hdcDest, nWidth, nHeight);
    hOldBitmap2 = SelectObject_k(hDC2, hBitmap2);

    /* draw using foregnd rop */
    BitBlt_k(hDC2, 0, 0, nWidth, nHeight, hdcDest, nXDest, nYDest, SRCCOPY);
    hbrTmp = SelectObject_k(hDC2, hbrDst);
    BitBlt_k(hDC2, 0, 0, nWidth, nHeight, hdcSrc, nXSrc, nYSrc, FRGND_ROP3(dwRop));

    /* combine both using the mask as a pattern brush */
    SelectObject_k(hDC2, hbrMask);
    BitBlt_k(hDC2, 0, 0, nWidth, nHeight, hDC1, 0, 0, 0xac0744 ); /* (D & P) | (S & ~P) */ 
    SelectObject_k(hDC2, hbrTmp);

    /* blit to dst */
    BitBlt_k(hdcDest, nXDest, nYDest, nWidth, nHeight, hDC2, 0, 0, SRCCOPY);

    /* restore all objects */
    SelectObject_k(hdcDest, hbrDst);
    SelectObject_k(hDC1, hOldBitmap1);
    SelectObject_k(hDC2, hOldBitmap2);

    /* delete all temp objects */
    DeleteObject_k(hBitmap1);
    DeleteObject_k(hBitmap2);
    DeleteObject_k(hbrMask);
 ...
 */
BOOL WINAPI GdiTransparentBlt_k( HDC hdcDest, int xDest, int yDest, int widthDest, int heightDest,
                            ...
                            UINT crTransparent )
{
    ...
    oldBackground = SetBkColor_k(hdcDest, RGB(255,255,255));
    oldForeground = SetTextColor_k(hdcDest, RGB(0,0,0));

    /* Stretch bitmap */
    oldStretchMode = GetStretchBltMode_k(hdcSrc);
    if(oldStretchMode == BLACKONWHITE || oldStretchMode == WHITEONBLACK)
        SetStretchBltMode_k(hdcSrc, COLORONCOLOR);
    hdcWork = CreateCompatibleDC_k(hdcDest);
    bmpWork = CreateCompatibleBitmap_k(hdcDest, widthDest, heightDest);
    oldWork = SelectObject_k(hdcWork, bmpWork);
    if(!StretchBlt_k(hdcWork, 0, 0, widthDest, heightDest, hdcSrc, xSrc, ySrc, widthSrc, heightSrc, SRCCOPY)) {
        TRACE("Failed to stretch\n");
        goto error;
    }
    SetBkColor_k(hdcWork, crTransparent);

    /* Create mask */
    hdcMask = CreateCompatibleDC_k(hdcDest);
    bmpMask = CreateCompatibleBitmap_k(hdcMask, widthDest, heightDest);
    oldMask = SelectObject_k(hdcMask, bmpMask);
    if(!BitBlt_k(hdcMask, 0, 0, widthDest, heightDest, hdcWork, 0, 0, SRCCOPY)) {
        TRACE("Failed to create mask\n");
        goto error;
    }

    /* Replace transparent color with black */
    SetBkColor_k(hdcWork, RGB(0,0,0));
    SetTextColor_k(hdcWork, RGB(255,255,255));
    if(!BitBlt_k(hdcWork, 0, 0, widthDest, heightDest, hdcMask, 0, 0, SRCAND)) {
        TRACE("Failed to mask out background\n");
        goto error;
    }

    /* Replace non-transparent area on destination with black */
    if(!BitBlt_k(hdcDest, xDest, yDest, widthDest, heightDest, hdcMask, 0, 0, SRCAND)) {
        TRACE("Failed to clear destination area\n");
        goto error;
    }

    /* Draw the image */
    if(!BitBlt_k(hdcDest, xDest, yDest, widthDest, heightDest, hdcWork, 0, 0, SRCPAINT)) {
    ...
    ret = TRUE;
error:
    SetStretchBltMode_k(hdcSrc, oldStretchMode);
    SetBkColor_k(hdcDest, oldBackground);
    SetTextColor_k(hdcDest, oldForeground);
    if(hdcWork) {
        SelectObject_k(hdcWork, oldWork);
        DeleteDC_k(hdcWork);
    }
    if(bmpWork) DeleteObject_k(bmpWork);
    if(hdcMask) {
        SelectObject_k(hdcMask, oldMask);
        DeleteDC_k(hdcMask);
    }
    if(bmpMask) DeleteObject_k(bmpMask);
    return ret;
}
 ...
 *           GdiAlphaBlend [GDI32.@]
 */
BOOL WINAPI GdiAlphaBlend_k(HDC hdcDst, int xDst, int yDst, int widthDst, int heightDst,
                          HDC hdcSrc, int xSrc, int ySrc, int widthSrc, int heightSrc,
                          BLENDFUNCTION blendFunction)
{
    ...
    if ((dcDst = get_dc_ptr( hdcDst )))
    {
        ...
        src.log_width  = widthSrc;
        src.log_height = heightSrc;
        src.layout     = GetLayout_k( hdcSrc );
        dst.log_x      = xDst;
        dst.log_y      = yDst;
        dst.log_width  = widthDst;
        dst.log_height = heightDst;
        dst.layout     = GetLayout_k( hdcDst );
        ret = !get_vis_rectangles( dcDst, &dst, dcSrc, &src );
 ...
 *
 */
BOOL WINAPI PlgBlt_k( HDC hdcDest, const POINT *lpPoint,
                        ...
                        INT nHeight, HBITMAP hbmMask, INT xMask, INT yMask)
{
    ...
    oldgMode = SetGraphicsMode_k(hdcDest,GM_ADVANCED);
    if (oldgMode == 0)
        return FALSE;
    ...
    if (fabs(det) < 1e-5)
    {
        SetGraphicsMode_k(hdcDest,oldgMode);
        return FALSE;
    }
    ...
    /* X components */
    xf.eM11 = (FLOAT)((plg[1].x*(rect[2].y - rect[0].y) - plg[2].x*(rect[1].y - rect[0].y) - plg[0].x*(rect[2].y - rect[1].y)) / det);
    xf.eM21 = (FLOAT)((rect[1].x*(plg[2].x - plg[0].x) - rect[2].x*(plg[1].x - plg[0].x) - rect[0].x*(plg[2].x - plg[1].x)) / det);
    xf.eDx  = (FLOAT)((rect[0].x*(rect[1].y*plg[2].x - rect[2].y*plg[1].x) - rect[1].x*(rect[0].y*plg[2].x - rect[2].y*plg[0].x) + rect[2].x*(rect[0].y*plg[1].x - rect[1].y*plg[0].x)) / det);

    /* Y components */
    xf.eM12 = (FLOAT)((plg[1].y*(rect[2].y - rect[0].y) - plg[2].y*(rect[1].y - rect[0].y) - plg[0].y*(rect[2].y - rect[1].y)) / det);
    xf.eM22 = (FLOAT)((plg[1].x*(rect[2].y - rect[0].y) - plg[2].x*(rect[1].y - rect[0].y) - plg[0].x*(rect[2].y - rect[1].y)) / det);
    xf.eDy  = (FLOAT)((rect[0].x*(rect[1].y*plg[2].y - rect[2].y*plg[1].y) - rect[1].x*(rect[0].y*plg[2].y - rect[2].y*plg[0].y) + rect[2].x*(rect[0].y*plg[1].y - rect[1].y*plg[0].y) ) / det);

    GetWorldTransform_k(hdcSrc,&SrcXf);
    CombineTransform_k(&xf,&xf,&SrcXf);

    /* save actual dest transform */
    GetWorldTransform_k(hdcDest,&oldDestXf);

    SetWorldTransform_k(hdcDest,&xf);
    /* now destination and source DCs use same coords */
    MaskBlt_k(hdcDest,nXSrc,nYSrc,nWidth,nHeight,
            hdcSrc, nXSrc,nYSrc,
            hbmMask,xMask,yMask,
            SRCCOPY);
    /* restore dest DC */
    SetWorldTransform_k(hdcDest,&oldDestXf);
    SetGraphicsMode_k(hdcDest,oldgMode);
