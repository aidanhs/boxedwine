...
{
    HeapFree_k( GetProcessHeap_k(), 0, path->points );
    HeapFree_k( GetProcessHeap_k(), 0, path->flags );
    HeapFree_k( GetProcessHeap_k(), 0, path );
}

static struct gdi_path *alloc_gdi_path( int count )
{
    struct gdi_path *path = HeapAlloc_k( GetProcessHeap_k(), 0, sizeof(*path) );
    ...
    path->points = HeapAlloc_k( GetProcessHeap_k(), 0, count * sizeof(*path->points) );
    path->flags = HeapAlloc_k( GetProcessHeap_k(), 0, count * sizeof(*path->flags) );
...
static struct gdi_path *copy_gdi_path( const struct gdi_path *src_path )
{
    ...
    path->points = HeapAlloc_k( GetProcessHeap_k(), 0, path->count * sizeof(*path->points) );
    path->flags = HeapAlloc_k( GetProcessHeap_k(), 0, path->count * sizeof(*path->flags) );
    if (!path->points || !path->flags)
    {
        free_gdi_path( path );
        SetLastError_k( ERROR_NOT_ENOUGH_MEMORY );
...
static BOOL PATH_ReserveEntries(struct gdi_path *pPath, INT count)
{
    ...
    if(count > pPath->allocated)
    {
        ...
        pPointsNew = HeapReAlloc_k( GetProcessHeap_k(), 0, pPath->points, count * sizeof(POINT) );
        if (!pPointsNew) return FALSE;
        pPath->points = pPointsNew;

        pFlagsNew = HeapReAlloc_k( GetProcessHeap_k(), 0, pPath->flags, count * sizeof(BYTE) );
...
static BYTE *add_log_points( struct path_physdev *physdev, const POINT *points, DWORD count, BYTE type )
{
    ...
    LPtoDP_k( physdev->dev.hdc, &path->points[path->count], count );
...
static BOOL start_new_stroke( struct path_physdev *physdev )
{
    ...
    GetCurrentPositionEx_k( physdev->dev.hdc, &pos );
...
static void PATH_CheckCorners( HDC hdc, POINT corners[], INT x1, INT y1, INT x2, INT y2 )
{
    ...
    if (GetGraphicsMode_k( hdc ) == GM_COMPATIBLE)
    {
        corners[1].x--;
        corners[1].y--;
    }
...
static HRGN PATH_PathToRegion(const struct gdi_path *pPath, INT nPolyFillMode)
{
    ...
    pNumPointsInStroke=HeapAlloc_k( GetProcessHeap_k(), 0, sizeof(int) * numStrokes );
    ...
    hrgn=CreatePolyPolygonRgn_k(rgn_path->points, pNumPointsInStroke,
                              numStrokes, nPolyFillMode);

    HeapFree_k(GetProcessHeap_k(), 0, pNumPointsInStroke );
    free_gdi_path( rgn_path );
    return hrgn;
}
 ...
 *           BeginPath    (GDI32.@)
 */
BOOL WINAPI BeginPath_k(HDC hdc)
{
    BOOL ret = FALSE;
    DC *dc = get_dc_ptr( hdc );
 ...
 *    Failure: FALSE
 */
BOOL WINAPI AbortPath_k( HDC hdc )
{
    BOOL ret = FALSE;
    DC *dc = get_dc_ptr( hdc );
 ...
 */
BOOL WINAPI CloseFigure_k(HDC hdc)
{
    BOOL ret = FALSE;
    DC *dc = get_dc_ptr( hdc );
 ...
 *           GetPath    (GDI32.@)
 */
INT WINAPI GetPath_k(HDC hdc, LPPOINT pPoints, LPBYTE pTypes, INT nSize)
{
   INT ret = -1;
   DC *dc = get_dc_ptr( hdc );
   ...
   if (!dc->path)
   {
      SetLastError_k(ERROR_CAN_NOT_COMPLETE);
      goto done;
   }
   ...
   else
   {
      ...
      if(!DPtoLP_k(hdc, pPoints, dc->path->count))
      {
 ...
 */
HRGN WINAPI PathToRegion_k(HDC hdc)
{
   HRGN  hrgnRval = 0;
   DC *dc = get_dc_ptr( hdc );
   ...
   if (!dc->path) SetLastError_k(ERROR_CAN_NOT_COMPLETE);
   else
   {
       if ((hrgnRval = PATH_PathToRegion(dc->path, GetPolyFillMode_k(hdc))))
       {
...
static BOOL PATH_FillPath( HDC hdc, const struct gdi_path *pPath )
{
   ...
   if ((hrgn = PATH_PathToRegion(pPath, GetPolyFillMode_k(hdc))))
   {
      ...
      mapMode=GetMapMode_k(hdc);
      GetViewportExtEx_k(hdc, &ptViewportExt);
      GetViewportOrgEx_k(hdc, &ptViewportOrg);
      GetWindowExtEx_k(hdc, &ptWindowExt);
      GetWindowOrgEx_k(hdc, &ptWindowOrg);
       ...
       */
      GetWorldTransform_k(hdc, &xform);

      /* Set MM_TEXT */
      SetMapMode_k(hdc, MM_TEXT);
      SetViewportOrgEx_k(hdc, 0, 0, NULL);
      SetWindowOrgEx_k(hdc, 0, 0, NULL);
      graphicsMode=GetGraphicsMode_k(hdc);
      SetGraphicsMode_k(hdc, GM_ADVANCED);
      ModifyWorldTransform_k(hdc, &xform, MWT_IDENTITY);
      SetGraphicsMode_k(hdc, graphicsMode);

      /* Paint the region */
      PaintRgn_k(hdc, hrgn);
      DeleteObject_k(hrgn);
      /* Restore the old mapping mode */
      SetMapMode_k(hdc, mapMode);
      SetViewportExtEx_k(hdc, ptViewportExt.cx, ptViewportExt.cy, NULL);
      SetViewportOrgEx_k(hdc, ptViewportOrg.x, ptViewportOrg.y, NULL);
      SetWindowExtEx_k(hdc, ptWindowExt.cx, ptWindowExt.cy, NULL);
      SetWindowOrgEx_k(hdc, ptWindowOrg.x, ptWindowOrg.y, NULL);

      /* Go to GM_ADVANCED temporarily to restore the world transform */
      graphicsMode=GetGraphicsMode_k(hdc);
      SetGraphicsMode_k(hdc, GM_ADVANCED);
      SetWorldTransform_k(hdc, &xform);
      SetGraphicsMode_k(hdc, graphicsMode);
      return TRUE;
   }
   return FALSE;
 ...
 *    Check that SetLastError is being called correctly
 */
BOOL WINAPI FillPath_k(HDC hdc)
{
    BOOL ret = FALSE;
    DC *dc = get_dc_ptr( hdc );
 ...
 *  Check that SetLastError is being called correctly
 */
BOOL WINAPI SelectClipPath_k(HDC hdc, INT iMode)
{
    BOOL ret = FALSE;
    DC *dc = get_dc_ptr( hdc );
...
static BOOL pathdrv_AbortPath( PHYSDEV dev )
{
    ...
    HeapFree_k(GetProcessHeap_k(), 0, physdev );
    release_dc_ptr( dc );
    return TRUE;
}
...
{
    struct path_physdev *physdev = HeapAlloc_k( GetProcessHeap_k(), 0, sizeof(*physdev) );
    DC *dc;
                      ...
                      INT xStart, INT yStart, INT xEnd, INT yEnd, INT lines )
{
    ...
    POINT centre;
    BOOL start, end;
    INT direction = GetArcDirection_k(dev->hdc);
    double temp;
   ...
   /* In GM_COMPATIBLE, don't include bottom and right edges */
   if (GetGraphicsMode_k(dev->hdc) == GM_COMPATIBLE)
   {
      corners[1].x--;
      corners[1].y--;
   }
   ...
   {
      centre.x = (LONG)((corners[0].x+corners[1].x)/2);
      centre.y = (LONG)((corners[0].y+corners[1].y)/2);
...
static BOOL pathdrv_AngleArc( PHYSDEV dev, INT x, INT y, DWORD radius, FLOAT eStartAngle, FLOAT eSweepAngle)
{
    ...
    arcdir = SetArcDirection_k( dev->hdc, eSweepAngle >= 0 ? AD_COUNTERCLOCKWISE : AD_CLOCKWISE);
    ret = PATH_Arc( dev, x-radius, y-radius, x+radius, y+radius, x1, y1, x2, y2, -1 );
    SetArcDirection_k( dev->hdc, arcdir );
    return ret;
}
...
{
    return PATH_Arc( dev, x1, y1, x2, y2, x1, (y1+y2)/2, x1, (y1+y2)/2, 0 ) && CloseFigure_k( dev->hdc );
}
...
static BOOL pathdrv_PolyDraw( PHYSDEV dev, const POINT *pts, const BYTE *types, DWORD cbPoints )
{
    ...
    GetCurrentPositionEx_k( dev->hdc, &orig_pos );
    lastmove = orig_pos;

    for(i = physdev->path->count - 1; i >= 0; i--){
        if(physdev->path->flags[i] == PT_MOVETO){
            lastmove = physdev->path->points[i];
            DPtoLP_k(dev->hdc, &lastmove, 1);
            break;
        }
    }

    for(i = 0; (DWORD)i < cbPoints; i++)
    {
        switch (types[i])
        {
        case PT_MOVETO:
            MoveToEx_k( dev->hdc, pts[i].x, pts[i].y, NULL );
            break;
        case PT_LINETO:
        case PT_LINETO | PT_CLOSEFIGURE:
            LineTo_k( dev->hdc, pts[i].x, pts[i].y );
            break;
        case PT_BEZIERTO:
            if (((DWORD)i + 2 < cbPoints) && (types[i + 1] == PT_BEZIERTO) &&
                (types[i + 2] & ~PT_CLOSEFIGURE) == PT_BEZIERTO)
            {
                PolyBezierTo_k( dev->hdc, &pts[i], 3 );
                i += 2;
                break;
            }
            /* fall through */
        default:
            if (i)  /* restore original position */
            {
                ...
                if (lastmove.x != orig_pos.x || lastmove.y != orig_pos.y)
                    MoveToEx_k( dev->hdc, orig_pos.x, orig_pos.y, NULL );
            }
            return FALSE;
        }
        ...
        if(types[i] & PT_CLOSEFIGURE){
            ...
            MoveToEx_k( dev->hdc, lastmove.x, lastmove.y, NULL );
        }
    }
                             ...
                             TTPOLYGONHEADER *header, DWORD size)
{
    ...
    while ((char *)header < (char *)start + size)
    {
        ...
        while ((char *)curve < (char *)header + header->cb)
        {
            ...
            case TT_PRIM_CSPLINE:
            {
                WORD i;
                POINTFX ptfx;
                POINT *pts = HeapAlloc_k(GetProcessHeap_k(), 0, (curve->cpfx + 1) * sizeof(POINT));
    ...
    return CloseFigure_k(physdev->dev.hdc);
}
                                ...
                                LPCWSTR str, UINT count, const INT *dx )
{
    ...
    for (idx = 0; idx < count; idx++)
    {
        ...
        dwSize = GetGlyphOutlineW_k(dev->hdc, str[idx], ggo_flags, &gm, 0, NULL, &identity);
        if (dwSize == GDI_ERROR) return FALSE;

        /* add outline only if char is printable */
        if(dwSize)
        {
            outline = HeapAlloc_k(GetProcessHeap_k(), 0, dwSize);
            if (!outline) return FALSE;

            GetGlyphOutlineW_k(dev->hdc, str[idx], ggo_flags, &gm, dwSize, outline, &identity);
            PATH_add_outline(physdev, x + offset.x, y + offset.y, outline, dwSize);

            HeapFree_k(GetProcessHeap_k(), 0, outline);
        }
...
static BOOL PATH_StrokePath( HDC hdc, const struct gdi_path *pPath )
{
    ...
    mapMode=GetMapMode_k(hdc);
    GetViewportExtEx_k(hdc, &szViewportExt);
    GetViewportOrgEx_k(hdc, &ptViewportOrg);
    GetWindowExtEx_k(hdc, &szWindowExt);
    GetWindowOrgEx_k(hdc, &ptWindowOrg);
    GetWorldTransform_k(hdc, &xform);
    ...
    pLinePts = HeapAlloc_k(GetProcessHeap_k(), 0, nAlloc * sizeof(POINT));
    nLinePts = 0;
	...
	case PT_MOVETO:
	    ...
	    if(nLinePts >= 2)
	        Polyline_k(hdc, pLinePts, nLinePts);
	    nLinePts = 0;
	...
	case PT_BEZIERTO:
		...
		{
		    nAlloc = nMinAlloc * 2;
		    pLinePts = HeapReAlloc_k(GetProcessHeap_k(), 0, pLinePts,
		                           nAlloc * sizeof(POINT));
		}
		memcpy(&pLinePts[nLinePts], &pBzrPts[1],
		       (nBzrPts - 1) * sizeof(POINT));
		nLinePts += nBzrPts - 1;
		HeapFree_k(GetProcessHeap_k(), 0, pBzrPts);
		i += 2;
	    }
	    break;
	default:
    ...
    HeapFree_k(GetProcessHeap_k(), 0, pLinePts);

    /* Restore the old mapping mode */
    SetMapMode_k(hdc, mapMode);
    SetWindowExtEx_k(hdc, szWindowExt.cx, szWindowExt.cy, NULL);
    SetWindowOrgEx_k(hdc, ptWindowOrg.x, ptWindowOrg.y, NULL);
    SetViewportExtEx_k(hdc, szViewportExt.cx, szViewportExt.cy, NULL);
    SetViewportOrgEx_k(hdc, ptViewportOrg.x, ptViewportOrg.y, NULL);
...
static struct gdi_path *PATH_WidenPath(DC *dc)
{
    ...
    size = GetObjectW_k( dc->hPen, 0, NULL );
    if (!size) {
        SetLastError_k(ERROR_CAN_NOT_COMPLETE);
        return NULL;
    }

    elp = HeapAlloc_k(GetProcessHeap_k(), 0, size );
    GetObjectW_k( dc->hPen, size, elp );

    obj_type = GetObjectType_k(dc->hPen);
    if(obj_type == OBJ_PEN) {
        ...
        SetLastError_k(ERROR_CAN_NOT_COMPLETE);
        HeapFree_k(GetProcessHeap_k(), 0, elp );
        return NULL;
    }
    ...
    for(i = 0, j = 0; i < flat_path->count; i++, j++) {
        ...
        switch(flat_path->flags[i]) {
            case PT_MOVETO:
                ...
                j = 0;
                if(numStrokes == 1)
                    pStrokes = HeapAlloc_k(GetProcessHeap_k(), 0, sizeof(*pStrokes));
                else
                    pStrokes = HeapReAlloc_k(GetProcessHeap_k(), 0, pStrokes, numStrokes * sizeof(*pStrokes));
    ...
    for(i = 0; i < numStrokes; i++) {
            ...
            if((!(pStrokes[i]->flags[pStrokes[i]->count - 1] & PT_CLOSEFIGURE)) && (j == 0 || j == pStrokes[i]->count - 1) ) {
                ...
                switch(endcap) {
                    case PS_ENDCAP_SQUARE :
                        pt.x = (LONG)(xo + round(sqrt(2) * penWidthOut * cos(M_PI_4 + theta)));
                        pt.y = (LONG)(yo + round(sqrt(2) * penWidthOut * sin(M_PI_4 + theta)));
                        PATH_AddEntry(pUpPath, &pt, (j == 0 ? PT_MOVETO : PT_LINETO) );
                        pt.x = (LONG)(xo + round(sqrt(2) * penWidthIn * cos(- M_PI_4 + theta)));
                        pt.y = (LONG)(yo + round(sqrt(2) * penWidthIn * sin(- M_PI_4 + theta)));
                        PATH_AddEntry(pUpPath, &pt, PT_LINETO);
                        break;
                    case PS_ENDCAP_FLAT :
                        pt.x = (LONG)(xo + round( penWidthOut * cos(theta + M_PI_2) ));
                        pt.y = (LONG)(yo + round( penWidthOut * sin(theta + M_PI_2) ));
                        PATH_AddEntry(pUpPath, &pt, (j == 0 ? PT_MOVETO : PT_LINETO));
                        pt.x = (LONG)(xo - round( penWidthIn * cos(theta + M_PI_2) ));
                        pt.y = (LONG)(yo - round( penWidthIn * sin(theta + M_PI_2) ));
                        PATH_AddEntry(pUpPath, &pt, PT_LINETO);
                        break;
		...
		struct gdi_path *pInsidePath, *pOutsidePath;
                ...
                if(alpha > 0) {
                    pt.x = (LONG)(xo + round( penWidthIn * cos(M_PI_2 + alpha + theta) ));
                    pt.y = (LONG)(yo + round( penWidthIn * sin(M_PI_2 + alpha + theta) ));
                }
                else {
                    pt.x = (LONG)(xo - round( penWidthIn * cos(M_PI_2 + alpha + theta) ));
                    pt.y = (LONG)(yo - round( penWidthIn * sin(M_PI_2 + alpha + theta) ));
                ...
                switch(_joint) {
                     case PS_JOIN_MITER :
                        ...
                        pt.x = (LONG)(xo + round( miterWidth * cos(theta + alpha / 2) ));
                        pt.y = (LONG)(yo + round( miterWidth * sin(theta + alpha / 2) ));
                    ...
                    case PS_JOIN_BEVEL :
                        ...
                        PATH_AddEntry(pOutsidePath, &pt, PT_LINETO);
                        if(alpha > 0) {
                            pt.x = (LONG)(xo - round( penWidthOut * cos(M_PI_2 + alpha + theta) ));
                            pt.y = (LONG)(yo - round( penWidthOut * sin(M_PI_2 + alpha + theta) ));
                        }
                        else {
                            pt.x = (LONG)(xo + round( penWidthOut * cos(M_PI_2 + alpha + theta) ));
                            pt.y = (LONG)(yo + round( penWidthOut * sin(M_PI_2 + alpha + theta) ));
                    ...
                    default :
                        ...
                        pt.x = (LONG)(xo + round( penWidthOut * cos(theta + alpha / 2) ));
                        pt.y = (LONG)(yo + round( penWidthOut * sin(theta + alpha / 2) ));
                        PATH_AddEntry(pOutsidePath, &pt, PT_BEZIERTO);
                        if(alpha > 0) {
                            pt.x = (LONG)(xo - round( penWidthOut * cos(M_PI_2 + alpha + theta) ));
                            pt.y = (LONG)(yo - round( penWidthOut * sin(M_PI_2 + alpha + theta) ));
    ...
    HeapFree_k(GetProcessHeap_k(), 0, pStrokes);
    free_gdi_path( flat_path );
    return pNewPath;
}
...
BOOL nulldrv_CloseFigure_k( PHYSDEV dev )
{
    SetLastError_k( ERROR_CAN_NOT_COMPLETE );
    return FALSE;
}
...
BOOL nulldrv_SelectClipPath( PHYSDEV dev, INT mode )
{
    ...
    if (!(hrgn = PATH_PathToRegion( dc->path, GetPolyFillMode_k(dev->hdc)))) return FALSE;
    ret = ExtSelectClipRgn_k( dev->hdc, hrgn, mode ) != ERROR;
    if (ret)
    {
        free_gdi_path( dc->path );
        dc->path = NULL;
