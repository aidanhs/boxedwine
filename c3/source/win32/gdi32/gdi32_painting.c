...
BOOL nulldrv_AngleArc( PHYSDEV dev, INT x, INT y, DWORD radius, FLOAT start, FLOAT sweep )
{
    ...
    INT arcdir = SetArcDirection_k( dev->hdc, sweep >= 0 ? AD_COUNTERCLOCKWISE : AD_CLOCKWISE );
    BOOL ret = ArcTo_k( dev->hdc, x - radius, y - radius, x + radius, y + radius, x1, y1, x2, y2 );
                    ...
                    INT xstart, INT ystart, INT xend, INT yend )
{
    ...
    LineTo_k( dev->hdc, GDI_ROUND( xcenter + cos(angle) * xradius ),
            GDI_ROUND( ycenter + sin(angle) * yradius ));
    return Arc_k( dev->hdc, left, top, right, bottom, xstart, ystart, xend, yend );
}
...
BOOL nulldrv_FillRgn( PHYSDEV dev, HRGN rgn, HBRUSH brush )
{
    ...
    if ((prev = SelectObject_k( dev->hdc, brush )))
    {
        ret = PaintRgn_k( dev->hdc, rgn );
        SelectObject_k( dev->hdc, prev );
    }
    return ret;
}
...
BOOL nulldrv_FrameRgn( PHYSDEV dev, HRGN rgn, HBRUSH brush, INT width, INT height )
{
    ...
    if (tmp)
    {
        if (REGION_FrameRgn( tmp, rgn, width, height )) ret = FillRgn_k( dev->hdc, tmp, brush );
        DeleteObject_k( tmp );
    }
    return ret;
}
...
{
    HBRUSH prev_brush = SelectObject_k( dev->hdc, GetStockObject_k(BLACK_BRUSH) );
    INT prev_rop = SetROP2_k( dev->hdc, R2_NOT );
    BOOL ret = PaintRgn_k( dev->hdc, rgn );
    SelectObject_k( dev->hdc, prev_brush );
    SetROP2_k( dev->hdc, prev_rop );
    return ret;
}
...
{
    BOOL ret = FALSE;
    POINT *pts = HeapAlloc_k( GetProcessHeap_k(), 0, sizeof(POINT) * (count + 1) );

    if (pts)
    {
        GetCurrentPositionEx_k( dev->hdc, &pts[0] );
        memcpy( pts + 1, points, sizeof(POINT) * count );
        ret = PolyBezier_k( dev->hdc, pts, count + 1 );
        HeapFree_k( GetProcessHeap_k(), 0, pts );
    }
    return ret;
}
...
BOOL nulldrv_PolyDraw( PHYSDEV dev, const POINT *points, const BYTE *types, DWORD count )
{
    ...
    space = count + 300;
    line_pts = HeapAlloc_k( GetProcessHeap_k(), 0, space * sizeof(POINT) );
    num_pts = 1;

    GetCurrentPositionEx_k( dev->hdc, &line_pts[0] );
    for (i = 0; i < count; i++)
    {
        switch (types[i])
        {
        case PT_MOVETO:
            if (num_pts >= 2) Polyline_k( dev->hdc, line_pts, num_pts );
            num_pts = 0;
        ...
        case PT_BEZIERTO:
            ...
            if ((bzr_pts = GDI_Bezier( bzr, 4, &num_bzr_pts )))
            {
                ...
                {
                    space = size * 2;
                    line_pts = HeapReAlloc_k( GetProcessHeap_k(), 0, line_pts, space * sizeof(POINT) );
                }
                memcpy( &line_pts[num_pts], &bzr_pts[1], (num_bzr_pts - 1) * sizeof(POINT) );
                num_pts += num_bzr_pts - 1;
                HeapFree_k( GetProcessHeap_k(), 0, bzr_pts );
            }
            i += 2;
            break;
        }
    ...
    if (num_pts >= 2) Polyline_k( dev->hdc, line_pts, num_pts );
    MoveToEx_k( dev->hdc, line_pts[num_pts - 1].x, line_pts[num_pts - 1].y, NULL );
    HeapFree_k( GetProcessHeap_k(), 0, line_pts );
    return TRUE;
}
...
BOOL nulldrv_PolylineTo( PHYSDEV dev, const POINT *points, INT count )
{
    ...
    if ((pts = HeapAlloc_k( GetProcessHeap_k(), 0, sizeof(POINT) * (count + 1) )))
    {
        GetCurrentPositionEx_k( dev->hdc, &pts[0] );
        memcpy( pts + 1, points, sizeof(POINT) * count );
        ret = Polyline_k( dev->hdc, pts, count + 1 );
        HeapFree_k( GetProcessHeap_k(), 0, pts );
    }
    return ret;
}
 ...
 *           LineTo    (GDI32.@)
 */
BOOL WINAPI LineTo_k( HDC hdc, INT x, INT y )
{
 ...
 *           MoveToEx    (GDI32.@)
 */
BOOL WINAPI MoveToEx_k( HDC hdc, INT x, INT y, LPPOINT pt )
{
    BOOL ret;
    PHYSDEV physdev;
    DC * dc = get_dc_ptr( hdc );
 ...
 *           Arc    (GDI32.@)
 */
BOOL WINAPI Arc_k( HDC hdc, INT left, INT top, INT right,
 ...
 *           Pie   (GDI32.@)
 */
BOOL WINAPI Pie_k( HDC hdc, INT left, INT top,
 ...
 *           Chord    (GDI32.@)
 */
BOOL WINAPI Chord_k( HDC hdc, INT left, INT top,
                       INT right, INT bottom, INT xstart, INT ystart,
                       INT xend, INT yend )
{
    BOOL ret;
 ...
 *           Ellipse    (GDI32.@)
 */
BOOL WINAPI Ellipse_k( HDC hdc, INT left, INT top,
                         INT right, INT bottom )
{
    BOOL ret;
    PHYSDEV physdev;
 ...
 *           Rectangle    (GDI32.@)
 */
BOOL WINAPI Rectangle_k( HDC hdc, INT left, INT top,
                           INT right, INT bottom )
{
    PHYSDEV physdev;
    BOOL ret;
 ...
 *           RoundRect    (GDI32.@)
 */
BOOL WINAPI RoundRect_k( HDC hdc, INT left, INT top, INT right,
 ...
 *           SetPixel    (GDI32.@)
 */
COLORREF WINAPI SetPixel_k( HDC hdc, INT x, INT y, COLORREF color )
{
    PHYSDEV physdev;
    COLORREF ret;
    DC * dc = get_dc_ptr( hdc );
 ...
 *           SetPixelV    (GDI32.@)
 */
BOOL WINAPI SetPixelV_k( HDC hdc, INT x, INT y, COLORREF color )
{
    PHYSDEV physdev;
    DC * dc = get_dc_ptr( hdc );
 ...
 *           GetPixel    (GDI32.@)
 */
COLORREF WINAPI GetPixel_k( HDC hdc, INT x, INT y )
{
    PHYSDEV physdev;
    COLORREF ret;
    DC * dc = get_dc_ptr( hdc );
 ...
 */
BOOL WINAPI GdiSetPixelFormat_k( HDC hdc, INT format, const PIXELFORMATDESCRIPTOR *descr )
{
    DC *dc;
    BOOL ret = TRUE;
 ...
 */
INT WINAPI GdiDescribePixelFormat_k( HDC hdc, INT format, UINT size, PIXELFORMATDESCRIPTOR *descr )
{
 ...
 */
BOOL WINAPI GdiSwapBuffers_k( HDC hdc )
{
    FIXME( "(%p): stub\n", hdc );
    return FALSE;
}
 ...
 *           PaintRgn    (GDI32.@)
 */
BOOL WINAPI PaintRgn_k( HDC hdc, HRGN hrgn )
{
    PHYSDEV physdev;
    BOOL ret;
    DC * dc = get_dc_ptr( hdc );
 ...
 *           FillRgn    (GDI32.@)
 */
BOOL WINAPI FillRgn_k( HDC hdc, HRGN hrgn, HBRUSH hbrush )
{
    PHYSDEV physdev;
    BOOL retval;
    DC * dc = get_dc_ptr( hdc );
 ...
 *           FrameRgn     (GDI32.@)
 */
BOOL WINAPI FrameRgn_k( HDC hdc, HRGN hrgn, HBRUSH hbrush,
 ...
 *           InvertRgn    (GDI32.@)
 */
BOOL WINAPI InvertRgn_k( HDC hdc, HRGN hrgn )
{
    PHYSDEV physdev;
    BOOL ret;
    DC *dc = get_dc_ptr( hdc );
 ...
 *          Polyline   (GDI32.@)
 */
BOOL WINAPI Polyline_k( HDC hdc, const POINT* pt, INT count )
{
    PHYSDEV physdev;
    BOOL ret;
    DC * dc = get_dc_ptr( hdc );
 ...
 *          PolylineTo   (GDI32.@)
 */
BOOL WINAPI PolylineTo_k( HDC hdc, const POINT* pt, DWORD cCount )
{
    DC * dc = get_dc_ptr( hdc );
    PHYSDEV physdev;
    BOOL ret;
 ...
 *          Polygon  (GDI32.@)
 */
BOOL WINAPI Polygon_k( HDC hdc, const POINT* pt, INT count )
{
    PHYSDEV physdev;
    BOOL ret;
    DC * dc = get_dc_ptr( hdc );
 ...
 *          PolyPolygon  (GDI32.@)
 */
BOOL WINAPI PolyPolygon_k( HDC hdc, const POINT* pt, const INT* counts,
                             UINT polygons )
{
    PHYSDEV physdev;
    BOOL ret;
 ...
 *          PolyPolyline  (GDI32.@)
 */
BOOL WINAPI PolyPolyline_k( HDC hdc, const POINT* pt, const DWORD* counts,
                            DWORD polylines )
{
    PHYSDEV physdev;
    BOOL ret;
 ...
 *          ExtFloodFill   (GDI32.@)
 */
BOOL WINAPI ExtFloodFill_k( HDC hdc, INT x, INT y, COLORREF color,
                              UINT fillType )
{
    PHYSDEV physdev;
    BOOL ret;
 ...
 *          FloodFill   (GDI32.@)
 */
BOOL WINAPI FloodFill_k( HDC hdc, INT x, INT y, COLORREF color )
{
    return ExtFloodFill_k( hdc, x, y, color, FLOODFILLBORDER );
}
 ...
 *    Failure: FALSE
 */
BOOL WINAPI PolyBezier_k( HDC hdc, const POINT* lppt, DWORD cPoints )
{
    PHYSDEV physdev;
    BOOL ret;
    DC * dc;
 ...
 *    Failure: FALSE
 */
BOOL WINAPI PolyBezierTo_k( HDC hdc, const POINT* lppt, DWORD cPoints )
{
    DC * dc;
    BOOL ret;
    PHYSDEV physdev;
 ...
 *      AngleArc (GDI32.@)
 */
BOOL WINAPI AngleArc_k(HDC hdc, INT x, INT y, DWORD dwRadius, FLOAT eStartAngle, FLOAT eSweepAngle)
{
    PHYSDEV physdev;
    BOOL result;
    DC *dc;
 ...
 *      PolyDraw (GDI32.@)
 */
BOOL WINAPI PolyDraw_k(HDC hdc, const POINT *lppt, const BYTE *lpbTypes,
                       DWORD cCount)
{
    DC *dc = get_dc_ptr( hdc );
    PHYSDEV physdev;
 ...
 *           LineDDA   (GDI32.@)
 */
BOOL WINAPI LineDDA_k(INT nXStart, INT nYStart, INT nXEnd, INT nYEnd,
				...
				INT *nPtsOut, INT level )
{
    if(*nPtsOut == *dwOut) {
        *dwOut *= 2;
	*PtsOut = HeapReAlloc_k( GetProcessHeap_k(), 0, *PtsOut,
			       *dwOut * sizeof(POINT) );
    }
...
POINT *GDI_Bezier( const POINT *Points, INT count, INT *nPtsOut )
{
    ...
    *nPtsOut = 0;
    out = HeapAlloc_k( GetProcessHeap_k(), 0, dwOut * sizeof(POINT));
 ...
 */
BOOL WINAPI GdiGradientFill_k( HDC hdc, TRIVERTEX *vert_array, ULONG nvert,
 ...
 *
 */
BOOL WINAPI GdiDrawStream_k( HDC hdc, ULONG in, void * pvin )
{
    FIXME("stub: %p, %d, %p\n", hdc, in, pvin);
    return FALSE;
}
