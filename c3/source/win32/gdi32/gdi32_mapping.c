...
static void MAPPING_FixIsotropic( DC * dc )
{
    ...
    if (xdim > ydim)
    {
        ...
        dc->vportExtX = (INT)(floor(dc->vportExtX * ydim / xdim + 0.5));
        if (!dc->vportExtX) dc->vportExtX = mincx;
    }
    else
    {
        INT mincy = (dc->vportExtY >= 0) ? 1 : -1;
        dc->vportExtY = (INT)(floor(dc->vportExtY * xdim / ydim + 0.5));
...
INT nulldrv_SetMapMode( PHYSDEV dev, INT mode )
{
    ...
    case MM_HIMETRIC:
        ...
        break;
    case MM_LOENGLISH:
        dc->wndExtX   = MulDiv_k(1000, horzSize, 254);
        dc->wndExtY   = MulDiv_k(1000, vertSize, 254);
        dc->vportExtX = horzRes;
        dc->vportExtY = -vertRes;
        break;
    case MM_HIENGLISH:
        dc->wndExtX   = MulDiv_k(10000, horzSize, 254);
        dc->wndExtY   = MulDiv_k(10000, vertSize, 254);
        dc->vportExtX = horzRes;
        dc->vportExtY = -vertRes;
        break;
    case MM_TWIPS:
        dc->wndExtX   = MulDiv_k(14400, horzSize, 254);
        dc->wndExtY   = MulDiv_k(14400, vertSize, 254);
        dc->vportExtX = horzRes;
        dc->vportExtY = -vertRes;
        break;
    case MM_ANISOTROPIC:
...
BOOL nulldrv_ModifyWorldTransform( PHYSDEV dev, const XFORM *xform, DWORD mode )
{
    ...
    case MWT_LEFTMULTIPLY:
        CombineTransform_k( &dc->xformWorld2Wnd, xform, &dc->xformWorld2Wnd );
        break;
    case MWT_RIGHTMULTIPLY:
        CombineTransform_k( &dc->xformWorld2Wnd, &dc->xformWorld2Wnd, xform );
        break;
    default:
        return FALSE;
    }
 ...
 *           DPtoLP    (GDI32.@)
 */
BOOL WINAPI DPtoLP_k( HDC hdc, LPPOINT points, INT count )
{
    ...
    if (dc->vport2WorldValid)
    {
        while (count--)
        {
            double x = points->x;
            double y = points->y;
            points->x = (LONG)(floor( x * dc->xformVport2World.eM11 +
                               y * dc->xformVport2World.eM21 +
                               dc->xformVport2World.eDx + 0.5 ));
            points->y = (LONG)(floor( x * dc->xformVport2World.eM12 +
 ...
 *           LPtoDP    (GDI32.@)
 */
BOOL WINAPI LPtoDP_k( HDC hdc, LPPOINT points, INT count )
{
    DC * dc = get_dc_ptr( hdc );
    if (!dc) return FALSE;
    ...
    while (count--)
    {
        double x = points->x;
        double y = points->y;
        points->x = (LONG)(floor( x * dc->xformWorld2Vport.eM11 +
                           y * dc->xformWorld2Vport.eM21 +
                           dc->xformWorld2Vport.eDx + 0.5 ));
        points->y = (LONG)(floor( x * dc->xformWorld2Vport.eM12 +
 ...
 *           SetMapMode    (GDI32.@)
 */
INT WINAPI SetMapMode_k( HDC hdc, INT mode )
{
    INT ret = 0;
    DC * dc = get_dc_ptr( hdc );
 ...
 */
BOOL WINAPI SetViewportExtEx_k( HDC hdc, INT x, INT y, LPSIZE size )
{
    INT ret = FALSE;
    DC * dc = get_dc_ptr( hdc );
 ...
 */
BOOL WINAPI SetViewportOrgEx_k( HDC hdc, INT x, INT y, LPPOINT pt )
{
    INT ret = FALSE;
    DC * dc = get_dc_ptr( hdc );
 ...
 */
BOOL WINAPI SetWindowExtEx_k( HDC hdc, INT x, INT y, LPSIZE size )
{
    INT ret = FALSE;
    DC * dc = get_dc_ptr( hdc );
 ...
 */
BOOL WINAPI SetWindowOrgEx_k( HDC hdc, INT x, INT y, LPPOINT pt )
{
    INT ret = FALSE;
    DC * dc = get_dc_ptr( hdc );
 ...
 */
BOOL WINAPI OffsetViewportOrgEx_k( HDC hdc, INT x, INT y, LPPOINT pt)
{
    INT ret = FALSE;
    DC * dc = get_dc_ptr( hdc );
 ...
 */
BOOL WINAPI OffsetWindowOrgEx_k( HDC hdc, INT x, INT y, LPPOINT pt )
{
    INT ret = FALSE;
    DC * dc = get_dc_ptr( hdc );
 ...
 */
BOOL WINAPI ScaleViewportExtEx_k( HDC hdc, INT xNum, INT xDenom,
 ...
 */
BOOL WINAPI ScaleWindowExtEx_k( HDC hdc, INT xNum, INT xDenom,
                                  INT yNum, INT yDenom, LPSIZE size )
{
    INT ret = FALSE;
    DC * dc = get_dc_ptr( hdc );
 ...
 */
BOOL WINAPI ModifyWorldTransform_k( HDC hdc, const XFORM *xform, DWORD mode )
{
    BOOL ret = FALSE;
    DC *dc;
 ...
 */
BOOL WINAPI SetWorldTransform_k( HDC hdc, const XFORM *xform )
{
    BOOL ret = FALSE;
    DC *dc;
 ...
 */
BOOL WINAPI SetVirtualResolution_k(HDC hdc, DWORD horz_res, DWORD vert_res,
                                 DWORD horz_size, DWORD vert_size)
{
    ...
    {
        horz_res  = GetDeviceCaps_k(hdc, HORZRES);
        vert_res  = GetDeviceCaps_k(hdc, VERTRES);
        horz_size = GetDeviceCaps_k(hdc, HORZSIZE);
        vert_size = GetDeviceCaps_k(hdc, VERTSIZE);
    }
