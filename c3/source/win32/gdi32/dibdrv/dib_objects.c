...
static COLORREF make_rgb_colorref( HDC hdc, const dib_info *dib, COLORREF color, BOOL *got_pixel, DWORD *pixel )
{
    ...
    *got_pixel = FALSE;
    ...
    {
        HPALETTE pal = GetCurrentObject_k( hdc, OBJ_PAL );
        PALETTEENTRY pal_ent;

        if (!GetPaletteEntries_k( pal, LOWORD(color), 1, &pal_ent ))
            GetPaletteEntries_k( pal, 0, 1, &pal_ent );
...
DWORD get_pixel_color( HDC hdc, const dib_info *dib, COLORREF color, BOOL mono_fixup )
{
    ...
    pixel = get_pixel_color( hdc, dib, GetBkColor_k(hdc), FALSE );
    if (color == GetBkColor_k(hdc)) return pixel;
    else return !pixel;
}
                                    ...
                                    INT bkgnd_mode, rop_mask *fg_mask, rop_mask *bg_mask )
{
    ...
    if (dib->bit_count != 1) color = get_pixel_color( hdc, dib, GetBkColor_k(hdc), FALSE );
    else if (colorref != GetBkColor_k(hdc)) color = !color;
...
static BOOL solid_pen_lines(dibdrv_physdev *pdev, int num, POINT *pts, BOOL close, HRGN region)
{
    ...
    else
    {
        ...
        calc_and_xor_masks( GetROP2_k(pdev->dev.hdc), color, &and, &xor );
...
static INLINE void skip_dash(dibdrv_physdev *pdev, unsigned int skip)
{
    ...
    do
    {
        if(pdev->dash_pos.left_in_dash > (int)skip)
        {
...
static BOOL dashed_pen_lines(dibdrv_physdev *pdev, int num, POINT *pts, BOOL close, HRGN region)
{
    ...
    {
        get_color_masks( pdev->dev.hdc, &pdev->dib, GetROP2_k(pdev->dev.hdc), pdev->pen_brush.colorref,
                         pdev->pen_is_ext ? TRANSPARENT : GetBkMode_k(pdev->dev.hdc),
...
static void add_cap( dibdrv_physdev *pdev, HRGN region, HRGN round_cap, const POINT *pt )
{
    ...
    case PS_ENDCAP_ROUND:
        OffsetRgn_k( round_cap, pt->x, pt->y );
        CombineRgn_k( region, region, round_cap, RGN_OR );
        OffsetRgn_k( round_cap, -pt->x, -pt->y );
        return;
                                 ...
                                 const struct face *face_1, const struct face *face_2 )
{
    ...
    GetMiterLimit_k( pdev->dev.hdc, &limit );
    ...
    pts[0] = face_2->start;
    pts[1] = face_1->start;
    pts[2].x = (LONG)round( x );
    pts[2].y = (LONG)round( y );
    pts[3] = face_2->end;
    pts[4] = face_1->end;

    return CreatePolygonRgn_k( pts, 5, ALTERNATE );
}
                      ...
                      const struct face *face_1, const struct face *face_2 )
{
    ...
    case PS_JOIN_BEVEL:
        ...
        pts[2] = face_1->end;
        pts[3] = face_2->start;
        join = CreatePolygonRgn_k( pts, 4, ALTERNATE );
        break;
    }

    CombineRgn_k( region, region, join, RGN_OR );
    DeleteObject_k( join );
    return;
}
                               ...
                               BOOL need_cap_1, BOOL need_cap_2, struct face *face_1, struct face *face_2 )
{
    ...
    else
    {
        ...
        narrow_half.x = (LONG)round( width_x / 2 );
        narrow_half.y = (LONG)round( width_y / 2 );
        wide_half.x   = (LONG)round( (width_x + 1) / 2 );
        wide_half.y   = (LONG)round( (width_y + 1) / 2 );
        ...
        segment = CreatePolygonRgn_k( seg_pts, 4, ALTERNATE );
        CombineRgn_k( total, total, segment, RGN_OR );
        DeleteObject_k( segment );
...
static BOOL wide_pen_lines(dibdrv_physdev *pdev, int num, POINT *pts, BOOL close, HRGN total)
{
    ...
    if (pdev->pen_join == PS_JOIN_ROUND || pdev->pen_endcap == PS_ENDCAP_ROUND)
        round_cap = CreateEllipticRgn_k( -(pdev->pen_width / 2), -(pdev->pen_width / 2),
    ...
    if (round_cap) DeleteObject_k( round_cap );
    return TRUE;
}
...
static BOOL dashed_wide_pen_lines(dibdrv_physdev *pdev, int num, POINT *pts, BOOL close, HRGN total)
{
    ...
    for (i = 0; i < (close ? num : num - 1); i++)
    {
        ...
        else
        {
            ...
            {
                skip_dash( pdev, (unsigned int)(len - cur_len) );
                cur_len = 0;
                continue;
            }
            cur_len += pdev->dash_pos.left_in_dash;
...
COLORREF dibdrv_SetDCPenColor( PHYSDEV dev, COLORREF color )
{
    ...
    if (GetCurrentObject_k(dev->hdc, OBJ_PEN) == GetStockObject_k( DC_PEN ))
...
static BOOL alloc_brush_mask_bits( dib_brush *brush )
{
    ...
    if (!(brush->masks.xor = HeapAlloc_k(GetProcessHeap_k(), 0, 2 * size))) return FALSE;
...
{
    if (brush->masks.xor != brush->dib.bits.ptr) HeapFree_k(GetProcessHeap_k(), 0, brush->masks.xor);
...
static BOOL create_hatch_brush_bits(dibdrv_physdev *pdev, dib_brush *brush, BOOL *needs_reselect)
{
    ...
    get_color_masks( pdev->dev.hdc, &pdev->dib, brush->rop, brush->colorref, GetBkMode_k(pdev->dev.hdc),
                     &fg_mask, &bg_mask );

    if (brush->colorref & (1 << 24))  /* PALETTEINDEX */
        *needs_reselect = TRUE;
    if (GetBkMode_k(pdev->dev.hdc) != TRANSPARENT && (GetBkColor_k(pdev->dev.hdc) & (1 << 24)))
        *needs_reselect = TRUE;
...
static BOOL select_pattern_brush( dibdrv_physdev *pdev, dib_brush *brush, BOOL *needs_reselect )
{
        ...
        (pdev->dib.bit_count != 1 || pdev->dib.color_table))
    {
        ...
        color = make_rgb_colorref( pdev->dev.hdc, &pdev->dib, GetTextColor_k( pdev->dev.hdc ),
                                   &got_pixel, &pixel );
        ...
        color = make_rgb_colorref( pdev->dev.hdc, &pdev->dib, GetBkColor_k( pdev->dev.hdc ),
                                   &got_pixel, &pixel );
    ...
    {
        brush->dib.bits.ptr     = HeapAlloc_k( GetProcessHeap_k(), 0, brush->dib.height * brush->dib.stride );
                          ...
                          int num, const RECT *rects, INT rop)
{
    ...
    GetBrushOrgEx_k(pdev->dev.hdc, &origin);
...
HBRUSH dibdrv_SelectBrush( PHYSDEV dev, HBRUSH hbrush, const struct brush_pattern *pattern )
{
    ...
    GetObjectW_k( hbrush, sizeof(logbrush), &logbrush );

    if (hbrush == GetStockObject_k( DC_BRUSH ))
        logbrush.lbColor = GetDCBrushColor_k( dev->hdc );
...
HPEN dibdrv_SelectPen( PHYSDEV dev, HPEN hpen, const struct brush_pattern *pattern )
{
    ...
    if (!GetObjectW_k( hpen, sizeof(logpen), &logpen ))
    {
        /* must be an extended pen */
        INT size = GetObjectW_k( hpen, 0, NULL );

        if (!size) return 0;

        elp = HeapAlloc_k( GetProcessHeap_k(), 0, size );

        GetObjectW_k( hpen, size, elp );
    ...
    if (hpen == GetStockObject_k( DC_PEN ))
        logbrush.lbColor = GetDCPenColor_k( dev->hdc );
...
COLORREF dibdrv_SetDCBrushColor( PHYSDEV dev, COLORREF color )
{
    ...
    if (GetCurrentObject_k(dev->hdc, OBJ_BRUSH) == GetStockObject_k( DC_BRUSH ))
    {
