...
static INLINE DWORD gradient_triangle_8888( const TRIVERTEX *v, int x, int y, int det )
{
    ...
    r = (BYTE)((v[0].Red   * l1 + v[1].Red   * l2 + v[2].Red   * (det - l1 - l2)) / det / 256);
    g = (BYTE)((v[0].Green * l1 + v[1].Green * l2 + v[2].Green * (det - l1 - l2)) / det / 256);
    b = (BYTE)((v[0].Blue  * l1 + v[1].Blue  * l2 + v[2].Blue  * (det - l1 - l2)) / det / 256);
    a = (BYTE)((v[0].Alpha * l1 + v[1].Alpha * l2 + v[2].Alpha * (det - l1 - l2)) / det / 256);
    return a << 24 | r << 16 | g << 8 | b;
}
...
static INLINE DWORD gradient_triangle_555( const TRIVERTEX *v, int x, int y, int det )
{
    ...
    triangle_weights( v, x, y, &l1, &l2 );
    r = (BYTE)((v[0].Red   * l1 + v[1].Red   * l2 + v[2].Red   * (det - l1 - l2)) / det / 128 + bayer_4x4[y % 4][x % 4]);
    g = (BYTE)((v[0].Green * l1 + v[1].Green * l2 + v[2].Green * (det - l1 - l2)) / det / 128 + bayer_4x4[y % 4][x % 4]);
    b = (BYTE)((v[0].Blue  * l1 + v[1].Blue  * l2 + v[2].Blue  * (det - l1 - l2)) / det / 128 + bayer_4x4[y % 4][x % 4]);
    r = min( 31, max( 0, r / 16 ));
    g = min( 31, max( 0, g / 16 ));
    b = min( 31, max( 0, b / 16 ));
    return (r << 10) | (g << 5) | b;
...
static INLINE DWORD gradient_triangle_8( const dib_info *dib, const TRIVERTEX *v, int x, int y, int det )
{
    ...
    triangle_weights( v, x, y, &l1, &l2 );
    r = (BYTE)(((v[0].Red   * l1 + v[1].Red   * l2 + v[2].Red   * (det - l1 - l2)) / det / 128 + bayer_16x16[y % 16][x % 16]) / 256);
    g = (BYTE)(((v[0].Green * l1 + v[1].Green * l2 + v[2].Green * (det - l1 - l2)) / det / 128 + bayer_16x16[y % 16][x % 16]) / 256);
    b = (BYTE)(((v[0].Blue  * l1 + v[1].Blue  * l2 + v[2].Blue  * (det - l1 - l2)) / det / 128 + bayer_16x16[y % 16][x % 16]) / 256);
    return rgb_to_pixel_colortable( dib, r * 127, g * 127, b * 127 );
}
