...
{
    if (pdev->clip) CombineRgn_k( region, region, pdev->clip, RGN_AND );
...
static RECT get_device_rect( HDC hdc, int left, int top, int right, int bottom, BOOL rtl_correction )
{
    ...
    rect.right  = right;
    rect.bottom = bottom;
    if (rtl_correction && GetLayout_k( hdc ) & LAYOUT_RTL)
    {
...
static int get_arc_points( PHYSDEV dev, const RECT *rect, POINT start, POINT end, POINT *points )
{
    ...
    if (GetArcDirection_k( dev->hdc ) != AD_CLOCKWISE)
    {
        start.y = -start.y;
        end.y = -end.y;
    }
                      ...
                      INT start_x, INT start_y, INT end_x, INT end_y, INT extra_lines )
{
    ...
    points = HeapAlloc_k( GetProcessHeap_k(), 0, (width + height) * 3 * sizeof(*points) );
    if (!points) return FALSE;

    if (extra_lines == -1)
    {
        GetCurrentPositionEx_k( dev->hdc, points );
        LPtoDP_k( dev->hdc, points, 1 );
    ...
    if (count < 2)
    {
        HeapFree_k( GetProcessHeap_k(), 0, points );
        return TRUE;
    }

    if (pdev->pen_uses_region && !(outline = CreateRectRgn_k( 0, 0, 0, 0 )))
    {
        HeapFree_k( GetProcessHeap_k(), 0, points );
        return FALSE;
    }
    ...
    if (pdev->brush.style != BS_NULL && extra_lines > 0 &&
        !(interior = CreatePolygonRgn_k( points, count, WINDING )))
    {
        HeapFree_k( GetProcessHeap_k(), 0, points );
        if (outline) DeleteObject_k( outline );
        return FALSE;
    }
    ...
    if (interior)
    {
        CombineRgn_k( interior, interior, outline, RGN_DIFF );
        ret = brush_region( pdev, interior );
        DeleteObject_k( interior );
    }
    if (outline)
...
{
    *min_comp = (BYTE)((ramp[aa] * text_comp) / 0xff);
    *max_comp = (BYTE)(ramp[16 - aa] + ((0xff - ramp[16 - aa]) * text_comp) / 0xff);
}
...
static struct cached_font *add_cached_font( HDC hdc, HFONT hfont, UINT aa_flags )
{
    ...
    GetObjectW_k( hfont, sizeof(font.lf), &font.lf );
    GetTransform_k( hdc, 0x204, &font.xform );
        ...
        for (i = 0; i < GLYPH_NBTYPES; i++)
        {
            for (j = 0; j < GLYPH_CACHE_PAGES; j++)
            {
                ...
                for (k = 0; k < GLYPH_CACHE_PAGE_SIZE; k++)
                    HeapFree_k( GetProcessHeap_k(), 0, ptr->glyphs[i][j][k] );
                HeapFree_k( GetProcessHeap_k(), 0, ptr->glyphs[i][j] );
            }
        }
        list_remove( &ptr->entry );
    }
    else if (!(ptr = HeapAlloc_k( GetProcessHeap_k(), 0, sizeof(*ptr) )))
    {
        LeaveCriticalSection_k( &font_cache_cs );
        return NULL;
    }
...
{
    if (font) InterlockedDecrement_k( &font->ref );
}
                                              ...
                                              struct cached_glyph *glyph )
{
    ...
    if (!font->glyphs[type][page])
    {
        ...
        ptr = HeapAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY, GLYPH_CACHE_PAGE_SIZE * sizeof(*ptr) );
        if (!ptr)
        {
            HeapFree_k( GetProcessHeap_k(), 0, glyph );
            return NULL;
        }
        if (InterlockedCompareExchangePointer_k( (void **)&font->glyphs[type][page], ptr, NULL ))
            HeapFree_k( GetProcessHeap_k(), 0, ptr );
    }
    ret = InterlockedCompareExchangePointer_k( (void **)&font->glyphs[type][page][entry], glyph, NULL );
    if (!ret) ret = glyph;
    else HeapFree_k( GetProcessHeap_k(), 0, glyph );
    return ret;
}
...
static INLINE void get_text_bkgnd_masks( HDC hdc, const dib_info *dib, rop_mask *mask )
{
    ...
    {
        COLORREF fg = GetTextColor_k( hdc );
...
static struct cached_glyph *cache_glyph_bitmap( HDC hdc, struct cached_font *font, UINT index, UINT flags )
{
    ...
    {
        index = indices[i];
        ret = GetGlyphOutlineW_k( hdc, index, ggo_flags, &metrics, 0, NULL, &identity );
    ...
    glyph = HeapAlloc_k( GetProcessHeap_k(), 0, FIELD_OFFSET( struct cached_glyph, bits[size] ));
    if (!glyph) return NULL;
    if (!ret) goto done;  /* zero-size glyph */

    if (bit_count == 8) pad = padding[ metrics.gmBlackBoxX % 4 ];

    ret = GetGlyphOutlineW_k( hdc, index, ggo_flags, &metrics, size, glyph->bits, &identity );
    if (ret == GDI_ERROR)
    {
        HeapFree_k( GetProcessHeap_k(), 0, glyph );
        return NULL;
    ...
    {
        for (y = 0, dst = glyph->bits; (UINT)y < metrics.gmBlackBoxY; y++, dst += stride)
                           ...
                           const struct clipped_rects *clipped_rects, RECT *bounds )
{
    ...
    text_color = get_pixel_color( hdc, dib, GetTextColor_k( hdc ), TRUE );
                                ...
                                UINT aa_flags, LPCWSTR str, UINT count, const INT *dx )
{
    ...
    if (!(font = add_cached_font( hdc, GetCurrentObject_k( hdc, OBJ_FONT ), aa_flags ))) return FALSE;
...
static INLINE BOOL is_interior( dib_info *dib, HRGN clip, int x, int y, DWORD pixel, UINT type)
{
    ...
    if (clip && !PtInRegion_k( clip, x, y )) return FALSE;
...
static INLINE void do_next_row( dib_info *dib, HRGN clip, const RECT *row, int offset, DWORD pixel, UINT type, HRGN rgn )
{
    ...
    while (next.right < row->right)
    {
        ...
        {
            if (next.left != next.right && !PtInRegion_k( rgn, next.left, next.top ))
...
BOOL dibdrv_ExtFloodFill( PHYSDEV dev, INT x, INT y, COLORREF color, UINT type )
{
    ...
    if (!(rgn = CreateRectRgn_k( 0, 0, 0, 0 ))) return FALSE;
    row.left = x;
    row.right = x + 1;
    row.top = y;
    row.bottom = y + 1;
...
BOOL dibdrv_LineTo( PHYSDEV dev, INT x, INT y )
{
    ...
    GetCurrentPositionEx_k(dev->hdc, pts);
    pts[1].x = x;
    pts[1].y = y;

    LPtoDP_k(dev->hdc, pts, 2);

    if (pdev->pen_uses_region && !(region = CreateRectRgn_k( 0, 0, 0, 0 ))) return FALSE;
...
BOOL dibdrv_PolyPolygon( PHYSDEV dev, const POINT *pt, const INT *counts, DWORD polygons )
{
    ...
    points = HeapAlloc_k( GetProcessHeap_k(), 0, total * sizeof(*pt) );
    if (!points) return FALSE;
    memcpy( points, pt, total * sizeof(*pt) );
    LPtoDP_k( dev->hdc, points, total );

    if (pdev->brush.style != BS_NULL &&
        !(interior = CreatePolyPolygonRgn_k( points, counts, polygons, GetPolyFillMode_k( dev->hdc ))))
    {
        HeapFree_k( GetProcessHeap_k(), 0, points );
        return FALSE;
    }
...
BOOL dibdrv_Rectangle( PHYSDEV dev, INT left, INT top, INT right, INT bottom )
{
    ...
    if (GetGraphicsMode_k( dev->hdc ) == GM_ADVANCED)
    {
    ...
    if (pdev->pen_uses_region && !(outline = CreateRectRgn_k( 0, 0, 0, 0 ))) return FALSE;
    ...
    if (outline)
    {
        if (pdev->brush.style != BS_NULL)
        {
            HRGN interior = CreateRectRgnIndirect_k( &rect );
                       ...
                       INT ellipse_width, INT ellipse_height )
{
    ...
    points = HeapAlloc_k( GetProcessHeap_k(), 0, (ellipse_width + ellipse_height) * 2 * sizeof(*points) );
    if (!points) return FALSE;
    ...
    if (pdev->brush.style != BS_NULL &&
        !(interior = CreateRoundRectRgn_k( rect.left, rect.top, rect.right + 1, rect.bottom + 1,
