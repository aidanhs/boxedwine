...
BOOL init_dib_info_from_bitmapobj(dib_info *dib, BITMAPOBJ *bmp)
{
    if (!is_bitmapobj_dib( bmp ))
    {
        ...
        if (!bmp->dib.dsBm.bmBits)
        {
            ...
            bmp->dib.dsBm.bmBits = HeapAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY,
...
int get_clipped_rects( const dib_info *dib, const RECT *rc, HRGN clip, struct clipped_rects *clip_rects )
{
    ...
    for (i = 0; i < region->numRects; i++)
    {
        ...
        {
            clip_rects->rects = HeapAlloc_k( GetProcessHeap_k(), 0, region->numRects * sizeof(RECT) );
...
{
    dibdrv_physdev *pdev = HeapAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(*pdev) );
...
static BOOL dibdrv_DeleteDC( PHYSDEV dev )
{
    ...
    HeapFree_k( GetProcessHeap_k(), 0, pdev );
    return TRUE;
}
...
{
    struct windrv_physdev *physdev = HeapAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(*physdev) );
