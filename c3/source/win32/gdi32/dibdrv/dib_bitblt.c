...
static DWORD copy_src_bits( dib_info *src, RECT *src_rect )
{
    ...
    void *ptr = HeapAlloc_k( GetProcessHeap_k(), 0, stride * height );
...
static DWORD create_tmp_dib( const dib_info *copy, int width, int height, dib_info *ret )
{
    ...
    ret->bits.ptr = HeapAlloc_k( GetProcessHeap_k(), 0, ret->height * ret->stride );
                       ...
                       struct bitblt_coords *dst, DWORD rop )
{
    ...
    if (clip && pdev->clip)
    {
        tmp_rgn = CreateRectRgn_k( 0, 0, 0, 0 );
        CombineRgn_k( tmp_rgn, clip, pdev->clip, RGN_AND );
        clip = tmp_rgn;
    }
    ...
    if (tmp_rgn) DeleteObject_k( tmp_rgn );
    return ret;
