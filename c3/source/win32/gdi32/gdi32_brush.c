...
static BOOL copy_bitmap( struct brush_pattern *brush, HBITMAP bitmap )
{
    ...
    if (!bits.free)
    {
        if (!(brush->bits.ptr = HeapAlloc_k( GetProcessHeap_k(), 0, info->bmiHeader.biSizeImage ))) goto done;
        memcpy( brush->bits.ptr, bits.ptr, info->bmiHeader.biSizeImage );
        brush->bits.free = free_heap_bits;
    }

    if (!(brush->info = HeapAlloc_k( GetProcessHeap_k(), 0, get_dib_info_size( info, DIB_RGB_COLORS ))))
    {
...
void free_brush_pattern( struct brush_pattern *pattern )
{
    ...
    HeapFree_k( GetProcessHeap_k(), 0, pattern->info );
}
 ...
 *   is used.
 */
HBRUSH WINAPI CreateBrushIndirect_k( const LOGBRUSH * brush )
{
    BRUSHOBJ * ptr;
    HBRUSH hbrush;

    if (!(ptr = HeapAlloc_k( GetProcessHeap_k(), 0, sizeof(*ptr) ))) return 0;
 ...
 *   when it is no longer required.
 */
HBRUSH WINAPI CreateHatchBrush_k( INT style, COLORREF color )
{
    LOGBRUSH logbrush;
    ...
    return CreateBrushIndirect_k( &logbrush );
}
 ...
 *   when it is no longer required.
 */
HBRUSH WINAPI CreatePatternBrush_k( HBITMAP hbitmap )
{
 ...
 *   be used instead.
 */
HBRUSH WINAPI CreateDIBPatternBrush_k( HGLOBAL hbitmap, UINT coloruse )
{
    LOGBRUSH logbrush;
 ...
 *   when it is no longer required.
 */
HBRUSH WINAPI CreateDIBPatternBrushPt_k( const void* data, UINT coloruse )
{
 ...
 *   when it is no longer required.
 */
HBRUSH WINAPI CreateSolidBrush_k( COLORREF color )
{
    LOGBRUSH logbrush;
 ...
 */
BOOL WINAPI SetBrushOrgEx_k( HDC hdc, INT x, INT y, LPPOINT oldorg )
{
 ...
 */
BOOL WINAPI FixBrushOrgEx_k( HDC hdc, INT x, INT y, LPPOINT oldorg )
{
    return SetBrushOrgEx_k(hdc,x,y,oldorg);
}
...
static BOOL BRUSH_DeleteObject( HGDIOBJ handle )
{
    ...
    HeapFree_k( GetProcessHeap_k(), 0, brush );
    return TRUE;
}
