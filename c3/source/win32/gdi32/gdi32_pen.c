 ...
 *           CreatePen    (GDI32.@)
 */
HPEN WINAPI CreatePen_k( INT style, INT width, COLORREF color )
{
    LOGPEN logpen;
    ...
    return CreatePenIndirect_k( &logpen );
}
 ...
 */
HPEN WINAPI CreatePenIndirect_k( const LOGPEN * pen )
{
    PENOBJ * penPtr;
    HPEN hpen;
    ...
    {
        hpen = GetStockObject_k(NULL_PEN);
        if (hpen) return hpen;
    }

    if (!(penPtr = HeapAlloc_k( GetProcessHeap_k(), HEAP_ZERO_MEMORY, sizeof(*penPtr) ))) return 0;
    ...
    if (!(hpen = alloc_gdi_handle( penPtr, OBJ_PEN, &pen_funcs )))
        HeapFree_k( GetProcessHeap_k(), 0, penPtr );
    return hpen;
}
...
HPEN WINAPI ExtCreatePen_k( DWORD style, DWORD width,
                              ...
                              const DWORD *style_bits )
{
    ...
    {
    case PS_NULL:
        return CreatePen_k( PS_NULL, 0, brush->lbColor );
    ...
    if ((style & PS_TYPE_MASK) == PS_GEOMETRIC)
    {
        if (brush->lbStyle == BS_NULL) return CreatePen_k( PS_NULL, 0, 0 );
    }
    else
    {
    ...
    if (!(penPtr = HeapAlloc_k(GetProcessHeap_k(), 0, FIELD_OFFSET(PENOBJ,logpen.elpStyleEntry[style_count]))))
        return 0;
...
static BOOL PEN_DeleteObject( HGDIOBJ handle )
{
    ...
    HeapFree_k( GetProcessHeap_k(), 0, pen );
    return TRUE;
}
