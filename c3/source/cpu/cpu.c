...
Callback callbacks[MAX_CALLBACK_COUNT];
U32 callbackCount;
U8 callbackCode[MAX_CALLBACK_COUNT*4];
...
void cpu_ret(struct CPU* cpu, U32 big, U32 eip) {
	...
	address = selector & ~7;
	//desc.fill = readq(cpu->thread->process->ldt[selector>>3].base_addr);
...
void cpu_iret(struct CPU* cpu, U32 big, U32 oldeip) {
	...
	if (cpu->flags & VM) {
		if ((cpu->flags & IOPL)!=IOPL) {
			...
			if (big) {
				U32 new_eip=readd(cpu->segAddress[SS] + ESP);
				U32 new_cs=readd(cpu->segAddress[SS] + ESP+4); // only first 16-bits are read
				U32 new_flags=readd(cpu->segAddress[SS] + ESP+8);
				...
				U16 new_eip=readw(cpu->segAddress[SS] + ESP);
				U16 new_cs=readw(cpu->segAddress[SS] + ESP+2);
				U16 new_flags=readw(cpu->segAddress[SS] + ESP+4);
	...
	if (big) {
		n_eip=readd(cpu->segAddress[SS] + ESP);
		n_cs_sel=readd(cpu->segAddress[SS] + ESP+4); // only read first 16-bits
		n_flags=readd(cpu->segAddress[SS] + ESP+8);
		...
		n_eip=readw(cpu->segAddress[SS] + ESP);
		n_cs_sel=readw(cpu->segAddress[SS] + ESP + 2);
		n_flags=readw(cpu->segAddress[SS] + ESP + 4);
...
void cpu_enter32(struct CPU* cpu, U32 bytes, U32 level) {
    ...
    sp_index-=4;
    writed(cpu->segAddress[SS] + sp_index, EBP);
    EBP = ESP - 4;
    if (level!=0) {
        U32 i;
        ...
        for (i=1;i<level;i++) {
            sp_index-=4;
            bp_index-=4;
            writed(cpu->segAddress[SS] + sp_index, readd(cpu->segAddress[SS] + bp_index));
        }
        sp_index-=4;
        writed(cpu->segAddress[SS] + sp_index, EBP);
    }
...
void cpu_enter16(struct CPU* cpu, U32 bytes, U32 level) {
    ...
    sp_index-=2;
    writew(cpu->segAddress[SS] + sp_index, BP);
    BP = SP - 2;
    if (level!=0) {
        U32 i;
        ...
        for (i=1;i<level;i++) {
            ...
            writew(cpu->segAddress[SS] + sp_index, readw(cpu->segAddress[SS] + bp_index));
        }
        sp_index-=2;
        writew(cpu->segAddress[SS] + sp_index, BP);
    }
...
void push16(struct CPU* cpu, U16 value) {
	if (cpu->big) {
        ...
        SP-=2;
		writew(cpu->segAddress[SS] + SP, value);
    }
}
...
U16 pop16(struct CPU* cpu) {
	if (cpu->big) {
		int result = readw(ESP);
        ESP+=2;
        return result;
    } else {
		int result = readw(cpu->segAddress[SS] + SP);
        SP+=2;
        return result;
    }
}
...
U32 peek32(struct CPU* cpu, U32 index) {
	if (cpu->big) {
		return readd(cpu->segAddress[SS] + ESP+4*index);
    } else {
		return readd(cpu->segAddress[SS] + SP+4*index);
    }
}
...
struct Block* getBlock(struct CPU* cpu) {
	struct Block** block = (struct Block**)pblMapGet(blocks, &cpu->eip.u32, sizeof(U32), 0);
	if (!block) {
		struct Block* b = decodeBlock(cpu, cpu->eip.u32);
        pblMapAdd(blocks, &cpu->eip.u32, sizeof(U32), &b, sizeof(struct Block*));
        return b;
	}
	return *block;
}
...
void cpu_callback(struct CPU* cpu, U16 index) {
    callbacks[index](cpu);
}