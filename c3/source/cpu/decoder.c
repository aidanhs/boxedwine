...
U16 FETCH16(struct DecodeData* data) {
	U16 result = readw(data->ip);
    data->ip+=2;
    return result;
}

U32 FETCH32(struct DecodeData* data) {
	U32 result = readd(data->ip);
    data->ip+=4;
    return result;
}
...
U32 aot(struct CPU* cpu, struct Block* block, U32 eip) {
    ...
    //func = getCompiledFunction(crc, ops, opPos, ip, &i);
    if (func) {
        block->ops->func = func;
...
void decodeBlockWithBlock(struct CPU* cpu, U32 eip, struct Block* block) {	
    ...
    struct Op* op;
    U32 baseStart = cpu->thread->process->mainModule->originalImageBase;
    U32 baseEnd = cpu->thread->process->mainModule->originalImageBase + cpu->thread->process->mainModule->size;
    U32 baseOffset = (U32)cpu->thread->process->mainModule->baseAddress - cpu->thread->process->mainModule->originalImageBase;
    ...
    while (op) {
        if (op->data1>=baseStart && op->data1<baseEnd) {
            op->data1+=baseOffset;
        }
        if (op->eData>=baseStart && op->eData<baseEnd) {
            op->eData+=baseOffset;
        }
        op = op->next;
    }
