...
WINECRT int printf(const char * fmt, ...)
{
  va_list ptr;
  int rv;
...
WINECRT int fprintf(FILE * fp, const char * fmt, ...)
{
  va_list ptr;
  int rv;
  va_strt(ptr, fmt);
...
WINECRT int vprintf(const char* fmt, va_list ap)
{
...
static int prtfld(FILE* op, char* buf, int ljustf, char sign, char pad, int width, int preci, int buffer_mode)
/*
...
WINECRT int vfprintf(FILE* op, const char* fmt, va_list ap)
{
